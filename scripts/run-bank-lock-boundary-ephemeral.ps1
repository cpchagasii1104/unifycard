# run-bank-lock-boundary-ephemeral.ps1
# Orquestrador do E2E F-BANK-LOCK-BOUNDARY-MATERIAL. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_bank_lock_boundary_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e bank-lock-boundary..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-bank-lock-boundary.ts
...
if ($exitCode -ne 0) { Write-Host "e2e bank-lock-boundary falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e bank-lock-boundary verde." -ForegroundColor Green
exit 0
