# run-referral-link-materialization-ephemeral.ps1
# Orquestrador do E2E DECISION-0119 (vínculo de indicação A->B puro/atômico).
# Cria DB EFEMERA, migra FULL (inclui seed unificard-inicial), roda o teste e DROPA.
...
$dbName = "unificard_referral_link_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e vinculo de indicacao A->B (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-referral-link-materialization.ts
...
if ($exitCode -ne 0) { Write-Host "vinculo de indicacao falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "vinculo de indicacao verde." -ForegroundColor Green
exit 0
