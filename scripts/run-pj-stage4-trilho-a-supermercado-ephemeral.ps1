# run-pj-stage4-trilho-a-supermercado-ephemeral.ps1
# Orquestrador da validação F-PJ-STAGE4-TRILHO-A-SUPERMERCADO (Op2, DECISION-0108 + Op1).
...
$dbName = "unificard_trilho_a_super_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-STAGE4-TRILHO-A-SUPERMERCADO..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-stage4-trilho-a-supermercado.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-STAGE4-TRILHO-A-SUPERMERCADO falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-STAGE4-TRILHO-A-SUPERMERCADO verde." -ForegroundColor Green
exit 0
