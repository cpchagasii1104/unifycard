# run-classic-channel-readers-binding-ephemeral.ps1
# Orquestrador do E2E F-0113-CLASSIC-CHANNEL-READERS-BINDING. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_classic_readers_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e classic channel readers binding..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-classic-channel-readers-binding.ts
...
if ($exitCode -ne 0) { Write-Host "e2e classic readers falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e classic readers verde." -ForegroundColor Green
exit 0
