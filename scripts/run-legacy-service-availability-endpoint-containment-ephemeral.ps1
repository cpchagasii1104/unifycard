# run-legacy-service-availability-endpoint-containment-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-AVAILABILITY-LEGACY-PUBLIC-ENDPOINT-CONTAINMENT-SLICE-A2B.
...
$dbName = "unificard_legacy_service_avail_endpoint_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e legacy service availability endpoint containment..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-legacy-service-availability-endpoint-containment.ts
...
if ($exitCode -ne 0) { Write-Host "e2e legacy service availability endpoint containment falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e legacy service availability endpoint containment verde." -ForegroundColor Green
exit 0
