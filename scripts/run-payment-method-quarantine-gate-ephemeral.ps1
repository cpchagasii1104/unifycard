# run-payment-method-quarantine-gate-ephemeral.ps1
# Orquestrador do E2E F-PAYMENT-METHOD-QUARANTINE-GATE. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_payment_method_quar_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e payment-method-quarantine-gate..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-payment-method-quarantine-gate.ts
...
if ($exitCode -ne 0) { Write-Host "e2e payment-method-quarantine-gate falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e payment-method-quarantine-gate verde." -ForegroundColor Green
exit 0
