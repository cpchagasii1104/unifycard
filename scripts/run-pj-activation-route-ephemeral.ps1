# run-pj-activation-route-ephemeral.ps1
# Orquestrador da validação F-PJ-ACTIVATION-ROUTE-WRITE-PAIR (rota HTTP de ativação operacional PJ).
# Cria DB EFEMERA, migra FULL, roda o teste de rota (app.inject) e DROPA. NUNCA toca unificard_dev.
...
$dbName = "unificard_activation_route_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-ACTIVATION-ROUTE-WRITE-PAIR (rota operational-activation)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-activation-route.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-ACTIVATION-ROUTE-WRITE-PAIR falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-ACTIVATION-ROUTE-WRITE-PAIR verde." -ForegroundColor Green
exit 0
