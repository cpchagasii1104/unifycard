# run-canonical-foundation-ephemeral.ps1
# Orquestrador da validação CP1 F-CANONICAL-CATALOG (fundação canônica + governança).
...
$dbName = "unificard_canonical_foundation_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste CP1 fundacao canonica + governanca (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-canonical-foundation-governance.ts
...
if ($exitCode -ne 0) { Write-Host "CP1 fundacao canonica falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "CP1 fundacao canonica verde." -ForegroundColor Green
exit 0
