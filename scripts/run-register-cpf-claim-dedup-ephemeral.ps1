# run-register-cpf-claim-dedup-ephemeral.ps1
# Orquestrador do E2E F-REGISTER-CPF-CLAIM-DEDUP. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_register_cpf_claim_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e register-cpf-claim-dedup..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-register-cpf-claim-dedup.ts
...
if ($exitCode -ne 0) { Write-Host "e2e register-cpf-claim-dedup falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e register-cpf-claim-dedup verde." -ForegroundColor Green
exit 0
