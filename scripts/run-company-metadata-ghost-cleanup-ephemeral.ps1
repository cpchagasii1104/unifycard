# run-company-metadata-ghost-cleanup-ephemeral.ps1
# Orquestrador do E2E F-COMPANY-METADATA-GHOST-CLEANUP.
...
$dbName = "unificard_company_metadata_gc_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e company metadata ghost cleanup..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-company-metadata-ghost-cleanup.ts
...
if ($exitCode -ne 0) { Write-Host "e2e company metadata ghost cleanup falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e company metadata ghost cleanup verde." -ForegroundColor Green
exit 0
