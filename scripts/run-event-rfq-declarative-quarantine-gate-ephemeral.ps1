# run-event-rfq-declarative-quarantine-gate-ephemeral.ps1
# Orquestrador do E2E F-EVENT-RFQ-DECLARATIVE-QUARANTINE-GATE. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_event_rfq_decl_quar_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e event-rfq-declarative-quarantine-gate..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-event-rfq-declarative-quarantine-gate.ts
...
if ($exitCode -ne 0) { Write-Host "e2e event-rfq-declarative-quarantine-gate falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e event-rfq-declarative-quarantine-gate verde." -ForegroundColor Green
exit 0
