# run-dispute-mutation-containment-ephemeral.ps1
# Orquestrador do E2E P1 F-DISPUTE-MUTATION-ACTOR-BODY-AUTHORITY-CONTAINMENT.
...
$dbName = "unificard_dispute_mutation_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e contencao dispute mutation (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-dispute-mutation-http-containment.ts
...
if ($exitCode -ne 0) { Write-Host "contencao dispute mutation falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "contencao dispute mutation verde." -ForegroundColor Green
exit 0
