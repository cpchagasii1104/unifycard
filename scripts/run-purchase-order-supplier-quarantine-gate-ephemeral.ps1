# run-purchase-order-supplier-quarantine-gate-ephemeral.ps1
# Orquestrador do E2E F-PURCHASE-ORDER-SUPPLIER-QUARANTINE-GATE. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_po_supplier_quar_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e purchase-order-supplier-quarantine-gate..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-purchase-order-supplier-quarantine-gate.ts
...
if ($exitCode -ne 0) { Write-Host "e2e purchase-order-supplier-quarantine-gate falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e purchase-order-supplier-quarantine-gate verde." -ForegroundColor Green
exit 0
