# run-service-salon-availability-bank-free-ephemeral.ps1
# Orquestrador da validação F-SERVICE-SALON-AVAILABILITY-BANK-FREE (DECISION-0109).
...
$dbName = "unificard_salon_availability_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-SERVICE-SALON-AVAILABILITY-BANK-FREE..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-service-salon-availability-bank-free.ts
...
if ($exitCode -ne 0) { Write-Host "F-SERVICE-SALON-AVAILABILITY-BANK-FREE falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-SERVICE-SALON-AVAILABILITY-BANK-FREE verde." -ForegroundColor Green
exit 0
