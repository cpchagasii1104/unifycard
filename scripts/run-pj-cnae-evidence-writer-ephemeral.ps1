# run-pj-cnae-evidence-writer-ephemeral.ps1
# Orquestrador da validação F-PJ-CNAE-EVIDENCE-WRITER
# (writer de evidência CNAE em fiscal_identity_economic_activities — DECISION-0103).
# Cria DB EFEMERA, migra FULL, roda o teste do writer e DROPA. NUNCA toca unificard_dev.
...
$dbName = "unificard_cnae_evidence_writer_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-CNAE-EVIDENCE-WRITER..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-cnae-evidence-writer.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-CNAE-EVIDENCE-WRITER falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-CNAE-EVIDENCE-WRITER verde." -ForegroundColor Green
exit 0
