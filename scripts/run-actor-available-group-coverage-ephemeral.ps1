# run-actor-available-group-coverage-ephemeral.ps1
# Orquestrador do E2E F-ACTOR-AVAILABLE-GROUP-COVERAGE.
...
$dbName = "unificard_actor_avail_group_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e actor available group coverage..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-actor-available-group-coverage.ts
...
if ($exitCode -ne 0) { Write-Host "e2e actor available group coverage falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e actor available group coverage verde." -ForegroundColor Green
exit 0
