# run-cultural-checkin-target-authority-ephemeral.ps1
# Orquestrador do E2E F-CULTURAL-CHECKIN-TARGET-AUTHORITY-BINDING-SLICE-A.
...
$dbName = "unificard_cultural_checkin_authority_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e cultural checkin target authority..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-cultural-checkin-target-authority.ts
...
if ($exitCode -ne 0) { Write-Host "e2e cultural checkin target authority falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e cultural checkin target authority verde." -ForegroundColor Green
exit 0
