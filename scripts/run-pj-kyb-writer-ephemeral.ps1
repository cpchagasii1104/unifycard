# run-pj-kyb-writer-ephemeral.ps1
# Orquestrador da validação F2-A KYB PJ (DECISION-0086).
# Cria DB EFÊMERA, migra MIGRATION_PROFILE=FULL, roda o teste do writer KYB e DROPA a DB no fim.
...
try {
    ...
    try {
        ...
        Write-Host "⏳ teste F2-A KYB writer..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-kyb-writer.ts
...
if ($exitCode -ne 0) { Write-Host "❌ F2-A KYB writer falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "✨ F2-A KYB writer verde." -ForegroundColor Green
exit 0
