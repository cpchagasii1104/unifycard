# run-dispute-reversal-containment-ephemeral.ps1
# Orquestrador do E2E P0 F-DISPUTE-REVERSAL-HTTP-AUTHORITY-CONTAINMENT.
...
$dbName = "unificard_dispute_reversal_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e contencao dispute reversal (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-dispute-reversal-http-containment.ts
...
if ($exitCode -ne 0) { Write-Host "contencao dispute reversal falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "contencao dispute reversal verde." -ForegroundColor Green
exit 0
