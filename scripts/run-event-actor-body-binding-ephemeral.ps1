# run-event-actor-body-binding-ephemeral.ps1
# Orquestrador do E2E F-0113-EVENT-ACTOR-BODY-BINDING. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_event_actor_binding_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e event actor body binding..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-event-actor-body-binding.ts
...
if ($exitCode -ne 0) { Write-Host "e2e event actor body binding falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e event actor body binding verde." -ForegroundColor Green
exit 0
