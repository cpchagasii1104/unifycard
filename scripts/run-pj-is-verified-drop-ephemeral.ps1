# run-pj-is-verified-drop-ephemeral.ps1
# Orquestrador da validação FASE 3.3-B2 (drop de companies.is_verified).
# Cria DB EFEMERA, migra FULL (inclui 20260604130000_drop_companies_is_verified), roda o teste e DROPA.
...
$dbName = "unificard_is_verified_drop_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste FASE 3.3-B2 (is_verified drop)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-is-verified-drop.ts
...
if ($exitCode -ne 0) { Write-Host "FASE 3.3-B2 falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "FASE 3.3-B2 verde." -ForegroundColor Green
exit 0
