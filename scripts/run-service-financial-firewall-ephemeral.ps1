...
# Orquestrador da validação F-SERVICE-FINANCIAL-FIREWALL-CODE (DECISION-0110).
...
$dbName = "unificard_fin_firewall_$stamp"
...
try {
    ...
    $env:PILOT_MODE = $null
    $env:SERVICE_FINANCIAL_RUNTIME_ENABLED = $null
    ...
    try {
        ...
        Write-Host "teste F-SERVICE-FINANCIAL-FIREWALL-CODE..." -ForegroundColor Yellow
...
if ($exitCode -ne 0) { Write-Host "F-SERVICE-FINANCIAL-FIREWALL-CODE falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-SERVICE-FINANCIAL-FIREWALL-CODE verde." -ForegroundColor Green
exit 0
