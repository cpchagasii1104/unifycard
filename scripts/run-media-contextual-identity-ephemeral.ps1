# run-media-contextual-identity-ephemeral.ps1
# Orquestrador do E2E DECISION-0118 D1 (identidade contextual de midia)
...
$dbName = "unificard_media_context_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste adversarial identidade contextual de midia (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-media-contextual-identity.ts
...
if ($exitCode -ne 0) { Write-Host "identidade contextual de midia falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "identidade contextual de midia verde." -ForegroundColor Green
exit 0
