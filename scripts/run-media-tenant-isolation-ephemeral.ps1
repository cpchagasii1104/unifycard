# run-media-tenant-isolation-ephemeral.ps1
# Orquestrador do E2E adversarial F-CANONICAL-MEDIA-BLOB-ASSET-TENANT-ISOLATION-CLOSURE
...
$dbName = "unificard_media_isolation_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste adversarial isolamento de midia cross-tenant (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-media-tenant-isolation.ts
...
if ($exitCode -ne 0) { Write-Host "Isolamento de midia cross-tenant falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "Isolamento de midia cross-tenant verde." -ForegroundColor Green
exit 0
