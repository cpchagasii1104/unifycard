# run-contextual-temporal-integrado-ephemeral.ps1
# Orquestrador do E2E INTEGRADO F-CANONICAL-CONTEXTUAL-MEDIA-AND-TEMPORAL-AUTHORITY-CLOSURE
...
$dbName = "unificard_ctx_temporal_integrado_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste adversarial integrado contextual+temporal (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-contextual-media-temporal-authority.ts
...
if ($exitCode -ne 0) { Write-Host "integrado contextual+temporal falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "integrado contextual+temporal verde." -ForegroundColor Green
exit 0
