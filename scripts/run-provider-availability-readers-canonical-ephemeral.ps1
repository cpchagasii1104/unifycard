# run-provider-availability-readers-canonical-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-AVAILABILITY-PROVIDER-READERS-CONTAINMENT-SLICE-A2E.
...
$dbName = "unificard_provider_avail_readers_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e provider availability readers canonical..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-provider-availability-readers-canonical.ts
...
if ($exitCode -ne 0) { Write-Host "e2e provider availability readers canonical falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e provider availability readers canonical verde." -ForegroundColor Green
exit 0
