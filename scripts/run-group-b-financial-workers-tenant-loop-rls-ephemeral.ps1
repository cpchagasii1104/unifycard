# run-group-b-financial-workers-tenant-loop-rls-ephemeral.ps1
# Orquestrador do E2E F-GROUP-B-FINANCIAL-WORKERS-TENANT-LOOP-RLS (DECISION-0149).
...
$dbName = "unificard_group_b_tenant_loop_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e group b tenant loop rls..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-group-b-financial-workers-tenant-loop-rls.ts
...
if ($exitCode -ne 0) { Write-Host "e2e group b tenant loop rls falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e group b tenant loop rls verde." -ForegroundColor Green
exit 0
