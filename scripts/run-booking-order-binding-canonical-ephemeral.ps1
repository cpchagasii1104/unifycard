# run-booking-order-binding-canonical-ephemeral.ps1
# Orquestrador do E2E F-BOOKING-ORDER-BINDING-CANONICAL.
# Cria DB EFEMERA, migra FULL (inclui a nova migration de integridade), roda o teste e DROPA.
...
$dbName = "unificard_booking_order_binding_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e booking-order binding canonical..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-booking-order-binding-canonical.ts
...
if ($exitCode -ne 0) { Write-Host "e2e booking-order binding falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e booking-order binding verde." -ForegroundColor Green
exit 0
