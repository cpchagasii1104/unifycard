# run-pj-kyb-documents-ephemeral.ps1
# Orquestrador da validação F2-B KYB DOCUMENTOS PJ (DECISION-0087).
# Cria DB EFÊMERA, migra MIGRATION_PROFILE=FULL, roda o teste de documentos KYB e DROPA a DB no fim.
...
$dbName = "unificard_kyb_docs_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "⏳ teste F2-B KYB documentos..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-kyb-documents.ts
...
if ($exitCode -ne 0) { Write-Host "❌ F2-B KYB documentos falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "✨ F2-B KYB documentos verde." -ForegroundColor Green
exit 0
