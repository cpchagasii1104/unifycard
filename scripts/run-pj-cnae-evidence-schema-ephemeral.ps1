# run-pj-cnae-evidence-schema-ephemeral.ps1
# Orquestrador da validação F-PJ-CNAE-EVIDENCE-SCHEMA-MIGRATION
# (schema da tabela fiscal_identity_economic_activities — evidência CNAE, DECISION-0103).
...
$dbName = "unificard_cnae_evidence_schema_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-CNAE-EVIDENCE-SCHEMA (fiscal_identity_economic_activities)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-cnae-evidence-schema.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-CNAE-EVIDENCE-SCHEMA falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-CNAE-EVIDENCE-SCHEMA verde." -ForegroundColor Green
exit 0
