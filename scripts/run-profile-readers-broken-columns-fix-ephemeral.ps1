# run-profile-readers-broken-columns-fix-ephemeral.ps1
# Orquestrador do E2E F-PROFILE-READERS-BROKEN-COLUMNS-FIX.
...
$dbName = "unificard_profile_readers_fix_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e profile readers broken columns fix..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-profile-readers-broken-columns-fix.ts
...
if ($exitCode -ne 0) { Write-Host "e2e profile readers broken columns fix falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e profile readers broken columns fix verde." -ForegroundColor Green
exit 0
