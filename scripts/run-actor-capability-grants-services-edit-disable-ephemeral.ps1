# run-actor-capability-grants-services-edit-disable-ephemeral.ps1
# Orquestrador do E2E F-ACTOR-CAPABILITY-GRANTS-SERVICES-EDIT-DISABLE. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_capability_edit_disable_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e capability-grants services:edit/disable..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-actor-capability-grants-services-edit-disable.ts
...
if ($exitCode -ne 0) { Write-Host "e2e capability-grants edit/disable falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e capability-grants edit/disable verde." -ForegroundColor Green
exit 0
