# run-pj-tenant-concept-offerings-rebuild-ephemeral.ps1
# Orquestrador da validação F-PJ-TENANT-CONCEPT-OFFERINGS-LEGACY-REBUILD-SCRIPT
# (rebuild idempotente de tenant_concept_offerings a partir de company_concept_publications).
...
$dbName = "unificard_offerings_rebuild_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-TENANT-CONCEPT-OFFERINGS-REBUILD (dry-run/apply/idempotencia)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-tenant-concept-offerings-rebuild.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-TENANT-CONCEPT-OFFERINGS-REBUILD falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-TENANT-CONCEPT-OFFERINGS-REBUILD verde." -ForegroundColor Green
exit 0
