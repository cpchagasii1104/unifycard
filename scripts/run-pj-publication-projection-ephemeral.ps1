# run-pj-publication-projection-ephemeral.ps1
# Orquestrador da validação F-PJ-PUBLICATION-OFFERING-PROJECTION-WRITER
# (projeção tenant_concept_offerings derivada de company_concept_publications, via app.inject).
...
$dbName = "unificard_publication_projection_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-PUBLICATION-OFFERING-PROJECTION (discovery derivado)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-publication-projection.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-PUBLICATION-OFFERING-PROJECTION falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-PUBLICATION-OFFERING-PROJECTION verde." -ForegroundColor Green
exit 0
