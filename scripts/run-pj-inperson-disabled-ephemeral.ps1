# run-pj-inperson-disabled-ephemeral.ps1
# Orquestrador da validação DECISION-0091 Fase 2.5 (validateInPerson / FASE 12 QR desabilitado).
...
$dbName = "unificard_inperson_disabled_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste DECISION-0091 Fase 2.5 (validateInPerson disabled)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-inperson-disabled.ts
...
if ($exitCode -ne 0) { Write-Host "DECISION-0091 Fase 2.5 falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "DECISION-0091 Fase 2.5 verde." -ForegroundColor Green
exit 0
