# run-service-category-ramo-guard-ephemeral.ps1
# Orquestrador da validação F-SERVICE-CREATION-CATEGORY-RAMO-GUARD (DECISION-0109).
...
$dbName = "unificard_service_guard_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-SERVICE-CREATION-CATEGORY-RAMO-GUARD..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-service-category-ramo-guard.ts
...
if ($exitCode -ne 0) { Write-Host "F-SERVICE-CREATION-CATEGORY-RAMO-GUARD falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-SERVICE-CREATION-CATEGORY-RAMO-GUARD verde." -ForegroundColor Green
exit 0
