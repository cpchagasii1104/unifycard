# run-module-menu-discovery-ephemeral.ps1
# Orquestrador da validação CP5 F-CANONICAL-CATALOG (menu projetado do registry + discovery canônica).
...
$dbName = "unificard_module_menu_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste CP5 menu+discovery (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-module-menu-canonical-discovery.ts
...
if ($exitCode -ne 0) { Write-Host "CP5 menu+discovery falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "CP5 menu+discovery verde." -ForegroundColor Green
exit 0
