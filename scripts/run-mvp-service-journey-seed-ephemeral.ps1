# run-mvp-service-journey-seed-ephemeral.ps1
# Orquestrador do E2E F-MVP-SERVICE-JOURNEY-SEED. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_mvp_service_journey_seed_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e mvp-service-journey-seed..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-mvp-service-journey-seed.ts
...
if ($exitCode -ne 0) { Write-Host "e2e mvp-service-journey-seed falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e mvp-service-journey-seed verde." -ForegroundColor Green
exit 0
