# run-pj-cnae-concept-suggestion-schema-ephemeral.ps1
# Orquestrador da validação F-PJ-CNAE-TO-CONCEPT-SUGGESTION-MATRIX-SCHEMA-MIGRATION
# (schema da tabela cnae_concept_suggestions — matriz de sugestão CNAE→concept, DECISION-0104).
# Cria DB EFEMERA, migra FULL, roda o teste de schema e DROPA. NUNCA toca unificard_dev.
...
$dbName = "unificard_cnae_concept_suggestion_schema_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-CNAE-CONCEPT-SUGGESTION-SCHEMA (cnae_concept_suggestions)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-cnae-concept-suggestion-schema.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-CNAE-CONCEPT-SUGGESTION-SCHEMA falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-CNAE-CONCEPT-SUGGESTION-SCHEMA verde." -ForegroundColor Green
exit 0
