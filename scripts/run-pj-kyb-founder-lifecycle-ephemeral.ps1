# run-pj-kyb-founder-lifecycle-ephemeral.ps1
# Orquestrador da validação CP2 F-PJ-HUMAN-TO-COMPANY (lifecycle KYB do fundador, HTTP).
...
$dbName = "unificard_pj_kyb_founder_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste CP2 PJ KYB founder lifecycle (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-kyb-founder-lifecycle-http.ts
...
if ($exitCode -ne 0) { Write-Host "CP2 PJ KYB founder lifecycle falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "CP2 PJ KYB founder lifecycle verde." -ForegroundColor Green
exit 0
