# run-pj-birth-membership-readers-ephemeral.ps1
# Orquestrador da validação CP1 F-PJ-HUMAN-TO-COMPANY (nascimento fiscal-first + leitura por vínculo, HTTP).
...
$dbName = "unificard_pj_birth_members_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste CP1 PJ birth + membership readers (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-birth-membership-readers-http.ts
...
if ($exitCode -ne 0) { Write-Host "CP1 PJ birth/membership falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "CP1 PJ birth/membership verde." -ForegroundColor Green
exit 0
