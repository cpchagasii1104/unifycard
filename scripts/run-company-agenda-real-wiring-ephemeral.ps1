# run-company-agenda-real-wiring-ephemeral.ps1
# Orquestrador do E2E F-COMPANY-AGENDA-REAL-WIRING.
...
$dbName = "unificard_company_agenda_wiring_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e company agenda real wiring..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-company-agenda-real-wiring.ts
...
if ($exitCode -ne 0) { Write-Host "e2e company agenda real wiring falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e company agenda real wiring verde." -ForegroundColor Green
exit 0
