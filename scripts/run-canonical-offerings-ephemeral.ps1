# run-canonical-offerings-ephemeral.ps1
# Orquestrador da validação CP4 F-CANONICAL-CATALOG (ofertas por variante canônica + inventory + serviço/agenda).
...
$dbName = "unificard_canonical_offerings_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste CP4 ofertas canonicas + inventory (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-canonical-offerings-inventory.ts
...
if ($exitCode -ne 0) { Write-Host "CP4 ofertas falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "CP4 ofertas verde." -ForegroundColor Green
exit 0
