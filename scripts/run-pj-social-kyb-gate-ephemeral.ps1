# run-pj-social-kyb-gate-ephemeral.ps1
# Orquestrador da validação DECISION-0094 Fase Social Gate 1 (gate KYB social publish_feed/cast_vote).
...
$dbName = "unificard_social_kyb_gate_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste DECISION-0094 Fase Social Gate 1 (gate KYB social)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-social-kyb-gate.ts
...
if ($exitCode -ne 0) { Write-Host "DECISION-0094 Fase Social Gate 1 falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "DECISION-0094 Fase Social Gate 1 verde." -ForegroundColor Green
exit 0
