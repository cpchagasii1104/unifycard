# run-canonical-media-ephemeral.ps1
# Orquestrador da validação CP2 F-CANONICAL-CATALOG (mídia canônica content-addressed).
...
$dbName = "unificard_canonical_media_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste CP2 midia canonica (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-canonical-media-assets.ts
...
if ($exitCode -ne 0) { Write-Host "CP2 midia canonica falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "CP2 midia canonica verde." -ForegroundColor Green
exit 0
