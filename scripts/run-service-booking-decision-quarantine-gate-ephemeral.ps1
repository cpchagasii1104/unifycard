# run-service-booking-decision-quarantine-gate-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-BOOKING-DECISION-QUARANTINE-GATE. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_booking_decision_quar_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e service-booking-decision-quarantine-gate..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-service-booking-decision-quarantine-gate.ts
...
if ($exitCode -ne 0) { Write-Host "e2e service-booking-decision-quarantine-gate falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e service-booking-decision-quarantine-gate verde." -ForegroundColor Green
exit 0
