# run-event-settlement-status-hold-containment-ephemeral.ps1
# Orquestrador do E2E F-EVENT-SETTLEMENT-STATUS-HOLD-CONTAINMENT. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_event_settlement_hold_$stamp"
...
try {
    ...
    Remove-Item Env:EVENT_SETTLEMENT_RUNTIME_ENABLED -ErrorAction SilentlyContinue
    ...
    try {
        ...
        Write-Host "e2e event-settlement-status-hold..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-event-settlement-status-hold-containment.ts
...
if ($exitCode -ne 0) { Write-Host "e2e event-settlement-status-hold falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e event-settlement-status-hold verde." -ForegroundColor Green
exit 0
