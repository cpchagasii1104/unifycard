# run-reports-transfers-sla-representation-ephemeral.ps1
# Orquestrador do E2E F-REPORTS-TRANSFERS-SLA-REPRESENTATION. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_reports_sla_repr_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e reports-transfers-sla-representation..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-reports-transfers-sla-representation.ts
...
if ($exitCode -ne 0) { Write-Host "e2e reports-transfers-sla-representation falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e reports-transfers-sla-representation verde." -ForegroundColor Green
exit 0
