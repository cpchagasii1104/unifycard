# run-pj-updatecompany-no-status-ephemeral.ps1
# Orquestrador da validação DECISION-0090 Fase 2.1 (updateCompany não escreve company_status).
# Cria DB EFÊMERA, migra FULL, roda o teste e DROPA a DB no fim. NUNCA toca unificard_dev.
...
$dbName = "unificard_updatecompany_nostatus_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste DECISION-0090 Fase 2.1 (updateCompany no-status)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-updatecompany-no-status.ts
...
if ($exitCode -ne 0) { Write-Host "DECISION-0090 Fase 2.1 falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "DECISION-0090 Fase 2.1 verde." -ForegroundColor Green
exit 0
