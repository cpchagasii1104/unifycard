# run-pj-concept-labels-schema-ephemeral.ps1
# Orquestrador da validação F-PJ-CONCEPT-LABELS-SCHEMA-MIGRATION
# (schema da tabela concept_labels — apresentação governada, DECISION-0107).
...
$dbName = "unificard_concept_labels_schema_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-CONCEPT-LABELS-SCHEMA (concept_labels)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-concept-labels-schema.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-CONCEPT-LABELS-SCHEMA falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-CONCEPT-LABELS-SCHEMA verde." -ForegroundColor Green
exit 0
