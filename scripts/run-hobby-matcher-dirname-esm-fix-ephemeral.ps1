# run-hobby-matcher-dirname-esm-fix-ephemeral.ps1
# Orquestrador do E2E F-HOBBY-MATCHER-DIRNAME-ESM-FIX.
...
$dbName = "unificard_hobby_dirname_esm_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e hobby matcher dirname esm fix..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-hobby-matcher-dirname-esm-fix.ts
...
if ($exitCode -ne 0) { Write-Host "e2e hobby matcher dirname esm fix falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e hobby matcher dirname esm fix verde." -ForegroundColor Green
exit 0
