# run-dispute-reversal-authority-model-ephemeral.ps1
# Orquestrador do E2E F-DISPUTE-REVERSAL-AUTHORITY-BINDING-MODEL (prova de contenção; nada reabilitado).
...
$dbName = "unificard_dispute_reversal_model_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e dispute-reversal authority model..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-dispute-reversal-authority-model.ts
...
if ($exitCode -ne 0) { Write-Host "e2e dispute-reversal model falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e dispute-reversal model verde." -ForegroundColor Green
exit 0
