# run-civil-identity-confirmation-ephemeral.ps1
# Orquestrador do E2E DECISION-0120 (confirmacao/trava de identidade civil em identity SSOT).
# Cria DB EFEMERA, migra FULL (inclui seed unificard-inicial + tabela civil), roda e DROPA.
...
$dbName = "unificard_civil_identity_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e confirmacao civil (HTTP + servico)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-civil-identity-confirmation.ts
...
if ($exitCode -ne 0) { Write-Host "confirmacao civil falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "confirmacao civil verde." -ForegroundColor Green
exit 0
