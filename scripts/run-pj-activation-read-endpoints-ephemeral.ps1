# run-pj-activation-read-endpoints-ephemeral.ps1
# Orquestrador da validação F-PJ-ACTIVATION-READ-ENDPOINTS (catálogo governado de seleção do par).
# Cria DB EFEMERA, migra FULL, roda o teste (app.inject GET) e DROPA. NUNCA toca unificard_dev.
...
$dbName = "unificard_activation_read_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-ACTIVATION-READ-ENDPOINTS (catalogo de selecao)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-activation-read-endpoints.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-ACTIVATION-READ-ENDPOINTS falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-ACTIVATION-READ-ENDPOINTS verde." -ForegroundColor Green
exit 0
