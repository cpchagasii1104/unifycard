# run-pj-kyb-gate-ephemeral.ps1
# Orquestrador da validação F2-C GATE KYB PJ (DECISION-0088).
# Cria DB EFÊMERA, migra FULL, roda o teste do gate KYB e DROPA a DB no fim. NUNCA toca unificard_dev.
# AUTHORITY_MODE=permissive (NODE_ENV=development) ISOLA o KYB (ATL/KYC skip) e prova strict-para-dinheiro.
...
$dbName = "unificard_kyb_gate_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "⏳ teste F2-C gate KYB (AUTHORITY_MODE=permissive)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-kyb-gate.ts
...
if ($exitCode -ne 0) { Write-Host "❌ F2-C gate KYB falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "✨ F2-C gate KYB verde." -ForegroundColor Green
exit 0
