# run-risk-dashboard-permission-spoof-ephemeral.ps1
# Orquestrador do E2E F-RISK-DASHBOARD-PERMISSION-SPOOF-CONTAINMENT. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_risk_dashboard_spoof_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e risk-dashboard permission spoof..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-risk-dashboard-permission-spoof.ts
...
if ($exitCode -ne 0) { Write-Host "e2e risk-dashboard spoof falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e risk-dashboard spoof verde." -ForegroundColor Green
exit 0
