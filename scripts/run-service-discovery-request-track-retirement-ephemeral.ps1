# run-service-discovery-request-track-retirement-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-DISCOVERY-REQUEST-TRACK-RETIREMENT.
...
$dbName = "unificard_discovery_retire_track_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e service discovery request track retirement..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-service-discovery-request-track-retirement.ts
...
if ($exitCode -ne 0) { Write-Host "e2e service discovery request track retirement falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e service discovery request track retirement verde." -ForegroundColor Green
exit 0
