# run-availability-quarantine-gate-ephemeral.ps1
# Orquestrador do E2E F-AVAILABILITY-WRITE-QUARANTINE-GATE. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_availability_quarantine_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e availability-quarantine-gate..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-availability-quarantine-gate.ts
...
if ($exitCode -ne 0) { Write-Host "e2e availability-quarantine-gate falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e availability-quarantine-gate verde." -ForegroundColor Green
exit 0
