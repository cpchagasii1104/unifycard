# run-groups-votes-closevote-quarantine-gate-ephemeral.ps1
# Orquestrador do E2E F-GROUPS-VOTES-CLOSEVOTE-QUARANTINE-GATE. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_groups_closevote_quar_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e groups-votes-closevote-quarantine-gate..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-groups-votes-closevote-quarantine-gate.ts
...
if ($exitCode -ne 0) { Write-Host "e2e groups-votes-closevote-quarantine-gate falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e groups-votes-closevote-quarantine-gate verde." -ForegroundColor Green
exit 0
