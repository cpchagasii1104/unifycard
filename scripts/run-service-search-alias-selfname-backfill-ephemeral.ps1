# run-service-search-alias-selfname-backfill-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-SEARCH-ALIAS-SELFNAME-BACKFILL.
...
$dbName = "unificard_alias_selfname_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e service search alias selfname backfill..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-service-search-alias-selfname-backfill.ts
...
if ($exitCode -ne 0) { Write-Host "e2e service search alias selfname backfill falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e service search alias selfname backfill verde." -ForegroundColor Green
exit 0
