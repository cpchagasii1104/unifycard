# run-pj-capability-kyb-ephemeral.ps1
# Orquestrador da validação DECISION-0092 Fase 3.0 (capability social + CNPJ-lock via kyb_status).
...
$dbName = "unificard_capability_kyb_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste DECISION-0092 Fase 3.0 (capability + CNPJ-lock via kyb_status)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-capability-kyb.ts
...
if ($exitCode -ne 0) { Write-Host "DECISION-0092 Fase 3.0 falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "DECISION-0092 Fase 3.0 verde." -ForegroundColor Green
exit 0
