# run-pj-concept-labels-seed-ephemeral.ps1
# Orquestrador da validação F-PJ-CONCEPT-LABELS-SEED-MVP (DECISION-0107 D10).
# Cria DB EFEMERA, migra FULL (inclui o seed), roda o teste e DROPA. NUNCA toca unificard_dev.
...
$dbName = "unificard_concept_labels_seed_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-CONCEPT-LABELS-SEED (concept_labels)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-concept-labels-seed.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-CONCEPT-LABELS-SEED falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-CONCEPT-LABELS-SEED verde." -ForegroundColor Green
exit 0
