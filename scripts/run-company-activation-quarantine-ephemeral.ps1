# run-company-activation-quarantine-ephemeral.ps1
# Orquestrador do E2E F-COMPANY-OPERATIONAL-ACTIVATION-QUARANTINE-GATE. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_company_activation_quar_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e company-activation-quarantine..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-company-activation-quarantine.ts
...
if ($exitCode -ne 0) { Write-Host "e2e company-activation-quarantine falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e company-activation-quarantine verde." -ForegroundColor Green
exit 0
