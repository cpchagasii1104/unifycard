# run-group-a-financial-tables-rls-ephemeral.ps1
# Orquestrador do E2E F-GROUP-A-FINANCIAL-TABLES-RLS (15 tabelas, achado B3 do auditoria.md).
...
$dbName = "unificard_group_a_rls_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e group a financial tables rls..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-group-a-financial-tables-rls.ts
...
if ($exitCode -ne 0) { Write-Host "e2e group a financial tables rls falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e group a financial tables rls verde." -ForegroundColor Green
exit 0
