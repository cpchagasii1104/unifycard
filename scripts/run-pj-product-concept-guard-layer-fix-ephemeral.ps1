# run-pj-product-concept-guard-layer-fix-ephemeral.ps1
# Orquestrador da validação F-PJ-PRODUCT-CONCEPT-GUARD-LAYER-FIX (DECISION-0108).
...
$dbName = "unificard_guard_layerfix_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-PRODUCT-CONCEPT-GUARD-LAYER-FIX..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-product-concept-guard-layer-fix.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-PRODUCT-CONCEPT-GUARD-LAYER-FIX falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-PRODUCT-CONCEPT-GUARD-LAYER-FIX verde." -ForegroundColor Green
exit 0
