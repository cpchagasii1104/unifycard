# run-events-assignstaff-sessionread-schema-drift-ephemeral.ps1
# Orquestrador do E2E F-EVENTS-ASSIGNSTAFF-SESSIONREAD-SCHEMA-DRIFT. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_staff_session_drift_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e events-assignstaff-sessionread-schema-drift..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-events-assignstaff-sessionread-schema-drift.ts
...
if ($exitCode -ne 0) { Write-Host "e2e events-assignstaff-sessionread-schema-drift falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e events-assignstaff-sessionread-schema-drift verde." -ForegroundColor Green
exit 0
