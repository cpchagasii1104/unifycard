# run-payment-intents-governance-funding-rls-ephemeral.ps1
# Orquestrador do E2E F-PAYMENT-INTENTS-GOVERNANCE-FUNDING-RLS (achado B3 do auditoria.md).
...
$dbName = "unificard_payment_intents_rls_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e payment intents / governance funding rls..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-payment-intents-governance-funding-rls.ts
...
if ($exitCode -ne 0) { Write-Host "e2e payment intents / governance funding rls falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e payment intents / governance funding rls verde." -ForegroundColor Green
exit 0
