# run-service-booking-requested-effect-emission-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-BOOKING-REQUESTED-EFFECT-EMISSION.
...
$dbName = "unificard_booking_requested_effect_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e service booking requested effect emission..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-service-booking-requested-effect-emission.ts
...
if ($exitCode -ne 0) { Write-Host "e2e service booking requested effect emission falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e service booking requested effect emission verde." -ForegroundColor Green
exit 0
