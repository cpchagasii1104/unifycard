# run-availability-conflict-detection-materialized-ephemeral.ps1
# Orquestrador do E2E F-AVAILABILITY-CONFLICT-DETECTION-STUB-FIX.
...
$dbName = "unificard_conflict_detection_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e availability conflict detection materialized..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-availability-conflict-detection-materialized.ts
...
if ($exitCode -ne 0) { Write-Host "e2e availability conflict detection materialized falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e availability conflict detection materialized verde." -ForegroundColor Green
exit 0
