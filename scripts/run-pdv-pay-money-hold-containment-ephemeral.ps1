# run-pdv-pay-money-hold-containment-ephemeral.ps1
# Orquestrador do E2E F-PDV-PAY-MONEY-HOLD-CONTAINMENT. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_pdv_pay_money_hold_$stamp"
...
try {
    ...
    # garante que o flag de runtime financeiro do PDV NÃO vaze ligado do ambiente.
    Remove-Item Env:PDV_FINANCIAL_RUNTIME_ENABLED -ErrorAction SilentlyContinue
    ...
    try {
        ...
        Write-Host "e2e pdv-pay-money-hold-containment..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pdv-pay-money-hold-containment.ts
...
if ($exitCode -ne 0) { Write-Host "e2e pdv-pay-money-hold-containment falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e pdv-pay-money-hold-containment verde." -ForegroundColor Green
exit 0
