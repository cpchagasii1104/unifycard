# run-unread-counts-feed-visibility-fix-ephemeral.ps1
# Orquestrador do E2E F-UNREAD-COUNTS-FEED-VISIBILITY-FIX.
...
$dbName = "unificard_unread_counts_feed_vis_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e unread counts feed visibility fix..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-unread-counts-feed-visibility-fix.ts
...
if ($exitCode -ne 0) { Write-Host "e2e unread counts feed visibility fix falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e unread counts feed visibility fix verde." -ForegroundColor Green
exit 0
