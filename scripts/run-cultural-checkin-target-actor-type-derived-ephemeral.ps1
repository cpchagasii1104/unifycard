# run-cultural-checkin-target-actor-type-derived-ephemeral.ps1
# Orquestrador do E2E F-CULTURAL-CHECKIN-TARGET-ACTOR-TYPE-DERIVED.
...
$dbName = "unificard_cultural_checkin_type_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e cultural checkin target actor type derived..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-cultural-checkin-target-actor-type-derived.ts
...
if ($exitCode -ne 0) { Write-Host "e2e cultural checkin target actor type derived falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e cultural checkin target actor type derived verde." -ForegroundColor Green
exit 0
