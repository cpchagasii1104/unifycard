# run-pj-publication-schema-ephemeral.ps1
# Orquestrador da validação F-PJ-PUBLICATION-OFFERING-SCHEMA-MIGRATION
# (schema da tabela soberana company_concept_publications).
...
$dbName = "unificard_publication_schema_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-PUBLICATION-OFFERING-SCHEMA (company_concept_publications)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-publication-schema.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-PUBLICATION-OFFERING-SCHEMA falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-PUBLICATION-OFFERING-SCHEMA verde." -ForegroundColor Green
exit 0
