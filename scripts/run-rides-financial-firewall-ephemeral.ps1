# run-rides-financial-firewall-ephemeral.ps1
# Orquestrador do E2E F-RIDES-FINANCIAL-FIREWALL (achado B1 do auditoria.md).
...
$dbName = "unificard_rides_firewall_$stamp"
...
try {
    ...
    Remove-Item Env:RIDES_FINANCIAL_RUNTIME_ENABLED -ErrorAction SilentlyContinue
    ...
    try {
        ...
        Write-Host "e2e rides financial firewall..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-rides-financial-firewall.ts
...
if ($exitCode -ne 0) { Write-Host "e2e rides financial firewall falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e rides financial firewall verde." -ForegroundColor Green
exit 0
