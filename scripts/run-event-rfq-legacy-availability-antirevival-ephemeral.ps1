# run-event-rfq-legacy-availability-antirevival-ephemeral.ps1
# Orquestrador do E2E F-EVENT-RFQ-LEGACY-SERVICE-AVAILABILITY-ANTI-REACTIVATION-GUARD.
...
$dbName = "unificard_rfq_legacy_avail_guard_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e rfq legacy availability antirevival..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-event-rfq-legacy-availability-antirevival.ts
...
if ($exitCode -ne 0) { Write-Host "e2e rfq legacy availability antirevival falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e rfq legacy availability antirevival verde." -ForegroundColor Green
exit 0
