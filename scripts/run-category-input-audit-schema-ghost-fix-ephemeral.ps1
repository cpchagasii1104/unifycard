# run-category-input-audit-schema-ghost-fix-ephemeral.ps1
# Orquestrador do E2E F-CATEGORY-INPUT-AUDIT-SCHEMA-GHOST-FIX.
...
$dbName = "unificard_category_audit_ghost_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e category input audit schema ghost fix..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-category-input-audit-schema-ghost-fix.ts
...
if ($exitCode -ne 0) { Write-Host "e2e category input audit schema ghost fix falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e category input audit schema ghost fix verde." -ForegroundColor Green
exit 0
