# run-pj-cnae-suggestion-read-endpoint-ephemeral.ps1
# Orquestrador da validação F-PJ-CNAE-TO-CONCEPT-SUGGESTION-READ-ENDPOINT (DECISION-0104).
# Cria DB EFEMERA, migra FULL (inclui o seed γ), roda o teste e DROPA. NUNCA toca unificard_dev.
...
$dbName = "unificard_cnae_suggestion_endpoint_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-CNAE-TO-CONCEPT-SUGGESTION-READ-ENDPOINT..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-cnae-suggestion-read-endpoint.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-CNAE-TO-CONCEPT-SUGGESTION-READ-ENDPOINT falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-CNAE-TO-CONCEPT-SUGGESTION-READ-ENDPOINT verde." -ForegroundColor Green
exit 0
