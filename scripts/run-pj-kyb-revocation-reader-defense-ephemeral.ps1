# run-pj-kyb-revocation-reader-defense-ephemeral.ps1
# Orquestrador da validação DT-PJ-KYB-REVOCATION-READER-DEFENSE
# (filtro defensivo KYB-approved no reader de discovery contextual — DECISION-0101 D9).
...
$dbName = "unificard_kyb_reader_defense_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste DT-PJ-KYB-REVOCATION-READER-DEFENSE..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-kyb-revocation-reader-defense.ts
...
if ($exitCode -ne 0) { Write-Host "DT-PJ-KYB-REVOCATION-READER-DEFENSE falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "DT-PJ-KYB-REVOCATION-READER-DEFENSE verde." -ForegroundColor Green
exit 0
