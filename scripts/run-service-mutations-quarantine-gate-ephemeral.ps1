# run-service-mutations-quarantine-gate-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-MUTATIONS-QUARANTINE-GATE. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_service_mutations_quar_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e service-mutations-quarantine-gate..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-service-mutations-quarantine-gate.ts
...
if ($exitCode -ne 0) { Write-Host "e2e service-mutations-quarantine-gate falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e service-mutations-quarantine-gate verde." -ForegroundColor Green
exit 0
