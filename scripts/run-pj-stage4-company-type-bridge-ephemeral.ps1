# run-pj-stage4-company-type-bridge-ephemeral.ps1
# Orquestrador da validação F-PJ-STAGE4-COMPANY-TYPE-BRIDGE (Op1, ponte Classificação→Stage 4).
...
$dbName = "unificard_stage4_bridge_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-STAGE4-COMPANY-TYPE-BRIDGE..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-stage4-company-type-bridge.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-STAGE4-COMPANY-TYPE-BRIDGE falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-STAGE4-COMPANY-TYPE-BRIDGE verde." -ForegroundColor Green
exit 0
