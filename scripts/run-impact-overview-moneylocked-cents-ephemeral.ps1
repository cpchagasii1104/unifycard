# run-impact-overview-moneylocked-cents-ephemeral.ps1
# Orquestrador do E2E F-IMPACT-OVERVIEW-MONEYLOCKED-CENTS-FIX.
...
$dbName = "unificard_impact_moneylocked_cents_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e impact-overview moneylocked cents..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-impact-overview-moneylocked-cents.ts
...
if ($exitCode -ne 0) { Write-Host "e2e impact-overview moneylocked cents falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e impact-overview moneylocked cents verde." -ForegroundColor Green
exit 0
