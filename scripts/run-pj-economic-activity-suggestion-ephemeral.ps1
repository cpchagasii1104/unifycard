# run-pj-economic-activity-suggestion-ephemeral.ps1
# Orquestrador do E2E F-PJ-ONBOARDING-WIZARD-ECONOMIC-ACTIVITY-SUGGESTION (B1). DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_economic_activity_suggestion_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e economic-activity-suggestion..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-economic-activity-suggestion.ts
...
if ($exitCode -ne 0) { Write-Host "e2e economic-activity-suggestion falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e economic-activity-suggestion verde." -ForegroundColor Green
exit 0
