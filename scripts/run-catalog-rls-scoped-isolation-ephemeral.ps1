# run-catalog-rls-scoped-isolation-ephemeral.ps1
# Orquestrador do E2E F-CATALOG-RLS-SCOPED-ISOLATION.
...
$dbName = "unificard_catalog_rls_isolation_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e catalog rls scoped isolation..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-catalog-rls-scoped-isolation.ts
...
if ($exitCode -ne 0) { Write-Host "e2e catalog rls scoped isolation falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e catalog rls scoped isolation verde." -ForegroundColor Green
exit 0
