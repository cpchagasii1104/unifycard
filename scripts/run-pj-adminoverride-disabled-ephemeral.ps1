# run-pj-adminoverride-disabled-ephemeral.ps1
# Orquestrador da validação DECISION-0090 Fase 2.2 (adminOverrideToVerified desabilitado).
...
$dbName = "unificard_adminoverride_disabled_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste DECISION-0090 Fase 2.2 (adminOverride disabled)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-adminoverride-disabled.ts
...
if ($exitCode -ne 0) { Write-Host "DECISION-0090 Fase 2.2 falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "DECISION-0090 Fase 2.2 verde." -ForegroundColor Green
exit 0
