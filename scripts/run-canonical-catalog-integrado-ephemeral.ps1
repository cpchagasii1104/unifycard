# run-canonical-catalog-integrado-ephemeral.ps1
# Orquestrador do E2E INTEGRADO F-CANONICAL-CATALOG-BUSINESS-TEMPLATES-AND-OFFERING-CLOSURE
# (GO §12: 3 empresas + templates + produto/variante/mídia + ofertas + serviço/agenda +
# menu + discovery + revogação + unidades + zero financeiro).
# Cria DB EFEMERA, migra FULL, roda o teste (app.inject) e DROPA. NUNCA toca unificard_dev.
...
$dbName = "unificard_canonical_integrado_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "E2E INTEGRADO canonico (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-canonical-catalog-templates-offering.ts
...
if ($exitCode -ne 0) { Write-Host "E2E integrado canonico falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "E2E integrado canonico verde." -ForegroundColor Green
exit 0
