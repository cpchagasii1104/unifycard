# run-legacy-service-availability-feed-badge-containment-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-AVAILABILITY-LEGACY-FEED-BADGE-CONTAINMENT-SLICE-A2C.
# DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_legacy_service_feed_badge_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e legacy service availability feed badge containment..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-legacy-service-availability-feed-badge-containment.ts
...
if ($exitCode -ne 0) { Write-Host "e2e legacy service availability feed badge containment falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e legacy service availability feed badge containment verde." -ForegroundColor Green
exit 0
