# run-capability-grant-quarantine-ephemeral.ps1
# Orquestrador do E2E F-CAPABILITY-GRANT-QUARANTINE-GATE. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_capability_grant_quar_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e capability-grant-quarantine..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-capability-grant-quarantine.ts
...
if ($exitCode -ne 0) { Write-Host "e2e capability-grant-quarantine falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e capability-grant-quarantine verde." -ForegroundColor Green
exit 0
