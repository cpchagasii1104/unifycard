# run-mvp-service-journey-pj-provider-ephemeral.ps1
# Orquestrador do E2E F-MVP-SERVICE-JOURNEY-PJ-PROVIDER. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_mvp_pj_provider_$stamp"
...
try {
    ...
    # Money-free: garante que nenhuma flag financeira/worker vaze do ambiente do operador.
    ...
    try {
        ...
        Write-Host "e2e mvp-service-journey-pj-provider..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-mvp-service-journey-pj-provider.ts
...
if ($exitCode -ne 0) { Write-Host "e2e mvp-service-journey-pj-provider falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e mvp-service-journey-pj-provider verde." -ForegroundColor Green
exit 0
