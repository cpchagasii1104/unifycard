# run-events-lifecycle-quarantine-gate-ephemeral.ps1
# Orquestrador do E2E F-EVENTS-LIFECYCLE-QUARANTINE-GATE. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_events_lifecycle_quar_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e events-lifecycle-quarantine-gate..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-events-lifecycle-quarantine-gate.ts
...
if ($exitCode -ne 0) { Write-Host "e2e events-lifecycle-quarantine-gate falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e events-lifecycle-quarantine-gate verde." -ForegroundColor Green
exit 0
