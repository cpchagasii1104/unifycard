# run-business-templates-ephemeral.ps1
# Orquestrador da validação CP3 F-CANONICAL-CATALOG (templates empresariais versionados).
...
$dbName = "unificard_business_templates_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste CP3 templates empresariais (HTTP)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-business-templates-application.ts
...
if ($exitCode -ne 0) { Write-Host "CP3 templates falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "CP3 templates verde." -ForegroundColor Green
exit 0
