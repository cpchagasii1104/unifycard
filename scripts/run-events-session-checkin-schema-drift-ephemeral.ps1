# run-events-session-checkin-schema-drift-ephemeral.ps1
# Orquestrador do E2E F-EVENTS-SESSION-CHECKIN-SCHEMA-DRIFT. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_session_checkin_drift_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e events-session-checkin-schema-drift..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-events-session-checkin-schema-drift.ts
...
if ($exitCode -ne 0) { Write-Host "e2e events-session-checkin-schema-drift falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e events-session-checkin-schema-drift verde." -ForegroundColor Green
exit 0
