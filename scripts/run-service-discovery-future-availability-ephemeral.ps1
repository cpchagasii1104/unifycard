# run-service-discovery-future-availability-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-DISCOVERY-FUTURE-AVAILABILITY-SLICE-B.
...
$dbName = "unificard_discovery_availability_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e service discovery future availability..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-service-discovery-future-availability.ts
...
if ($exitCode -ne 0) { Write-Host "e2e service discovery future availability falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e service discovery future availability verde." -ForegroundColor Green
exit 0
