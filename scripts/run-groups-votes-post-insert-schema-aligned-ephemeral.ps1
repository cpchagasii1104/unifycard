# run-groups-votes-post-insert-schema-aligned-ephemeral.ps1
# Orquestrador do E2E F-GROUPS-VOTES-POST-INSERT-SCHEMA-DRIFT. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_votes_schema_drift_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e groups-votes-post-insert-schema-aligned..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-groups-votes-post-insert-schema-aligned.ts
...
if ($exitCode -ne 0) { Write-Host "e2e groups-votes-post-insert-schema-aligned falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e groups-votes-post-insert-schema-aligned verde." -ForegroundColor Green
exit 0
