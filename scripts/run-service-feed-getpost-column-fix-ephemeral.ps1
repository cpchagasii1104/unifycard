# run-service-feed-getpost-column-fix-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-FEED-GETPOST-COLUMN-FIX.
...
$dbName = "unificard_service_feed_getpost_fix_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e service feed getpost column fix..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-service-feed-getpost-column-fix.ts
...
if ($exitCode -ne 0) { Write-Host "e2e service feed getpost column fix falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e service feed getpost column fix verde." -ForegroundColor Green
exit 0
