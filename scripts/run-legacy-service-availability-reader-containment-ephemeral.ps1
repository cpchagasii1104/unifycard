# run-legacy-service-availability-reader-containment-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-AVAILABILITY-LEGACY-SERVICE-OWNER-READER-CONTAINMENT-SLICE-A2.
...
$dbName = "unificard_legacy_service_avail_containment_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e legacy service availability reader containment..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-legacy-service-availability-reader-containment.ts
...
if ($exitCode -ne 0) { Write-Host "e2e legacy service availability reader containment falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e legacy service availability reader containment verde." -ForegroundColor Green
exit 0
