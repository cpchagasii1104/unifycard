# run-service-booking-confirm-canonical-lock-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-BOOKING-CONFIRM-CANONICAL-LOCK-SLICE-A1. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_booking_confirm_lock_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e service-booking confirm canonical lock..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-service-booking-confirm-canonical-lock.ts
...
if ($exitCode -ne 0) { Write-Host "e2e service-booking confirm canonical lock falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e service-booking confirm canonical lock verde." -ForegroundColor Green
exit 0
