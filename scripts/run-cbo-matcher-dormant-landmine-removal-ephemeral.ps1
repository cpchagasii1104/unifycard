# run-cbo-matcher-dormant-landmine-removal-ephemeral.ps1
# Orquestrador do E2E F-CBO-MATCHER-DORMANT-LANDMINE-REMOVAL.
...
$dbName = "unificard_cbo_matcher_removal_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e cbo matcher dormant landmine removal..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-cbo-matcher-dormant-landmine-removal.ts
...
if ($exitCode -ne 0) { Write-Host "e2e cbo matcher dormant landmine removal falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e cbo matcher dormant landmine removal verde." -ForegroundColor Green
exit 0
