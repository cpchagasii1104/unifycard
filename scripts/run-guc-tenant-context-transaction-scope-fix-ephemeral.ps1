# run-guc-tenant-context-transaction-scope-fix-ephemeral.ps1
# Orquestrador do E2E F-GUC-TENANT-CONTEXT-TRANSACTION-SCOPE-FIX.
# DB EFEMERA, migra FULL, roda, DROPA. Cria/dropa um role probe temporario (login+senha aleatoria,
# nunca persiste fora deste processo).
...
$dbName = "unificard_guc_tenant_context_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e guc tenant context transaction scope fix..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-guc-tenant-context-transaction-scope-fix.ts
...
if ($exitCode -ne 0) { Write-Host "e2e guc tenant context transaction scope fix falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e guc tenant context transaction scope fix verde." -ForegroundColor Green
exit 0
