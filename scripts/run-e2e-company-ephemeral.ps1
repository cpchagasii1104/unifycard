# run-e2e-company-ephemeral.ps1
# Orquestrador do E2E company (G2) em DB EFEMERA — usado p/ provar DECISION-0090 Fase 2.4
# (approved legado DESLIGADO). Cria DB efemera, migra FULL, roda o E2E e DROPA. NUNCA toca unificard_dev.
...
$dbName = "unificard_e2e_company_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "E2E company (G2) — approved legado DESLIGADO..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-company.ts
...
if ($exitCode -ne 0) { Write-Host "E2E company falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "E2E company verde (approved legado desligado)." -ForegroundColor Green
exit 0
