# run-service-order-direct-create-containment-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-ORDER-DIRECT-CREATE-AUTHORITY-CONTAINMENT.
# Cria DB EFEMERA, migra FULL, roda o teste de contencao e DROPA. NUNCA toca unificard_dev.
...
$dbName = "unificard_service_order_direct_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e service-order direct create containment..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-service-order-direct-create-containment.ts
...
if ($exitCode -ne 0) { Write-Host "e2e service-order direct containment falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e service-order direct containment verde." -ForegroundColor Green
exit 0
