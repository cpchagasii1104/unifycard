# run-pj-kyb-revocation-cascade-ephemeral.ps1
# Orquestrador da validação F-PJ-KYB-APPROVED-REVOCATION-WRITER
# (revogação KYB approved→suspended|closed + cascata de publicação/projeção — DECISION-0101).
...
$dbName = "unificard_kyb_revocation_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-KYB-APPROVED-REVOCATION-WRITER..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-kyb-revocation-cascade.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-KYB-APPROVED-REVOCATION-WRITER falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-KYB-APPROVED-REVOCATION-WRITER verde." -ForegroundColor Green
exit 0
