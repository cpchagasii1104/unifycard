# run-service-offering-canonical-binding-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-OFFERING-CANONICAL-BINDING. DB EFEMERA, migra FULL, roda, DROPA.
...
$dbName = "unificard_service_offering_binding_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e service-offering canonical binding..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-service-offering-canonical-binding.ts
...
if ($exitCode -ne 0) { Write-Host "e2e service-offering binding falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e service-offering binding verde." -ForegroundColor Green
exit 0
