# run-core-feed-batch-post-id-column-fix-ephemeral.ps1
# Orquestrador do E2E F-CORE-FEED-BATCH-GETPOSTSBATCH-COLUMN-FIX.
...
$dbName = "unificard_core_feed_batch_fix_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e core feed batch post_id column fix..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-core-feed-batch-post-id-column-fix.ts
...
if ($exitCode -ne 0) { Write-Host "e2e core feed batch post_id column fix falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e core feed batch post_id column fix verde." -ForegroundColor Green
exit 0
