# run-discovery-has-availability-canonical-filter-ephemeral.ps1
# Orquestrador do E2E F-SERVICE-DISCOVERY-HAS-AVAILABILITY-CANONICAL-FILTER-SLICE-A2D.
...
$dbName = "unificard_discovery_has_avail_filter_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "e2e discovery has_availability canonical filter..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-discovery-has-availability-canonical-filter.ts
...
if ($exitCode -ne 0) { Write-Host "e2e discovery has_availability canonical filter falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "e2e discovery has_availability canonical filter verde." -ForegroundColor Green
exit 0
