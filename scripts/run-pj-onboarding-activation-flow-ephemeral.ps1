# run-pj-onboarding-activation-flow-ephemeral.ps1
# Orquestrador do E2E ENCADEADO F-PJ-ONBOARDING-ACTIVATION-FLOW:
#   GET catálogo → escolhe par real → POST ativação → primary_* persistido + invariantes.
...
$dbName = "unificard_onboarding_flow_$stamp"
...
try {
    ...
    try {
        ...
        Write-Host "teste F-PJ-ONBOARDING-ACTIVATION-FLOW (cadeia catalogo->ativacao)..." -ForegroundColor Yellow
        & npx tsx src/scripts/validate-pipeline-e2e-pj-onboarding-activation-flow.ts
...
if ($exitCode -ne 0) { Write-Host "F-PJ-ONBOARDING-ACTIVATION-FLOW falhou (exit $exitCode)" -ForegroundColor Red; exit $exitCode }
Write-Host "F-PJ-ONBOARDING-ACTIVATION-FLOW verde." -ForegroundColor Green
exit 0
