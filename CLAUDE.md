...
| **Migrations** | Lei 2 — **forward-only**; tag `GENESIS_CONSTITUCIONAL_v1` existe. ⚠️ O que a Lei protege de edição é o **GENESIS** (0001-0005) e migration **já aplicada** em ambiente persistente — verificado na fonte pela auditoria de 2026-08-01, quando este roteador dizia "nunca editar migration existente" (mais estrito que a Lei) e quase transformou em violação um ato legítimo: reescrever migration **nunca aplicada** (`schema_migrations` prova) em vez de criar sucessora que deixaria lixo. Roteador não endurece norma |
...
| **migration** | **forward-only** (Lei 2). Nunca editar o GENESIS nem migration **já aplicada**; nunca aplicada em lugar nenhum (`schema_migrations` é a prova) pode ser reescrita — sucessora para desfazer o que ninguém rodou é lixo. Formato canônico `YYYYMMDDHHMMSS_desc.sql` | ver `backend/migrations/README.md` (numeração, sufixo alfabético, gaps) |
