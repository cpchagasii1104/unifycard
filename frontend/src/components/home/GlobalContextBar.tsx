...
export default function GlobalContextBar({ onContextChange }: GlobalContextBarProps) {
  ...
  // REGRA CRÍTICA: Só fazer chamadas quando sessão estiver pronta E activeActor definido E tiver escopo bancário
  useEffect(() => {
    ...
    // 🔴 GUARD CRÍTICO: Verificar escopo bancário ANTES de executar loadBalances
    // Apenas actors do tipo 'user' têm acesso bancário garantido
    // Actors 'page' (empresas) podem não ter conta bancária ativa
    const hasBankAccess = activeActor.actor_type === 'user';
    
    if (!hasBankAccess) {
      // Sem escopo bancário: NÃO executar loadBalances, apenas definir como indisponível
      setWalletBalance(null);
      setRegionalFundBalance(null);
      setIsLoadingBalances(false);
      return; // 🔴 CRÍTICO: Retornar ANTES de executar loadBalances
    }
    ...
    const loadBalances = async () => {
      ...
      // ✅ Guard já foi verificado no useEffect - actor tem escopo bancário garantido
      ...
      try {
        ...
        try {
          ...
          // ✅ Guard já verifica escopo - se chegou aqui, é erro inesperado
          ...
          {!isLoadingBalances && (
            <>
              {walletBalance !== null ? (
                ...
                <div className="context-balance-item context-balance-unavailable">
                  <span className="context-balance-label">Saldo:</span>
                  <span className="context-balance-value">Indisponível</span>
                </div>
              )}
