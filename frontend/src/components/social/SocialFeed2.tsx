...
export default function SocialFeed2() {
  ...
  const loadSidebarData = async () => {
    ...
    try {
      const suggestions = await getGroupSuggestions(3);
      setGroupSuggestions(suggestions.groups || []);
    } catch (err) {
      console.warn('Erro ao carregar sugestões de grupos:', err);
    } finally {
      setGroupsLoading(false);
    }

    // Carregar dados do fundo regional
    setFundLoading(true);
    try {
      const dashboard = await getFundDashboard(30);
      setFundData(dashboard);
    } catch (err) {
      console.warn('Erro ao carregar dados do fundo:', err);
    } finally {
    ...
    try {
      ...
      try {
        ...
        if (unifiedData.items) {
          ...
          const uniqueEvents = Array.from(
            new Map(standaloneEvents.map(e => [e.eventId, e])).values()
          );
          setEvents(uniqueEvents);
        } else if (unifiedData.posts) {
          // Fallback: compatibilidade com formato antigo
          const eventItems = unifiedData.posts
            .filter((p) => p.eventId && p.event && (p.event.status === 'PUBLISHED' || p.event.status === 'ONGOING'))
            .map((p) => ({
              eventId: p.event!.id,
              title: p.event!.title,
      ...
      if (!nextCursor) {
        try {
          ...
          // Não bloqueia o feed se eventos culturais falharem
                ...
                <div className="account-balance">
                  <div className="balance-amount">
                    {new Intl.NumberFormat('pt-BR', {
                      style: 'currency',
                      currency: myAccount.currency || 'BRL',
                    }).format(myAccount.balance)}
                  </div>
                  <div className="account-status">
                    <span className={`status-badge status-${myAccount.status === 'active' ? 'active' : 'inactive'}`}>
                      {myAccount.status === 'active' ? 'Conta ativa' : 'Conta inativa'}
                    </span>
                  </div>
                </div>
              ) : (
                <div className="widget-empty">Conta não disponível</div>
              )}
            </div>
          </div>

          {/* Bloco 2: Sugestões de Comunidade */}
          <div className="sidebar-section">
            <h3>Sugestões de Comunidade</h3>
            <div className="widget-content">
              {groupsLoading ? (
                <div className="widget-loading">Carregando...</div>
                ...
                <div className="widget-empty">Nenhuma sugestão disponível</div>
              )}
            </div>
          </div>

          {/* Bloco 3: Economia Local */}
          <div className="sidebar-section">
            ...
            <div className="widget-content">
              {fundLoading ? (
                  ...
                  <div className="fund-balance">
                    ...
                    <div className="fund-amount">
                      {new Intl.NumberFormat('pt-BR', {
                        style: 'currency',
                        currency: 'BRL',
                      }).format(fundData.summary.currentBalance)}
                    </div>
                  </div>
                  <button
                ...
                <div className="widget-empty">Dados não disponíveis</div>
              )}
            </div>
          </div>
        </aside>
