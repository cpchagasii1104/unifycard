...
import { type CulturalEvent, getCheckInStatus, checkInToEvent, type CheckInStatus, getCheckInCount, type CheckInResponse } from '../../api/cultural';
import { validateActiveActor, safeApiCall, safeNumber } from '../../utils/guardrails';
import { devLog } from '../../utils/devLog';
import { showToast } from '../common/Toast';
import EventCheckoutModal from '../events/EventCheckoutModal';
import EventCheckInModal from '../events/EventCheckInModal';
...
export default function CulturalEventCard({
  ...
  // Bloco 3.3: Check-in com guardrails e exibição de impacto
  const handleCheckIn = async () => {
    if (!validateActiveActor(activeActor) || isCheckingIn) {
      if (!validateActiveActor(activeActor)) {
        showToast('Você precisa estar autenticado para fazer check-in', 'error');
      }
      return;
    }
    ...
    try {
      // Bloco 3.3: Usar safeApiCall para check-in
      const result = await safeApiCall(
        async () => checkInToEvent(event.id, {
          method: 'AUTO', // Método automático (pode ser QR_CODE ou MANUAL no futuro)
          target_actor_id: activeActor!.actor_id,
          target_actor_type: activeActor!.actor_type as 'user' | 'page',
        }),
        null,
      ...
      if (!result || !result.success) {
        throw new Error((result as any)?.error || 'Erro ao fazer check-in');
      }

      // Bloco 3.3: Salvar resultado para exibir no modal
      ...
      // Bloco 3.3: Exibir modal de confirmação com impacto
      ...
      // Bloco 3.3: Toast de sucesso
      const impactGenerated = safeNumber(result.impact_generated, 0);
      if (impactGenerated > 0) {
        showToast('Check-in realizado! Impacto gerado no ecossistema.', 'success');
      } else {
      ...
      devLog.error('Erro ao fazer check-in:', err);
      ...
      <div className="event-actions">
        ...
        {!((event as any).stateInfo) && 
         ...
         (event.status === 'PUBLISHED' || event.status === 'CONFIRMED') && (
          ...
          >
            🎟️ Comprar Ingresso ({new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(event.ticket_price_cents / 100)})
          </button>
        )}
      </div>

      {/* Modal de checkout - Bloco 3.2: Processa compra real */}
      {showCheckout && (
        <EventCheckoutModal
          event={event}
      ...
      {showCheckInModal && checkInResult && (
        <EventCheckInModal
          ...
          onBackToFeed={() => {
            setShowCheckInModal(false);
            setCheckInResult(null);
            // Recarregar status de check-in para atualizar UI
            if (validateActiveActor(activeActor)) {
              getCheckInStatus(event.id)
                .then(setCheckInStatus)
                .catch((err) => {
                  devLog.warn('Erro ao recarregar status de check-in:', err);
                });
            }
          }}
        />
