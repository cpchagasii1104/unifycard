...
| `DT-DB-GUARANTEE-SWEEP-INCOMPLETE` | antes da **próxima migração de substrato**. ✅ **PAGO para os dois pares que o nomearam:** `bookings`+`availability` (varredura completa no cartório) e **o par da locação** — migration `20260806235000` levou **12 garantias** que ficaram em `rentable_resources`, **4 delas de dinheiro**. 🔴 Agora tem guard **dinâmico e GENÉRICO**: `audit-rental-guarantee-parity` compara o par **por COLUNA COMUM**, não por lista de nomes. ⚠️ **Um pacote pediu para marcar esta dívida como PAGA sem comparar o conteúdo dos CHECKs** — teria enterrado as 12. O resto do banco (outros pares migrados, FKs, índices parciais) segue **`?`, não `0`** |
