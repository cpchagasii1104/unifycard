...
**Data**: 2026-05-07 (atualizado 2026-05-12 — §20/§21; atualizado 2026-05-13 — §22 reconciliação, §-5 AGENT PROTOCOL obrigatório, §-4 visão fundacional, §23 como pensar antes de codar, §24 camadas N0/N1/N2/CATEGORIES/CONCEPT; atualizado 2026-05-24 — §-3 sub-seção "Lições da retomada pós-marco-zero" com 5 lições materiais para próxima IA não re-descobrir; atualizado 2026-05-24 — §-3 sub-seção "Lições da sessão social" com 5 lições materiais novas: ver-vence-deduzir levado ao runtime, grep callers antes de mexer em signature, mesmo bug em camadas, assumed-fantasma como inverso do drift, reauditoria de dimensionamento de DT antiga)
...
### Lições da sessão social (2026-05-24)

Cinco lições materiais consolidadas na sessão de destrave do módulo social (6 commits funcionais: Fatia getFeed + A1 + A2 + B + C + D, ancorada em 4 DECISIONs pré-existentes de 2026-05-19). Para próxima IA não re-descobrir.

6. **Ver vence deduzir — levado ao runtime, não só à leitura** — para ESCRITA: leitura estática cuidadosa + tsc verde + 4 gates verdes + grep órfão zero **não basta**. HTTP 500 do PostgreSQL é o detector final. Aplicado retroativamente na fatia getFeed (cobri 9 drifts por leitura, runtime expôs 2 a mais — `g.group_id` e `g.visibility` — que a leitura não pegou); depois aplicado preditivamente em A1/A2/B/C/D via SELECT confirmatório direto no banco. **Para escrita, "rodou sem erro" ≠ "gravou certo"** — 3 ramificações de toggleReaction (INSERT/DELETE/UPDATE) cada uma com SELECT pós para confirmar colunas/valores canônicos. Sem o SELECT, não distinguiria "gravou certo" de "gravou no lugar errado e não bateu CHECK" (família do bug do resolver gravando `'completed'` fora do enum).

7. **Grep callers ANTES de mexer em signature** (memória persistente: `feedback_grep_callers_antes_de_mexer_em_signature.md`) — "Não usado no corpo do método" ≠ "Não passado por nenhum caller". Antes de remover/renomear param em método (especialmente ESCRITA), grep TODOS os callers no codebase inteiro (backend/scripts/workers/tests) e confirmar quantos argumentos cada um passa. **Custo: 10s de grep. Custo de pular: deslocar argumento silenciosamente em escrita crítica (família do resolver).** Origem A1: removi 2 params órfãos sem grep prévio e acertei por sorte (único caller backend era o que eu já estava editando). A2/B/C/D aplicaram como protocolo. Confirmação inversa em B: grep revelou 6 callers passando args nas posições — preservação obrigatória, salvou refator amplo errado.

8. **"Mesmo bug em camadas diferentes da cadeia" como categoria diagnóstica** — antes de classificar dois sintomas como bugs independentes, mapear a cadeia para ver se compartilham origem. Fatia D provou que o gate de authority `canActAs` rejeitando "Actor has capability but user lacks access" **não era bug independente** — era o mesmo bug do slot dual da rota observado em outra camada. Hipótese de "ownership/delegation ausente para dev user" REFUTADA materialmente: dev actor sempre teve `actor.user_id` correto e `actor_registry.capabilities_json.can_publish_feed: true`. **Authority/registry/seed: zero mudanças necessárias.** Reduziu o escopo da Fatia D de "refator de signature em 7 arquivos + investigação de ownership" para "1 linha em 1 arquivo + smoke material em 2 caminhos". Padrão estrutural: dois sintomas em camadas adjacentes que parecem independentes podem ser o mesmo bug observado de ângulos diferentes — mapear a cadeia antes de classificar.

9. **"Assumed-fantasma" como inverso do drift de schema** — categoria nova descoberta na Fatia C. Drift clássico de schema: código referencia coluna/tabela que não existe no DDL. Inverso: **frontend declara endpoint backend como FANTASMA quando o endpoint EXISTE**. Caso material: `frontend/src/api/social.ts:133` faz `throw new Error('NOT_IMPLEMENTED: getComments — endpoint backend ausente. DT-PRESSURE-COMMENTS-FANTASMA pendente')` quando o endpoint backend `/social/posts/:id/comments` está vivo há tempos (rota `social-2.0.routes.ts:340`, HTTP 200 confirmado pela Fatia C). Drift institucional do outro lado da cadeia — frontend mentindo sobre disponibilidade do backend. Categoria para grep futuro: helpers stub que declaram features ausentes podem estar errados em ambas as direções (declarar como existente quando ausente OU declarar como ausente quando existente).

10. **Decisão arquitetural pode estar tomada há dias e ninguém ter executado** — repetição do padrão Fase 1 da DECISION-0032 (lição #5 da retomada anterior). Aqui as 4 DECISIONs do social (0031/0032-social/0033/0034) estavam tomadas no mesmo dia 19/05 da DT-DRIFT-SOCIAL-2.0 que disse "2-4 sessões dedicadas". A DT foi escrita antes de saber que as decisões existiam. Reaplicação da lição #5: grep no DECISIONS_LOG por termo do tema **antes** de aceitar o dimensionamento de uma DT antiga. Padrão estrutural confirmado em 2 frentes consecutivas (payment + social). **Toda DT antiga merece reauditoria do dimensionamento contra DECISIONs registradas em datas próximas** — pode ter encolhido sem que a DT registre.

**Princípio condensado das 5 lições novas:** disciplina cresce contra a tentação de extrapolar (deduzir, classificar dois sintomas como dois bugs, aceitar escopo escrito antes de evidência). Ver é mais barato que deduzir. Greppar é mais barato que sortear. Mapear a cadeia é mais barato que classificar por nome. Reauditar dimensionamento é mais barato que aceitar escopo escrito. **A sessão social fez 6 commits funcionais convergindo todo o módulo SQL para schema canônico sem pagar nenhuma decisão arquitetural nova** — toda decisão soberana já estava no log; o trabalho foi executar com disciplina material.
