...
export default function CTAModal({ cta, postId: _postId, postIntent, onClose, onConfirm }: CTAModalProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const result: ConfirmCTAResponse = await confirmCTA(cta.cta_id, {
      ...
      // FASE 10: Disparar evento de mudança de impacto
      if (activeActor) {
      ...
      setStep('error');
      showToast('Erro ao confirmar ação. Tente novamente.', 'error');
    }
  };
  ...
  if (step === 'success') {
    ...
    // Calcular total baseado nos entries reais ou preço do CTA
    const totalAmount = confirmationResult?.revenue_entry?.amount_cents || 
                       (cta.price ? Math.round(cta.price * 100) : 0);
