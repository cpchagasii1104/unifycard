      ...
      try {
        ...
        if (continueResult.status === 'fulfilled') {
          ...
          if (statementResult.entries.length > 0) {
            const recentTransaction = statementResult.entries[0];
          ...
          if (eventsResult.events.length > 0) {
            const upcomingEvent = eventsResult.events[0];
          ...
          console.warn('Erro ao carregar itens de continuidade:', continueResult.reason);
        }
