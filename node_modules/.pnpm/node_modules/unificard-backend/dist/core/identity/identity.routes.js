...
const identityRoutes = async (fastify) => {
    ...
    fastify.post('/update', {
        ...
        try {
            ...
            let validated;
            try {
                validated = identity_schemas_1.updateGlobalIdentitySchema.parse(req.body);
            }
            catch (zodError) {
                fastify.log.error({
                    zodError,
            ...
            const verifyAfterSave = await identity_service_1.identityService.getGlobalIdentity(profile.global.globalUserId);
            fastify.log.info({
                globalUserId: profile.global.globalUserId,
                ...
                match: verifyAfterSave ? (verifyAfterSave.fullName === response.fullName &&
                    (verifyAfterSave.birthdate ? (verifyAfterSave.birthdate instanceof Date ? verifyAfterSave.birthdate.toISOString().substring(0, 10) : String(verifyAfterSave.birthdate).substring(0, 10)) : null) === response.birthdate) : false,
            }, '🔍 VERIFICAÇÃO FINAL: Dados após salvar vs dados enviados na resposta');
            // 🔴 CRÍTICO: Se os dados não correspondem, lançar erro
            if (verifyAfterSave && (verifyAfterSave.fullName !== response.fullName ||
            ...
            const isValidationError = error instanceof Error && (error.message.includes('inválido') ||
    ...
    fastify.get('/configurations', async (req, reply) => {
        ...
        try {
            ...
            const userType = globalUser.metadata?.userType || 'physical';
            return { userType };
        }
        catch (error) {
