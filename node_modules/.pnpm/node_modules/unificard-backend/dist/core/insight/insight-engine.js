...
exports.insightEngine = exports.InsightEngine = void 0;
...
class InsightEngine {
     ...
     */
    generateProjectionInsights(events) {
        const insights = [];
        // Filtrar apenas eventos de pagamento processado
        const paymentEvents = events.filter((e) => e.eventType === 'payment.processed' && e.amount !== undefined);
        if (paymentEvents.length === 0) {
        ...
        const recentEvents = paymentEvents.filter((e) => new Date(e.occurredAt) >= thirtyDaysAgo);
     ...
     */
    generateAnomalyInsights(events) {
        const insights = [];
        // Filtrar apenas eventos de pagamento processado
        const paymentEvents = events.filter((e) => e.eventType === 'payment.processed' && e.amount !== undefined);
        if (paymentEvents.length === 0) {
        ...
        const recentEvents = paymentEvents.filter((e) => new Date(e.occurredAt) >= fourWeeksAgo);
