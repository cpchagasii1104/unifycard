...
const createDonationSchema = zod_1.z.object({
    targetType: zod_1.z.enum(['user', 'project', 'group'], {
...
const donationRoutes = async (fastify) => {
    /**
     * POST /bank/donate
     ...
     * - 500: Erro inesperado
     */
    fastify.post('/donate', async (req, reply) => {
        ...
        try {
            // 4. Criar doação
            const result = await donation_service_1.donationService.createDonation(tenantId, {
                fromUserId,
                targetType,
                targetId,
                amount,
