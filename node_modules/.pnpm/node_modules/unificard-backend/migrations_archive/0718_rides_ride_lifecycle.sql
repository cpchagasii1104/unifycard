/*
Arquivo: 011_rides_part3_ride_lifecycle.sql
Projeto: UnifyCard
Banco: PostgreSQL 14+
Escopo: Ciclo de vida das corridas

Objetivo:
- Controlar pedidos, ofertas, corridas e eventos
- Registrar histórico e telemetria

Dependências:
- rides_drivers, rides_vehicles, geography
*/

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "postgis";

-- =========================================================
-- FUNÇÃO PADRÃO updated_at (defensivo)
-- =========================================================

CREATE OR REPLACE FUNCTION update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = now();
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- =========================================================
-- RIDE REQUESTS
-- =========================================================

CREATE TABLE IF NOT EXISTS rides_ride_requests (
  request_id      UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  tenant_id       UUID NOT NULL REFERENCES tenants(id),

  passenger_id    UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,

  origin          GEOGRAPHY(POINT) NOT NULL,
  destination     GEOGRAPHY(POINT) NOT NULL,

  passenger_count INTEGER NOT NULL DEFAULT 1 CHECK (passenger_count > 0),

  stops           JSONB NOT NULL DEFAULT '[]'::jsonb,
  stops_count     INTEGER GENERATED ALWAYS AS (
    jsonb_array_length(COALESCE(stops, '[]'::jsonb))
  ) STORED,

  service_type_id UUID NOT NULL,

  estimated_distance_km  NUMERIC(10,2),
  estimated_duration_min INTEGER,

  status          VARCHAR(40) NOT NULL DEFAULT 'pending',

  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);

ALTER TABLE rides_ride_requests ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'rides_ride_requests'
      AND policyname = 'rides_ride_requests_rls'
  ) THEN
    CREATE POLICY rides_ride_requests_rls
      ON rides_ride_requests
      USING (tenant_id::text = current_setting('app.current_tenant', true));
  END IF;
END $$;

DROP TRIGGER IF EXISTS trg_rides_ride_requests_updated_at ON rides_ride_requests;
CREATE TRIGGER trg_rides_ride_requests_updated_at
  BEFORE UPDATE ON rides_ride_requests
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

-- =========================================================
-- REQUEST OFFERS
-- =========================================================

CREATE TABLE IF NOT EXISTS rides_request_offers (
  offer_id      UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  tenant_id     UUID NOT NULL REFERENCES tenants(id),
  request_id    UUID NOT NULL REFERENCES rides_ride_requests(request_id) ON DELETE CASCADE,
  driver_id     UUID NOT NULL REFERENCES rides_drivers(driver_id) ON DELETE CASCADE,

  status        VARCHAR(30) NOT NULL DEFAULT 'pending',
  created_at    TIMESTAMPTZ NOT NULL DEFAULT now(),

  UNIQUE (tenant_id, request_id, driver_id)
);

ALTER TABLE rides_request_offers ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'rides_request_offers'
      AND policyname = 'rides_request_offers_rls'
  ) THEN
    CREATE POLICY rides_request_offers_rls
      ON rides_request_offers
      USING (tenant_id::text = current_setting('app.current_tenant', true));
  END IF;
END $$;

-- =========================================================
-- RIDES
-- =========================================================

CREATE TABLE IF NOT EXISTS rides_rides (
  ride_id        UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  tenant_id      UUID NOT NULL REFERENCES tenants(id),

  request_id     UUID NOT NULL REFERENCES rides_ride_requests(request_id) ON DELETE CASCADE,
  driver_id      UUID NOT NULL REFERENCES rides_drivers(driver_id) ON DELETE CASCADE,
  vehicle_id     UUID NOT NULL REFERENCES rides_vehicles(vehicle_id),

  status         VARCHAR(40) NOT NULL DEFAULT 'assigned',

  accepted_at    TIMESTAMPTZ,
  arrived_at     TIMESTAMPTZ,
  started_at     TIMESTAMPTZ,
  completed_at   TIMESTAMPTZ,
  cancelled_at   TIMESTAMPTZ,

  total_distance_km   NUMERIC(10,2),
  total_duration_min INTEGER,
  final_price        NUMERIC(10,2),

  created_at     TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at     TIMESTAMPTZ NOT NULL DEFAULT now()
);

ALTER TABLE rides_rides ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'rides_rides'
      AND policyname = 'rides_rides_rls'
  ) THEN
    CREATE POLICY rides_rides_rls
      ON rides_rides
      USING (tenant_id::text = current_setting('app.current_tenant', true));
  END IF;
END $$;

DROP TRIGGER IF EXISTS trg_rides_rides_updated_at ON rides_rides;
CREATE TRIGGER trg_rides_rides_updated_at
  BEFORE UPDATE ON rides_rides
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

-- =========================================================
-- RIDE LOCATIONS (TELEMETRIA)
-- =========================================================

CREATE TABLE IF NOT EXISTS rides_ride_locations (
  ride_location_id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  tenant_id        UUID NOT NULL REFERENCES tenants(id),
  ride_id          UUID NOT NULL REFERENCES rides_rides(ride_id) ON DELETE CASCADE,

  location         GEOGRAPHY(POINT) NOT NULL,
  speed_kmh        NUMERIC(10,2) CHECK (speed_kmh >= 0),
  heading          INTEGER,
  recorded_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);

ALTER TABLE rides_ride_locations ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'rides_ride_locations'
      AND policyname = 'rides_ride_locations_rls'
  ) THEN
    CREATE POLICY rides_ride_locations_rls
      ON rides_ride_locations
      USING (tenant_id::text = current_setting('app.current_tenant', true));
  END IF;
END $$;

-- =========================================================
-- RIDE EVENTS
-- =========================================================

CREATE TABLE IF NOT EXISTS rides_ride_events (
  event_id      UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  tenant_id     UUID NOT NULL REFERENCES tenants(id),
  ride_id       UUID NOT NULL REFERENCES rides_rides(ride_id) ON DELETE CASCADE,

  event_type    VARCHAR(100) NOT NULL,
  payload       JSONB NOT NULL DEFAULT '{}',

  created_at    TIMESTAMPTZ NOT NULL DEFAULT now()
);

ALTER TABLE rides_ride_events ENABLE ROW LEVEL SECURITY;

DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'rides_ride_events'
      AND policyname = 'rides_ride_events_rls'
  ) THEN
    CREATE POLICY rides_ride_events_rls
      ON rides_ride_events
      USING (tenant_id::text = current_setting('app.current_tenant', true));
  END IF;
END $$;
