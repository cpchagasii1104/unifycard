...
const tenantPluginImpl: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.addHook('preHandler', async (req, reply) => {
    ...
    let jwtPayload: any;
    try {
      jwtPayload = jwt.decode(token, { complete: false });
    } catch (err) {
    ...
    if (!jwtTenantId || typeof jwtTenantId !== 'string') {
      ...
      if (isDev) {
        ...
        const isApiRoute =
          urlPathForApiCheck.startsWith('/marketplace') ||
        ...
        if (isApiRoute) {
          ...
          return reply.status(400).send({
            ...
            code: 'MISSING_TENANT',
            message:
              'API não deve ser acessada diretamente no browser. Use o frontend em http://localhost:5173',
            devHint:
              'Esta é uma API REST. Acesse http://localhost:5173 para usar a interface web.',
          });
        }
        ...
        return reply.status(400).send({
          ...
          code: 'MISSING_TENANT',
          message:
            'Tenant ausente. Faça login pelo frontend em http://localhost:5173',
        });
      }

      // 🔴 PRODUÇÃO: rígido, sem exceção
      throw fastify.httpErrors.badRequest(
        'Missing tenant context (JWT tenantId not found)'
      );
    }
