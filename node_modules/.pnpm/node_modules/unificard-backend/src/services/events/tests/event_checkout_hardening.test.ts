// src/services/events/tests/event_checkout_hardening.test.ts
//
...
describe('Event Checkout Hardening', () => {
  ...
  beforeAll(async () => {
    ...
    const { accountService } = await import('../../../core/economy/accounts/account.service');
  ...
  describe('Split de Eventos - EVENT_ORGANIZER', () => {
    it('Deve enviar 70% do split para o organizador (usuário)', async () => {
      ...
      const organizerTransactions = await runQueriesWithTenant<{
        ...
        tenantId,
        `
          SELECT transaction_id, amount, to_account
          FROM transactions
          WHERE to_account = $1
      ...
      expect(parseFloat(organizerTx.amount)).toBeCloseTo(testEvent.ticketPrice * 0.70, 2);
    });
    ...
    it('Deve enviar 70% do split para o organizador (usuário)', async () => {
      ...
      expect(parseFloat(userTransactions[0].amount)).toBeCloseTo(50.00 * 0.70, 2);
    });
