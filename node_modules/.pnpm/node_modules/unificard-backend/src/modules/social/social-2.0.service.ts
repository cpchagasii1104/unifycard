...
export class Social2Service {
  ...
  async getFeed(
    ...
    if (!currentActorId) {
      ...
      if (user) {
        const actor = await actorRepository.findOrCreateUserActor(tenantId, user.user_id, globalUserId);
    ...
    const calculateContentWeight = (row: any, mode: 'user' | 'page', userPreferences?: any, userLocation?: any): number => {
      ...
      if (metadata.event_id || intent === 'event') {
        baseWeight = 120;
    ...
    const postsWithScores = await Promise.all(rows.map(async (row) => {
      ...
      if (postMetadata.cultural_event_id || row.event_id) {
        try {
          const { culturalEventService } = await import('../cultural/cultural-event.service');
          const eventId = postMetadata.cultural_event_id || row.event_id;
  ...
  async createPost(
    ...
    if (actorId) {
      ...
      actor = await actorRepository.findOrCreateUserActor(tenantId, userId, globalUserId);
    }
    ...
    const permissions = await reputationService.getPermissions(
      tenantId,
      actor.actor_id,
      actor.actor_type as 'user' | 'page',
      companyStatus || undefined
    );

    // REGRA: Empresas PROVISIONAL não podem criar votações públicas
    if (intent === 'vote' && actor.actor_type === 'page' && companyStatus === 'PROVISIONAL') {
      throw new Error('Empresas em validação não podem criar votações públicas. Complete a validação presencial para habilitar esta funcionalidade.');
    }

    // FASE 11: Verificar permissão para votar (baseado em reputação)
    if (intent === 'vote' && !permissions.canVote) {
      throw new Error('Esta funcionalidade está disponível após uso contínuo da plataforma. Continue interagindo para desbloquear.');
    }

    // FASE 11: Verificar permissão para criar projetos
    if (intent === 'project' && !permissions.canCreateProject) {
      throw new Error('Criar projetos está disponível após uso contínuo da plataforma. Continue interagindo para desbloquear.');
    }

    // FASE 11: Verificar permissão para criar CTA
    if (cta && !permissions.canCreateCTA) {
      throw new Error('Criar ações (CTAs) está disponível após uso contínuo da plataforma. Continue interagindo para desbloquear.');
    }
    ...
    // FASE 10: Registrar impacto quando post é publicado
    try {
  ...
  async createComment(
    ...
    let actor = null;
    if (user) {
      actor = await actorRepository.findOrCreateUserActor(tenantId, user.user_id, globalUserId);
    }
   ...
   */
  async getUserActor(tenantId: string, globalUserId: string): Promise<{ actor_id: string } | null> {
    try {
      const user = await runQueryWithTenant<{ user_id: string }>(
        tenantId,
        `
