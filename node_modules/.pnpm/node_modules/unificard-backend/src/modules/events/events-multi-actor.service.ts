...
class EventsMultiActorService {
  ...
  async createEvent(
    ...
    // Criar evento
    const eventRows = await runQueryWithTenant<EventRow & {
      datetime_start: Date;
      datetime_end: Date;
      location_name: string | null;
      capacity: number | null;
  ...
  async addActorToEvent(
    ...
    const eventRows = await runQueryWithTenant<{ id: string; tenant_id: string }>(
      tenantId,
    ...
    const existingRows = await runQueryWithTenant<{ id: string }>(
      tenantId,
  ...
  async getEventsByActor(
    ...
    const countRows = await runQueryWithTenant<{ total: string }>(
      tenantId,
      countQuery,
      countParams
    );
