...
async function fixPizzaioloHierarchy() {
  ...
  try {
    ...
    const gastronomiaResult = await pool.query(
      `
    ...
    const cozinharResult = await pool.query(
      `
      SELECT category_id, name, slug, level, parent_id, path
      FROM categories
    ...
    // Buscar Pizzaiolo
    const pizzaioloResult = await pool.query(
      `
      SELECT category_id, name, slug, level, parent_id, path
      FROM categories
    ...
    async function calculatePathRecursive(categoryId: string): Promise<string[]> {
      const catResult = await pool.query(
        `SELECT category_id, parent_id, slug FROM categories WHERE category_id = $1`,
        [categoryId]
      );
