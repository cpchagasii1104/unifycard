...
export async function executeHireService(
  ...
  try {
    ...
    if (!categoryId && typeof serviceType === 'string') {
      const categories = await categoriesService.searchCategories(serviceType, 1);
...
export async function executeBuyProduct(
  ...
  try {
    ...
    if (!categoryId && parameters.category) {
      const categories = await categoriesService.searchCategories(parameters.category, 1);
      if (categories.length > 0) {
        categoryId = categories[0].categoryId;
      }
    }
