...
class RegionalFundGovernanceService {
  ...
  private async transferWithClient(
    ...
    const existingTx = await client.query(
      ...
      [eventId]
    ) as { rows: Array<{ transaction_id: string }> };
    ...
    const accountsMap = new Map(
      accountsResult.rows.map((row: any) => [row.account_id, parseFloat(row.balance)])
    );

    const accountsInfoMap = new Map(
      accountsResult.rows.map((row: any) => [
        row.account_id,
        {
    ...
    const fromAccountInfo = accountsInfoMap.get(fromAccount) as { balance: number; ownerType: string } | undefined;
    ...
    const txResult = await client.query(
      ...
      [tenantId, fromAccount, toAccount, fromGlobalUserId, toGlobalUserId, amount, eventId, 'completed', JSON.stringify(metadata)]
    ) as { rows: Array<{ transaction_id: string }> };
  ...
  async executeProposal(tenantId: string, proposalId: string): Promise<{
    ...
    try {
      ...
      if (proposalData.target_type === 'project') {
        ...
        targetAccountId = await groupAccountService.createOrGetGroupAccount(
          tenantId,
