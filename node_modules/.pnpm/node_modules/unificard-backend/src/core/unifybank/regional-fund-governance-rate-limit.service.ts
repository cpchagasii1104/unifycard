...
class RegionalFundGovernanceRateLimitService {
  private readonly MAX_PROPOSALS_PER_MONTH = 3;
  private readonly MAX_VOTES_PER_MINUTE = 10;

  /**
   * Verifica rate limit para criar proposta (3/mês)
   */
  async checkProposalRateLimit(
    tenantId: string,
    globalUserId: string
  ): Promise<{ allowed: boolean; currentCount: number; maxCount: number; resetAt: Date }> {
    const now = new Date();
