...
const categoryReviewRoutes: FastifyPluginAsync = async (fastify) => {
  /**
  ...
  fastify.get('/pending', {
    ...
    try {
      const categories = await categoryReviewService.getPendingCategories();
      const count = await categoryReviewService.getPendingCount();
  ...
  fastify.post<{
    ...
    try {
      const category = await categoryReviewService.approveCategory({
  ...
  fastify.post<{
    ...
    try {
      const category = await categoryReviewService.rejectCategory({
        categoryId: req.params.id,
        adminId: req.user.id || req.user.globalUserId || '',
        tenantId: req.tenant.id,
...
export default categoryReviewRoutes;
