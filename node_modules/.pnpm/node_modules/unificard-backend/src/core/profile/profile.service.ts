...
import type { Profile, ProfileRow, UpdateProfileInput } from './profile.types';
...
class ProfileService {
  ...
  async getProfile(tenantId: string, userId: string): Promise<Profile | null> {
    ...
    const txidInGet = await runQueryWithTenant<{ txid: string }>(
      tenantId,
    ...
    const countResult = await runQueryWithTenant<{ count: string }>(
      tenantId,
      `SELECT COUNT(*) as count FROM profiles WHERE tenant_id = $1 AND user_id = $2`,
      [tenantId, userId]
    );
    ...
    const row = await runQueryWithTenant<ProfileRow & { txid: string }>(
      tenantId,
      `
    ...
    console.log('[ProfileService] 🔍 TXID NO GET:', {
      ...
      userId,
      txid: txidInGet?.txid,
      txidInRow: row ? (row as any).txid : null,
    });
  ...
  async upsertProfile(
    ...
    const txidBefore = await runQueryWithTenant<{ txid: string }>(
      tenantId,
      `SELECT txid_current() as txid`
    );
    ...
    const txidAfter = await runQueryWithTenant<{ txid: string }>(
      tenantId,
      `SELECT txid_current() as txid`
    );
    ...
    const verifyRow = await runQueryWithTenant<ProfileRow & { txid: string }>(
      tenantId,
      `
        SELECT profile_id, tenant_id, user_id, full_name, phone, metadata, created_at, updated_at, txid_current() as txid
        FROM profiles
    ...
    console.log('[ProfileService] 🔍 TXID NO GET APÓS UPDATE:', {
      ...
      txidBefore: txidBefore?.txid,
      txidAfter: txidAfter?.txid,
      txidInVerify: verifyRow ? (verifyRow as any).txid : null,
      txidConsistent: txidBefore?.txid === (verifyRow as any)?.txid || txidAfter?.txid === (verifyRow as any)?.txid,
    });
