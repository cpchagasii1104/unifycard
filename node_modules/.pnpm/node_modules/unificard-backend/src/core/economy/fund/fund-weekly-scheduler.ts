...
class FundWeeklyScheduler {
  ...
  private lastRunWeek: string | null = null;
  ...
  private async checkAndGenerate(): Promise<void> {
    ...
    try {
      ...
      const tenantsResult = await pool.query<{ tenant_id: string }>(
        `
        SELECT tenant_id
        FROM tenants
        `
      ...
      for (const tenantId of tenants) {
        try {
          const report = await fundWeeklyReportService.generateWeeklyReport(tenantId);
          ...
          console.log(
            `[FundWeeklyScheduler] Relatório gerado para tenant ${tenantId}, semana ${currentWeek}`
          );
        } catch (error) {
          console.error(
            `[FundWeeklyScheduler] Erro ao gerar relatório para tenant ${tenantId}:`,
            error
          );
        }
      }
   ...
   */
  async forceGenerate(tenantId: string): Promise<void> {
    this.lastRunWeek = null; // Reset para forçar geração
    await this.checkAndGenerate();
  }
}

export const fundWeeklyScheduler = new FundWeeklyScheduler();
