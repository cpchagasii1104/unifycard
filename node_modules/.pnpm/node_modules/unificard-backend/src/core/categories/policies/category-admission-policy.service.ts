...
import type {
  ...
  CategoryDomainType,
  AIContext,
} from './category-admission-policy.types';
import {
  buildOccupationalClassificationPrompt,
  type OccupationalClassificationResult,
} from './occupational-classification.prompt';

class CategoryAdmissionPolicyService {
  // Listas de bloqueio explícito (BLOCK)
  private readonly BLOCKED_TERMS: Set<string> = new Set([
    // Sexual explícito
  ...
  private readonly REVIEW_TERMS: Set<string> = new Set([
  ...
  private readonly ALLOWED_PROFESSIONS: Set<string> = new Set([
    // Saúde
  ...
  private readonly BLOCK_PATTERNS: RegExp[] = [
   ...
   */
  async validateProfessionalCategory(
    input: string,
    aiContext?: AIContext
    ...
    let aiClassification: OccupationalClassificationResult | null = null;
    try {
      const { AIKernel } = await import('../../ai/ai-kernel');
      const aiKernel = AIKernel.getInstance();
      const prompt = buildOccupationalClassificationPrompt(input, 'professional');
   ...
   */
  async validateInterestCategory(
    input: string,
    aiContext?: AIContext
  ): Promise<AdmissionDecision> {
    const normalizedInput = this.normalizeInput(input);
    ...
    try {
      const { AIKernel } = await import('../../ai/ai-kernel');
      const aiKernel = AIKernel.getInstance();
      const prompt = buildOccupationalClassificationPrompt(input, 'interest');
   ...
   */
  private checkBlockedTerms(normalizedInput: string): AdmissionDecision | null {
   ...
   */
  private checkBlockPatterns(normalizedInput: string): AdmissionDecision | null {
   ...
   */
  private checkReviewTerms(normalizedInput: string): AdmissionDecision | null {
   ...
   */
  private checkAllowedProfessions(normalizedInput: string): AdmissionDecision | null {
   ...
   */
  private isSportAsProfession(normalizedInput: string): boolean {
   ...
   */
  private inferDomainType(term: string): CategoryDomainType {
   ...
   */
  private inferDomainTypeFromPattern(pattern: RegExp): CategoryDomainType {
...
export const categoryAdmissionPolicyService = new CategoryAdmissionPolicyService();
