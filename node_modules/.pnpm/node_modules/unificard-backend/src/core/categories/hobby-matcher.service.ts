...
class HobbyMatcherService {
   ...
   */
  private async loadHobbiesDataset(): Promise<void> {
    ...
    try {
      ...
      if (Array.isArray(data.hobbies)) {
        this.hobbies = data.hobbies.map((h: string) => this.normalizeInput(h));
        this.initialized = true;
      } else {
   ...
   */
  private findExactMatch(normalized: string): string | null {
   ...
   * Limite: 3 resultados
   */
  private async findFuzzyMatch(normalized: string): Promise<{ hobby: string; similarity: number } | null> {
    try {
      // Verificar se pg_trgm está disponível
      const extensionCheck = await pool.query<{ exists: boolean }>(
      ...
      const result = await pool.query<{ hobby: string; similarity: number }>(
        `
        SELECT 
          hobby,
   ...
   */
  private findSimpleFuzzyMatch(normalized: string): { hobby: string; similarity: number } | null {
    let bestMatch: { hobby: string; similarity: number } | null = null;
   ...
   * 3. Senão → DENY
   */
  async findMatch(input: string): Promise<HobbyMatchResult> {
   ...
   */
  async isInDataset(input: string): Promise<boolean> {
    await this.loadHobbiesDataset();
    const normalized = this.normalizeInput(input);
...
export const hobbyMatcherService = new HobbyMatcherService();
