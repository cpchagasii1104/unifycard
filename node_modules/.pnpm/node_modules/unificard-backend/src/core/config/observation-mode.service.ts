  ...
  throwIfEnabled(operation: string): void {
    if (this.isEnabled()) {
      const error = new Error(
        `OBSERVATION MODE ATIVO: Operação "${operation}" bloqueada. Nenhuma feature estrutural deve ser adicionada.`
      );
...
export const observationModeService = new ObservationModeService();
