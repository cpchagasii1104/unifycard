...
Objetivo:
- Gerenciar motoristas, veículos e serviços habilitados
- Controlar localização e disponibilidade em tempo real

Dependências:
- users
- 009_rides_part1_geography

Observações:
- Dados altamente voláteis (tracking)
- rating_avg NUMERIC(3,2) suporta 0.00–5.00
*/
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = current_schema()
      AND tablename = 'rides_driver_services'
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = current_schema()
      AND tablename = 'rides_driver_locations'
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = current_schema()
      AND tablename = 'rides_driver_availability'
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE schemaname = current_schema()
      AND tablename = 'rides_driver_sessions'
