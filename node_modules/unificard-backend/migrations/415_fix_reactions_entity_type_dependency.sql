...
-- Verificar se tabela reactions existe e se possui coluna entity_type
DO $$
BEGIN
    ...
    IF EXISTS (
        ...
        -- Verificar se coluna entity_type NÃO existe
        IF NOT EXISTS (
            SELECT 1 
        ...
        IF EXISTS (
            ...
            -- Criar constraint nova se não existir
            IF NOT EXISTS (
                SELECT 1 
                FROM information_schema.table_constraints 
                WHERE table_schema = 'public' 
                ...
                -- Apenas criar se entity_type e entity_id não forem NULL
                -- Usar constraint parcial para compatibilidade
                ALTER TABLE reactions 
