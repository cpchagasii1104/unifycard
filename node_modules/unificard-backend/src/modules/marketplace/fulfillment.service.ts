...
class FulfillmentService {
  ...
  async shipFulfillment(
    ...
    // 2. Validar que todos os itens estão PICKED
    const items = await fulfillmentRepository.listFulfillmentItems(
      tenantId,
      fulfillmentOrder.id
    );
    ...
    // 3. Gerar inventory_movements OUT para cada item
    ...
    // 4. Consumir reservas de estoque (se ainda não consumidas)
    // SPRINT 54: Consumir reservas quando SHIPPED
    try {
      ...
      // Log mas não bloqueia (pode já ter sido consumido)
      console.warn(
        `[FulfillmentService] Erro ao consumir reservas após SHIP para pedido ${fulfillmentOrder.orderId}:`,
        consumeError
      );
    }

    // 5. Atualizar status para SHIPPED
    const shippedFulfillment = await fulfillmentRepository.updateFulfillmentStatus(
      tenantId,
      fulfillmentOrder.id,
      'SHIPPED',
      undefined,
