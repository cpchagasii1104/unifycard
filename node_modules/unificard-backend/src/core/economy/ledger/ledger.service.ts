...
class LedgerService {
   ...
   */
  private toLedgerEntry(row: LedgerRow): LedgerEntry {
    return {
   ...
   */
  async getLedgerEntry(tenantId: string, entryId: string): Promise<LedgerEntry | null> {
    const client = await getClientWithTenant(tenantId);
   ...
   */
  async verifyLedgerIntegrity(tenantId: string, accountId: string): Promise<boolean> {
    const client = await getClientWithTenant(tenantId);
