...
class GroupAccountService {
   ...
   */
  async createOrGetGroupAccount(
    tenantId: string,
    groupId: string
  ): Promise<string> {
    // Verificar se já existe conta vinculada
    const existingLink = await groupsRepository.getGroupAccount(tenantId, groupId);
    if (existingLink) {
      return existingLink.accountId;
    }

    // Buscar ou criar conta
    const groupAccounts = await this.accountService.getAccountsByOwner(
      tenantId,
      groupId,
      'group',
    );
    ...
    await groupsRepository.linkAccount(tenantId, groupId, groupAccount.accountId);
   ...
   */
  async resolveGroupAccountIds(
    params: ResolveGroupAccountParams
  ): Promise<string[]> {
...
export const groupAccountService = new GroupAccountService(
  accountService
);
