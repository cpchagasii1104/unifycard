...
class OccupationFormCheckerService {
   ...
   */
  private hasOccupationalSuffix(term: string): boolean {
   ...
   */
  private hasFunctionPrefix(term: string): boolean {
    const normalized = this.normalizeInput(term);
   ...
   */
  private hasValidStructure(term: string): boolean {
    ...
    const hasPreposition = words.some(word => 
      word.toLowerCase() === 'de' || word.toLowerCase() === 'em'
    );
   ...
   */
  private generateSuggestion(term: string): string {
    const normalized = this.normalizeInput(term);
   ...
   */
  validate(input: string, context: CategoryContext = 'professional'): OccupationFormResult {
...
export const occupationFormCheckerService = new OccupationFormCheckerService();
