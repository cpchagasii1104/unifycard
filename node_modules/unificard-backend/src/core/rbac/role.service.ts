// backend/src/core/rbac/role.service.ts

import { runQueryWithTenant, runQueriesWithTenant } from '@core/database/pool';
import type { Role, CreateRoleInput, UpdateRoleInput } from './rbac.types';

type RoleRow = {
  role_id: string;
  tenant_id: string;
  name: string;
  description: string | null;
  is_system_role: boolean;
  created_at: Date;
  updated_at: Date;
};

class RoleService {
  private toRole(row: RoleRow): Role {
    return {
      roleId: row.role_id,
      tenantId: row.tenant_id,
      name: row.name,
      description: row.description,
      isSystemRole: row.is_system_role,
      createdAt: row.created_at,
      updatedAt: row.updated_at,
    };
  }

  /**
   * Cria uma nova role
   */
  async createRole(tenantId: string, input: CreateRoleInput): Promise<Role> {
    const { name, description } = input;

    // Verifica se já existe role com esse nome
    const existing = await runQueryWithTenant<RoleRow>(
      tenantId,
      'SELECT * FROM roles WHERE name = $1 LIMIT 1',
      [name]
    );

    if (existing) {
      const error = new Error('Role with this name already exists');
      (error as any).statusCode = 409;
      throw error;
    }

    const row = await runQueryWithTenant<RoleRow>(
      tenantId,
      `INSERT INTO roles (tenant_id, name, description)
       VALUES ($1, $2, $3)
       RETURNING *`,
      [tenantId, name, description || null]
    );

    if (!row) {
      throw new Error('Failed to create role');
    }

    return this.toRole(row);
  }

  /**
   * Busca role por ID
   */
  async getRoleById(tenantId: string, roleId: string): Promise<Role | null> {
    const row = await runQueryWithTenant<RoleRow>(
      tenantId,
      'SELECT * FROM roles WHERE role_id = $1 LIMIT 1',
      [roleId]
    );

    return row ? this.toRole(row) : null;
  }

  /**
   * Lista todas as roles
   */
  async listRoles(tenantId: string): Promise<Role[]> {
    const rows = await runQueriesWithTenant<RoleRow>(
      tenantId,
      'SELECT * FROM roles ORDER BY name'
    );

    return rows.map((row) => this.toRole(row));
  }

  /**
   * Atualiza uma role
   */
  async updateRole(tenantId: string, roleId: string, input: UpdateRoleInput): Promise<Role> {
    const { name, description } = input;

    // Verifica se a role existe
    const existing = await this.getRoleById(tenantId, roleId);
    if (!existing) {
      const error = new Error('Role not found');
      (error as any).statusCode = 404;
      throw error;
    }

    // Não permite editar roles de sistema
    if (existing.isSystemRole) {
      const error = new Error('Cannot modify system role');
      (error as any).statusCode = 403;
      throw error;
    }

    // Se está tentando mudar o nome, verifica se já existe outro com esse nome
    if (name && name !== existing.name) {
      const duplicate = await runQueryWithTenant<RoleRow>(
        tenantId,
        'SELECT * FROM roles WHERE name = $1 AND role_id != $2 LIMIT 1',
        [name, roleId]
      );

      if (duplicate) {
        const error = new Error('Role with this name already exists');
        (error as any).statusCode = 409;
        throw error;
      }
    }

    const row = await runQueryWithTenant<RoleRow>(
      tenantId,
      `UPDATE roles 
       SET name = COALESCE($1, name),
           description = COALESCE($2, description),
           updated_at = now()
       WHERE role_id = $3
       RETURNING *`,
      [name || null, description !== undefined ? description : null, roleId]
    );

    if (!row) {
      throw new Error('Failed to update role');
    }

    return this.toRole(row);
  }

  /**
   * Deleta uma role
   */
  async deleteRole(tenantId: string, roleId: string): Promise<void> {
    // Verifica se a role existe
    const existing = await this.getRoleById(tenantId, roleId);
    if (!existing) {
      const error = new Error('Role not found');
      (error as any).statusCode = 404;
      throw error;
    }

    // Não permite deletar roles de sistema
    if (existing.isSystemRole) {
      const error = new Error('Cannot delete system role');
      (error as any).statusCode = 403;
      throw error;
    }

    // Deleta a role (CASCADE vai remover user_roles e role_permissions automaticamente)
    await runQueryWithTenant(tenantId, 'DELETE FROM roles WHERE role_id = $1', [roleId]);
  }
}

export const roleService = new RoleService();