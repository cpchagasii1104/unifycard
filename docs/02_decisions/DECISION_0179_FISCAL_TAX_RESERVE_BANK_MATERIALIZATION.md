...
- `bank_transactions.reference_id` é **TEXT** (convertido historicamente de UUID→TEXT pela migration `20260530531000_bank_transactions_reference_id_uuid_to_text.sql` — C19 FIX/DECISION-0029, para alinhar referência externa heterogênea com as demais tabelas `financial_*`/`payment_intents`); `bank_accounts` **não tem coluna currency**. *(Errata factual pós-Veredito B da Yala: a caracterização anterior como UUID estava incorreta; a escolha de fingerprint dedicado — D11 — permanece inalterada, apenas com fundamento corrigido.)*
...
Raiz canônica: `(tenant_id, reference_type, reference_id)` + **fingerprint imutável** do contexto fiscal/econômico. Embora `reference_id` seja **TEXT**, ele **integra a raiz canônica e única de idempotência** da operação junto com `tenant_id` e `reference_type` (UNIQUE `uq_bank_transactions_reference`). O fingerprint fiscal-econômico **não** pode ser embutido em `reference_id` — não por causa do tipo, mas porque isso **misturaria a identidade externa da operação com a identidade do payload**, alteraria a chave usada em retries e reconciliação, e impediria a comparação explícita de payload divergente. Por isso, o fingerprint deve viver em coluna **dedicada, imutável, comparável e indexável**: promulga-se **`bank_transactions.fiscal_economic_context_fingerprint`** (ou nome físico canônico equivalente): dedicada · imutável após criação · comparável no retry · indexável · não escondida em JSON mutável · nullable apenas para histórico/transactions não fiscalizadas · **obrigatória no caminho fiscal 4e**.
