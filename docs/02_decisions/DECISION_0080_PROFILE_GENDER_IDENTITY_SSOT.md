...
**Parcialmente superada — apenas no §4 (enum de valores) — pela `DECISION-0115` D3** (ratificada expressamente por Clayton): vocabulário soberano = 5 valores. Todo o restante desta DECISION permanece vigente. Ver ADENDO abaixo.

## ADENDO FACTUAL — VOCABULÁRIO EXPANDIDO PARA 5 VALORES + F2/F3 IMPLEMENTADOS (2026-06-11)

> Adendo **factual** registrado pela executora sob GO `F-C1-HUMAN-JOURNEY-END-TO-END-CLOSURE` (IA Diretora, 2026-06-11). **Redação de precedência corrigida em 2026-06-12 sob GO `F-C1-GENDER-NORMATIVE-PRECEDENCE-CORRECTION`** (a versão anterior deste adendo atribuía incorretamente a promulgação ao GO executivo).

- **Precedência normativa (correta):** a **`DECISION-0115` D3, ratificada expressamente por Clayton**, estabeleceu o vocabulário soberano de **5 valores** — `male | female | non_binary | other | prefer_not_to_say` — e **superou a regra de 3 valores do §4 desta DECISION exclusivamente nesse ponto** (D3 é emenda explícita ao enum; todo o resto da 0080 — casa canônica `global_users.gender`, "perfil coleta, identidade guarda", lock/set-once — permanece vigente). O **GO da macrofrente C1 NÃO promulga norma e NÃO supera DECISION**: apenas **executou** a DECISION-0115 D3. O commit/runtime implementam a decisão soberana; este adendo registra a implementação e a relação de superação — não cria soberania. Evidência material que motivou a D3: a UI de Register já oferecia os 5 valores e `non_binary`/`prefer_not_to_say` **evaporavam silenciosamente** (DT-GENDER-INPUT-PERSISTENCE-VOCABULARY-DIVERGENCE).
