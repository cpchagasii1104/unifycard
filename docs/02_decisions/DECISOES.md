...
## Tabela mestra — Bloco 0112–0153 (pós-0111 · reindex A1 + DECISION-0142..0153 · 2026-06-23)
...
> Sequência 0112→0148 (37 decisões, sem buraco, sem duplicata; **0142** materializada via U1b; **0143/0144/0145/0146/0147** docs-only/réguas F-OFFER-0/2/3/5+6 + ativação-segura P3; **0148** docs-only/contrato do core de booking — subject normalizado; **0149** docs-only/conexão cross-tenant RLS — tenant-loop canônico B-heavy; **0150** política onboarding enforcement — Opção B+b1 progressivo/UX-hint + marcos read-only thin; **0151** modelo de locação resource-based — Opção B, recurso bloqueado no tempo; **0152** assinatura/billing recorrente HOLD até Camada 1 + quarentena de fragmentos; **0153** modelo de cascata de referral — Modelo B por-usuário, C multinível rejeitado, cascade material FINANCEIRO/HOLD). Próximo nº livre: **0154**.
...
| 0153 | DECISION_0153_REFERRAL_CASCADE_MODEL | PROMULGADA / DECISÃO DE PRODUTO (REFERRAL) / DOCS + PROVA PRÉ-MONEY (Modelo B) | MATERIALIZADO-THIN (guard + e2e pré-money; cascade material HOLD) | referral por-usuário (Modelo B): indicador=owner_actor_id do código participa de receita elegível do indicado (e futuros actors dele, server-side); Modelo C multinível REJEITADO; referral=lookup não-autoridade, autoindicação/actor_system proibidos, split só no Bank; DIRETO user-level já vivo (getActiveReferral→bank_splits/wallet); cascade actor→owner-user = FINANCEIRO/HOLD (Camada 1, 3 paralelas); e2e pré-money prova vínculo+resolução Δbank=0; guard audit-referral-cascade-model (single-level, anti-multinível, cascade não-wired) |
