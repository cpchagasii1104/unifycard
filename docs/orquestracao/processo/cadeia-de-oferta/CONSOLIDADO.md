...
**CAMINHO B1 CLOSED/PROMULGADA** (material `3cee7d1c`, MODO B + condicional): **prova de jornada pré-dinheiro** que a cadeia CONECTA ponta-a-ponta. `e2e-offer-journey-pre-money.ts` (funções reais, fixture+teardown): discover→`canonicalServiceId`→by-canonical(active-only/draft oculto)→offering→availability→createBooking(requested)→confirm→2º sobreposto mesmo provider→**409**→back-to-back→confirma. **9/9 PASS**; ledger=0; `canonicalServiceId` já viaja (sem fix de DTO). **IA-YALA PASS 6/6** (baseline idêntico). Reconciliação: o gap da IA-OFERTA era integração/frontend, não backend. **HOLD:** Caminho A (ativação segura) = pré-req de público/dinheiro; B2 (frontend) = frente própria. A prova NÃO abençoa ativação self-serve.

**B2 FRONTEND CLOSED/PROMULGADA** (material `949e134c`, MODO B): a jornada provada virou **tangível na tela** (P2 fechado). discovery transporta `canonicalServiceId` → `ServiceOfferingSelector` (novo) lista ofertas ACTIVE via by-canonical → availability(owner=service_offering) → `createBooking('requested')` canônico → confirm OWNER-only com UX honesta p/ **409**. `api/offerings.ts` novo; `requesterActorId` só de sessão (`waitForActorContext`); placebo de agendamento removido; front projeta verdade. **IA-YALA PASS 8/8**; front tsc 0 / back 34 / B1 e2e 9/9 / regression EXIT 0. **ZERO** dinheiro/legado-booking/migration. **Resíduo (fora de B2):** `ServicePostCard.handlePayment` placebo de pagamento = frente de dinheiro. **Próximo:** Caminho A (ativação segura, P3) p/ público; depois dinheiro.

**P3 / ATIVAÇÃO SEGURA CLOSED/PROMULGADA** (DECISION-0147 `e0e8e4ff` + material `a152b38a`, MODO C / promulgação manual): `active`=elegibilidade VIVA revalidada na ativação. state-machine fail-closed (active→draft→409; status nunca do body); gate PJ (publicação+operacional+KYB page-actor) + PF (declaração + KYC-lite civil V1: global_user+CPF+full_name+identities + sem-ATL; sem metadata); booking-gate (só offering active); **cascata Q5 atômica** (KYB/publicação/declaração revogada → suspende offerings active; não apaga/dinheiro). Guard `audit-offering-activation-safe.mjs` + e2e (6/6). **IA-YALA PASS 8/8.** Invariante 0147 fechado: **nenhuma offering fica active se a base cair.** **CADEIA DE OFERTA COMPLETA: semântica→identidade→autoridade→service→offering→discovery→tempo→jornada(B1/B2)→ativação segura.** Próximo = dinheiro (fee-bps/payout-golive/RLS-runtime) ou produto-card/locação/assinatura, por decisão de Clayton.
