...
9. **settlement/release workers (35p)** — **GATE PRÉ-RLS / DECISÃO** (READ-FIRST 2026-06-24, F-RLS-PREFLIGHT-SETTLEMENT-RELEASE-WORKER-GATE). `settlement-worker` (BOOT:216) e `release-worker` (BOOT:225) são **default-ON e UNGATED** (≠ bank-settlement/payout/reversal, que são `isFinancialWorkerEnabled` default-OFF). Ambos fazem **claim cross-tenant** de `payment_intents` (`WHERE payment_status='escrowed'|'settled'`, SEM filtro de tenant e SEM tenant-context antes do claim; `set_config('app.current_tenant')` só DEPOIS, por intent). `release` move dinheiro real (transfer seller_pending→seller_available, escreve bank_ledger). **Achado material (live-DB 2026-06-24):** `payment_intents` tem **RLS enabled=false/forced=false** → **o flip RLS-físico NÃO os torna inertes** (RLS só gateia tabelas com RLS; bank_ledger/transactions/accounts são forced, mas o CLAIM é sobre payment_intents, sem RLS). Inertes **HOJE** só porque `payment_intents` está **vazia** (firewall de checkout) — comportamental, não estrutural. **Recomendação (próxima fatia, com GO):** gatear ambos default-OFF (`ENABLE_SETTLEMENT_WORKER`/`ENABLE_RELEASE_WORKER` via `financial-worker-gate`) ANTES da virada RLS-física — converte inerte-comportamental em contido-estrutural. Conecta com #34 (tenant-loop/0149). NÃO tocado neste READ-FIRST.
