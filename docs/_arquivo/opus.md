...
## Sessão 2026-06-15 — F-C1-MONEY-PO-RECEIVE-EXPLICIT-CONTAINMENT (acidental→explícito)

Trocar a contenção acidental de receivePO (purchase order) por hard-stop explícito fail-closed. Decisão Clayton: PO não é creator-owned; created_by_actor_id = autoria, não autoridade; destino = company-owned (frente futura de schema). READ-FIRST: receivePO acionava inventory IN (usando order.createdByActorId como actor — o vetor proibido) + status RECEIVED/COMPLETED + accounts payable. Schema vivo não tem company_id/owner_actor_id → owner material hoje = só tenant_id. Fix: receivePO vira throw AppError(403, PURCHASE_ORDER_RECEIVE_CONTAINED) na 1ª linha; rota /receive vira 403 explícito sem chamar o service. PEGADINHA tsc importante: meu primeiro approach (throw no topo + corpo original logo abaixo) quebrou o strict (43→72!) — um throw incondicional no topo torna o resto UNREACHABLE, e o TS para de aplicar o null-narrowing (if(!order)throw) em código unreachable, então 'order possibly null' explodiu em 29 erros. Solução limpa: MOVER toda a implementação material para um método privado receivePOContainedImpl NÃO chamado — receivePO fica só com o throw (corpo reachable trivial), o impl é reachable-normal (narrowing funciona), strict volta a 43. Bônus: a separação física é uma contenção MAIS FORTE (as mutações não estão no mesmo método que pode "vazar" abaixo do throw; estão num método sem caller). Guard audit-po-receive-containment: receivePO sem mutação + impl sem caller + rota 403 sem chamar service. Neg-proof 5 mordidas. e2e 9/9, com um teste esperto (T2b): chamar receivePO com orderId INEXISTENTE retorna CONTAINED, não "Ordem não encontrada" — prova que o hard-stop precede getPurchaseOrderById (mais forte que seedar um PO real). Seedei um PO real (tenant→actor→supplier→PO; sem item, evitei a cadeia category→product→variant) p/ provar status SUBMITTED inalterado. DT-PO-RECEIVE-COMPANY-OWNER-PENDING (company-owned = frente futura). Lição重: throw-no-topo + dead-code quebra strict null-narrowing; extrair para método separado é o padrão limpo (e fortalece a contenção). HOLD pra Yala.

## Sessão 2026-06-15 — F-C1-MONEY-SPR-SCHEMA-INTEGRITY-FK-INDEX (primeira migration da onda 0131)

Terceira fatia SPR: integridade material (FK+índices) de service_payment_requests, fechando a parte FK/índice do DT R3. Primeira frente da onda 0131 que CRIA migration (as anteriores foram todas zero-migration/guard-lock). Disciplina pré-migration (provas SELECT/catálogo obrigatórias): payer/receiver/tenant uuid NOT NULL; actors.id=uuid PK; 0 FK existentes; 0 órfãos (row_count=0, então trivial); índices (tenant,booking)+(tenant,service) JÁ existem mas SEM índice em payer/receiver. Tudo seguro → criei migration 20260615200000 (timestamp 14-díg; o check-migration-numbering ignora 14-díg, só valida o formato 4-díg legado). Migration idempotente: guard DO-block (IF NOT EXISTS pg_constraint) p/ as 2 FKs + CREATE INDEX IF NOT EXISTS p/ os 2 índices. Forward-only, sem DROP/dados/lifecycle/RLS/trigger/amount. Apliquei em dev com EXPECTED_DATABASE_NAME=unificard_dev (CORE_ONLY default — confirmei via shouldExecuteMigration que minha migration não é LATENT/IGNORED, então roda sob CORE_ONLY): 1 pendente→executada, dev 385→386, catálogo confirma 2 FKs + 2 índices. Provei que não quebrou runtime rodando os e2es SPR read 9/9 + create 10/10 COM a FK ativa (os ephemeral usam FULL profile → aplicam minha migration; o seeding cria actors antes do payment_request, então a FK é satisfeita — bom sinal de que o runtime real respeita a ordem). RLS deixei explicitamente OPEN (GO: não criar RLS; R3 não totalmente fechado enquanto RLS ficar fora). Gates: actor-writer/bank-ledger OK, regression rc=0 (migration-numbering + sql-lint passam), arch critical_new=0, tsc 25/43 (zero .ts). Lição: frente de schema = as provas vêm ANTES da migration (órfãos/FK-existente/tipos/índice-equivalente); migration idempotente com guard DO-block é o padrão seguro; e aplicar em dev exige confirmar o profile (CORE_ONLY vs FULL) p/ não pular a migration. HOLD pra Yala.

## Sessão 2026-06-15 — F-C1-MONEY-SPR-CREATE-AUTHORITY-HARDENING (Opção A: receiver emite a cobrança)

Segunda cirurgia C1_MONEY: o POST create de service-payment-request. Clayton promulgou a decisão de produto que ficou pendente na frente READ (DT R1): Opção A — o RECEIVER/PROVIDER emite a cobrança, o payer paga. O achado que DEFINIU a arquitetura: createPaymentRequest tem 3 callers (POST create + service-hire + event-rfq) — então NÃO pude colocar a autoridade no SERVICE (afetaria hire/RFQ, fora de escopo). Foi route-level no POST create apenas. Fix: resolve service+booking server-side → receiver=service.actorId, payer=booking.requesterActorId (derivados) → canRepresentActor(req.user.userId, receiver) → 403. body payer/receiver tornados opcionais e IGNORADOS (derivados usados); o service re-valida (defesa em profundidade + booking_decision accepted). Estendi o guard spr-read-authority com a fatia do POST create (slice do 1º fastify.post até o 1º fastify.get): exige canRepresentActor + service.actorId + booking.requesterActorId + 403 + PROÍBE parsed.data.{payer,receiver}ActorId como autoridade. Neg-proof 3→5 (add: create-sem-receiver-derivado, create-body-authority). e2e create 10/10 (receiver cria; payer/terceiro/spoof 403; spoof body→derivado persiste; booking de outro service/não-aceito 400; zero dinheiro). DT R1 → RESOLVIDO. Interação B1f: o arquivo já estava clareado pelo file-level (READ); agora o POST está materialmente vinculado, então o mascaramento deixou de importar; baseline B1f intacto=31 (não toquei; B1f neg-proof passa). Pegadinha de escopo evitada: a tentação era colocar a authority no service (mais robusto/DRY), mas os 3 callers proibiam — confirmar callers ANTES de escolher route vs service foi o que salvou a frente de vazar para hire/RFQ. tsc 25/43, chain rc=0, arch critical_new=0. NÃO abri payer-initiated payment (fluxo futuro). HOLD pra Yala.

## Sessão 2026-06-15 — F-C1-MONEY-SPR-READ-AUTHORITY-HARDENING (primeira cirurgia runtime do C1_MONEY)

Primeira frente de RUNTIME depois da onda leve (que foi toda guard-lock/tombstone). Tapar o leak vivo de leitura de service-payment-request. Ultracode: workflow read-only de 4 mappers (registration/callers/execution-shape/e2e-seed). Achado decisivo do READ-FIRST: o owner está NO PRÓPRIO recurso — service_payment_requests.payer_actor_id/receiver_actor_id (uuid NOT NULL), validados na criação (payer=booking.requester, receiver=service.actor_id). Então não precisei resolver booking/service separado — o PR carrega payer/receiver. Fix idêntico ao padrão exemplar (E1/PDV): resolve recurso server-side → deriva payer/receiver → canRepresentActor(req.user.userId, payer) OR (receiver) → 403. Incluí o GET /payments/:id/execution (mesma cadeia; firewall só no POST execute, GET estava nu). e2e 9/9 (payer/receiver 200, terceiro 403, spoof actionContext 403, execution bound, zero dinheiro). Guard novo audit-spr-read-authority + neg-proof 3 mordidas. Pegadinha importante do guard B1f: ao adicionar canRepresentActor ao arquivo SPR (que estava no baseline B1f das 31 canal-1), o guard FILE-LEVEL passou a CLAREAR o arquivo inteiro → flagged 31→30, stale_baseline=1. Isso MASCARA o canal-1 do POST create (que NÃO toquei — depende de decisão de produto "quem cria cobrança"). Verifiquei que o B1f neg-proof segue passando (baseline=31 é o tamanho do OBJETO, intacto; new=0). Registrei o POST-create residual explicitamente no DT pra a dívida não se perder no mascaramento file-level. Schema: a coluna é `amount` (bigint), não amount_cents; bookings é a tabela real (não unified_bookings, que nem existe); seedei via os mk* do e2e canonical. Resíduos DT: POST create canal-1 + decisão produto, POST execute firewall, FK/índice/RLS ausentes (sem migration). Lição: quando um fix adiciona binding a um arquivo file-level-baselined, confirmar que (a) o baseline-count não muda (neg-proof não quebra) e (b) registrar o que o file-level passou a mascarar. tsc 25/43, chain rc=0. HOLD pra Yala.

## Sessão 2026-06-15 — A1 higiene cartorial da onda leve 0131 (cartório puro, zero runtime)

Fechamento da onda leve 0131 (PDV-seal · B3f · C4 · B1f · E1 · E2) com um pass de alinhamento cartorial. Ultracode: workflow read-only de 4 auditores (STATUS / DT-log / execution-logs / stale-sweep) p/ caçar inconsistências sem tocar runtime. Achado-chave do meu READ-FIRST de 1ª mão: os 6 selos já estavam corretamente registrados (escrevi cada um DURANTE a execução — lição "registrar durante a fatia"), os 6 DTs presentes e statusados. O valor do A1 = um ÍNDICE CONSOLIDADO no topo do STATUS (tabela: frente → commit → selo EXATO → o que NÃO fecha → DT) como artefato único de alinhamento, + correção de qualquer stale. Disciplina importante: a doc normativa que diz "REVOKE não aplicada" (SSOT_REGISTRY/RFC_C63) é NORMA SOBERANA — NÃO posso editar sem GO específico; mantive como resíduo registrado (DT-TEMPORAL R4), o cartório (STATUS/DT/oplog) reflete a verdade do DB (aplicada). opus.md é TRACKED (não gitignored como o header dele afirma) — incluí no A1. REMEDIATION_DECISIONS_LOG NÃO precisou de entrada: a onda leve é guard-lock/tombstone, não DECISION soberana entre alternativas. Invariante que registrei: todos os 6 são lock/tombstone de regressão zero-runtime — travam o bom/morto, NÃO corrigem em massa nem fecham domínio; cada selo é estreito por design; resíduos (C1_MONEY, rides bridge, runner órfão E1, descomissionamento físico, leitura legada) seguem como frentes próprias. Zero .ts/.sql/migration/guard tocado. Lição: higiene cartorial pós-onda = consolidar a verdade num índice, não reabrir nada; e respeitar a fronteira cartório-vs-norma (não "corrigir" norma soberana mesmo quando stale — registra-se o resíduo). HOLD pra Yala.

## Sessão 2026-06-15 — E2 temporal tombstone / legacy write-path lock (zero runtime)

Tombstone/regression-lock do trilho temporal legado schedules/schedule_slots (DECISION-0014/C63). Ultracode: workflow read-only de 3 mappers + DB vivo de 1ª mão. Brakes do GO eram o ponto sensível — confirmei AMBOS de 1ª mão antes de qualquer patch: (#1 writer vivo) grep raw-SQL + query-builder em todo src = vazio 2×; os 3 serviços (SlotGenerator/EventScheduleService/EmployeeService) são tombstones puros (cada método lança *LegacyError, zero DB, ZERO callers — confirmado por grep dos símbolos exportados); checkout-ticket migrou p/ unified_availability (slotId=null); weekly-template grava só availability; social-work-schedule lança 501. (#2 REVOKE aplicado) descobri uma pegadinha: has_table_privilege('public', schedules, INSERT)=false MAS isso podia ser só o default no-grant, não a REVOKE — então confirmei que a migration 20260428200000_schedules_revoke_write.sql ESTÁ em schema_migrations (filename, não version — minha 1ª query usou coluna errada e deu 0; corrigi). Doc (workflow leu SSOT_REGISTRY/RFC_C63) diz "não aplicada" → STALE, o DB contradiz → DT R4. Nuance importante: o app conecta como postgres=OWNER, que bypassa REVOKE → a garantia EFETIVA contra write legado é CÓDIGO (zero writer), não o REVOKE. Por isso a lacuna objetiva real = NÃO havia guard impedindo um writer NOVO. Guard novo audit-temporal-legacy-tombstone.mjs (mirror do bank-ledger §4.6): zero INSERT/UPDATE/DELETE runtime contra as tabelas (LEITURA não casa — preservei o read legado legítimo do EventAvailabilityPreviewService, brake #3) + pin do REVOKE migration + os 3 serviços continuam lançando *LegacyError. CLOSED=5. Neg-proof 5 mordidas byte-idêntico (probe temporário p/ writers; mutate+restore p/ migration/tombstone). Resíduos DT (não corrigidos): leitura legada viva, coluna morta schedule_slot_id, rota 501 montada, doc stale. NÃO criei migration (brake), zero .ts. Lição: em tombstone, o read legado legítimo NÃO pode ser bloqueado (guard só morde WRITE); e "PUBLIC negado" ≠ "REVOKE aplicado" — verifiquei a migration em schema_migrations p/ não confundir default-no-grant com a trava. HOLD pra Yala.

## Sessão 2026-06-15 — E1 availability owner-authority exemplar + regression lock (zero runtime)

Primeira frente com ULTRACODE ligado: usei um Workflow (4 mappers read-only: normative/guard/resolver/e2e) p/ o READ-FIRST, mantendo as conclusões e não os dumps. A Diretora foi explícita: E1 NÃO corrige availability/booking/agenda — registra o PADRÃO bom de autoridade server-side como exemplar DECISION-0131 e trava só se houver lacuna objetiva. Confirmei de 1ª mão (workflow + leitura direta do resolver/guard): o padrão availability-owner-authority é o exemplar mais COMPLETO do projeto — (owner_type, owner_id) é RECURSO (HINT), não actor; OWNER_AUTHORITY_POLICIES é Record<enum,policy> (TS força cobertura 100%); cada policy deriva o authority actor do schema vivo; gate final canRepresentActor(req.user.userId, authorityActorId) server-side; 6 owner types; triplo fail-closed (400 unknown / 404 not-found / 403 not-representable). O guard existente (audit-availability-owner-authority.mjs, já no chain) provava COBERTURA (enum↔CHECK↔policy, sem as-never/actor-cure, rotas usam resolver, sem ownerId cru) mas — lacuna objetiva confirmada pelo mapper guard — NÃO pinava a REJEIÇÃO (os 3 throws). Então a única coisa que TRAVEI (zero .ts): bloco 4b no guard com 3 checks de fail-closed (throw+código+catch→deny) → CLOSED 23→26; estendi o neg-proof existente (negative-proofs-contextual-temporal.ps1) com P-T7/8/9 → 19/19 byte-idêntico. NÃO criei guard paralelo (estendi o existente — lição B1f). Deliverable principal = doc exemplar (o padrão de 4 passos + tabela "por que copiável p/ PDV/CRM/ERP/social"). Resíduos REAIS achados pelo READ-FIRST (DT, não corrigidos): R1 o e2e exemplar cita um runner run-availability-owner-authority-ephemeral.ps1 que NÃO existe (prova órfã!); R2 gate não cobre writer fora de core/availability; R3 raw-ownerId guards por nome-fixo; R4 rotas reimplementam helpers inline e não importam o primitivo combinado resolveAvailabilityOwnerAuthority (drift risk); + headers normativos stale. NÃO toquei availability/agenda/booking/etc. Lição: quando o padrão já é bom E já tem guard, a frente "exemplar" entrega valor por (a) DOCUMENTAR o padrão copiável e (b) pinar o contrato COMPORTAMENTAL (fail-closed throws) que o guard estrutural deixava passar — não por reimplementar. E o READ-FIRST achou um resíduo que ninguém via (o runner órfão). HOLD pra Yala.

## Sessão 2026-06-15 — B1f canal-1 transversal lock (actionContext.actorId entra no guard 0113)

A Diretora pediu B1f como a trava transversal do princípio "cliente declara intenção, servidor decide autoridade" — generalizando PDV/groups/reversal. READ-FIRST: o guard 0113 (audit-actor-authority-boundary.mjs) já existia e cobria 4 canais client-declared (body/params/query/x-actor-id + 6º body.actor) mas NÃO o canal-1 actionContext.actorId — exatamente o gap da DT-mãe 0113 que vinha sendo tratado caso-a-caso. Não criei guard novo: ESTENDI o existente (é O detector). Medição de 1ª mão (throwaway scan deletado): 55 routes usam actionContext.actorId → 22 já bound, 6 safe-subject readers, 1 Forma A (requirePermission), 31 RESÍDUO. Design-chave que evitou um falso-massacre: requirePermission([ vincula SÓ o canal-1 (requirePermission→canPerformAction→canActAs liga req.user→actionContext.actorId), NÃO body/params/query (que declaram outro ator) — então o clearing por requirePermission só vale quando onlyActionContext (sem canal STRICT). Sem isso, eu marcaria toda rota autenticada como violação. Os 31 resíduos (debt PRÉ-EXISTENTE — o guard só não enxergava o canal) congelei no BASELINE com DT-0113-CANAL1-ACTIONCONTEXT-UNBOUND-BASELINE (subconjunto money-adjacent marcado C1_MONEY = prioridade), NÃO corrigi (frente ampla: marketplace ERP/CRM, services, social, profile-c1, events, votes...). B1f = detector que impede a dívida CRESCER, não que a zera. Estendi o neg-proof existente (não criei paralelo): +2 fases (canal-1 nova sem binding FALHA; com requirePermission([ fica VERDE), e mudei o assert baseline=0→31. Zero .ts tocado → tsc 25/43 inalterado; chain rc=0; arch critical_new=0. Lição: quando já existe O guard do tema, estender > criar paralelo (senão dois detectores divergem); e o design do "binder" tem que respeitar QUAL ator cada canal declara — requirePermission não é binder universal, é binder do canal-1. HOLD pra Yala.

## Sessão 2026-06-15 — C4 reversal containment guard-lock (READ-FIRST → guard-lock, zero runtime)

Segui o C4 em dois tempos pedidos pela Diretora: READ-FIRST ONLY, depois guard-lock. O READ-FIRST achou um quadro melhor do que o esperado: o motor de reversal já está bem contido — money sempre system-authored (buildSystemAuthorship), todo HTTP humano de dispute/reversal é 403 fail-closed (F-DISPUTE-*-CONTAINMENT), e o único bridge com autor frouxo (bank-integration.reverseTransaction, fallback "1º actor do tenant") só é alcançável por rides.service.cancelRide que tem 0 callers (DEAD — o cancelamento vivo é lifecycle.routes /cancel, que faz assertRideAuthority + participação e NÃO reverte). O gap real era regressão: contenção protegida só por e2e, fora do chain. Guard-lock: audit-reversal-containment.mjs com 7 invariantes (403 das 4 rotas mutáveis; events read-only; authorship system nos 2 transfers; nenhuma rota chama motor; tombstone bank-transaction.reverseTransaction; baseline dos arquivos que referenciam reverseTransaction/cancelRide; post-D-money block released_to_actor_wallet/refunded_via_recovery). Neg-proof 6 mordidas byte-idêntico. DT-RIDES-CANCEL-REVERSAL-DEAD-BRIDGE (não amputar). Pegadinha do guard: INV5 (tombstone "não executa motor") deu falso-positivo porque a STRING da mensagem de erro do throw cita 'bankIntegrationService.reverseTransaction()' — stripComments não remove strings; resolvi (a) excluir o próprio nome reverseTransaction do forbidden-set (o risco é chamar requestAndExecuteReversalSync/executeReversal/transfer, não o self-name) e (b) stripStrings no slice do método. Lição reforçada (igual B3f): guards estruturais sobre money precisam distinguir CHAMADA real de MENÇÃO em string/comentário — senão mordem o texto certo pela razão errada. Outra: INV6 como baseline-de-arquivos (não contagem) é robusto pra "morde caller novo" quando AST exata é frágil — a Diretora explicitamente abriu essa porta no GO. Zero .ts tocado → tsc 25/43 inalterado; arch critical_new=0. Fecha SÓ como "C4 — REVERSAL CONTAINMENT / SYSTEM-AUTHORSHIP REGRESSION LOCK", não o motor/bridge/recovery. HOLD pra Yala.

## Sessão 2026-06-15 — B3f (GET /groups/mine): guard-lock do já-corrigido + DT do irmão invites/mine

Primeiro batch pós-selo PDV, de volta à onda 0131. A Diretora pediu B3f /groups/mine ("baixo risco, corrige desvio de namespace user_id/actorId"). READ-FIRST READ-ONLY mudou a premissa: /groups/mine JÁ estava corrigido+committado (c00435da derive subject from authenticated user + 7c76cfb5 decouple from action context) — handler usa req.user?.userId (JWT), 401 fail-closed, read-only, bypass EXATO no action-context.plugin (method==='GET' && rawPath==='/groups/mine'), repo filtra gm.user_id=$2, e2e HTTP real existente. NÃO refiz o patch (seria re-trabalho). Surfacei à Diretora os dois resíduos reais: R1 = sem guard de regressão (runtime certo mas destravado); R2 = o IRMÃO GET /groups/invites/mine + POST /:id/request usam namespace divergente (req.user.globalUserId||id, não userId) = drift das 3 identidades. Ela escolheu R1 (guard-lock, zero runtime) + R2 vira DT (não abrir arco de identidade — sequestraria a onda). Fiz guard estrutural audit-groups-mine-auth-derived.mjs travando as 7 invariantes (mirror dos B1-B10 do e2e), neg-proof 6 mordidas byte-idêntico, wired no regression-guards, DT-GROUPS-INVITES-MINE-NAMESPACE-DIVERGENT registrado. Zero .ts tocado → tsc inalterado 25/43. Lição重: quando o GO mira um alvo que o READ-FIRST revela JÁ FEITO, o valor não é refazer — é (a) travar o ganho contra regressão e (b) nomear o resíduo adjacente como DT sem puxá-lo. Confirmar "já feito" exige git log dos arquivos + ler o handler vivo, não confiar na lista da onda. HOLD pra Yala (reseal curto). Depois: C4 reversal READ-FIRST.

## Sessão 2026-06-15 — ARCO PDV SELADO/CLOSED (decisão IA Diretora pós-PASS F2C)

Após o PASS do F2C a IA Diretora ENCERROU o arco PDV ("não abrir PDV-F2D" — continuar puxando PDV = buraco infinito). Cadeia selada: PDV-F0 (matrix+lock) → F2A (payment seller-ownership) → F2B (10 rotas authority/authorship bound) → F2C (service hardening + summary amount_cents). Autoridade PDV selada em ROTA e SERVICE. NÃO misturar com: modelo operador×empresa (decisão produto Clayton), actingUserId=session.actorId (autoria operacional, hardening futuro), Bank/Core/paymentExecutionService (não tocados), RBAC/FASE 6 (não tocados). Registrei o selo no STATUS_EXECUCAO_GLOBAL.md. Direção: voltar à ONDA DECISION-0131 restante. Candidatos próximos: B3f GET /groups/mine (baixo risco) OU C4 reversal operator trace (materialidade, toca reversão/audit financeiro → READ-FIRST forte, money = paro-e-consulto). Aguardei a escolha do batch antes de abrir (executor não se autoriza, ainda mais com C4 = money).

## Sessão 2026-06-15 — PDV-F2C (micro-batch final): defesa própria do service payOrderFromPdv + bug pi.amount→amount_cents

Micro-batch fechando os resíduos locais do PDV (a IA Diretora foi explícita: micro-batch, não frente grande). Dois alvos. (1) HARDENING service-level de payOrderFromPdv: READ-FIRST confirmou caller ÚNICO (a rota PDV), e que o service JÁ buscava a ordem persistida via getOrderById mas usava input.seller/buyerActorId (body) nas chamadas money (createPaymentIntent actorId + executePayment buyer/seller). Side-effect money começa em createPaymentIntent (passo 4). Correção: após buscar a ordem e ANTES do side-effect, derivo sellerActorId/buyerActorId da ORDEM persistida e falho fail-closed ("must match the persisted order") se o body divergir; createPaymentIntent/executePayment passam a usar os valores derivados. Agora o service tem defesa própria independente da rota — um caller futuro não redireciona dinheiro pelo body. (2) Bug pi.amount (resíduo que a Yala registrou no F2B): consultei o schema vivo (information_schema) — payment_intents tem amount_cents (não amount), order_id EXISTE, pt.status válido. Logo é rename puro de coluna: pi.amount→pi.amount_cents, em getSessionSummary E closeSessionWithSummary (query idêntica). Query-only, sem migration, sem semântica — bug de leitura, não autoridade/dinheiro. Pegadinha do guard: meu primeiro check de "body-trust" (regex `(buyer|seller)ActorId: input.*ActorId`) mordeu o createOrderFromPdv legítimo — esse método USA input.* p/ criar a ordem NOVA (o body É a fonte de verdade de uma ordem nova, gateado na rota pela representabilidade do operador). Solução: escopar os 3 checks do service ao CORPO de payOrderFromPdv (slice de `async payOrderFromPdv(` até a próxima declaração de método em indentação 2). Pegadinha do e2e: pdv_sessions tem exclusion constraint one_open_per_actor — criar 2ª sessão OPEN p/ Alice (T8 já criara uma) deu 23P01; reusei sessAlice (o service só exige sessão OPEN, não checa ownership — isso é da rota). TS1 (body divergente)→fail-closed antes do side-effect; TS2 (body casa)→passa validação, falha downstream SUBMITTED (ordem 'cancelled', sem money); TS3 prova payment_intents==0 (side-effect nunca começou). T9 agora 200 (summary não-500). neg-proof 7→10, e2e 9→12. Gates: actor-writer/bank-ledger OK, regression rc=0, arch patterns critical_new=0, tsc 25/43 (0 atribuível). DECISÃO consciente: NÃO declarei o modelo operador×empresa resolvido (residue aberto, decisão de produto Clayton) — o GO exigia isso explicitamente no critério de aceite. Lição: quando o guard morde código legítimo, o erro é do ESCOPO do check (genérico demais), não do código — escopar ao método certo > relaxar a regra. HOLD pra Yala.

## Sessão 2026-06-16 — PDV-F2B (9 rotas restantes + audit hardening): toda rota PDV prova representabilidade; DT-PDV-CANAL1 FECHADO

Fechei o que PDV-F0-LOCK travou e PDV-F2A começou: as 9 rotas PDV restantes (sessions ×6, orders ×1, items ×2). READ-FIRST confirmou que TODAS têm vínculo material resolvível server-side (ZERO STOP): sessions→pdv_sessions.actor_id (operador, via novo findSessionById nullable); order-create→input.sessionId→session.actor_id; items→getOrderById→seller_actor_id. Como `orders` NÃO tem session_id (sem FK order→session — mesma descoberta do F2A), os ITEMS bindam pelo SELLER da ordem (não pela sessão). Helper assertRepresents(req,reply,tenantId,targetActorId)=canRepresentActor, 403 fail-closed, aplicado nas 9 rotas por modelo A/B/C/D. Os readers (GET /sessions, /sessions/open, /sessions/:id/summary) eram o ponto MAIS leakável (qualquer user lia sessões de qualquer operador por spoof de actorId; nem tinham requirePermission) — agora gateados só pelo binding, o que os torna PERFEITOS p/ isolar o gate no e2e (sem requirePermission interferindo, ao contrário das rotas POST). Audit hardening: as 4 auditorias que gravavam `actor_id: actionContext.actorId` CRU (orders, items×2, pay) passam a gravar o actor VALIDADO (operador/seller resolvido); a pay (F2A) ficou intacta no gate, só o audit endureceu. 10 rotas DIVERGENT→CANONICAL. Guard estendido com 3 checks novos: helper presente+primitivo real, cobertura ≥9 chamadas assertRepresents, e proibição de `actor_id: actionContext.actorId` cru (regex). Neg-proof 5→7 mordidas (add: cobertura cai / autoria crua reintroduzida). e2e 5→9/9 (add T6/T7 reader binding, T8 summary Modelo B de sessão alheia→403, T9 dono→passa). Detalhe importante do guard: como o pay usa canRepresentActor INLINE (com a mensagem específica "represent the order seller" que o e2e T2 testa) e as 9 outras usam o HELPER, contei cobertura como ≥9 (não ≥10) e mantive o pay-deep-check aceitando assertRepresents OU canRepresentActor — assim não quebrei a mensagem do F2A nem o e2e existente. Resíduo achado (fora de escopo): getSessionSummary realtime usa pi.amount (coluna ausente nesse schema)→500 downstream DO gate; T9 prova que o gate passa, o 500 é bug latente de query, não-autoridade — NÃO corrigi (disciplina de escopo). Gates: guard OK, regression rc=0, tsc build 25/strict 43 (0 atribuível), arch 33 CRITICAL pré-existentes (REGRA 2/3 profile/categories, 0 em modules/pdv). DT-PDV-CANAL1-AUTHORITY-NO-BINDING FECHADO. Lição: quando uma frente "lock" foi bem mapeada (F0), a correção (F2B) flui — a matriz READ-FIRST já dizia quais rotas bindavam por operador vs seller; bastou seguir o vínculo material que EXISTE, sem fabricar FK. HOLD pra Yala.

## Sessão 2026-06-15 — PDV-F2A (payment authority binding): pay deixa de ser role/canal-1 → ownership do seller

Logo após PDV-F0-LOCK (PASS Yala), corrigi a rota money POST /pdv/orders/:id/pay (a parte DIVERGENT-MONEY). READ-FIRST decisivo: orders NÃO tem session_id (sem FK order→session) — então ir "pela sessão" exigiria improvisar o vínculo order→session (que é só client-declared), e o GO proíbe improvisar empresa-actor/vínculo. O actor autorizado REALMENTE resolvível é o SELLER da ordem (orders.seller_actor_id), server-side via getOrderById. Bindei: canRepresentActor(req.user.id, order.sellerActorId) ANTES de payOrderFromPdv (o side-effect money começa em createPaymentIntent, passo 4) + consistência input.seller/buyer === order.seller/buyer (sem redirecionar dinheiro pelo body). requirePermission fica como camada AND adicional (não OR-fallback → não é ADAPTER, é CANONICAL). DIVERGENT-MONEY → CANONICAL. Guard atualizado (pay CANONICAL exige canRepresentActor+getOrderById antes do payOrderFromPdv) + neg-proof 5 mordidas + e2e 5/5. O e2e foi o mais difícil: a cadeia requirePermission→canPerformAction→canActAs DENY-a tudo sem actor_registry (denyIfMissingRequiredRegistryCapability nega se registry null e a permission exige can_hold_assets) — então semeei actor_registry com can_hold_assets p/ o operador, fazendo requirePermission passar via OWNERSHIP, e aí o diferencial provado é o MEU gate (representa o seller da ordem), não a capability. T2 prova exatamente "tem capability mas não representa o seller → 403". Quando o GO PDV-F2 chegou (no meio do meu cartório), ele pedia EXATAMENTE o que eu já tinha feito — respondi apontando que PDV-F2A o atende, com o STOP-handling correto (não improvisei order→session; bindei no seller resolvível) e sem expansão silenciosa (as 9 rotas seguem DIVERGENT). Lição: quando o "owner material" que o GO assume (sessão→empresa) NÃO existe no schema (sem FK order→session), o caminho honesto é bindar no vínculo que EXISTE (order→seller) e declarar a ausência do outro — não fabricar a FK nem o empresa-actor. HOLD pra Yala. Restam 9 rotas PDV (PDV-F2B) + possível decisão Clayton sobre modelo operador/empresa.

## Sessão 2026-06-15 — ONDA 0131 BATCH 5 (PDV-F0-LOCK): matriz + lock guard, read-only, zero runtime

IA Diretora pivotou de "volume" para "materialidade": PDV-F0-LOCK = lock/read-only + guard, NÃO correção. Escopo só modules/pdv. READ-FIRST de 1ª mão das 10 rotas (pdv.routes.ts): TODAS gravam/filtram autoria por actionContext.actorId (canal-1 0113) SEM binding. Confirmações-chave: (1) o hook do módulo só checa EXISTÊNCIA de actionContext.actorId; o require-permission.guard (≠ fastify.requirePermission do rbac.plugin — é @core/authorization/require-permission.guard) resolve a capability do actor DECLARADO via actorRegistryService.findByActorId, NÃO vincula req.user→actor → confirmado NÃO binda; (2) POST /orders/:orderId/pay = DIVERGENT-MONEY (requirePermission marketplace_execute_payments + actionContext.actorId + paga via paymentExecutionService); (3) PDV NÃO toca bank_ledger direto — grep vazio, money via Core/marketplace (sem STOP crítico); (4) schema: SÓ pdv_sessions (actor_id=operador), orders/payments vão pelo marketplace. Gap relevante: actionContext.actorId (canal-1) NÃO é detectado pelo audit-actor-authority-boundary (cobre body/params/query/x-actor-id, não actionContext) — por isso PDV não aparece no baseline 0113 e precisa de lock próprio. Guard audit-pdv-authority-lock: registro das 10 rotas classificadas; morde rota nova não classificada / pay declassificada de DIVERGENT-MONEY sem binding / bank_ledger direto / perda da marca canal-1. Neg-proof 3 mordidas (rota fake / pay→CANONICAL no registro / INSERT bank_ledger no service) byte-idêntico. SEM e2e (nenhum runtime tocado). DT-PDV-CANAL1-AUTHORITY-NO-BINDING (correção=PDV-F2, possível decisão Clayton sobre modelo de autoridade do operador: sessão×empresa×delegação). Gates: regression rc=0, arch critical_new=0, tsc build 25/strict 43 (inalterado — só guard .mjs + package.json + docs, zero compilado). Lição: F0-LOCK é a disciplina certa para superfície money-adjacente grande+não-coberta — mapear+classificar+travar ANTES de corrigir evita tanto "corrigir cego" quanto "deixar regredir"; o pay-declassification-check (falha se money for apresentada canônica sem binding) é o cadeado que impede futuro teatro. NÃO declarei PDV corrigido. HOLD pra Yala. Restantes: PDV-F2 (correção), C4/B3f/A1/E1/E2/B1f.

## Sessão 2026-06-15 — ONDA 0131 BATCH 4 (RBAC-V2 ownership, money-first): GET payments deixa de ser role-solo

IA Diretora liberou B4 estreito: corrigir SÓ a superfície money GET /social/work/posts/:postId/payments (DIVERGENT-MONEY do batch 3). READ-FIRST confirmou: autorizada por requirePermission (RBAC-V2 role-based) + inline userHasAnyRole → role-solo; o dono do post SEM role era barrado pelo preHandler (era o que eu previra no batch 3 — por isso a contenção lá, não fix). Plano original: usar socialService.getPost + post.globalUserId. **Descoberta que mudou o fix: getPost está QUEBRADO** — seu SQL faz SELECT post_id, global_user_id FROM posts, mas posts no schema vivo é actor-based (id/actor_id, sem post_id/global_user_id) → "coluna post_id não existe". validatePostAccess (que usa getPost) também está quebrado. Então o primitivo "post owner via globalUserId" que o batch 3 assumiu como canônico É ELE PRÓPRIO inexistente no runtime. Pivotei para o primitivo que REALMENTE funciona: ler posts.actor_id (autor) direto + canRepresentActor(tenantId, req.user.id, postActorId). canRepresentActor é o primitivo canônico vivo (ownership/company/group/delegação, fail-closed). Removi requirePermission+userHasAnyRole da GET payments; troquei import rbacService→pool+authorizationService. Resultado: DIVERGENT-MONEY → CANÔNICO-OWNERSHIP (sem role). Tripé: guard (FROM posts+postActorId+canRepresentActor+403; sem role/requirePermission) + neg-proof (3 mordidas) + e2e 5/5 — o teste-chave T3: Bob COM role admin SEMEADA → AINDA 403 (prova material que role não autoriza money). e2e gotcha: registrar o plugin inteiro quebra em fastify.requirePermission (rbac.plugin ausente) → decorei um no-op requirePermission (a GET payments não usa, a POST /pay não é testada). Containment guard atualizado (payment removido; 7 callers). Resíduos honestos (DT): POST /pay (money-WRITE ainda role-based, fora do escopo estreito), apply/schedule/categories, e o schema-drift getPost/resolveJobFromPost. Gates: regression rc=0, arch critical_new=0, tsc build 25/strict 43 (zero atribuível). Lição dupla: (1) quando o "primitivo canônico" assumido (getPost/globalUserId) está QUEBRADO no runtime, não dá pra fingir que existe — pivotar para o que funciona (posts.actor_id + canRepresentActor) é a correção honesta; (2) provar "role não basta" exige SEMEAR a role e mostrar o 403 mesmo assim (T3), senão é só asserção. HOLD pra Yala. Não declarei RBAC-V2/POST-pay/apply/schedule/categories corrigidos.

## Sessão 2026-06-15 — F3 correção FAIL Yala: role-fallback não é CANÔNICO (ADAPTER_TRANSITIONAL)

Yala FAIL estreito no batch 3: rotulei 4 callers como CANONICAL, mas eles têm `primitivo canônico OR userHasAnyRole(['admin','owner'])` — role ainda é caminho de autoridade SECUNDÁRIA, logo são ADAPTADOR_TRANSITÓRIO, não CANÔNICO definitivo. Yala certíssima — eu vendi role-fallback como canônico. Correção cartorial/guard, ZERO runtime: troquei os 4 de CANONICAL→ADAPTER_TRANSITIONAL no guard + legenda (CANÔNICO=não depende de role; ADAPTADOR=primitivo primário+role-fallback; DIVERGENT=role-sole) + DT-ROLE-FALLBACK-TRANSITIONAL-ADAPTER (convergir p/ remover role-fallback). Guard agora: 0 CANONICAL puro + 4 ADAPTER + 4 DIVERGENT. Não reabri os DIVERGENT, não declarei F3/RBAC-V2/payment corrigidos. neg-proof segue mordendo; gates verdes; tsc inalterado. Lição: "CANONICAL" é palavra forte — um OR-fallback por role NÃO é canônico; a distinção CANÔNICO×ADAPTADOR×DIVERGENT (vocab DECISION-0131 B7) tem que ser aplicada com rigor, senão o cartório mente que algo está mais limpo do que está. Batch 3 fecha como F3-CONTAINMENT/CLASSIFICATION/REGRESSION-LOCK. HOLD pra re-reseal.

## Sessão 2026-06-15 — ONDA 0131 BATCH 3 (F3 role-as-authority): contenção+classificação, escape do GO (frente grande)

Yala PASS no B2f, IA Diretora liberou F3 como frente própria. F3 NÃO é tombstone — userHasAnyRole (rbac.service:373) lê user_roles como autoridade em 8 callers vivos (Art.17). READ-FIRST de 1ª mão (2 agentes Explore p/ classificar cada caller): 4 CANONICAL (role = fallback APÓS primitivo canônico — social-work create-job tem post-owner check, event-lifecycle tem owner/company, work-insights/worker-status têm self) · 4 DIVERGENT (role-sole, sem primitivo: social-work-payment[MONEY GET payments], apply, schedule, categories.createCategory-active) · 1 DEAD duplicate (services/events/event-lifecycle, não registrado). A descoberta decisiva que tornou a frente GRANDE: as 3 rotas social-work têm preHandler requirePermission que é ELE PRÓPRIO role-based (rbac.plugin → actorHasAnyRole → actor_has_any_role → user_roles) — então o post-owner (primitivo canônico, via validatePostAccess que existe em social-work.routes) seria BARRADO pelo preHandler ANTES do meu check inline; deixar o owner passar exige mexer no preHandler RBAC-V2, que o GO PROÍBE (não ativar/alterar RBAC, sem refactor amplo). Então acionei o ESCAPE explícito do GO ("se a frente se revelar grande demais: READ-FIRST + classificação + guard de bloqueio + reportar subfrentes"): guard audit-role-as-authority-containment (registro dos 8 callers; FALHA em caller NOVO não classificado — bloqueia regressão Art.17) + removi o duplicata morto + DT-ROLE-AS-AUTHORITY-DIVERGENT com 3 sub-frentes nomeadas (F-RBAC-V2-PERMISSION-OWNERSHIP → correção social-work → F-CATEGORIES-TAXONOMY-AUTHORITY). NÃO improvisei correção de autoridade (o preHandler RBAC-V2 sobreporia qualquer fix inline; improvisar seria pior que conter). Sem e2e (nenhuma autoridade de caller foi TOCADA — só guard + remoção de morto; a prova de comportamento vem nas sub-frentes com spoof e2e). Tripé: guard + neg-proof (novo caller user/actorHasAnyRole não classificado → 2 mordidas byte-idêntico). Gates: regression rc=0, arch critical_new=0, tsc build 25/strict 43 (zero atribuível). Lição central: a disciplina mais difícil deste batch foi NÃO "consertar" o money DIVERGENT (social-work-payment) com um check de owner inline — porque o preHandler RBAC-V2 (role-based) o barraria antes, tornando o fix teatro; reconhecer que a correção real é uma frente RBAC-V2 maior (proibida aqui) e CONTER+DT honestamente vale mais que um patch que parece fechar mas não fecha. Mesma família das decisões honestas de F3-reclassificação e B2f-owner≠target. HOLD pra Yala. Restantes da onda: C4/B3f/A1/E1/E2/B1f.

## Sessão 2026-06-14 (cont.) — ONDA 0131 BATCH 2 (B2f money unbound): rotas econômicas v2 vinculadas ao dono do evento

Pós-PASS Yala do batch 1, IA Diretora liberou B2f (money crítico) como frente própria. Verifiquei de 1ª mão (STOP do GO): POST .../economic/v2/custody (lê body.economic_owner_id) e /split (lê parts[].target_id) — NEM rota NEM service bindam. custody persiste em event_custody+outbox (declarativo); split é IN-MEMORY (Map, nem DB). Nenhuma move dinheiro, mas autoria spoofável = gap real. Decisão de modelo (a parte difícil): a autoridade correta é sobre o EVENTO (canRepresentActor no event.actorId via o helper userRepresentsActor já no arquivo), NÃO sobre economic_owner_id/target_id — esses são DADO (beneficiários), e binding de split por target seria SEMANTICAMENTE ERRADO (split multi-parte não exige representar cada beneficiário). A IA Diretora esperava "prova de representação do economic_owner_id/target_id", mas isso está errado p/ split — bindei no dono do evento (controle correto/fail-closed) e expliquei no cartório. Patch mínimo: em cada rota, antes do side-effect, getEvent → userRepresentsActor(event.actorId) → 403. Sem tocar service, Core financeiro, nem inventar modelo. Tripé: guard (createCustody/calculateSplit exigem binding antes) + neg-proof (2 mordidas byte-idêntico) + e2e 6/6 (Bob não-rep→403 custody+split, sem side-effect; Alice dona→≠403; Bank intacto). Descobertas no caminho: (1) event_custody NÃO existe nas migrations FULL (tabela aspiracional) → o e2e quebrou ao contar event_custody; envolvi em try/catch (o 403 acontece ANTES do service, então não há insert independente da tabela existir) — e isso CONFIRMA que o motor econômico v2 é protótipo (custody aspiracional + split in-memory), o que torna o binding seguro (não quebra fluxo de produção); (2) reusei o e2e precedente F-0113-EVENT-ACTOR-BODY-BINDING como molde (social ports + stub x-test-user-id). Resolvi também a ressalva da Yala sobre tsc: build config (strict:false) = 25, strict config (tsconfig.json) = 43 — ambos baseline herdado, ZERO atribuível aos meus arquivos; reportei os dois números. Lição: quando o GO especifica a FORMA da prova (binding em economic_owner_id/target_id) mas a forma é semanticamente errada p/ o caso (split multi-parte), entregar o controle CORRETO + explicar vale mais que seguir a letra — mesma disciplina da reclassificação de F3/B1f. HOLD pra Yala. Restantes: C4/B3f/A1/E1/E2/B1f + F3(Art.17) em batches próprios.

## Sessão 2026-06-14 (cont.) — PRIMEIRA ONDA INDEPENDENTE 0131, BATCH 1 (B4f + F1/F2): vazamento cross-tenant fechado + locks stub/tombstone

Pós-PASS Yala de 0131, IA Diretora liberou a primeira onda (11 frentes independentes, cada uma com tripé). A onda é GRANDE e várias têm surpresas que exigem juízo — então executei em BATCHES por correção, não tudo de uma vez (gate honesto > gate verde). Scoping paralelo (3 agentes Explore READ-ONLY) revelou: B1f JÁ está fechado (R2 anteriores); **B2f achou 2 rotas money UNBOUND** (economic/v2/custody lê body.economic_owner_id; economic/v2/split lê body.parts[].target_id) sem userRepresentsActor visível — RESPEITEI o STOP do GO (classificar, não concluir seguro sem provar o gate service-level); **F3 não é tombstone dormente — actor_has_any_role está LIVE** (rbac.service:210 + 7 callers de userHasAnyRole lendo user_roles como autoridade) → reclassifiquei como frente Art.17 própria, NÃO um guard de tombstone. Batch 1 entregue com tripé completo: **B4f** (daily-metrics rodava pool.query SEM tenant_id → vazamento cross-tenant platform-wide; reescrevi com filtro tenant_id em toda query + rota exige req.tenant.id; no caminho achei 3 nomes de coluna defasados — current_period_end→ends_at, type→metric_type — que deixavam o endpoint 500 latente, corrigi pois estava reescrevendo o service mesmo; e2e efêmero 4/4 provando A≠B). **F1+F2** (guard audit-rbac-stub-and-tombstones: F1 trava o swap do actor_has_permission verificando a def EFETIVA = maior-timestamp = RETURN FALSE — a def histórica 20260421010000 era RETURN TRUE dev-stub SUPERADA pela fail-closed 20260422000100, então "toda def tem RETURN FALSE" estava errado, corrigi para "a efetiva"; F2 bloqueia CREATE TABLE de organization_members/user_identity_links). Lições: (1) quando a onda tem frentes com premissas que o plano assumiu mas o vivo contradiz (F3 dormente×live, B1f pendente×feito), VERIFICAR de 1ª mão e reclassificar honestamente vale mais que executar a label cega; (2) guard de "swap travado" tem que olhar a def EFETIVA (última migration), não toda def histórica; (3) achar bug de coluna defasada no meio de um refactor de tenant-scope é correção em-escopo (eu estava reescrevendo o service), não scope-creep. Gates: regression rc=0 (+2 guards), arch critical_new=0, tsc 25. Restantes (A1/B1f/B2f/B3f/C4/E1/E2/F3) scoped com findings, cada um vira batch próprio. HOLD pra Yala.

## Sessão 2026-06-14 (cont.) — DECISION-0131 (eu: análise + redação docs-only): gramática de autoridade, índice que cita 0013→0130

Clayton pediu PRIMEIRO análise (não execução) de 3 docs de planejamento (PLANO-DEFINITIVO/MAP-v2/INSTRUMENTO) + a recomendação de outra IA (slate A0/B1-B7). Li os 3, verifiquei de 1ª mão no dev 385 os fatos load-bearing (financial_approval_*=0/0/0 → payout fail-closed; 3 chaves de identidade divergentes company_users/tenant_operator_grants→global_user_id vs financial→user_id vs delegations→actor; 3 vocabulários temporais; RLS=0 nos 6 planos vs bank_ledger/user_roles já rls=t) e CONCORDEI com o slate, adicionando 3 emendas materiais (não echo): (1) cargo-template NÃO materializa financial_approval_authorities — senão reabre a indireção que a 0129/0130 (que EU construí) fechou; (2) o mapper B3 é resolvedor de COMPOSIÇÃO, não pode re-keyar o Core financeiro pra actor_id — financial_approval_authorities fica user_id-bound porque a segregação requester≠approver é user-level (a redação do plano C1 "migration ampla" podia atropelar isso); (3) RLS pre-flight é BLOQUEANTE (getClientWithTenant seta GUC mas seed/migration/worker rodam como infra role → precisam BYPASSRLS ou quebram silenciosamente). Clayton assumiu direção, aprovou slate + 3 emendas, GO SÓ redação docs-only. Redigi DECISION-0131 como DECISION-ÍNDICE (cita a cadeia + textos soberanos com paths reais que localizei: AUTHORITY_LAW/AUTHORITY_ENFORCEMENT_MODEL/08_AUTORIDADE_CANONICA/SSOT_REGISTRY_UNIFICARD/AUTHORITY_PRECEDENCE; promulga só B1-B7 com as 3 emendas embutidas). Disciplina respeitada: papel de reviewer quando pedido (Clayton pediu análise explicitamente — não é eu planejando por conta própria); docs-only estrito (zero código/migration/seed/RLS/cargo/mapper/delegação/financial-seed/platform/cartão); seed da 1ª row financial_approval_* = PORTA-1 ato soberano, FORA da 0131 (não smuggle de seed); âncoras citadas não reescritas. Cartório: DECISION file + LOG append-only + STATUS + execução log + opus. NÃO toquei DECISOES.md (memoria de especialista, pre-modificada no working tree, não minha) nem fiz a higiene A1 (header 0116 stale, §10.2) — fora do GO, frente própria. Commit docs-only, HOLD pra Yala. Lição: quando Clayton pede análise antes de execução, verificar de 1ª mão os fatos que sustentam a decisão (não confiar no snapshot da outra IA) transforma "concordo" em "concordo porque medi" — e as 3 emendas saíram justamente de cruzar o slate com o que EU sei das frentes 0128/0129/0130 que construí.

## Sessão 2026-06-14 (cont.) — F-PAYOUT-APPROVAL-POLICY-MATERIALIZATION (eu executora full): aprovação material de payout viva na faixa MVP, CAMINHO A

Materializa DECISION-0130. A frente anterior (CAMINHO B) deixou o approve fail-closed; esta cria o substrato Core e liga a aprovação real DENTRO da faixa. READ-FIRST de 1ª mão (schema dev) provou que TODO o D7 tem substrato material (KYC `identities.kyc_status`, ATL `atl_blocked_actors`, recovery `actor_wallet_recovery_obligations` com vocab pending_approval/approved/partially_recovered, risco `actor_risk_profile.risk_level` low/medium/high/blocked, destino `internal_settlement`); aprovador FK = `users.id` (casa `requested_by_user_id` e o bridge); KYB (`fiscal_identity_kyb_requests`) é p/ payout EXTERNO PJ → fora do MVP internal-only (DT, destino externo bloqueado). Migration 20260614150000 (dev 384→385): 3 tabelas Core (policies/authorities/policy_events) com BIGINT/TIMESTAMPTZ, **CHECK MVP ceiling 50000/150000 no banco** (ratifica D4 — não hardcode arbitrário, é materialização de DECISION), trigger append-only (D9), índices, sem backfill/autoridade automática. Arquitetura-chave (3 lições material): (1) **execution-seal guard (f) escaneia TODAS as *.routes.ts por approveActorWalletPayout** — então a rota NÃO pode chamar o bridge; criei um ORQUESTRADOR de módulo (`payout-approval.service.ts`) que chama o bridge, e a rota delega a ele → seal verde intocado (CAMINHO A teria forçado reescrever o seal se a rota chamasse o bridge). (2) **Core não importa @modules** (arch:core) — então a DECISÃO (policy/authority/D7/diário/evento) vive no Core (`@core/financial-approval`, só SQL, zero import de módulo) e o bridge fica no orquestrador do módulo; o Core faz SQL nas tabelas D7 (actors/identities/atl/risk/recovery) sem importar TS de módulo (SQL ≠ import). (3) **daily-limit race**: advisory lock `pg_advisory_xact_lock(hashtext(tenant),hashtext(actor))` na TX da decisão + idempotency_key por payout (1 evento approved/payout) + active-gate (1 payout ativo/actor) → 2 aprovações concorrentes do mesmo payout = 1 evento, daily contado 1× (provado T35). Erros/correções no caminho: (a) **tsc strict:false de novo** — discriminante boolean (`approved:true|false`) não estreita non-strict; troquei p/ `kind:'approved'|'blocked'` string-literal (lição 49 reaplicada — já sabia, mas reincidi por hábito; a partir de agora SEMPRE string-literal em union de produção). (b) o guard NOVO mordeu meu próprio NOT_CONFIGURED_REASON... não, isso foi a frente anterior. (c) **financial-approval-core-boundary guard FALHOU** sobre meu Core service: o token de import `\/payout` (substring cru) é amplo demais e casou `from './payout-approval-policy.constants'` (arquivo INTERNO do Core, não o módulo). Precisei p/ `modules/payout` — tightening honesto (Core segue proibido de importar o módulo payout/Bank via modules/payout|payout.service|actor-wallet-payout|bank-*); documentei como correção de falso-positivo, não enfraquecimento. Decidi tightening em vez de renomear meus arquivos (rename cascata em 5 lugares vs 1 linha precisa + correta). (d) seed e2e: clearing coverage credit precisa de actor_id (bank_transactions.actor_id NOT NULL) — movi a coberta p/ dentro de mkActorPayout (atribuída ao actor) em vez do mkTenant (sem actor ainda). e2e 37/37 com 12 actors (happy/faixa/daily-preseed/D7×4/conc/revoke/spoof) + 3 tenants (principal/sem-policy/cross-tenant), auth stub mutável (flip tenant+user por request), recovery obligation seedada com payment_intent real, concorrência via Promise.all. Gates: regression rc=0, core-boundary 5 arquivos NÃO-EXECUTOR, arch critical_new=0, tsc 25. Cartório: execução log + STATUS + DT-NOT-CONFIGURED→CLOSED + DT-KYB-DEFERRED (nova) + opus. Hard stops: HTTP não executa, bank_* intocado, grants comuns não aprovam (autoridade só via financial_approval_authorities), faixa travada no banco+resolver. IMPLEMENTED, HOLD pra Yala. Próximas: ativar worker (decisão própria), destino externo+KYB, multi-approval.
