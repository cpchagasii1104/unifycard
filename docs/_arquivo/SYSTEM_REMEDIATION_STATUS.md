...
| Atualização 2026-05-24 [social-1] | Fatia getFeed FIXED — DT-DRIFT-SOCIAL-2.0-SERVICE-SCHEMA-MISMATCH CRITICAL CLOSED via 11 drifts convergidos (`0c478dec`) aplicando DECISIONs 0031/0032-social/0033 de 19/05; 2 drifts adicionais descobertos pelo runtime (`groups.group_id`, `groups.visibility`); HTTP 500 → HTTP 200 confirmado; DT-FEED-MEDIA-HIDRATATION-PENDING e DT-PRESSURE-GROUPS-VISIBILITY-FANTASMA abertas com escopo concreto |
| Atualização 2026-05-24 [social-2] | Fatia A1 toggleReaction FIXED — convergência polimórfica DECISION-0031 (4 SQLs + signature rename `globalUserId → actorId`) (`b4200afd`); 3 ramificações de escrita exercitadas via runtime com SELECT confirmatório direto no banco (INSERT/DELETE-toggle-off/UPDATE); lição "grep callers antes de mexer em signature" registrada em memória persistente após acerto-por-sorte (único caller backend); sub-achado drift zod↔CHECK em `reaction_type` registrado |
| Atualização 2026-05-24 [social-3] | Fatia A2 FIXED — 5 escritas/leituras sociais convergidas em 1 fatia (createComment com signature `globalUserId → actorId` + pivot duplo via users eliminado, followActor/unfollowActor/isFollowing/getActorCounts com drift SQL `follows.actor_id → followed_actor_id`) (`05470985`); disciplina A1 aplicada como protocolo (grep callers + grep contrato JSON) ANTES dos Edits; prova material por endpoint com SELECT direto no banco; pendência da rota composta HTTP empurrada para Fatia C |
| Atualização 2026-05-24 [social-4] | Fatia C FIXED — getActorPosts (10 drifts mecânicos, query-irmã do getFeed) + getComments (4 drifts + JOIN direto sem pivot via users) convergidos (`720946a3`); rota composta `GET /social/actors/:id` HTTP 200 destravada (pendência A2 fechada); sub-achado lateral material: frontend `api/social.ts:133` declara `getComments` como `NOT_IMPLEMENTED` quando endpoint backend EXISTE — categoria nova "assumed-fantasma" (inverso do drift de schema), backlog frontend |
| Atualização 2026-05-24 [social-5] | Fatia B createPost FIXED (cirúrgica) — corpo convergido com INSERT canônico `media_ids` direto + 3 blocos PREMATURO removidos (post_media DECISION-0033, post_projects DECISION-0034, post_cta DECISION-0032-social) (`5241c1e9`); signature intacta protegendo 6 callers internos (descoberta pelo grep); DT-CREATEPOST-SIGNATURE-DUAL-USERID aberta para refator de signature futuro; INSERT canônico provado via tsx + cross-check no feed |
| Atualização 2026-05-24 [social-6] | Fatia D FIXED — 1 arquivo, 1 linha (`social-2.0.routes.ts:242`: `req.actionContext.actorId` → `req.user.id`) (`9414c0db`); destrava ponta a ponta os 2 caminhos do POST `/social/posts` (com e sem `actor_id` no body); micro-auditoria prévia provou que (a) `posts.global_user_id` nunca existiu em DDL — drift puro, (b) gate de authority não era bug independente — era o mesmo bug do slot dual da rota observado em outra camada; **hipótese "ownership/delegation ausente" REFUTADA materialmente** (dev actor sempre teve estado canônico); DT-CREATEPOST-SIGNATURE-DUAL-USERID atualizada: HIGH → LOW, escopo encolhido para "remover 1 param morto ~6-7 LOC" |
| Atualização 2026-05-24 [social-final] | **Módulo social SQL inteiro convergido para schema canônico em runtime real** — toda interação humana funcional ponta a ponta: ver feed / curtir / comentar / seguir / contadores / perfil / ler comentários / criar post via UI. Bloqueio remanescente conhecido: frontend stub `NOT_IMPLEMENTED` em `getComments` helper (escopo frontend, categoria assumed-fantasma) |
