...
```yaml
domain: bank
description: Visualizar termos financeiros de ordem de serviço
capability_required: null
applies_to:
  - GET /services/orders/:orderId/financial-terms
who_can_have:
  - owner da ordem
  - prestador do serviço
  - delegated users (se scopes contém financial_terms:view)
```

#### `financial_terms:confirm`
```yaml
domain: bank
description: Confirmar termos financeiros de ordem de serviço
capability_required: can_hold_assets
applies_to:
  - POST /services/orders/:orderId/financial-terms/confirm
who_can_have:
  - owner da ordem
  - delegated users (se scopes contém financial_terms:confirm)
```

#### `split:view`
```yaml
domain: bank
description: Visualizar splits de pagamento
capability_required: null
applies_to:
  - GET /bank/splits/:splitId
  - GET /bank/splits
who_can_have:
  - owner do split
  - participantes do split
  - delegated users (se scopes contém split:view)
```

#### `split:create`
```yaml
domain: bank
description: Criar split de pagamento
capability_required: can_hold_assets
applies_to:
  - POST /bank/splits
who_can_have:
  - owner do pagamento
  - delegated users (se scopes contém split:create)
```

#### `financial:execute_payout`
```yaml
domain: bank
description: Executar payout (repasses financeiros)
capability_required: can_hold_assets
applies_to:
  - POST /bank/payouts/execute
who_can_have:
  - owner do actor
  - delegated users (se scopes contém financial:execute_payout)
```

#### `financial:view_ledger`
```yaml
domain: bank
description: Visualizar ledger (próprio actor)
capability_required: null
applies_to:
  - GET /bank/ledger
who_can_have:
  - owner do actor
  - delegated users (se scopes contém financial:view_ledger)
```

#### `financial:view_all_ledger`
```yaml
domain: bank
description: Visualizar ledger consolidado (todos os actors)
capability_required: null
applies_to:
  - GET /bank/ledger/all
who_can_have:
  - usuários com permissão explícita (atribuição manual)
  - delegated users (se scopes contém financial:view_all_ledger)
```

#### `calendar:view`
```yaml
domain: bank
description: Visualizar agenda (Unified Availability)
capability_required: null
applies_to:
  - GET /availability/calendar
  - GET /availability/calendar/:actorId
who_can_have:
  - owner do actor
  - delegated users (se scopes contém calendar:view)
```

#### `calendar:block`
```yaml
domain: bank
description: Bloquear agenda (criar availability bloqueada)
capability_required: null
applies_to:
  - POST /availability/block
who_can_have:
  - owner do actor
  - delegated users (se scopes contém calendar:block)
```

#### `calendar:unblock`
```yaml
domain: bank
description: Desbloquear agenda (remover availability bloqueada)
capability_required: null
applies_to:
  - DELETE /availability/block/:availabilityId
who_can_have:
  - owner do actor
  - delegated users (se scopes contém calendar:unblock)
```
...
who_can_have:
  - user actors → ownership suficiente
...
who_can_have:
  - owner da ordem
  - prestador do serviço
  - delegated users (se scopes contém service_order:view)
```

#### `service_order:confirm`
```yaml
domain: services
description: Confirmar ordem de serviço
capability_required: null
applies_to:
  - POST /services/orders/:orderId/confirm
who_can_have:
  - owner da ordem
  - delegated users (se scopes contém service_order:confirm)
```

#### `service_order:start`
```yaml
domain: services
description: Iniciar execução de ordem de serviço
capability_required: null
applies_to:
  - POST /services/orders/:orderId/start
who_can_have:
  - prestador do serviço
  - delegated users (se scopes contém service_order:start)
```

#### `service_order:complete`
```yaml
domain: services
description: Completar ordem de serviço
capability_required: null
applies_to:
  - POST /services/orders/:orderId/complete
who_can_have:
  - prestador do serviço
  - delegated users (se scopes contém service_order:complete)
```
...
capability_required: null
applies_to:
  - POST /services/orders/:orderId/cancel
who_can_have:
  - owner da ordem
  - prestador do serviço
  - delegated users (se scopes contém service_order:cancel)
```

#### `rfq:create`
```yaml
domain: services
description: Criar RFQ (Request for Quote)
capability_required: null
applies_to:
  - POST /services/rfqs
who_can_have:
...
```yaml
domain: services
description: Visualizar RFQ
capability_required: null
applies_to:
  - GET /services/rfqs/:rfqId
  - GET /services/rfqs
who_can_have:
  - owner do RFQ
  - prestadores de serviço (para responder)
  - delegated users (se scopes contém rfq:view)
```

#### `rfq:close`
```yaml
domain: services
description: Fechar RFQ
capability_required: null
applies_to:
  - POST /services/rfqs/:rfqId/close
who_can_have:
  - owner do RFQ
  - delegated users (se scopes contém rfq:close)
```

#### `rfq:convert`
```yaml
domain: services
description: Converter RFQ em ordem de serviço ou bundle
capability_required: null
applies_to:
  - POST /services/rfqs/:rfqId/convert
who_can_have:
  - owner do RFQ
  - delegated users (se scopes contém rfq:convert)
```

#### `quote:submit`
```yaml
domain: services
description: Submeter cotação para RFQ
capability_required: null
applies_to:
  - POST /services/rfqs/:rfqId/quotes
who_can_have:
  - prestadores de serviço
  - delegated users (se scopes contém quote:submit)
```

#### `quote:view`
```yaml
domain: services
description: Visualizar cotações de RFQ
capability_required: null
applies_to:
  - GET /services/rfqs/:rfqId/quotes
  - GET /services/quotes/:quoteId
who_can_have:
  - owner do RFQ
  - autor da cotação
  - delegated users (se scopes contém quote:view)
```

#### `bundle:create`
```yaml
domain: services
description: Criar bundle de serviços
capability_required: null
applies_to:
  - POST /services/bundles
who_can_have:
  - user actors → ownership suficiente
  - company actors → ownership suficiente
```
...
```yaml
domain: services
description: Visualizar bundle de serviços
capability_required: null
applies_to:
  - GET /services/bundles/:bundleId
  - GET /services/bundles
who_can_have:
  - owner do bundle
  - prestadores envolvidos
  - delegated users (se scopes contém bundle:view)
```

#### `bundle:confirm`
```yaml
domain: services
description: Confirmar bundle de serviços
capability_required: null
applies_to:
  - POST /services/bundles/:bundleId/confirm
who_can_have:
  - owner do bundle
  - delegated users (se scopes contém bundle:confirm)
```
...
### v1.5 (2026-01-XX) - Hardening de Permissões Administrativas

**Permissions adicionadas:**
```
institutional: admin:view_regional_fund, admin:view_consolidated_balance, admin:view_fund_reports, admin:view_audit_logs
reports: reports:view_operational, dashboard:view
```

**Substituições:**
```
economy:accounts:read → admin:view_fund_reports
```

**Justificativa:**
- Correção de vazamento de autoridade administrativa
- Substituição de permissão não canônica (economy:accounts:read) por permissão institucional explícita
- Blindagem de consolidação: relatórios/dashboards com consolidated=true requerem view_consolidated_reports
- Separação clara entre permissões operacionais (próprio actor) e administrativas (sistema)

---

### v1.4 (2026-01-XX) - Fase 4.2: Core de Decisão Permissions

**Permissions adicionadas:**
```
services: service_order:create, service_order:view, service_order:confirm, service_order:start, service_order:complete, service_order:cancel
services: rfq:create, rfq:view, rfq:close, rfq:convert
services: quote:submit, quote:view
services: bundle:create, bundle:view, bundle:confirm
bank: financial_terms:view, financial_terms:confirm
bank: split:view, split:create
bank: financial:execute_payout, financial:view_ledger, financial:view_all_ledger
bank: calendar:view, calendar:block, calendar:unblock
marketplace: MARKETPLACE_STORE_CREATE, MARKETPLACE_STORE_VIEW
marketplace: MY_ORDERS_VIEW
```

**Justificativa:**
- Core de Decisão requer Permission Keys explícitas para todos os BusinessActions
- Separação entre visualização e execução de ações críticas
- Permissões financeiras requerem capability can_hold_assets quando aplicável
- Permissões de agenda conectadas ao Core Temporal (Unified Availability)
...
```yaml
domain: marketplace
description: Criar loja no marketplace
capability_required: null
applies_to:
...
```yaml
domain: marketplace
description: Visualizar loja no marketplace
capability_required: null
applies_to:
...
who_can_have:
  - owner da loja
  - delegated users (se scopes contém MARKETPLACE_STORE_VIEW)
```

#### `MY_ORDERS_VIEW`
```yaml
domain: marketplace
description: Visualizar próprios pedidos
capability_required: null
applies_to:
  - GET /marketplace/orders/my
  - GET /marketplace/orders/my/:orderId
who_can_have:
  - qualquer user actor autenticado (próprios pedidos)
note: |
  Permissão para visualizar apenas os próprios pedidos do usuário.
  Não requer ownership de actor específico, apenas autenticação.
```
...
```yaml
domain: reports
description: Visualizar relatórios operacionais do próprio actor
capability_required: null
applies_to:
  - GET /reports/operational
  - GET /reports/operational/:reportId
who_can_have:
  - owner do actor
  - delegated users (se scopes contém reports:view_operational)
note: |
  Se consolidated=true, requer view_consolidated_reports
```

#### `dashboard:view`
```yaml
domain: reports
description: Visualizar dashboard do próprio actor
capability_required: null
applies_to:
  - GET /dashboard
...
who_can_have:
  - owner do actor
  - delegated users (se scopes contém dashboard:view)
note: |
  Se consolidated=true, requer view_consolidated_reports
```
...
```yaml
domain: institutional
description: Visualizar fundos regionais
capability_required: null
applies_to:
  - GET /admin/fund/regional
...
```yaml
domain: institutional
description: Visualizar e reconciliar balanço consolidado
capability_required: null
applies_to:
...
```yaml
domain: institutional
description: Visualizar relatórios administrativos do fundo
capability_required: null
applies_to:
  - GET /admin/fund/reports
  - GET /economy/fund/admin/*
who_can_have:
  - admin users (atribuição manual)
  - system operators
```
...
```
Total de permissions: 61 (v1.5)
Total de capabilities: 5
Total de domínios: 11

Distribuição:
- feed: 2 permissions
- bank: 11 permissions (3 originais + 8 novas: financial_terms, split, financial, calendar)
- events: 3 permissions
- groups: 3 permissions
- services: 18 permissions (2 originais + 16 novas: service_order, rfq, quote, bundle)
- rides: 3 permissions
...
- marketplace: 13 permissions (10 originais + 3 novas: store, orders)
...
**Status:** ✅ v1.5 (Hardening de Permissões Administrativas)  
**Data:** 12 de Janeiro de 2026 (v1.0), XX de Janeiro de 2026 (v1.1), XX de Janeiro de 2026 (v1.3), XX de Janeiro de 2026 (v1.4), XX de Janeiro de 2026 (v1.5)  
**Próxima revisão:** Após piloto REAL  
**Evolução:** v1.5+ apenas com justificativa  
