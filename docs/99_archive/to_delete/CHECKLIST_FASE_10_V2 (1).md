# 📋 CHECKLIST FASE 10 v2.1 — EVENTOS + ESCROW + PENALIDADES + RESPONSABILIZAÇÃO
...
## PARTE F: REFINAMENTOS (IDENTIFICADOS NOS EDGE CASES)

### F.1 Validação Geolocalização para Check-in

**Problema:** Check-in falso (pessoa marca sem estar no local)

**Solução:**

```typescript
// backend/src/modules/events/check-in.validator.ts
...
async function validateCheckInLocation(params: CheckInValidation): Promise<boolean> {
  ...
  if (distance > GEO_RADIUS_METERS) {
    throw new Error(`Check-in fora do raio permitido (${distance}m > ${GEO_RADIUS_METERS}m)`);
  }
...
**Arquivos:**
- Criar: `backend/src/modules/events/check-in.validator.ts`
- Modificar: `backend/src/modules/events/events-v2.service.ts` (checkIn method)

### Checklist F.1
- [ ] Criar validador de geolocalização
- [ ] Integrar no fluxo de check-in
- [ ] Adicionar fallback para check-in manual (validado por organizador)
- [ ] Commit: `feat(events): add geo validation for check-in`

---

### F.2 Sistema de Reclamação Automática

**Problema:** Conluio entre organizador e prestador

**Solução:**

```typescript
// Após evento, sistema envia pesquisa automática para compradores
// Se reclamação > 30% → flag no evento e nos atores

interface ComplaintThresholds {
  flagThreshold: 0.30;       // 30% = flag
  reviewThreshold: 0.50;     // 50% = revisão obrigatória
  penaltyThreshold: 0.70;    // 70% = penalidade automática
}

// Migration: complaints table
CREATE TABLE event_complaints (
  id UUID PRIMARY KEY,
  event_id UUID NOT NULL,
  actor_id UUID NOT NULL,      -- Quem reclamou
  complaint_type VARCHAR(50),  -- 'no_show', 'poor_quality', 'fraud', 'other'
  description TEXT,
...
**Arquivos:**
- Criar: `migrations/095_event_complaints.sql`
- Criar: `backend/src/modules/events/complaints.service.ts`

### Checklist F.2
- [ ] Criar migration de complaints
- [ ] Criar service de reclamações
- [ ] Implementar job de envio de pesquisa pós-evento
- [ ] Implementar cálculo de taxa de reclamação
- [ ] Commit: `feat(events): add complaint system`

---

### F.3 Limites Progressivos por Histórico

**Problema:** Usuário novo criando evento de R$500

**Solução:**

```typescript
const PROGRESSIVE_LIMITS = {
  // Novato (0-2 eventos bem-sucedidos)
  novice: {
    maxTicketPriceCents: 5000,    // R$50
    maxAttendeesPerEvent: 50,
    maxActiveEvents: 1,
  },
  
  // Intermediário (3-9 eventos bem-sucedidos)
  intermediate: {
    maxTicketPriceCents: 15000,   // R$150
    maxAttendeesPerEvent: 200,
    maxActiveEvents: 3,
  },
  
  // Avançado (10+ eventos bem-sucedidos)
  advanced: {
    maxTicketPriceCents: 50000,   // R$500
    maxAttendeesPerEvent: 1000,
    maxActiveEvents: 10,
  },
  
  // Verificado (identidade confirmada + score >80)
  verified: {
    maxTicketPriceCents: null,    // Sem limite
...
**Arquivos:**
- Criar: `backend/src/modules/events/limits.service.ts`
- Modificar: `backend/src/modules/events/events-v2.validator.ts`

### Checklist F.3
- [ ] Criar service de limites progressivos
- [ ] Integrar no validator de criação de evento
- [ ] Adicionar cálculo de tier baseado em histórico
- [ ] Commit: `feat(events): add progressive limits by history`

---

### F.4 Regras de Evento Híbrido (Online + Presencial)

**Problema:** Evento com parte presencial e parte online

**Solução:**

```typescript
// Novos campos em events
interface HybridEventFields {
  is_hybrid: boolean;
  online_capacity: number | null;
  presential_capacity: number | null;
  streaming_url: string | null;
}

// Check-in diferenciado
interface CheckInType {
  'presential': 'geo' | 'qr' | 'manual';
  'online': 'login' | 'time_spent';  // Mínimo 30 min conectado
}

// Split pode ser diferenciado
interface HybridTicketType {
  type: 'presential' | 'online' | 'both';
  price_cents: number;
  split_profile: 'standard' | 'reduced';  // Online pode ter split menor
}
```

**Arquivos:**
- Criar: `migrations/096_hybrid_events.sql`
- Modificar: `backend/src/modules/events/events-v2.types.ts`

### Checklist F.4
- [ ] Criar migration para campos híbridos
- [ ] Implementar check-in online (por tempo de permanência)
- [ ] Permitir tipos de ingresso diferenciados
- [ ] Commit: `feat(events): add hybrid event support`

---

### F.5 Fluxo de Disputa Formal

**Problema:** Organizador e prestador discordam sobre quem causou falha

**Solução:**

```typescript
// Estados de disputa
type DisputeStatus = 'OPEN' | 'EVIDENCE_COLLECTION' | 'REVIEW' | 'RESOLVED';

interface Dispute {
  id: string;
  event_id: string;
  opener_actor_id: string;       // Quem abriu
  disputed_actor_id: string;     // Contestado
  reason: string;
  evidence: Evidence[];
  status: DisputeStatus;
  resolution: 'OPENER_WINS' | 'DISPUTED_WINS' | 'BOTH_PENALIZED' | 'NO_ACTION';
}

// Evidências válidas
interface Evidence {
  type: 'CHECK_IN_LOG' | 'GEO_DATA' | 'PHOTO' | 'MESSAGE' | 'SYSTEM_LOG';
  content: string;
  timestamp: Date;
}
```
...
**Arquivos:**
- Criar: `migrations/097_disputes.sql`
- Criar: `backend/src/modules/disputes/disputes.service.ts`

### Checklist F.5
- [ ] Criar migration de disputas
- [ ] Criar service de disputas
- [ ] Implementar coleta de evidências automática
- [ ] Definir prazo máximo (7 dias para resolver)
- [ ] Commit: `feat(disputes): add formal dispute flow`

---

### F.6 Eventos Gratuitos com Custos

**Problema:** Evento gratuito onde colaboradores têm valores acordados, mas não há escrow

**Solução:**

```typescript
// Para eventos gratuitos com custos, organizador deposita antecipadamente

interface FreeEventWithCosts {
  is_free: true;
  has_collaborator_costs: true;
  estimated_costs_cents: number;
  
  // Organizador deve depositar antes de publicar
  organizer_deposit_required: true;
  deposit_status: 'PENDING' | 'DEPOSITED' | 'RELEASED';
}

// Fluxo:
// 1. Organizador cria evento gratuito
// 2. Adiciona colaboradores com valores
// 3. Sistema calcula total de custos
// 4. Organizador deposita valor no escrow
// 5. Só então evento é publicado
// 6. Pós-evento: escrow paga colaboradores
```

**Arquivos:**
- Modificar: `backend/src/modules/events/events-v2.service.ts`
- Modificar: `backend/src/core/economy/escrow.service.ts`

### Checklist F.6
- [ ] Implementar depósito antecipado para eventos gratuitos com custos
- [ ] Bloquear publicação sem depósito
- [ ] Adaptar fluxo de escrow
- [ ] Commit: `feat(events): add organizer deposit for free events with costs`

---

## PARTE G: DASHBOARD DE CONFIANÇA

### G.1 Backend — Endpoints de Trust

**Criar:** `backend/src/modules/trust/trust.routes.ts`

```typescript
// GET /api/trust/me — Dashboard completo (privado)
// GET /api/trust/actor/:actorId — Perfil público
// GET /api/trust/me/history — Histórico de score
// GET /api/trust/me/timeline — Linha do tempo
// GET /api/trust/me/penalties — Penalidades
// GET /api/trust/me/badges — Badges
```

**Arquivos:**
- Criar: `backend/src/modules/trust/trust.routes.ts`
- Criar: `backend/src/modules/trust/trust.service.ts`
- Criar: `backend/src/modules/trust/badges.service.ts`

### Checklist G.1
- [ ] Criar módulo trust
- [ ] Implementar endpoints
- [ ] Implementar cálculo de badges
- [ ] Commit: `feat(trust): add trust dashboard endpoints`

---

### G.2 Frontend — Componentes de Trust

**Criar:** `frontend/src/components/trust/`
...
├── TrustScoreCard.tsx
├── TrustStatsGrid.tsx
├── TrustFinancialHistory.tsx
├── TrustResponsibilityCard.tsx
├── TrustPenaltiesList.tsx
├── TrustScoreTimeline.tsx
├── TrustBadges.tsx
└── TrustPublicProfile.tsx
```

### Checklist G.2
- [ ] Criar componentes de trust
- [ ] Criar página /trust (dashboard)
- [ ] Integrar no menu
- [ ] Commit: `feat(frontend): add trust dashboard`
...
1. [ ] Compra de ingresso vai para escrow
2. [ ] Escrow bloqueia antes do evento
3. [ ] Split só acontece pós-evento
4. [ ] Prestador sem check-in não recebe
5. [ ] Colaborador com check-in SEMPRE recebe
6. [ ] Causador é responsabilizado automaticamente
7. [ ] Penalidades aplicadas automaticamente
8. [ ] Score impacta permissões
9. [ ] Validação geo para check-in
10. [ ] Sistema de reclamação funciona
11. [ ] Limites progressivos ativos
12. [ ] Dashboard de Confiança visível
13. [ ] Smoke tests passam 100%

---

*Checklist derivado do CONTRATO_EVENTOS_V1.3.md*
