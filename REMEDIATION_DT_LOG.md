...
- **Resolução prevista:** **D-GENDER ✅ (DECISION-0080, docs-only)** → **F1 ✅ (`global_users.gender` + CHECK + backfill idempotente; blob preservado)** → **F2 ✅ (writers/readers → Identity SSOT; strip do blob; set-once lock; espelho p/ targeting; `other` reconciliado)** → **F3** frontend/contratos (provavelmente DISPENSÁVEL — contrato `metadata.gender` preservado por espelho) → **F4** cleanup `profiles.metadata.gender` (guard fail-closed) → **F5** selo + CLOSE. PF apenas; não fechar antes do selo.
