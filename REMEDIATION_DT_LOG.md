...
- **Status:** **PARTIALLY MITIGATED** (2026-06-01) — neutralizado/redesenhado nas abas Aprendizado (Fatia 4b) **e Interesses** (Fatia 4c); CLOSE pendente de varredura de outros fluxos (ex.: aba Profissional / outros usos de `createCategoryWithAI`).
- **Origem:** auditoria read-only Interesses/Aprendizado Lei 7.
...
- **Resolução prevista:** mover criação/associação semântica para **pipeline governado backend/CONCEPT**, com UI apenas **sugerindo intenção** (não persistindo taxonomia). Abas Aprendizado e Interesses já não dirigem taxonomia; **CLOSE pendente** de varredura/neutralização de **outros fluxos** que ainda chamem `createCategoryWithAI`/`suggestCategoryPath` (ex.: aba Profissional).
