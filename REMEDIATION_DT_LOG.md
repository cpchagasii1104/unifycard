...
- **Status:** OPEN (2026-06-05) — registrada por `DECISION-0108` (docs-only). O `product-concept-guard` (`assertProductConceptAllowedForTenant`, chamado em `product.repository.createProduct:102`) **nasceu antes da DECISION-0105** (1º commit `14111f7c` 2026-05-04; 0105 em 2026-06-05) e **reconflata camadas de `concepts.domain`**: compara `canonical_products.concept_id` (camada **item-comercial**/SKU) contra `company_type_allowed_concepts.concept_id` (camada **vendor/atuação**). Por 0105 são camadas distintas → **interseção zero** → o guard rejeita **todo** produto industrial quando há `company_type` setado; só "passa" pelo bypass quando `tenants.company_type_id` é NULL (**falso-verde**). Bloqueou a materialização da Op2 (supermercado).
