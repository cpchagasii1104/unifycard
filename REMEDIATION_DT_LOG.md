...
- **Resolução prevista:** sequência de fatias: **F-SAUDE-501 ✅** → **F-TARGETING-DECOUPLE ✅** → **F1 schema** (SSOT lifestyle actor-first + consent/visibility/audit) → **F2 backend** → **F3 frontend** (remover sexualOrientation; rever score de completude do core p/ drinks/smokes) → **F4 readers** → **F5 cleanup do blob + selo + CLOSE**. Ordem inegociável: política (DECISION-0071) antes de schema/código.
