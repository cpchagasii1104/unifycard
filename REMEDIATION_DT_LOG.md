...
- **Status:** PARTIALLY MITIGATED / GOVERNED (2026-06-04) — `F-PJ-CNAE-EVIDENCE-SCHEMA-MIGRATION` instalou a **tomada fiscal**: migration `20260604150000_create_fiscal_identity_economic_activities.sql` (satélite 1:N ancorado em `fiscal_identities` ON DELETE CASCADE; `cnae_code`/`cnae_description`/`is_primary`/`source`/`fetched_at`; CHECKs não-vazios; `uq_fiea_fiscal_cnae` UNIQUE(fiscal_identity_id, cnae_code); `uq_fiea_one_primary` partial-unique ≤1 principal; **sem QSA/dados pessoais**, LGPD D5). Schema-only — **NÃO há writer** (D14). Aplicada em `unificard_dev` (358→**359**). e2e schema efêmero **24/24 verde**. **NÃO CLOSED** — falta o writer (persistir do `fetchCNPJFromRevenue` já existente, fail-open) → frente `F-PJ-CNAE-EVIDENCE-WRITER`. _(antes: GOVERNED/DECISIONED 2026-06-04; OPEN 2026-06-04.)_
- **Status anterior 1:** GOVERNED / DECISIONED (2026-06-04) — governada por `DECISION-0103`: a evidência CNAE/atividade mora na **casa fiscal** (`fiscal_identities` + satélite 1:N `fiscal_identity_economic_activities` com `source`/`fetched_at`), buscada backend-side, **fail-open** no nascimento, **sem QSA bruto** (LGPD); CNAE é evidência (não SSOT; só sugere). Modelo definido; resíduos viram DTs próprias (`DT-PJ-COMPANY-ACTIVITY-COLUMNS-GHOST`, `DT-PJ-CNAE-TO-CONCEPT-SUGGESTION-MATRIX-MISSING`).
- **Status anterior 2:** OPEN (2026-06-04) — criada por `DECISION-0102` (D4).
- **Origem:** auditoria read-only `F-PJ-ONBOARDING-DOMAIN-ELIGIBILITY`.
