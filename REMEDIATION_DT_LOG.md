...
- **Mitigação atual:** DECISION-0073 fixa Educação MVP = **declaração não-verificada**; veto de apresentar formação como verificada sem emissor/prova/autoridade/terceiro. Órfão inalcançável (sem rota/tabela/caller). **Nada alterado em código nesta fatia (docs-only).**
- **Resolução prevista:** **D1 ✅ (DECISION-0073)** → **F1** neutralizar/quarentenar órfão morto (`profile-education-companies.*` + `EducationSection.tsx`; classificar §4-A) → **F2** ajustar vocabulário/UI/event types (reservar — preferida — ou rebaixar `validada_institucionalmente`/`confirmada`/`contestada`/`validator`/`evidence` para autodeclaração honesta) → **F3** selo Educação. Credencial verificada real (emissor/prova/autoridade/audit) = frente própria futura, **não** implícita.
