...
- **Resolução prevista:** **D1 ✅** → **F1 ✅** → **F2 ✅** → **D2 ✅** → **D-GEO ✅ (0077)** → **F-GEO-1a ✅ (infra)** → **D-GEO-1b ✅ (0078)** → **F-GEO-1b ✅ (cache + service cache-first + script backfill)** → **F-GEO-2** (rodar backfill com provider real env-gated p/ enriquecer os 3 addresses) → **F-GEO-3** core lê city/state do catálogo (sem blob) → **F4** cleanup `profiles.metadata.address` (depende de decisão de neighborhood) → **F5** selo + CLOSE. Compartilhável PF/PJ; PJ não cria resolver/cache paralelo (DECISION-0075 §7/0077). Não fechar nesta fatia.
