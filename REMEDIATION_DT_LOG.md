...
- **Resolução prevista:** **D1 ✅** → **F1 ✅** → **F2 ✅** → **D2 ✅** → **D-GEO ✅ (0077)** → **F-GEO-1a ✅** → **D-GEO-1b ✅ (0078)** → **F-GEO-1b ✅** → **F-GEO-2a ✅ (dry-run)** → **F-GEO-2b ✅ (backfill real: state_id resolvido p/ 3; city_id pendente de provider com IBGE)** → **(provider IBGE/ViaCEP — follow-up)** → **F-GEO-3** core lê **state** do catálogo (city ainda via blob até IBGE) → **F4** cleanup `profiles.metadata.address` (depende de city/neighborhood) → **F5** selo + CLOSE. Compartilhável PF/PJ; PJ não cria resolver/cache paralelo. Não fechar nesta fatia.
