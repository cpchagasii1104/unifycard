...
- **Mitigação atual:** **rota viva do par** (`F-PJ-ACTIVATION-ROUTE-WRITE-PAIR`) — `companies.routes.ts` `POST /:companyId/operational-activation` chama o writer soberano `activateCompanyOperationally` com guard contextual `companiesService.canManageCompany` (company_users) + validação `company_type_allowed_concepts`; e2e de rota `validate-pipeline-e2e-pj-activation-route.ts` (15/15, efêmero). NÃO escreve `tenant_concept_offerings`; NÃO toca marketplace/hybrid. O **gap remanescente** é o onboarding (frontend) ainda projetar a classificação em `metadata.business_category`/`businessType` em vez de chamar a rota do par.
