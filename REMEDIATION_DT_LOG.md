...
- **Status:** GOVERNED / DECISIONED (2026-06-04) — governada por `DECISION-0104`: a matriz CNAE→concept é **sinal de sugestão governado**, não autoridade. Modelo fixado: mapeia `cnae_code` → `suggested_concept_id` (CONCEPT é alvo/SSOT; **não** `MarketplaceDomain`, **não** N0, **não** company_type como identidade); multi-candidato; `confidence` (principal pesa mais que secundário); `rationale`/`source`/`version`/`review_status`; **escopo MVP seletivo** (só CNAEs das 7 verticais vivas — açougue/farmácia/hortifruti/padaria/restaurante/salão/supermercado), **proibido importar CNAE oficial inteiro**; exact-match primeiro (prefixo futuro); sugestão **`pending`** com aplicação explícita; **autoativação proibida** (não escreve `primary_*`/`company_concept_publications`/`tenant_concept_offerings`, não chama ativação); company_type **derivado** via `company_type_allowed_concepts` (D4); Empregos fora (D15); consultoria/imóveis/veículos → nenhuma sugestão ou revisão (D14). Ancorada no precedente de sinais semânticos (`RFC_SEMANTIC_SIGNALS_ONBOARDING_BRIDGE` RASCUNHO + `SEMANTIC_CATALOG_GOVERNANCE`). **NÃO CLOSED** — falta **schema + seed MVP + read endpoint** (frentes próprias). _(antes: OPEN 2026-06-04.)_
- **Status anterior:** OPEN (2026-06-04) — criada por `DECISION-0103` (D10), posterior à persistência de evidência.
