...
- **Resolução prevista:** **D1 ✅** → **F1 ✅ (backend + backfill; core prefere Location Core + enriquece blob; blob preservado)** → **F2 ✅ (frontend grava no endpoint canônico; parou de escrever no blob; load segue via core enriquecido)** → **F3** readers/core sem depender do blob (remover o enriquecimento transitório) → **F4** cleanup `profiles.metadata.address` → **F5** selo + CLOSE. Ordem inegociável respeitada: **F1 antes de F2**. Não fechar nesta fatia.
