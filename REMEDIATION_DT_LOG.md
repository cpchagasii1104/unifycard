...
## DT-PJ-COMPANY-USER-ROLE-VOCABULARY-MISMATCH — OPEN (2026-06-06) — achado lateral

- **Status:** **OPEN (2026-06-06)** — achado durante `F-PJ-ONBOARDING-ROLE-DEDUP`, **fora de escopo** (registrado, não corrigido). Há **fragmentação de vocabulário** no papel da empresa: o formulário de cadastro e o contrato `CompanyUserRole` usam **`owner/partner/director/manager/employee/other`**, mas o CHECK vivo **`chk_company_users_role_valid`** (migration `20260530541000`) aceita **`owner/admin/staff/contractor/member`**. Conjuntos só coincidem em `owner`.
- **Risco:** `createCompany` com `role` ∈ {partner, director, manager, employee, other} **viola o CHECK** (`23514`) e falha — na prática só `owner` passa hoje. O dropdown de cargo do cadastro oferece opções que o banco rejeita. (A frente de dedup mitigou a exibição: o wizard projeta defensivamente o papel cru se não houver rótulo PT, então não quebra; mas a **escrita** segue frágil.)
- **Mitigação atual:** dedup do wizard é vocab-agnóstico (CONFIRMA o que está em `company_users`, com fallback de rótulo). Nenhuma escrita nova introduzida nesta fatia.
- **Resolução prevista:** frente própria que reconcilie os vocabulários (alinhar o contrato/`CompaniesManagerForm` ao conjunto do CHECK, OU evoluir o CHECK + mapear na borda do `createCompany`), governada por decisão de Clayton/RFC. Relaciona-se à família `DT-ACTOR-TYPE-VOCABULARY-FRAGMENTATION`. Sem migration nesta fatia.
