...
## DT-PJ-LEGACY-COMPANY-DOCUMENTS-READERS-GHOST — OPEN (2026-06-06)

- **Status:** **OPEN (2026-06-06)** — achado lateral do tombstone: além do upload (neutralizado), os métodos de LEITURA/ADMIN do substrato legado ainda referenciam a tabela fantasma `company_documents` — `listCompanyDocuments`, get/metadata e o `UPDATE company_documents` da validação admin (`companies.service.ts` ~l.2140/2211/2255/2271/2286), expostos por `GET /companies/:id/documents`, `GET /companies/admin/documents/pending`, `PUT /companies/admin/documents/:id/status`. Todos **dead-on-arrival** (tabela inexistente → erro de runtime). NÃO tocados nesta fatia (escopo = upload). **Resolução prevista:** junto com o writer/UX canônico de `fiscal_identity_documents` (próxima frente), migrar/tombstonar esses leitores e o backoffice de validação para o SSOT 0087.

## DT-PJ-KYB-DOCUMENTS-NOT-IN-ONBOARDING — OPEN (2026-06-06) — STOP de substrato

- **Atualização 2026-06-06 (`F-PJ-LEGACY-DOC-UPLOAD-TOMBSTONE`):** o writer legado `uploadCompanyDocument` foi **tombstoned** (501 + throw) — o risco de "etapa documental contra o legado consolidaria SSOT paralelo" está **neutralizado** (o legado não grava mais). Segue **OPEN** porque o **fluxo canônico** (`fiscal_identity_documents`; writer/rota já existem em `fiscal-identity-document.service.ts`/`identity.routes.ts`) **ainda não foi colocado no wizard** — próxima frente (writer/UX canônico).
