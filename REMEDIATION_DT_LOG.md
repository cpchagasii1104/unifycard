...
- **Resolução prevista:** **D1 ✅ (DECISION-0073)** → **F1 ✅ (órfão morto removido)** → **F2** ajustar vocabulário/UI/event types (reservar — preferida — ou rebaixar `validada_institucionalmente`/`confirmada`/`contestada`/`validator`/`evidence` para autodeclaração honesta) → **F3** selo Educação. Credencial verificada real (emissor/prova/autoridade/audit) = frente própria futura, **não** implícita.
