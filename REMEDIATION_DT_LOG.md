...
- **Mitigação atual:** **rota viva do par** (`F-PJ-ACTIVATION-ROUTE-WRITE-PAIR`) — `companies.routes.ts` `POST /:companyId/operational-activation` chama o writer soberano `activateCompanyOperationally` com guard contextual `companiesService.canManageCompany` (company_users) + validação `company_type_allowed_concepts`; e2e de rota `validate-pipeline-e2e-pj-activation-route.ts` (15/15, efêmero). **+ catálogo governado de seleção** (`F-PJ-ACTIVATION-READ-ENDPOINTS`) — `GET /companies/operational-activation/company-types` e `.../company-types/:companyTypeId/concepts` (read-only, derivam de `company_types` e `company_type_allowed_concepts ⋈ concepts`, sem businessType/businessCategory/hybrid/metadata); e2e `validate-pipeline-e2e-pj-activation-read-endpoints.ts` (13/13, efêmero). NÃO escreve `tenant_concept_offerings`; NÃO toca marketplace/hybrid. O **gap remanescente** é o onboarding (frontend) ainda projetar a classificação em `metadata.business_category`/`businessType` em vez de consumir o catálogo e chamar a rota do par.
- **Resolução prevista (frentes seguintes):** (1) read-endpoints `company_types` + concepts permitidos; (2) onboarding chama a rota do par (para de gravar metadata); (3) marketplace aposenta `hybrid` → trilhos. "ambos" = duas seleções/trilhos (GRAPH), nunca valor atômico. **DT permanece OPEN (parcial).** **NÃO** executar onboarding/marketplace antes da palavra de Clayton (DECISION-0097 §9.2 / D10).
...
- **Mitigação atual (parcial — backend pronto):** o backend já fornece **catálogo governado** para a seleção (`F-PJ-ACTIVATION-READ-ENDPOINTS`: `GET company-types` + `GET company-types/:id/concepts`) **e** o writer/rota do par (`F-PJ-ACTIVATION-ROUTE-WRITE-PAIR`). Falta a peça de **onboarding (frontend)** que consome o catálogo e chama a rota do par — hoje o wizard ainda grava `businessType` em `metadata`. `DECISION-0097` fixa o modelo (D5/D6).
- **Resolução prevista (próxima frente — onboarding frontend):** wizard consome `GET company-types` + `GET .../concepts` (frontend NÃO inventa concept — envia o que o backend expôs), monta o par e chama `POST /:companyId/operational-activation`; "ambos" = dois trilhos por GRAPH (frente própria); sem fallback/default (`REGRA_CANONICA_CRIACAO_DE_CONTEXT`). **DT permanece OPEN.** **NÃO executar** frontend antes da palavra de Clayton (DECISION-0097 §9.1 / D10).
