...
- **Resolução prevista:** **D-GENDER ✅ (DECISION-0080, docs-only)** → **F1** migration `global_users.gender` + backfill `metadata.gender → global_users.gender` → **F2** writers/readers usam a coluna (strip do metadata, espelhando CPF; reconciliar `other`) → **F3** frontend/contratos s/n → **F4** cleanup `profiles.metadata.gender` (guard fail-closed) → **F5** selo + CLOSE. PF apenas; não fechar antes do selo.
