...
- **Resolução prevista:** **F-SAUDE-501 ✅** → **F-TARGETING-DECOUPLE ✅** → **F1a ✅ (desenho)** → **F1b ✅ (migration/schema)** → **F2** backend service/read-write consent-aware → **F3** frontend ProfilePhysical (remover sexualOrientation) → **F4** readers/completude (`core.service` 388/765) → **F5** cleanup do blob (+ micro-decisão backfill) → **F6** selo + CLOSE. Ordem inegociável: política (DECISION-0071) antes de schema/código.
