...
- **Mitigação atual:** nenhuma (docs-only — `DECISION-0097` declara o SSOT e marca os outros como legado). Sem código tocado.
- **Resolução prevista (frente futura — read-only → DECISION de reconciliação):** mapear callers/readers de `businessType`/`businessCategory`; decidir aposentar/promover; garantir escritor único do par `(primary_company_type_id, primary_concept_id)` no Momento 2; "ambos" = duas seleções/trilhos (GRAPH), nunca valor atômico. **DT permanece OPEN.** **NÃO executar** antes da palavra de Clayton (DECISION-0097 §9.2 / D10).
