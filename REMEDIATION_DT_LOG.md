...
- **Risco:** crash entre `INSERT companies` e criação do actor → **company + company_users órfãos sem actor**; `address`/`address_assignment` criados (`:499-518`) **não** são desfeitos no cleanup → endereço órfão; se um `DELETE` de compensação falhar no meio → compensação parcial. Sem atomicidade real, o nascimento pode deixar estado meio-construído.
