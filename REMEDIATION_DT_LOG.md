...
- **Status:** PARTIALLY MITIGATED — GOVERNED (2026-06-04) — governada por `DECISION-0099` (norma) + `DECISION-0100` (modelo). **Schema** (`F-PJ-PUBLICATION-OFFERING-SCHEMA-MIGRATION`, migration `20260604140000`, dev=358): tabela soberana `company_concept_publications` (company/page-actor×concept; FK tenant/company(company_id)/actor×3/concept; CHECK status+lifecycle+published_at; UNIQUE parcial active; índices). **+ Writer** (`F-PJ-PUBLICATION-OFFERING-WRITER-NO-PROJECTION`, commit desta fatia): `company-publications.service.ts` + rotas `POST /companies/:companyId/publications` (publish) e `.../publications/:conceptId/retire` (unpublish), gated por autoridade contextual (`canManageCompany`) + KYB approved (novo helper focado `authorityDecisionService.evaluatePageActorKybApproved`, reusa `evaluateKybLayer` — single-source) + empresa operacional (primary_* not null) + concept=`primary_concept_id`; idempotente; audit inline (created_by/retired_by = actor humano via `ensureUserActor`); reversível. e2e `validate-pipeline-e2e-pj-publication-writer.ts` (20/20, efêmero). **NÃO atualiza `tenant_concept_offerings`** (projeção/discovery é frente própria) → **NÃO CLOSED**: a publicação grava no SSOT mas ainda **não acende no discovery**. _(antes: PARTIALLY MITIGATED schema-only; GOVERNED/DECISIONED; OPEN — 2026-06-04.)_
