...
- **Resolução prevista:** **D1 ✅** → **F1 ✅ (backend + backfill; core prefere Location Core + enriquece blob; blob preservado)** → **F2** frontend ProfilePersonal → rota canônica (`UX_INPUT`) → **F3** readers/core sem depender do blob → **F4** cleanup `profiles.metadata.address` → **F5** selo + CLOSE. Ordem inegociável: **F1 antes de F2**. Não fechar nesta fatia.
