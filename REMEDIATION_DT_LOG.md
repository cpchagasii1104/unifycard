...
- **Status:** **PARTIALLY MITIGATED (2026-06-05)** — `F-SERVICE-TAXONOMY-BRIDGE-SCHEMA-MIGRATION` criou a **ponte governada** (Opção A ratificada por Clayton): tabela `company_type_service_categories` (`company_type_id`→`company_types.id`, `service_category_id`→`categories.category_id`, `is_department`, `source`, `uq_ctsc_type_category`, índices em ambas as FKs). Schema-only (dev 363→**364**, registrada, sem fantasma). **NÃO** repontou `default_*_slugs`, **NÃO** reusou `company_type_allowed_concepts`, **NÃO** alterou `services.category_id`. **Não CLOSED** — falta o **seed do salão** (5 linhas: `servicos-estetica-bem-estar` dept + `servicos-cabeleireiro`/`servicos-barbearia`/`servicos-manicure`/`servicos-estetica-facial`) em `F-SERVICE-TAXONOMY-BRIDGE-SEED-SALON`, e o guard de domínio (`services.category_id ∈ servicos + ramos`) em fatia posterior. _Contexto original abaixo._
