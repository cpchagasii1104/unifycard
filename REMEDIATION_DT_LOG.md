...
- **Resolução prevista (Fase Presential UX 1 — executor pequeno, SEM schema):** **frontend (Codex)** esconde/desabilita botão + modal + texto PROVISIONAL, mata a promessa de `VERIFIED`, não abre o QR como caminho vivo; **backend (Claude)** `requestValidation`→501/410 honesto (ou bloqueia QR órfão) e `validate/in-person` com tombstone honesto. **Fase UX 2** (higiene): avaliar `validation-history`, remover exports mortos, limpar textos. **DT permanece OPEN** até o executor. **Distinção:** esta DT é a **UX/QR órfã** (cleanup); a **evidência presencial KYB** (greenfield) fica em `DT-PJ-FASE12-QR-KYB-EVIDENCE-DESIGN-MISSING`.
