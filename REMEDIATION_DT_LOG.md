...
- **Status:** PARTIALLY MITIGATED (2026-06-04) — backend 1A (501 honesto) + frontend 1B (UI viva removida) feitos; **a mentira institucional/CTA órfã não existe mais na UI viva nem no backend**. Resta só **código morto** (UX 2/higiene): `CompanyValidationModal.tsx`/`.css` órfãos, exports mortos `requestCompanyValidation`/`getCompanyValidationHistory`, state `validationModalCompany` + props plumbing não-usados, rota `validation-history` inerte. _(antes: OPEN 2026-06-04.)_
