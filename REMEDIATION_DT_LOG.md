...
- **Resolução prevista:** **D1 ✅** → **F1 ✅** → **F2 ✅** → **D2 ✅** → **D-GEO ✅ (0077)** → **F-GEO-1a ✅** → **D-GEO-1b ✅ (0078)** → **F-GEO-1b ✅** → **F-GEO-2a ✅ (dry-run)** → **F-GEO-2b ✅ (real BrasilAPI: state-only)** → **F-GEO-2c ✅ (ViaCepProvider IBGE + cache-incompleto re-resolve)** → **F-GEO-2d ✅ (execução real `CEP_PROVIDER=viacep`: 3 `city_id` preenchidos = Curitiba reusada)** → **F-GEO-3 ✅ (core.service lê city=`cities.name` / UF=`states.abbreviation` por FK; blob só fallback; bairro ainda residual)** → **D-NEIGHBORHOOD ✅ (DECISION-0079: bairro = texto de exibição, não FK)** → **F-GEO-4a** (campo textual de bairro) → **F-GEO-4b** (migrar bairro blob→core) → **F-GEO-4c** (core sem bairro do blob) → **F-GEO-4d** cleanup `profiles.metadata.address` → **F-GEO-5** selo + CLOSE. Compartilhável PF/PJ; PJ não cria resolver/cache paralelo. Não fechar nesta fatia.
