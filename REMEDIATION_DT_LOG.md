...
- **Resolução prevista:** sequência de fatias (não autorizada aqui): **F-SAUDE-501** (desativar Saúde) → **F-TARGETING-DECOUPLE** (drinks/smokes fora do targeting) → **F1 schema** (SSOT lifestyle actor-first + consent/visibility/audit) → **F2 backend** → **F3 frontend** (remover sexualOrientation) → **F4 readers** → **F5 cleanup do blob + selo + CLOSE**. Ordem inegociável: política (DECISION-0071) antes de schema/código.
