...
- **Status:** PARTIALLY MITIGATED — GOVERNED (2026-06-04) — governada por `DECISION-0099` (norma) + `DECISION-0100` (modelo). **F-PJ-PUBLICATION-OFFERING-SCHEMA-MIGRATION** (commit desta fatia) **criou a tabela soberana** `company_concept_publications` (migration `20260604140000`, aplicada em dev = 358 migrations): cols tenant_id/company_id/page_actor_id→actors(id)/concept_id + status active|retired + published_at/retired_at + created_by/retired_by_actor_id + source/intent; FK tenant/company(company_id)/actor×3/concept; CHECK status+lifecycle+published_at; UNIQUE parcial `(company_id,concept_id) WHERE status='active'`; índices ativos (tenant×concept / page_actor / company / concept). 0 linhas (sem backfill/auto-publicação). `tenant_concept_offerings` intocada (read-model). e2e schema `validate-pipeline-e2e-pj-publication-schema.ts` (25/25, efêmero). **NÃO CLOSED** — falta o **writer** (publish/unpublish gated por KYB approved + `company_users` + page-actor). _(antes: GOVERNED/DECISIONED 2026-06-04; OPEN 2026-06-04.)_
