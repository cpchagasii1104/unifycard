...
- **Resolução prevista:** **D1 ✅** → **F1 ✅** → **F2 ✅** → **D2 ✅ (DECISION-0076)** → **D-GEO ✅ (DECISION-0077)** → **F-GEO-1a ✅ (infra geo: port/provider/service/repo + hook PF best-effort)** → **F-GEO-1b** (cache persistente `cep_resolution_cache` + script backfill dos addresses existentes) → **F-GEO-2/F3** core lê city/state do catálogo (sem blob) → **F4** cleanup `profiles.metadata.address` (depende de decisão de neighborhood) → **F5** selo + CLOSE. Frente Location/Geo é compartilhável PF/PJ; PJ não cria resolver paralelo (DECISION-0075 §7/0077). Não fechar nesta fatia.
