...
| **B7** — tríade de identidade + FK trocada | FAIL | 🟡 **PARCIALMENTE RESOLVIDO** (2026-07-03, GO de Clayton). **Parte (c) — a FK que mente — FECHADA MATERIAL** (`f63c8e7e8`): `event_reservations.global_user_id → actors(id)` era um duplicado redundante/vazio do `actor_id` genesis (escrito só por código morto; o leitor vivo home-feed já usava `actor_id`). Código re-apontado ao `actor_id` canônico + coluna dropada por migration **fail-closed** (aborta se houver dado divergente) + guard; provado em DB efêmera 7/7. **Partes (a)/(b) — convergência da tríade CPF (F4/F5) — NORM-BLOCKED pela própria DECISION-0062**: D9 (*"sem audit+backfill provados, F4 não pode iniciar"*) + D10 (cada fase exige GO próprio) + o audit F1 exige o banco vivo (inacessível). Respeitar a sequência da norma É fazer o que o B7 pede. A tríade `user_id`/`global_user_id`/`actor_id` segue como dívida estrutural (dependência de KYC/payout), mas a armadilha de join aguda foi eliminada. |
