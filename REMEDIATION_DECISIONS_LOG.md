...
## DECISION-0154 — DISCARD do webhook PIX ghost SPRINT-85 (F-CAMADA-1-GATE-IDEMPOTENCIA-OUTBOX-G1 / D1)
...
- **Contexto:** O READ-FIRST do #35 (idempotência/outbox/replay) encontrou DOIS caminhos de webhook PIX: (1) **canônico vivo** `POST /gateway/pix/webhook` (`pix-webhook.controller`) com HMAC fail-closed + dedup de ingestão por `gateway_webhook_events` ON CONFLICT (provider, reference_id); (2) **ghost SPRINT-85** `POST /webhooks/pix/:provider` (`pixWebhookRoutes`) + repo `pix-webhook.repository.ts`, que escreve na tabela **`pix_webhook_events` — NUNCA criada em nenhuma migration** → quebrava em runtime. Dois caminhos para a mesma função, um deles morto.
- **Decisão soberana (Clayton · D1):** **DISCARD** do ghost SPRINT-85. **NÃO reviver** (`REVIVAL_REQUIRED` só com migration + decisão própria). **NÃO criar** a tabela `pix_webhook_events`, **NÃO criar** migration, **NÃO criar** webhook novo. Caminho canônico de webhook PIX permanece **exclusivamente** `/gateway/pix/webhook`.
- **Materialização/Prova (commit desta fatia):** removido o `register(pixWebhookRoutes)` em `app.builder.ts`; removida a export `pixWebhookRoutes` de `pix.routes.ts` (preservada a leitura GET de status de cobrança usada por `marketplace.routes`); deletado o repo órfão `pix-webhook.repository.ts`. Guard `audit-webhook-resolver-idempotency` (anti-revival: migration/app.builder/export) + **NP 3×**. tsc 43 (0 novos) · regression EXIT 0 · bank-ledger/actor-writer OK · architectural critical_new=0. **ZERO bank_*/migration/worker/PORTA-1/payout/dinheiro.**
- **Consequências:** [[DT-PIX-WEBHOOK-SPRINT85-GHOST]] CLOSED/DISCARD. Matriz #35/#18 → FECHADO. RLS-live físico segue NÃO VIRADO; dinheiro/payout/worker HOLD.
- **Responsável:** Clayton / IA-DIRETORA (executor: Claude Opus 4.8).
- **Supera:** —
- **Superada por:** —
...
## DECISION-0156 — Reconciliation of Service Availability SSOT and Honest Opportunity Notification (F-SERVICE-AVAILABILITY-SSOT-RECONCILIATION-PACK / SLICE-0)

- **Data:** 2026-07-01
- **Tipo:** Arquitetural / temporal-autoritativa (DOCS-ONLY) — **NÃO MATERIAL** (não toca runtime/migration/backend/frontend/schema/`docs/01_normative`/DB/dinheiro). Promulga sequência de reconciliação e ratifica SSOT temporal já existente; a materialização é fatia própria futura.
- **Frente:** F-SERVICE-AVAILABILITY-SSOT-RECONCILIATION-PACK · **Slice 0 (docs-only)** · **HEAD (pré-commit):** `085917f86` · branch `rescue-structural`.
- **Status:** **PROMULGADA / DOCS-ONLY / DECISION_PROMULGATED.** Ratifica o READ-FIRST PACK aprovado por Clayton com alta confiança. **Classificação da agenda: `AGENDA_SSOT_COM_CONFLITO`** (SSOT temporal já decidido na norma; runtime ainda não obedece totalmente) — **NÃO** `AGENDA_INDECISA`.
- **Contexto:** Auditoria estratégica READ-ONLY (busca/orquestração/agenda/dispatch) + READ-FIRST temporal revelaram que o marketplace de serviços é hoje **pull** ("prateleira inteligente"), sem orquestrador automático, sem push ao prestador, e que a **agenda não filtra vitrine** — só barra no ato do pedido. Achado decisivo: **o SSOT temporal NÃO é uma decisão em aberto** — já foi promulgado (`unified_availability`/`unified_bookings` = SSOT temporal soberano, `SSOT_REGISTRY_UNIFICARD.md:1-9`; owner canônico do tempo contratável da oferta = `owner_type='service_offering'`, DECISION-0117 D / 0118 D2 / 0146 §A.5). O conflito é **drift de runtime** contra norma já batida: (a) reader legado `owner_type='service'` segue vivo (norma manda CONTER — F-OFFER-5A da DECISION-0146 promulgada mas sem GO); (b) blob `services.metadata.availability` (já rejeitado como não-SSOT por DECISION-0132 §9) governa um trilho de descoberta paralelo; (c) tabela `service_discovery_requests` é 2º trilho de reserva que NÃO converge em `bookings`/`service_orders`; (d) a função `detect_availability_conflicts()` é STUB vazio; (e) `SERVICE_BOOKING_REQUESTED` é definido/consumido mas NUNCA emitido; (f) a Superfície B de confirm (`createFromBookingAndDecision`) BYPASSA o advisory lock; (g) create de booking não valida futuro/pausa/capacity. A Lei de Coerência Sistêmica exige: SSOT decide o estado real, módulo usa o pilar mas não o controla, navegação organiza mas não define verdade. Reconciliar = fazer o runtime obedecer a norma existente, NÃO inventar novo SSOT.
- **Decisão soberana (Clayton · marteladas D1–D7):**
  1. **D1 — SSOT temporal da oferta (RATIFICAÇÃO, não decisão nova):** o SSOT temporal é `availability`/`bookings` via Unified Availability; para a oferta contratável, o owner canônico é `owner_type='service_offering'` (→ `service_offerings.provider_actor_id`). **NÃO** criar novo SSOT temporal; **NÃO** tratar `services.metadata.availability` como autoridade; **NÃO** tratar `owner_type='service'` como caminho novo (é legado a **conter**).
  2. **D2 — Vitrine sem data:** discovery sem data escolhida deve **esconder** oferta sem nenhuma disponibilidade futura ativa. Filtro mínimo = "tem janela futura ativa no SSOT temporal canônico".
  3. **D3 — Vitrine com data/hora:** discovery com data/hora deve filtrar por **janela compatível no SSOT temporal canônico**. **PROIBIDO** usar o blob JSON como fonte soberana.
  4. **D4 — Aviso ao prestador (aprovado, ordem posterior):** `SERVICE_BOOKING_REQUESTED` deve nascer no **create booking canônico**, mirando o **dono resolvido via `resolveAvailabilityOwner`** (offering = `provider_actor_id`), **não** hint de `payload.actorId`. **NÃO emitir antes de A+B** (agenda + discovery honestos) — para não notificar oportunidade falsa.
  5. **D5 — `services.metadata.availability`:** **aposentar** como caminho de agenda. Pode existir apenas como legado/transição documentada, **sem autoridade** e sem governar booking/discovery.
  6. **D6 — `service_discovery_requests`:** precisa **convergir** ao trilho canônico (`bookings`/`service_orders`) **ou ser aposentado**. **NÃO** pode continuar como 2ª verdade/reserva paralela.
  7. **D7 — confirmação/lock:** **obrigatório** — todo caminho que transforma booking aceito em compromisso/`service_order` deve passar pelo **lock/conflito canônico**. A Superfície B sem lock é risco material (vira DT própria).
- **Ordem obrigatória de execução (fatias materiais futuras, cada uma com GO próprio):** **Slice A** (reconciliar runtime ao SSOT temporal canônico: conter reader legado `service` via F-OFFER-5A/DECISION-0146; destino do blob D5; materializar OU remover a menção à função de conflito STUB) → **Slice B** (discovery respeitar agenda futura: D2 sem data / D3 com data; corrigir bug TZ) → **Slice C** (emitir `SERVICE_BOOKING_REQUESTED` honesto). **Slice A antes de B antes de C** — inverter a ordem faria o sistema avisar sobre horário que não existe.
- **HOLD explícito:** dispatch automático estilo Uber (`marketplace-dispatch` morto/in-memory, `/work/instant` latente que toca dinheiro, `/matching` stub, rides morto com guard anti-reativação ativo) · dinheiro · checkout · payout · PORTA-1 · firewall · ranking material · 2º provider material · worker/cron de distribuição. **`SERVICE_BOOKING_REQUESTED` aprovado SÓ depois de agenda/discovery honestos.**
- **Deriva de / mantém:** DECISION-0072 (Unified Availability = SSOT temporal) · DECISION-0014 (consolidação SSOT temporal) · DECISION-0117 D (canonical_services/service_offerings, agenda por referência) · DECISION-0118 D2 (autoridade polimórfica do owner temporal) · DECISION-0132 §9 (finalidade temporal como concept em `availability`; metadata rejeitado) · DECISION-0146 (integridade temporal da oferta + conflito por provider; §A.5 conter reader legado `service`) · DECISION-0147/0148 (publicação/subject de booking). **DECISION-0037 (convergência temporal soberana) segue apenas PROPOSTA — não promulgada; esta DECISION não a promulga.**
- **Materialização/Prova:** **NENHUMA** (docs-only). Registra a escritura do terreno temporal; a executora só mexe no "cano" após este cartório. **ZERO** dinheiro/checkout/order/payment/payout/PORTA-1/migration/DB/runtime/frontend/evento/worker. **Δbank=0.**
- **Consequências:** abre 7 DTs OPEN para os achados materiais (ver `REMEDIATION_DT_LOG.md`): [[DT-SERVICE-AVAILABILITY-RUNTIME-DRIFT-FROM-SSOT]] · [[DT-SERVICE-DISCOVERY-IGNORES-FUTURE-AVAILABILITY]] · [[DT-SERVICE-METADATA-AVAILABILITY-BLOB-PARALLEL]] · [[DT-SERVICE-DISCOVERY-REQUESTS-PARALLEL-TRAIL]] · [[DT-AVAILABILITY-CONFLICT-DETECTION-STUB]] · [[DT-SERVICE-BOOKING-CONFIRM-BYPASSES-LOCK]] · [[DT-SERVICE-BOOKING-REQUESTED-EFFECT-NOT-EMITTED]]. **Nenhuma DT material fechada agora.** Slice A material = **HOLD até GO próprio** após este cartório.
- **Responsável:** Clayton / IA-DIRETORA (executor: Claude Opus 4.8). **Validação prévia:** Clayton (promulgação soberana, marteladas D1–D7).
- **Supera:** —
- **Superada por:** —
- **Referências:** `docs/01_normative/SSOT_REGISTRY_UNIFICARD.md` (SSOT TEMPORAL) · `docs/01_normative/CORE_TEMPORAL_CONTRACT.md` · `docs/memorias/IA-TEMPO-BOOKING.md` · DECISION-0072/0014/0117/0118/0132/0146/0147/0148 · `REMEDIATION_DT_LOG.md` (7 DTs acima) · `STATUS_EXECUCAO_GLOBAL.md` (READ-FIRST PACK).

## DECISION-0157 — Vocabulário canônico de `actor_type` (D-C2: `user`/`page`/`group`/`channel`) + congelamento anti-fragmentação (F-ACTOR-TYPE-VOCABULARY-CANONICAL / achado B6 de `auditoria.md`)

- **Data:** 2026-07-02
- **Tipo:** Ontológica / identidade-autoritativa. **PROMULGAÇÃO docs-only** da ontologia + **1 fatia material money-free** (guard de congelamento; NÃO toca CHECK/readers/triggers/dados/migration/dinheiro). É a **D-C2** que o código aguardava (citada em `seed-dev-test-actors.ts:13`, `DT-ACTOR-TYPE-VOCABULARY-FRAGMENTATION` OPEN desde DECISION-0062 F3.1).
- **Frente:** F-ACTOR-TYPE-VOCABULARY-CANONICAL · **HEAD (pré-commit):** `f9e6f0cd0` · branch `rescue-structural`.
- **Status:** **PROMULGADA.** Classificação: **VOCABULÁRIO_CONVERGIDO_EM_DADOS_COM_DÍVIDA_DE_LEITURA** — a decisão não inventa ontologia; ratifica a que os DADOS já escolheram.
- **Contexto (READ-FIRST vivo, 2026-07-02):** `actors.actor_type` tem CHECK com **10 valores de 3 gerações** (`person/company/system` · `actor_human/actor_organizational/actor_system` · `user/page/group/channel`). Mas o **runtime já convergiu**: em `unificard_dev` só existem `user`(8)/`page`(3)/`group`(1); **zero** linhas em qualquer valor legado ou `channel`. Os **writers** vivos escrevem canônico (`actor.repository.ts` → `user`/`page`; page/group via actor-writer), com **UMA exceção**: `identity.service.ts:314` (`ensureGenesisActorForUser`, schema Genesis onde `actor.id = user.id`) ainda escreve `'actor_human'`. Os **readers** ainda ramificam pelo vocabulário legado defensivamente (`authorization.service.ts:129/349`, `authority-decision.service.ts:157`, trigger `20260510100000:28/38`). Fonte da fragmentação: a normalização histórica `0012_unify_actor_and_kyc_ontology.sql` (`person/user→actor_human`, `company/page→actor_organizational`) foi depois **revertida em direção** pela geração `user/page/group/channel`, sem drenar o CHECK/readers.
- **Decisão soberana (Clayton · D-C2):**
  1. **Vocabulário canônico de `actor_type` = `user` · `page` · `group` · `channel`.** É a resposta única para "que tipo de actor é?". `channel` é canônico e reservado (zero writers hoje — permitido, sem ação).
  2. **Valores legados (`person`/`company`/`system` · `actor_human`/`actor_organizational`/`actor_system`) = CONGELADOS-LEGADO.** Não são novo caminho. **Nenhum writer NOVO** pode produzi-los (enforçado por guard). Os **readers** que ainda os toleram permanecem por ora (norma assintótica — conter o drift agora, drenar depois com prova); tolerar ≠ ratificar.
  3. **Exceção documentada (pré-existente):** `identity.service.ts::ensureGenesisActorForUser` (`actor_human`) e a migration histórica `0012` são allowlistadas no guard como writers legados **conhecidos**. A convergência do writer Genesis (`actor_human`→`user`) NÃO é troca trivial de string — está atada ao modelo Genesis (`actor.id = user.id` vs actor canônico com id próprio) — e fica para a frente de drenagem, não para esta fatia.
  4. **NÃO tocar agora (norma assintótica, exige GO próprio):** o CHECK permissivo (drop dos 6 valores mortos), os ramos legados dos readers/triggers, e a convergência do writer Genesis. Frente futura `F-ACTOR-TYPE-VOCABULARY-DRAIN-*` (material, com E2E + prova de que nenhum dado/reader depende do legado).
- **Fatia material desta DECISION (money-free):** guard `audit-actor-type-vocabulary-freeze.mjs` (em `validate:regression-guards`) — **morde** se qualquer arquivo FORA do allowlist (`identity.service.ts` Genesis + migration `0012` histórica) escrever `actor_type` legado (INSERT VALUES / `actor_type: 'legado'` / `SET actor_type = 'legado'`). Congela o vocabulário no estado atual: nenhum novo writer legado. NÃO toca CHECK/readers/dados/dinheiro.
- **Deriva de / mantém:** DECISION-0062 (CPF/CNPJ SSOT — identidade canônica) · `project_actor_unidade_operacional_soberana` (actor como unidade soberana; capabilities transitórias substituem fragmentação de tipos). **Resolve parcialmente:** [[DT-ACTOR-TYPE-VOCABULARY-FRAGMENTATION]] (passa de OPEN para CONTAINED/FROZEN — fecha a **expansão** da fragmentação; a drenagem material segue aberta como frente própria).
- **Materialização/Prova:** guard money-free (regression-guard, backend-only, sem runtime/migration/DB/dinheiro). Negative-proof: guard morde um writer legado sintético; allowlist cobre os 2 writers legados reais. **Δbank=0.**
- **Consequências:** B6 do `auditoria.md` **contido** (fragmentação congelada; drenagem = frente própria com GO). B7 (tríade + FK que mente) **NÃO** é resolvido aqui — Clayton escolheu "só documentar" (ver DT); execução de DECISION-0062 F4/F5 + renomeação da FK exige GO + revisão adversarial.
- **Responsável:** Clayton / IA-DIRETORA (executor: Claude Fable 5). **Validação prévia:** Clayton (promulgação soberana, D-C2 via AskUserQuestion com evidência viva).
- **Supera:** — · **Superada por:** —
- **Referências:** `auditoria.md` (B6) · `seed-dev-test-actors.ts:13` (D-C2 aguardada) · migrations `0002`/`0012`/`0064` (gerações do CHECK) · `20260510100000_actor_responsibility.sql` (trigger 3-vocabulários) · DECISION-0062 · `REMEDIATION_DT_LOG.md` ([[DT-ACTOR-TYPE-VOCABULARY-FRAGMENTATION]]).
- **📌 ADENDO R3 (2026-07-02, append-only — corrige fato, não muda a decisão soberana; achados R3-1/R3-2 da re-auditoria adversarial rodada 3):** a afirmação "EXATAMENTE 1 writer legado vivo" acima estava **imprecisa**. Inventário correto: **3 writers legados pré-existentes** (todos allowlistados no guard, produção nasce só canônico): (1) `identity.service.ts:314` `'actor_human'` — **SCRIPT-ONLY** (R3-2): `ensureGenesisActorForUser` é private, chamado só por `ensureCanonicalActorChain` cujo único caller é `validate-financial-flow-real.ts` (gate `validate:financial-e2e`); NÃO é caminho de produção vivo (a fragmentação NÃO nasce em produção — melhor que o descrito); (2) migration histórica `0012`; (3) **`seeds/036_seed_e2e_c52_payment_intents.sql:118`** `'company'` (R3-1) — fixture E2E, escapava do guard por dupla cegueira (walk não cobria `seeds/` + P3 não pegava genérico posicional). **A decisão soberana (vocabulário canônico + freeze) permanece inalterada.** Correção material do guard: HEAD `7d311484e` (`fix(identity): close R3-1 freeze-guard blind spots`) — walk cobre `seeds/`, detecção posicional em `INSERT INTO actors`, C52 allowlistado. Efeito colateral notado (R3-2): rodar o gate `validate:financial-e2e` semeia `actor_human` no banco-alvo — some com a convergência do writer Genesis (frente de drenagem).

## DECISION-0158 — Baseline formal (ratchet só-desce) para os gates vermelhos: typecheck ZERADO + financial-vocabulary/ssot congelados no teto (F-RED-GATES-BASELINE / achado B4 de `auditoria.md`)
...
- **Frente:** F-RED-GATES-BASELINE · **HEAD (pré-commit):** `f2e7680ef` · branch `rescue-structural`.
- **Status:** **PROMULGADA / MATERIAL.** Resolve o "Exige DECISION: parcial (baseline formal vs zeragem)" do blocker B4.
- **Contexto:** o laudo `auditoria.md` (B4) apontou o **ponto cego institucional**: 3 gates vermelhos (typecheck 35 no config do gate · `financial-vocabulary` 3.8k · `financial-ssot` ~580) conviviam com um pipeline "verde" — a suíte `validate:regression-guards` não os enxergava. Zeragem das ~4.4k violações financeiras exigiria mexer em massa em código money-adjacent dos trilhos em HOLD (rides/marketplace/treasury) — contra a norma "não-agir em dívida classificada" enquanto o dinheiro está deliberadamente contido.
- **Decisão soberana (Clayton, via AskUserQuestion com recomendação):** **Baseline formal + drenagem do typecheck** (não zeragem):
  1. **Typecheck do gate (`tsconfig.build`) = ZERADO e mantido em zero.** Os 35 erros foram drenados na fatia (34 = um único padrão: narrowing de união discriminada quebrado sob `strict:false` — fix `status?: never` nos 3 producers, type-only, zero runtime; 1 = anotação de tipo em E2E). Baseline = 0: **qualquer erro novo FALHA**.
  2. **`financial-vocabulary` e `financial-ssot` = congelados no teto ratificado** (`red-gates-baseline.json`: 3.846 e 587 respectivamente — os +32/+10 vs o laudo são menções financeiras nos próprios firewalls/guards da remediação, transparente). **Ratchet só-desce:** count > baseline = GATE FAIL (violação nova proibida); count < baseline = abaixar o baseline no mesmo commit. **Subir o teto exige DECISION própria.**
  3. **O pipeline passa a ENXERGAR os 3 gates:** guard `audit-red-gates-baseline.mjs` wired na cadeia `validate:regression-guards` (via agregador — a extensão direta da cadeia estourou o limite de linha de comando do Windows; exceção de custo ~60-90s documentada no agregador) + script standalone `validate:red-gates-baseline`.
  4. **A drenagem real das violações financeiras** (4.4k) segue como frente contínua (`DT-FINANCIAL-SSOT-RED-SERVICE-PAYMENT-EXECUTION-REPOSITORY` e correlatas permanecem OPEN) — o baseline contém a EXPANSÃO, não fecha a dívida.
- **Materialização/Prova:** HEAD material `8ce514db3`. Typecheck gate 35→0 (dev strict 47→46; resto = ruído `req.tenant` pré-existente fora do config do gate). Negative-proof do ratchet: baseline abaixado temporariamente → FALHOU nos 2 mecanismos; restaurado → OK. Cadeia completa EXIT 0 com o baseline vigiando; bank-ledger/actor-writer OK (type-only em rotas money-read); arch strict `critical_new=0`. **Δbank=0.**
- **Consequências:** **B4 fechado como ponto cego** (a dívida financeira continua, contida com teto e rampa). Com isto, **TODOS os pré-requisitos técnicos de PORTA-1 listados no laudo (§2) estão fechados/contidos**: B1 (firewall rides) · B2 (event_settlements contido) · B3 (RLS 23/25) · B4 (baseline) · B5 (contratos). O que resta do laudo = PORTA-1 em si (B8/B9 + rail bancário externo — decisão soberana com IA-DINHEIRO) e frentes de drenagem documentadas (B7/DECISION-0062 F4-F5, actor_type drain, N4/A3/S4-1).
- **Responsável:** Clayton / IA-DIRETORA (executor: Claude Fable 5). **Validação prévia:** Clayton (escolha soberana baseline-vs-zeragem via AskUserQuestion).
- **Supera:** — · **Superada por:** —
- **Referências:** `auditoria.md` (B4, §2 pré-condições PORTA-1) · `backend/scripts/red-gates-baseline.json` (teto ratificado) · `backend/scripts/audit-red-gates-baseline.mjs` (ratchet) · `tsconfig.build.json` (config do gate, strict:false) · [[DT-FINANCIAL-SSOT-RED-SERVICE-PAYMENT-EXECUTION-REPOSITORY]] (drenagem segue OPEN).

## DECISION-0159 — Ratificação de trilhos separados + substrato comum (rejeita ordem canônica única); definição precisa de "ERP Social" como composição (fecha DT-ERP-CRM-CANONICAL-ORDER-CONVERGENCE)

- **Data:** 2026-07-03
- **Tipo:** Arquitetural / ontológica (DOCS-ONLY) — **NÃO MATERIAL** (não toca runtime/migration/backend/frontend/schema/dinheiro). Ratifica um fork que a própria DT já havia nomeado, sem inventar caminho novo.
- **Frente:** análise doutrinária pós-imagens (`visao-do-unificard.png`, `BASE.png`, `mapa.png`, `fluxo.png`) + parecer técnico cruzando `DT-ERP-CRM-CANONICAL-ORDER-CONVERGENCE` contra os diagramas · HEAD (pré-commit) `b4a9d7670` · branch `rescue-structural`.
- **Status:** **PROMULGADA / DOCS-ONLY / DECISION_PROMULGATED.** Resolve o `DECISION_REQUIRED` registrado em `DT-ERP-CRM-CANONICAL-ORDER-CONVERGENCE` (28/06).
- **Contexto:** Clayton apontou que ERP, CRM e PDV, apesar de nomes separados, tocam a mesma pergunta material (serviços, produtos, locações). Antes de qualquer código, releu-se a doutrina arquitetural registrada nos diagramas da raiz do projeto e cruzou-se contra a DT que já mapeara os 4 trilhos de ordem (serviços/produtos-PDV/locações/rides) sem camada comum. A DT já apresentava dois caminhos e já apontava, ela mesma, qual é o doutrinariamente favorecido — citando a tese `actor_unidade_operacional_soberana` — mas não fora promulgado.
- **Decisão soberana (Clayton, via GO ao parecer técnico):**
  1. **Trilhos separados + substrato comum é o modelo canônico.** Serviços, produtos/PDV, locações e (quando auditado) rides permanecem 4 trilhos de escrita distintos, unidos exclusivamente por: `actor` (identidade) + `unified_availability`/`bookings` (tempo) + `bank_ledger` (dinheiro) + `my-orders` (projeção de leitura). **NÃO** nasce uma tabela `orders`/`ordem` canônica única por cima dos 4 trilhos.
  2. **Fundamento doutrinário explícito** (`mapa.png`/`BASE.png`, painel "Alertas e Freios"): uma ordem canônica única violaria **"sem terceira entidade criadora de estado"**, **"sem engine universal"** e o anti-padrão **"segunda fonte de verdade"**. O PDV já prova o modelo correto em produção — reusa `orders`/`order_items`/catálogo do marketplace em vez de ter modelo próprio (achado da pesquisa de 2026-07-03); isso é o desenho de `fluxo.png` ("Trilho A — Produtos" terminando em "PDV/Checkout/Entrega" como nó único) funcionando como projetado, não um atalho a corrigir.
  3. **O gap real não é a ausência de Order único — é a maturação das camadas de orquestração.** Os diagramas marcam **Coordination** como `EMERGENTE (não aberta)` e **Publication** como `EMERGENTE (runtime parcial)`. É exatamente aí que `DT-ERP-CRM-CANONICAL-ORDER-CONVERGENCE` registrava os gaps reais (CRM_GAP, ERP_GAP) — não na inexistência de uma tabela comum.
  4. **Definição precisa de "ERP Social"** (fecha ambiguidade de vocabulário, referências: `07_NOMENCLATURA_CANONICA.md` "ERP Financeiro: 🔴 Ausente" e `mapa.png` Camada 3): **"ERP" não é módulo a construir.** É a composição já nomeada no diagrama: **Identidade real + Autoridade real + Settlement (liquidação centralizada) + Publication (relatórios = projeção)**. Materializar "ERP" significa consolidar Coordination/Publication como camadas reais e compor uma superfície sobre os substratos já soberanos — não desenhar um motor novo. Frentes futuras que mencionarem "construir o ERP" devem ser lidas à luz desta definição.
  5. **Locações confirmadas no caminho certo:** o substrato `rentable_resources` (migration `20260624120000`, DECISION-0151) estende o mesmo pilar TEMPO/`unified_availability` que serviços usa (`owner_type='rentable_resource'`), com guard de conflito real (advisory lock + checagem de sobreposição, não stub) — exatamente "um substrato, N verticais, muda só o concept" (`BASE.png`). Falta completar a superfície HTTP equivalente aos passos 6-8 do Trilho B (agenda→orçamento/booking→dispatch) para recurso; não falta desenho novo.
- **Materialização/Prova:** **NENHUMA** (docs-only). Não altera runtime/migration/schema/dinheiro. **Δbank=0.**
- **Consequências:** `DT-ERP-CRM-CANONICAL-ORDER-CONVERGENCE` passa de **OPEN/DECISION_REQUIRED** para **CLOSED/DECIDED** (ver entrada própria no `REMEDIATION_DT_LOG.md`). Abre precondição formal para `F-RENTAL-RESOURCE-SURFACE-SLICE-A` (rotas HTTP de locação, frente seguinte já autorizada por Clayton). Registra separadamente (DT nova) o achado não previsto nesta DT original: `contacts.tax_id` como candidata a fonte paralela de CPF/CNPJ (cruza com DECISION-0062 D7) — não resolvido por esta DECISION, é precondição própria antes de qualquer plano de tirar as tabelas de CRM do arquivo.
- **Responsável:** Clayton / IA-DIRETORA (executor: Claude Fable 5). **Validação prévia:** Clayton (GO explícito ao parecer técnico, 2026-07-03).
- **Supera:** parte do `DECISION_REQUIRED` aberto em `DT-ERP-CRM-CANONICAL-ORDER-CONVERGENCE` (28/06). **Superada por:** —
- **Referências:** `visao-do-unificard.png` · `BASE.png` · `mapa.png` · `fluxo.png` · `07_NOMENCLATURA_CANONICA.md` (ERP Financeiro) · `REMEDIATION_DT_LOG.md` ([[DT-ERP-CRM-CANONICAL-ORDER-CONVERGENCE]] · [[DT-CRM-CONTACTS-PARALLEL-IDENTITY-RISK]] · [[DT-RIDES-USER-SCOPE-LEGACY-NOT-ACTOR]]) · DECISION-0151 (rental resource model) · DECISION-0062 (CPF/CNPJ SSOT, D7) · `project_actor_unidade_operacional_soberana`.
