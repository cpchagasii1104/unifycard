...
**PRÓXIMA ETAPA (espera Clayton):** após verificação READ-ONLY do `eba663b5`, `go` em **`F-PJ-DOCUMENT-STORAGE-PORT`** (port S3-compatible + `LocalPrivateDocumentStorageProvider` dev + prod-fail-closed + hash/MIME/size/anti-path-traversal + `file_reference` opaco; eventual mime/size). Depois: `F-PJ-DOCUMENT-MALWARE-SCAN-PORT` → `F-PJ-KYB-DOCUMENTS-USER-SUBMIT` → `ADMIN-REVIEW-UI` → `KYB-RELEASE-GATE`. **Nada de upload/wizard antes do port.** Delete-guard / KYB release gate = eixos paralelos não-documentais.
...
> **⚠️ Enquadramento (correção Clayton 2026-06-06).** O commit `dd4e202c` é aprovável **como `F-PJ-LEGACY-DOC-READERS-TOMBSTONE`** (circuito fantasma de leitura/admin em `company_documents` neutralizado), **NÃO** como fluxo documental KYB canônico completo. **`F-PJ-KYB-DOCUMENTS-CANONICAL-FLOW` = PARTIAL / STOPPED.** Subentrega concluída: tombstone dos readers/admin legados. **Bloqueio real (STOP legítimo):** (1) storage/provider documental; (2) autoridade de submit user-facing (`companyId → fiscal_identity_id` + `canManageCompany`/vínculo autorizado). O fluxo canônico de ponta a ponta (usuário envia documento → storage → `file_reference` → `fiscal_identity_documents` → admin revisa) **ainda não existe** — foi BLOQUEADO por STOP, não concluído.
...
**O que entregou:** a evidência CNAE/atividade que `fetchCNPJFromRevenue` (ReceitaWS/BrasilAPI) retornava e era **descartada** agora é **persistida** na casa fiscal. Writer novo `backend/src/core/identity/fiscal-identity-economic-activity.service.ts` (sibling do `fiscal-identity-document.service.ts`): `persistEconomicActivities` normaliza o retorno do provider → lista canônica (**1º principal = `is_primary`**, ≤1 garantido por `uq_fiea_one_primary`; demais principais + secundários = `is_primary=false`), **dedup** por `cnae_code` (principal precede secundário), descarta entradas com code/description vazios, e é **idempotente** via `ON CONFLICT (fiscal_identity_id, cnae_code) DO UPDATE` (atualiza descrição/is_primary/source/fetched_at). Integrado **pós-commit, FAIL-OPEN (D8)** em `createCompany`: usa `birthResult.fiscalIdentityId` + o `revenueData` já em escopo, `source='receita_federal'`, `fetched_at` carimbado no momento da coleta; erro de evidência NÃO derruba a empresa já committada. **NÃO persiste QSA/sócios** (D5). CNAE permanece **evidência, não identidade** — não toca o par/CONCEPT, não autoriza domínio/publicação.

**Arquivos:** `backend/src/core/identity/fiscal-identity-economic-activity.service.ts` (writer), `backend/src/core/companies/companies.service.ts` (`createCompany`: captura `revenueFetchedAt` + bloco pós-commit best-effort), `backend/src/scripts/validate-pipeline-e2e-pj-cnae-evidence-writer.ts` (e2e), `scripts/run-pj-cnae-evidence-writer-ephemeral.ps1` (orquestrador). **Provider `fetchCNPJFromRevenue` NÃO alterado** (endpoints/fallback/null-behavior intactos); nenhuma migration nesta fatia.

**Prova:** e2e writer efêmero **17/17 verde** (createCompany→provider mock persiste 1 principal+2 secundários; exatamente 1 primary; secundários is_primary=false; source/fetched_at preenchidos; FK p/ fiscal_identities; idempotência 2× sem duplicar `(fiscal,cnae)`+update; **sem QSA** mesmo com fixture trazendo sócio; **fail-open** provider-null → empresa nasce com zero CNAE; normalização múltiplos-principais→só-1-primary + dedup principal∩secundário; companies sem colunas de atividade; Bank intocado; tco/ccp intocados; KYB=pending; 359 migrations sem nova migration). **atomic-company-birth 18/18** (createCompany não regrediu). Backend tsc só os 2 baseline geo. Gates: actor-writer/bank-ledger/regression-guards OK; **architectural-patterns `--strict` exit=0** (`critical_new=0`, `warning_new=1` = `validate-pipeline-e2e-c3-actor-wallet-debit-recovery.ts:334`, baseline pré-existente, fora desta fatia).

**DTs:** `DT-PJ-CNAE-EVIDENCE-NOT-PERSISTED` → **CLOSED** (schema+writer prontos, persistência provada). Resíduos = frentes próprias: source provider-granular (deferido p/ não tocar fetch), refresh/reconciliação (D8 futuro), matriz CNAE→concept (`DT-PJ-CNAE-TO-CONCEPT-SUGGESTION-MATRIX-MISSING` OPEN), elegibilidade (DECISION-0102). `DT-PJ-COMPANY-ACTIVITY-COLUMNS-GHOST` permanece CLOSED.

**PRÓXIMA ETAPA:** `F-PJ-CNAE-TO-CONCEPT-SUGGESTION-MATRIX-READONLY` (desenho: matriz CNAE→suggested concept/company_type como **sugestão governada**, nunca autoridade — fecha o ciclo evidência→sugestão de elegibilidade) OU `F-PJ-MARKETPLACE-DOMAIN-VOCABULARY-FORK-READONLY` (ortogonal, antes de religar marketplace). **REGRA:** o aparelho está ligado na tomada fiscal; não deixa o aparelho dirigir a empresa.
