...
const reputation_service_1 = require("@core/reputation/reputation.service"); // ⭐ IMPORTAÇÃO NOVA
class WorkerService {
    toWorker(row) {
        return {
    ...
    // ============================================================
    async getById(tenantId, workerId) {
      ...
      `, [workerId]);
        ...
        worker.reputation = await reputation_service_1.reputationService.getScore(tenantId, 'worker', workerId);
        return worker;
    }
    // ============================================================
    ...
    async getByUserId(tenantId, userId) {
      ...
      `, [userId]);
        ...
        const worker = this.toWorker(row);
        // ⭐ ADICIONA REPUTAÇÃO UNIVERSAL
        worker.reputation = await reputation_service_1.reputationService.getScore(tenantId, 'worker', worker.workerId);
        return worker;
    }
    ...
    `;
      ...
      `, params);
        ...
        for (const w of workers) {
            w.reputation = await reputation_service_1.reputationService.getScore(tenantId, 'worker', w.workerId);
        }
