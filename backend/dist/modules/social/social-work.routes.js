...
const socialWorkRoutes = async (fastify) => {
    /**
     * Helper para validar se usuário é dono do post ou admin/owner
     */
    async function validatePostAccess(req, postId) {
     ...
     */
    fastify.post('/posts/:postId/create-job', {
        ...
        try {
            const job = await social_work_service_1.socialWorkService.createJobFromPost(postId, globalUserId, tenantId);
            req.log.info({
                requestId,
                tenantId,
                globalUserId,
     ...
     */
    fastify.get('/posts/:postId/work-offer', {
        preHandler: fastify.requirePermission(['work:job:read', 'social:post:read']),
    }, async (req, reply) => {
        const tenantId = req.tenant.id;
        const { postId } = req.params;
        const job = await social_work_service_1.socialWorkService.getWorkOfferForPost(postId, tenantId);
        if (!job) {
