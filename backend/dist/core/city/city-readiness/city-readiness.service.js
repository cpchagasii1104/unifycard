...
exports.cityReadinessService = void 0;
...
class CityReadinessService {
    ...
    async getCityReadiness(cityId) {
        ...
        const missingDependencies = await this.detectMissingDependencies(tenantId, cityId);
        // Verificar economia
        const economyReady = await this.checkEconomyReady(tenantId);
        const usersCount = await this.getUsersCount(tenantId);
        const transactionsLast30Days = await this.getTransactionsLast30Days(tenantId);
    ...
    async getCityData(cityId) {
        ...
        const cityResult = await pool_1.pool.query(`
      ...
      `, [cityId]);
        if (cityResult.rows.length > 0) {
            ...
            if (regionId === 'unknown') {
                try {
                    const cityPath = await world_service_1.worldService.getCityFullPath(cityId);
        ...
        try {
            const worldCity = await world_service_1.worldService.getCityById(cityId);
            if (worldCity) {
     ...
     */
    async detectActiveModules(tenantId, cityId) {
        const modules = [];
        // Verificar WORK (workers ativos)
        const workersCount = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        ...
        const ridesActive = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        SELECT is_active
        FROM rides_cities
        ...
        if (ridesActive?.is_active === true) {
            const driversCount = await (0, pool_1.runQueryWithTenant)(tenantId, {
                text: `
          SELECT COUNT(*)::text AS count
          FROM rides_drivers
          WHERE tenant_id = $1
        ...
        const merchantsCount = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        ...
        const regionAccount = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        SELECT EXISTS(
     ...
     */
    async detectMissingDependencies(tenantId, cityId) {
        const missing = [];
        // Verificar economy (contas)
        const accountsCount = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        SELECT COUNT(*)::text AS count
        FROM accounts
        WHERE tenant_id = $1
        ...
        const zonesCount = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        SELECT COUNT(*)::text AS count
        FROM rides_zones
        ...
        const serviceTypesCount = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        SELECT COUNT(*)::text AS count
        ...
        const pricingCount = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        SELECT COUNT(*)::text AS count
     ...
     */
    async checkEconomyReady(tenantId) {
        const accountsCount = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        SELECT COUNT(*)::text AS count
        FROM accounts
    ...
    async getUsersCount(tenantId) {
        const usersCount = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        SELECT COUNT(*)::text AS count
        FROM users
        WHERE tenant_id = $1
     ...
     */
    async getTransactionsLast30Days(tenantId) {
        const transactionsCount = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        SELECT COUNT(*)::text AS count
        FROM transactions
        WHERE tenant_id = $1
     ...
     */
    async canActivateWork(tenantId, cityId) {
     ...
     */
    async canActivateRides(tenantId, cityId) {
        ...
        // Deve haver veículos
        const vehiclesCount = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        SELECT COUNT(*)::text AS count
        FROM rides_vehicles
        WHERE tenant_id = $1
     ...
     */
    async canActivateMarketplace(tenantId, cityId) {
     ...
     */
    async canActivateFund(tenantId, cityId) {
        // Economy deve estar pronto
        if (!(await this.checkEconomyReady(tenantId))) {
            return false;
        }
     ...
     */
    async getWorkBlockers(tenantId, cityId) {
        const blockers = [];
        if (!(await this.checkEconomyReady(tenantId))) {
     ...
     */
    async getRidesBlockers(tenantId, cityId) {
        const blockers = [];
        const city = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        SELECT is_active
     ...
     */
    async getMarketplaceBlockers(tenantId, cityId) {
        const blockers = [];
        const city = await (0, pool_1.runQueryWithTenant)(tenantId, {
            text: `
        SELECT is_active
     ...
     */
    async getFundBlockers(tenantId, cityId) {
        const blockers = [];
        if (!(await this.checkEconomyReady(tenantId))) {
            blockers.push('Economy não está pronto (contas não existem)');
        }
     ...
     * Log estruturado para diagnóstico
     */
    async logReadinessCheck(tenantId, cityId, result) {
        ...
        try {
            await decision_log_service_1.decisionLogService.createObservation('economy', 'city_readiness_check', {
                cityId,
                regionId: result.regionId,
            }, 0, {
                metadata: {
