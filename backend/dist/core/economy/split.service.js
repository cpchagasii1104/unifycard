...
const split_logger_service_1 = require("../logging/split-logger.service");
...
class SplitEngineService {
     ...
     */
    getDefaultConfigForTenant(tenantId, currency = 'BRL', module) {
        // 🔴 REGRA IMUTÁVEL: Contextos EVENT nunca usam WORKER
        const isEventContext = module === 'EVENT_TICKET' || module === 'EVENT_CONSUMPTION';
        ...
        const primaryPercentage = policy_registry_1.policyRegistry.getPolicyValue('economy', isEventContext ? 'split_event_organizer_percentage' : 'split_worker_percentage', 0.70) || 0.70;
        const tenantPercentage = policy_registry_1.policyRegistry.getPolicyValue('economy', 'split_tenant_percentage', 0.15) || 0.15;
        const regionPercentage = policy_registry_1.policyRegistry.getPolicyValue('economy', 'split_region_percentage', 0.10) || 0.10;
        const groupPercentage = policy_registry_1.policyRegistry.getPolicyValue('economy', 'split_group_percentage', 0.05) || 0.05;
     ...
     */
    calculateSplits(context) {
        // 🔴 CRÍTICO: Passa module do metadata para detectar contexto EVENT
        const module = context.metadata?.module;
        const config = this.getDefaultConfigForTenant(context.tenantId, context.currency, module);
        const splits = [];
        ...
        if (Math.abs(difference) > ROUNDING_TOLERANCE) {
            ...
            const primarySplitIndex = splits.findIndex((s) => s.rule.targetType === 'WORKER' || s.rule.targetType === 'EVENT_ORGANIZER');
        ...
        for (const split of calculation.splits) {
            ...
            switch (split.rule.targetType) {
                ...
                case 'GROUP':
                    ...
                    for (let i = 0; i < context.groupAccountIds.length; i++) {
                        ...
                        try {
                            const transferResult = await transaction_service_1.transactionService.transfer(context.tenantId, {
                            ...
                            try {
                                const groupAccount = await (0, pool_1.runQueryWithTenant)(context.tenantId, `
            ...
            if (!targetAccountId) {
                ...
                if (split.rule.targetType === 'EVENT_ORGANIZER') {
                    throw new Error(`CRITICAL: Event organizer account not resolved for event ${context.metadata?.eventId}. ` +
            ...
            try {
                ...
                if (context.metadata?.idempotencyKey) {
                    ...
                    const hash = (0, crypto_1.createHash)('sha256').update(deterministicSeed).digest('hex');
                ...
                split_logger_service_1.splitLoggerService.logSplit({
                ...
                if (split.rule.targetType === 'REGION' && context.regionAccountId) {
                    ...
                    split_logger_service_1.splitLoggerService.logRegionCredit({
                        timestamp: new Date().toISOString(),
