    ...
    fastify.put('/professional', async (req, reply) => {
        ...
        try {
            ...
            fastify.log.info({
                ...
                servicesInfo: parsedBody?.skills?.flatMap((s) => (s.predefinedServices || []).map((ps) => ({
                    name: ps.name,
                    nameType: typeof ps.name,
                    nameLength: ps.name?.length,
                    hasSpaces: ps.name?.includes(' '),
                    nameCharCodes: ps.name ? Array.from(ps.name).map((c) => c.charCodeAt(0)) : [],
            ...
            const updated = await profile_professional_service_1.profileProfessionalService.updateProfessionalProfile(req.tenant.id, req.user.id, bodyToProcess);
            // 🔴 DIAGNÓSTICO: Log do que foi retornado
            fastify.log.info({
                servicesReturned: updated.skills?.flatMap(s => (s.predefinedServices || []).map(ps => ({
                    name: ps.name,
                    hasSpaces: ps.name?.includes(' ')
                }))) || []
