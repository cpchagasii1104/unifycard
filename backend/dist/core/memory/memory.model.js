...
class MemoryModel {
    ...
    static preferencesFromRows(rows) {
        return rows.map((row) => this.preferenceFromRow(row));
    }
    static interactionFromRow(row) {
        return {
    ...
    static interactionsFromRows(rows) {
        return rows.map((row) => this.interactionFromRow(row));
    }
    static entityFromRow(row) {
        return {
            entityId: row.entity_id,
            tenantId: row.tenant_id,
            globalUserId: row.global_user_id,
    ...
    static entitiesFromRows(rows) {
        return rows.map((row) => this.entityFromRow(row));
    }
    static shortcutFromRow(row) {
        return {
