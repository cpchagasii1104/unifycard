-- ================================================
-- UNIFICARD - MIGRATION 056
-- Categories Governance - Consolidated
-- Sistema de governança e rastreamento para categorias
-- CONSOLIDA: 055_categories_ai_blindage.sql + 055_categories_ai_validation.sql
-- ================================================
-- NOTA: Esta migration é idempotente e consolida as duas migrations 055 anteriores
...
-- GOVERNANÇA: Atualizar categorias existentes
-- Se status é NULL, definir como 'active' (categorias antigas pré-governança)
-- Se status já existe, manter
UPDATE categories 
SET status = COALESCE(status, 'active'),
    requires_review = COALESCE(requires_review, false)
WHERE status IS NULL OR requires_review IS NULL;
...
-- ===========================
-- TABELA DE LOGS DE CRIAÇÃO (AUDITORIA COMPLETA)
-- ===========================
CREATE TABLE IF NOT EXISTS category_ai_logs (
  ...
  ai_confidence NUMERIC(3,2) CHECK (ai_confidence >= 0 AND ai_confidence <= 1), -- Confiança da IA (0.00 a 1.00)
  
  -- Metadados
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  
  -- Constraint único por categoria (último log prevalece)
...
-- RLS (Row Level Security) - Apenas se não existir
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE tablename = 'category_ai_logs' AND policyname = 'category_ai_logs_rls'
  ) THEN
...
-- COMENTÁRIOS
-- ===========================
COMMENT ON TABLE category_ai_logs IS 'Logs de rastreamento para categorias criadas por IA, scripts, migrations ou manualmente';
COMMENT ON COLUMN categories.status IS 'Status da categoria: active (aprovada), pending (aguardando aprovação), rejected (rejeitada), archived (arquivada)';
COMMENT ON COLUMN categories.requires_review IS 'Indica se a categoria requer revisão humana antes de ser ativada. GOVERNANÇA: Por padrão true para novas categorias';
