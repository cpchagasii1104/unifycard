...
--   1) célula da grade service_offering_config_prices(config_id,day_of_week,period)  [mais específica]
...
-- 2. Grade de preço por CÉLULA (config, dia-da-semana ISO Mon=1..Sun=7, período pt-BR). Nível 1 da cascata.
...
CREATE TABLE IF NOT EXISTS service_offering_config_prices (
  ...
  -- ISO-8601: Segunda=1 .. Domingo=7 (alinha com PG isodow); CHECK-not-enum (§4.9.7).
  ...
  CONSTRAINT uq_socp_cell UNIQUE (config_id, day_of_week, period)
);
...
COMMENT ON TABLE service_offering_config_prices IS
  'FATIA PREÇO: grade ESPARSA de preço DECLARADO por CONFIG × dia-da-semana (ISO 1-7) × período (manha/tarde/'
