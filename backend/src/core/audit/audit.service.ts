...
export type AuditSource = 'impact' | 'validation' | 'reputation' | 'social';
...
class AuditService {
  ...
  async evaluateImpactPatterns(
    ...
    try {
      ...
      const impactHistory = await runQueryWithTenant<{
        impact_delta: number;
        created_at: string;
      }>(
        tenantId,
      ...
      if (actorType === 'page' && delta24h >= 50) {
        ...
        if (company && company.length > 0) {
          const companyAge = (Date.now() - new Date(company[0].created_at).getTime()) / (1000 * 60 * 60 * 24);
          if (companyAge < 7) {
  ...
  async evaluateReputationPatterns(
    ...
    try {
      ...
      const previousReputation = await runQueryWithTenant<{
  ...
  async evaluateActionDiversity(
    ...
    try {
      ...
      const eventDistribution = await runQueryWithTenant<{
        event_type: string;
        count: number;
      }>(
        tenantId,
