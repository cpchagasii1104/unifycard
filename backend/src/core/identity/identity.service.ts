...
class IdentityService {
  ...
  async createGlobalIdentityForUser(
    ...
    const existingLink = await pool.query<UserIdentityLinkRow>(
      `
    ...
    const newGlobalUser = await pool.query<GlobalUserRow>(
      `
   ...
   */
  async getGlobalIdentity(globalUserId: string): Promise<GlobalUser | null> {
  ...
  async linkLocalUserToGlobal(
    ...
    if (existing.rows.length > 0) {
      ...
      const updated = await pool.query<UserIdentityLinkRow>(
        `
  ...
  async getIdentityProfile(
    ...
    let resolvedGlobalUserId: string | null = localUser.global_user_id;
    ...
    if (!resolvedGlobalUserId) {
      ...
      const linkResult = await pool.query<{ global_user_id: string }>(
        `
    ...
    if (!globalUser) {
      ...
      const finalCheck = await pool.query<{ global_user_id: string }>(
        `
          SELECT global_user_id
          FROM user_identity_links
          WHERE user_id = $1 AND tenant_id = $2
