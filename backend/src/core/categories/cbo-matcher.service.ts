...
class CBOMatcherService {
   ...
   */
  async findMatch(input: string): Promise<CBOMatchResult> {
   ...
   */
  async findExactMatch(input: string): Promise<CBOMatchResult> {
    const normalized = this.normalizeInput(input);
    ...
    try {
      const result = await pool.query<{
        ...
        `SELECT occupation_id, cbo_code, title
         FROM occupations_reference
         WHERE normalized_title = $1
            OR $1 = ANY(synonyms)
         LIMIT 1`,
...
export const cboMatcherService = new CBOMatcherService();
