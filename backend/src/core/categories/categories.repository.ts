...
export class CategoryRepository {
   ...
   */
  async findChildren(parentId: string, countryCode?: string | null, client?: any): Promise<CategoryRow[]> {
    const statusCondition = await this.getStatusCondition();
    let query = `
      SELECT category_id, parent_id, name, slug, description, level, path, 
             COALESCE(keywords, '[]'::jsonb) as keywords, country_code, created_at, updated_at
   ...
   * @param countryCode - Se fornecido, filtra por país (incluindo categorias globais)
   */
  async search(term: string, limit: number = 50, countryCode?: string | null, client?: any): Promise<CategoryRow[]> {
    ...
    const queryParams: any[] = [searchTerm, `${normalizedTerm}%`, searchPattern];
    let paramOffset = 3;
   ...
   * @param countryCode - Se fornecido, filtra por país (incluindo categorias globais)
   */
  async findAll(countryCode?: string | null, client?: any): Promise<CategoryRow[]> {
    const statusCondition = await this.getStatusCondition();
    let query = `
      SELECT category_id, parent_id, name, slug, description, level, path, 
             COALESCE(keywords, '[]'::jsonb) as keywords, country_code, created_at, updated_at
