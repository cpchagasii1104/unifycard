...
class CategoriesService {
   ...
   */
  async getCategoryTree(countryCode?: string | null, useCache: boolean = true): Promise<CategoryTree[]> {
   ...
   * @param countryCode - Se fornecido, filtra por país (incluindo categorias globais)
   */
  async searchCategories(term: string, limit: number = 50, countryCode?: string | null): Promise<Category[]> {
  ...
  async autocompleteCategories(
    ...
    if (allSlugs.size > 0) {
      ...
      const pathCategories = await pool.query<{ slug: string; name: string }>(
