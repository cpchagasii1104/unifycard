...
export class MemoryRepository {
  ...
  async upsertInteraction(data: {
    ...
    if (!row) {
      ...
      if (existing) {
        // Atualizar contador
        const updated = await runQueryWithTenant<UserMemoryInteractionRow>(
          data.tenantId,
          `
  ...
  async upsertEntity(data: {
    ...
    if (!row) {
      ...
      if (existing) {
        ...
        const updated = await runQueryWithTenant<UserMemoryEntityRow>(
          data.tenantId,
          `
  ...
  async upsertShortcut(data: {
    ...
    if (!row) {
      ...
      if (existing) {
        // Atualizar contador
        const updated = await runQueryWithTenant<UserMemoryShortcutRow>(
          data.tenantId,
          `
