...
class CompaniesService {
  ...
  async createCompany(
    ...
    const existing = await pool.query<{ company_id: string }>(
      `
      SELECT company_id
      FROM companies
    ...
    const userId = await this.resolveUserIdFromGlobalUserId(globalUserId);
    if (!userId || !isTestOverrideUser(userId)) {
      // 🔴 ANTI-FRAUDE: Limite de 3 empresas PROVISIONAL por CPF
      const provisionalCount = await pool.query<{ count: string }>(
        `
    ...
    // Isso garante que a empresa apareça no seletor de contexto sem depender de validação
    try {
  ...
  async getCompanyById(companyId: string, globalUserId: string, tenantId?: string): Promise<Company | null> {
    ...
    if (userId && isTestOverrideUser(userId)) {
      // Buscar empresa sem verificar ownership
    ...
    // Comportamento normal (sem override)
    const result = await pool.query<{
      company_id: string;
      global_user_id: string;
      cnpj: string;
  ...
  async listCompanies(globalUserId: string, tenantId?: string): Promise<Array<Company & { userRole: CompanyUser }>> {
    ...
    const userId = await this.resolveUserIdFromGlobalUserId(globalUserId, tenantId);
    if (userId && isTestOverrideUser(userId)) {
      // Retornar TODAS as empresas (sem filtro de ownership)
   ...
   */
  async getCompanyUserById(companyUserId: string, globalUserId: string): Promise<CompanyUser | null> {
    const result = await pool.query<{
      company_user_id: string;
      company_id: string;
      global_user_id: string;
   ...
   * 🔴 REGRA: Bloquear remoção apenas se company_status === 'APPROVED'
   * 🔴 REGRA: Permitir remoção se company_status === 'PROVISIONAL' (pending)
   */
  async deleteCompany(companyId: string, globalUserId: string): Promise<boolean> {
    ...
    // 🔴 REGRA: Bloquear remoção apenas se status for APPROVED
    if (company.companyStatus === 'APPROVED') {
      throw new Error(
        'Não é possível excluir uma empresa aprovada. ' +
        'Empresas com status APPROVED têm histórico financeiro e não podem ser removidas.'
      );
    }

    // 🔴 PROTEÇÃO: Verificar se há transações associadas (apenas para empresas não-PROVISIONAL)
    if (company.companyStatus !== 'PROVISIONAL') {
  ...
  async adminOverrideToVerified(
    ...
    // Buscar empresa
    const company = await this.getCompanyById(companyId, adminGlobalUserId);
    if (!company) {
      throw new Error('Empresa não encontrada');
    }
    ...
    return await this.getCompanyById(companyId, adminGlobalUserId) as Company;
  }
