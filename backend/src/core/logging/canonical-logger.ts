...
class CanonicalLogger {
  ...
  /**
   * Log de divergência shadow authorization (Fase 2B)
   * Loga apenas quando decision_legacy !== decision_shadow
   */
  shadowAuthDivergence(req: FastifyRequest | null, message: string, context?: CanonicalLogContext): void {
    const correlation = extractCorrelationContext(req);
    const securityMetadata: SecuritySignalMetadata = {
      securitySignal: true,
      signalType: 'RBAC',
    ...
    console.warn(`[CANONICAL] 🔀 SHADOW AUTH DIVERGENCE: ${message}`, fullContext);
  }
}
