...
export async function validateSchemaOrDie(): Promise<void> {
  ...
  if (!DATABASE_URL) {
    ...
    if (isProduction) {
      ...
      console.warn('⚠️  AVISO: DATABASE_URL não está configurada no arquivo .env');
      console.warn('   Servidor continuará iniciando em modo desenvolvimento, mas funcionalidades de banco não funcionarão.\n');
      return;
    }
  }
  ...
  try {
    ...
    // C. Verificar se tabela tenant_contexts existe (obrigatória para categories)
    const tenantContextsExists = await tableExists(client, 'tenant_contexts');
    
    if (!tenantContextsExists) {
      const isProduction = process.env.NODE_ENV === 'production';
      if (isProduction) {
        console.error('❌ ERRO FATAL: Tabela "tenant_contexts" não existe!');
        console.error('   Esta tabela é obrigatória para o funcionamento de /categories/tree');
        console.error('   Execute a migration 312: 312_tenant_default_context_permissions.sql\n');
        process.exit(1);
      } else {
        console.warn('⚠️  AVISO: Tabela "tenant_contexts" não existe!');
        console.warn('   Execute a migration 312: 312_tenant_default_context_permissions.sql');
        console.warn('   Funcionalidades de categories podem não funcionar corretamente.\n');
      }
    } else {
      console.log('✅ Tabela "tenant_contexts" existe\n');
    }

    // D. Verificar se coluna token_version existe
    ...
    // E. Coluna não existe - tentar corrigir automaticamente
    ...
    try {
      ...
      if (!tokenVersionExistsAfter) {
        ...
        if (isProduction) {
          ...
          console.warn('\n⚠️  AVISO: Migration 089 foi executada, mas coluna "users.token_version" ainda não existe!');
          console.warn('   Servidor continuará iniciando em modo desenvolvimento.\n');
        }
      }
      ...
      if (isProduction) {
        ...
        console.warn('\n⚠️  AVISO: Falha ao executar migration 089 automaticamente (desenvolvimento):');
        console.warn(error);
        console.warn('\n   Servidor continuará iniciando. Execute manualmente: npm run migrate\n');
      }
    }
    ...
    if (isProduction) {
      ...
      // Em desenvolvimento, não bloquear o servidor
      console.warn('\n⚠️  AVISO no Schema Guard (desenvolvimento):');
      console.warn(error);
      console.warn('\n   Servidor continuará iniciando, mas algumas funcionalidades podem não funcionar.');
      console.warn('   Para corrigir: verifique a conexão com o banco de dados e execute as migrations.\n');
    }
  } finally {
