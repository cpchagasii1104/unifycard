...
export const GOVERNED_VOCABULARIES: GovernedVocabulary[] = [
  ...
  },
  {
    ...
    values: ['fee', 'regional_fund', 'reserve', 'escrow', 'revenue_share', 'referral', 'tax_reserve'],
    canonRef: 'DECISION-0180 D2/D3 + DECISION-0181 + DECISION-0179 D5 (FISCAL-4E: extensão governada 6→7 com tax_reserve). (Governed Vocabulary Source and Derived Reference Contract). sourceSymbol=BANK_SPLIT_TYPES (tuple `as const`, única fonte dos valores) em src/modules/bank/bank-split.types.ts; derivedTypeSymbol=BankSplitType (DERIVADO: (typeof BANK_SPLIT_TYPES)[number]). Consumidores referenciam o TIPO DERIVADO por `import type` (SPE, bank-integration, bank-split-engine). Vocabulário governado do split (bank_splits.split_type), NÃO autoridade financeira (dinheiro = bank_ledger/bank_splits). Ordem viva preservada. tax_reserve FORA desta frente (extensão 6→7 = FISCAL-4E). Guard prova referência ESTRUTURAL (menção textual não legitima; import canônico não perdoa declaração paralela; uso escalar legítimo não é segunda fonte). ANTI-DRIFT atual manifesto ⊆ fonte; igualdade bidirecional reservada à 4e.',
  },
];
