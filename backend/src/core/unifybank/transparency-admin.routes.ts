...
const transparencyAdminRoutes: FastifyPluginAsync = async (fastify) => {
  /**
   ...
   * - 500: Erro inesperado
   */
  fastify.get<{ Params: { regionId: string } }>('/regional-fund/:regionId', async (req, reply) => {
    ...
    try {
      const result = await transparencyService.getAdminRegionalFund(tenantId, regionId, {
...
export default transparencyAdminRoutes;
