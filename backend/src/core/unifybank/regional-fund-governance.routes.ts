...
import { regionalFundGovernanceService } from './regional-fund-governance.service';
import { regionalFundGovernanceRateLimitService } from './regional-fund-governance-rate-limit.service';
...
const governanceRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/proposals', async (req, reply) => {
    ...
    try {
      const proposals = await regionalFundGovernanceService.listProposals(tenantId, {
   ...
   * Autenticação: OBRIGATÓRIA (JWT)
   */
  fastify.get<{ Params: { proposalId: string } }>('/proposals/:proposalId', async (req, reply) => {
    // 1. Verificar autenticação
    if (!req.user || !req.user.id) {
      return reply.status(401).send({ error: 'Authentication required' });
    }
    ...
    try {
      const proposal = await regionalFundGovernanceService.getProposal(tenantId, proposalId);
   ...
   */
  fastify.post<{ Params: { proposalId: string }; Body: { vote: 'YES' | 'NO' } }>(
  ...
  fastify.post<{ Params: { proposalId: string } }>(
    ...
    async (req, reply) => {
      ...
      const isAdmin = (req.user as any).role === 'admin' || 
                      ...
                      regionalFundGovernanceService.isAdmin(globalUserId);
      ...
      try {
        const proposal = await regionalFundGovernanceService.openVoting(tenantId, proposalId);
  ...
  fastify.post<{ Params: { proposalId: string } }>(
    ...
    async (req, reply) => {
      ...
      try {
        const proposal = await regionalFundGovernanceService.closeVoting(tenantId, proposalId);
  ...
  fastify.post<{ Params: { proposalId: string } }>(
    ...
    async (req, reply) => {
      ...
      try {
        const result = await regionalFundGovernanceService.executeProposal(tenantId, proposalId);
