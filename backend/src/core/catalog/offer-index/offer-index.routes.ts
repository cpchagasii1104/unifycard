...
const offerIndexRoutes: FastifyPluginAsync = async (fastify) => {
  /**
  ...
  fastify.get<{
    ...
    const result = await offerIndexService.search(tenantId, {
      productId,
      cityId,
  ...
  }>('/merchant/:merchantId', async (req, reply) => {
    const tenantId = req.tenant!.id;
    const { merchantId } = req.params;
    const { productId, limit, offset } = req.query;

    const result = await offerIndexService.findByMerchant(tenantId, merchantId, {
      productId,
