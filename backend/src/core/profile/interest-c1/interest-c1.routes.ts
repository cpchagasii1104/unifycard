// backend/src/core/profile/interest-c1/interest-c1.routes.ts
//
// C1 — rotas actor-first de INTERESSE (DECISION-0067). ActionContext OBRIGATÓRIO.
// actorId = req.actionContext.actorId (NUNCA req.user.id). Caminho NOVO /interest/c1.
// Binário (sem weight/priority). Body camelCase; interno snake.
...
export function isEmptyPatch(data: z.infer<typeof patchSchema>): boolean {
  return data.sourceCategoryId === undefined && data.reactivate === undefined;
}

export const conceptParamSchema = z.object({ conceptId: z.string().uuid() });

const interestC1Routes: FastifyPluginAsync = async (fastify) => {
  ...
  // R3 — PATCH /profile/interest/c1/concepts/:conceptId (reativação / breadcrumb)
  ...
  // R4 — DELETE /profile/interest/c1/concepts/:conceptId (desativação lógica)
...
export default interestC1Routes;
