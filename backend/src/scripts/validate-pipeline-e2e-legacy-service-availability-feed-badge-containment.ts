/**
 * E2E F-SERVICE-AVAILABILITY-LEGACY-FEED-BADGE-CONTAINMENT-SLICE-A2C
 ...
 *    Orquestrado por scripts/run-legacy-service-availability-feed-badge-containment-ephemeral.ps1.
 *
 * Prova que o feed de serviços NÃO habilita mais a ação BOOK a partir do sinal legado `owner_type='service'`:
 * o predicado `hasAvailability` (que gate o `FeedAction.BOOK`) retorna FALSE para serviço canônico-bound, mesmo
 * havendo janela legada `owner_type='service'` ativa/futura — a agenda reservável vive na OFERTA
 * (owner_type='service_offering'). Sem apagar dado, sem remover enum, sem tocar writers/booking/dinheiro.
 *
 * NOTA HONESTA: o caminho público `getAvailableActions` depende de `getPost` (`SELECT ... FROM posts WHERE
 * post_id=$1`), que tem MISMATCH de schema pré-existente (posts usa PK `id`, não `post_id`) — logo o CTA BOOK do
 * feed já está inalcançável por esse bug (fora do escopo desta fatia). Por isso provamos o SINAL de contenção
 * (`hasAvailability`) diretamente (acesso test-only via cast), que é exatamente o que gate o BOOK.
 */
...
async function assertEphemeralDb(): Promise<void> {
  ...
  if (!/legacy|service|availability|feed|badge|containment|ephemeral|smoke|test/i.test(db)) throw new Error(`ABORT: nome de banco "${db}" não parece efêmero.`);
...
async function mkAvailability(tenantId: string, ownerType: 'service' | 'service_offering', ownerId: string): Promise<string> {
  return (await pool.query<{ id: string }>(
    ...
    [tenantId, ownerType, ownerId, new Date('2026-12-08T12:00:00Z'), new Date('2026-12-08T13:00:00Z')]
  )).rows[0].id;
}
...
async function main(): Promise<void> {
  ...
  await tenantService.createTenant({ id: TENANT_ID, name: 'Legacy Service Availability Feed Badge Containment', slug: `lsafbc-${Date.now()}` });
  ...
  const serviceId = await mkService(TENANT_ID, alice.actorId, 'ServicoFeed', canonicalServiceId); // canônico-bound
  const legacyAvailId = await mkAvailability(TENANT_ID, 'service', serviceId); // janela LEGADA owner_type='service' ATIVA/futura
  ...
  // ── PRE — a janela legada owner_type='service' ATIVA existe (o feed antigo habilitaria BOOK por ela) ──
  {
    const n = await count(`SELECT count(*)::int AS n FROM availability WHERE tenant_id=$1 AND owner_type='service' AND owner_id=$2 AND status='active'`, [TENANT_ID, serviceId]);
    record('PRE janela legada owner_type=service ATIVA existe (feed antigo habilitaria BOOK por esse sinal)', n === 1, `n=${n}`);
  }

  // ── T1 (CENTRAL) — o SINAL que gate o FeedAction.BOOK (hasAvailability) é FALSE p/ serviço canônico ──
  //     (acesso test-only via cast: getAvailableActions público depende de getPost com mismatch de schema
  //      pré-existente — post_id vs id — fora do escopo; provamos o sinal de contenção diretamente).
  {
    const has = await (servicesFeedPlugin as any).hasAvailability(TENANT_ID, serviceId);
    record('T1 feed hasAvailability=FALSE p/ serviço canônico (BOOK não é habilitado pelo sinal legado owner_type=service)', has === false, `hasAvailability=${has}`);
  }
  ...
  // ── T4 — enum owner_type='service' preservado ──
  {
  ...
  // ── T5 — A2 não regride: discoverServices segue suprimindo availability_summary legado ──
  {
  ...
  // ── T6 — A2b não regride: GET público /services/:id/availability segue contido ──
  {
    const app = Fastify();
    ...
    await app.close();
    const contained = res.statusCode === 200 && Array.isArray(body?.data) && body.data.length === 0 && body?.contained === true;
    record('T6 A2b não regride: GET /services/:id/availability segue contido (data vazio + contained=true)', contained, `status=${res.statusCode} body=${res.body?.slice(0, 160)}`);
  }

  // ── T7 — Bank intocado (Δbank=0) ──
  {
    const bankAfter = await count(`SELECT ((SELECT count(*) FROM bank_ledger)+(SELECT count(*) FROM bank_transactions))::int AS n`);
    record('T7 Bank intocado (bank_ledger + bank_transactions inalterados) — Δbank=0', bankAfter === bankBefore, `before=${bankBefore} after=${bankAfter}`);
  }
  ...
  await pool.end();
  console.log('✨ Feed contido: o sinal de BOOK não deriva mais de owner_type=service para serviço canônico; dado/enum preservados; SSOT na oferta; A2/A2b intactos. R3 provado.');
}
