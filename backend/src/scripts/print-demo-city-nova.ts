...
async function printDemoInfo() {
  ...
  try {
    ...
    const companyResult = await client.query<CompanyRow>(
      `SELECT company_id, name, description, is_active 
       FROM companies 
    ...
    const userResult = await client.query<UserRow>(
      `SELECT user_id, email, global_user_id 
       FROM users 
    ...
    const workerResult = await client.query<WorkerRow>(
      `SELECT worker_id, bio, hourly_rate, reputation_score 
       FROM workers 
       WHERE tenant_id = $1 AND user_id = $2 
       LIMIT 1`,
      [tenant.tenant_id, user.user_id]
    ...
    if (user.global_user_id) {
      ...
      const scheduleResult = await client.query<ScheduleRow>(
        `SELECT schedule_id, global_user_id, company_id 
         FROM schedules 
    ...
    const categoriesResult = await client.query<CategoryRow>(
      `SELECT category_id, name, slug, level 
       FROM categories 
