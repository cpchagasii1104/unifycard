...
async function main(): Promise<void> {
  ...
  try {
    ...
    await cleanup();
    const left = await pool.query(`SELECT (SELECT COUNT(*) FROM users WHERE email LIKE $1)+(SELECT COUNT(*) FROM global_users WHERE full_name LIKE $2) total`, [`${MARKER}-%`, `${MARKER}%`]);
