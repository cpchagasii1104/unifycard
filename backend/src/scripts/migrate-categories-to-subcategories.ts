 ...
 */
const PROFESSION_MIGRATIONS: Array<{
  categoryName: string; // Nome da categoria principal
  professionName: string; // Nome da profissão a mover
  newSubcategoryName: string; // Nome da nova subcategoria
  newSubcategorySlug: string;
...
async function migrateCategories() {
  ...
  try {
    ...
    const migrationsByCategory = new Map<string, Map<string, typeof PROFESSION_MIGRATIONS>>();
    ...
    for (const [categoryName, subcategoryMap] of migrationsByCategory) {
      ...
      for (const [subcategoryName, professions] of subcategoryMap) {
        ...
        const existingSubcategory = await pool.query<{ category_id: string }>(
          `
          SELECT category_id
          FROM categories
        ...
        if (existingSubcategory.rows.length > 0) {
          ...
          const newSubcategory = await categoriesService.createCategory(
            {
        ...
        for (const profession of professions) {
          ...
          if (professionResult.rows.length === 0) {
            ...
            const existingProfession = await pool.query<{
              category_id: string;
              parent_id: string;
            }>(
              `
