...
export class CEPService {
   ...
   */
  async fetchCEP(cep: string, retries: number = 1): Promise<CEPData | null> {
    ...
    const apis = [
      {
        ...
        transform: (data: any): CEPData | null => {
      ...
      try {
        ...
        try {
          ...
          const isTimeout = fetchError instanceof Error && (
            ...
            (fetchError as any).code === 'UND_ERR_CONNECT_TIMEOUT' ||
            ((fetchError as any).cause && (fetchError as any).cause.code === 'UND_ERR_CONNECT_TIMEOUT')
          );
