...
export class EventScheduleService {
   ...
   */
  async ensureEventSchedule(eventId: string, tenantId: string): Promise<string> {
    return runTenantTransaction(tenantId, async (trx) => {
      ...
      const schedule = scheduleResult[0] as ScheduleRow;
   ...
   */
  async generateEventSlots(eventId: string, tenantId: string): Promise<number> {
    return runTenantTransaction(tenantId, async (trx) => {
      const eventResult = await trx.query({
        text: `
  ...
  private buildSlotsByType(event: EventRow): any[] {
