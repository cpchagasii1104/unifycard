...
const createSchema = z.object({
  ...
  // Quilometragem (só veículo; backend valida a coerência tipo↔política). Taxa em cents (anúncio).
  mileagePolicy: z.enum(MILEAGE_POLICIES).nullable().optional(),
...
const rentableResourceRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{ Body: z.infer<typeof createSchema> }>('/', async (req, reply) => {
    ...
    try {
      const resource = await rentableResourceService.create(req.tenant.id, req.actionContext.actorId, {
        ...
        mileagePolicy: parsed.data.mileagePolicy ?? null,
  ...
  fastify.get<{ Querystring: { originCityId?: string; originCep?: string; radiusKm?: string; resourceType?: string; startAt?: string; endAt?: string } }>(
    ...
    async (req, reply) => {
      if (!req.tenant?.id) return reply.status(400).send({ error: 'Tenant não encontrado' });
      const viewerActorId = req.actionContext?.actorId;
      if (!viewerActorId) return reply.status(400).send({ error: 'ActionContext obrigatório para descoberta' });
      ...
      return reply.send({ ok: true, data: cards });
    }
  );
   ...
   */
  fastify.post<{ Params: { bookingId: string } }>('/my-bookings/:bookingId/cancel', async (req, reply) => {
    if (!req.tenant?.id) return reply.status(400).send({ error: 'Tenant não encontrado' });
    const userId = (req.user as { userId?: string } | undefined)?.userId;
    if (!userId) return reply.status(401).send({ error: 'Autenticação obrigatória' });
    const actorId = req.actionContext?.actorId;
    ...
    let canRep = false;
    try { canRep = await authorizationService.canRepresentActor(req.tenant.id, userId, actorId); } catch { canRep = false; }
    if (!canRep) return reply.status(403).send({ ok: false, error: 'Sem autoridade sobre o actor declarado', code: 'RENTAL_CANCEL_NOT_REPRESENTABLE' });
    try {
      const data = await rentableResourceService.cancelMyBooking(req.tenant.id, req.params.bookingId, actorId, userId);
      return reply.send({ ok: true, data });
    } catch (err: any) {
  ...
  fastify.put<{ Params: { id: string }; Body: z.infer<typeof updateOfferSchema> }>('/:id', async (req, reply) => {
    ...
    try {
      const resource = await rentableResourceService.update(req.tenant.id, req.params.id, userId, {
        ...
        mileagePolicy: parsed.data.mileagePolicy,
