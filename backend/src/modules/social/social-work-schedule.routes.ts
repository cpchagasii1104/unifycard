...
import { socialWorkScheduleService } from './social-work-schedule.service';
import { rbacService } from '@core/rbac/rbac.service';
import { z } from 'zod';

const scheduleFromPostSchema = z.object({
  startTime: z.string().datetime(),
  endTime: z.string().datetime(),
});

const socialWorkScheduleRoutes: FastifyPluginAsync = async (fastify) => {
  /**
  ...
  fastify.post<{
    ...
    async (req, reply) => {
      ...
      try {
        ...
        const job = await socialWorkScheduleService.resolveJobFromPost(postId, tenantId);
        if (!job) {
          return reply.status(400).send({
        ...
        // Criar schedule
        const slot = await socialWorkScheduleService.createScheduleFromPost(
          postId,
          tenantId,
          userId,
          startTime,
  ...
  fastify.get<{ Params: { postId: string } }>(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        // Buscar schedules
        const slots = await socialWorkScheduleService.getSchedulesForPost(postId, tenantId);
