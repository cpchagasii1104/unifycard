...
class CulturalEventService {
  ...
  async createEvent(
    ...
    const creator = await runQueriesWithTenant<{
      id: string;
      type: string;
      active: boolean;
    }>(
    ...
    if (input.location_cultural_profile_id) {
      const location = await runQueriesWithTenant<{
        id: string;
        type: string;
        active: boolean;
      }>(
    ...
    if (input.revenue_split && input.revenue_split.length > 0) {
      await runTenantTransaction(tenantId, async (trx) => {
        for (const split of input.revenue_split) {
          await trx.query({
            text: `
  ...
  async publishEvent(
    ...
    const event = await runQueriesWithTenant<{
      id: string;
      status: string;
      created_by_cultural_profile_id: string;
    }>(
  ...
  async completeEvent(
    ...
    try {
      ...
      const creatorProfile = await runQueriesWithTenant<{
      ...
      if (creatorProfile && creatorProfile.length > 0) {
        ...
        await impactService.recordImpact({
          ...
          eventType: 'POST_PUBLISHED', // Reutilizar tipo existente por enquanto (FASE 16: impacto simbólico)
          delta: 5, // +5 impacto por evento completado
  ...
  async generateCheckInQR(
    tenantId: string,
    eventId: string
  ): Promise<{ qr_code: string; expiresAt: string; event: CulturalEvent }> {
    // Buscar evento
    ...
    if (!secret) {
      throw new Error('JWT_SECRET environment variable is required for QR code generation');
    }
