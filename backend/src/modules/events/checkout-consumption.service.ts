// Checkout legado (HTTP /core/checkout) — consumo no evento.
...
export class CheckoutConsumptionService {
    ...
    return runTenantTransaction(tenantId, async (trx) => {
      ...
      if (params.idempotencyKey) {
        ...
        if (existingConsumptions.length > 0) {
          ...
          const existing = existingConsumptions.map((row: ConsumptionRow) => row as ConsumptionRow);
          return {
