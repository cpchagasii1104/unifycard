...
import { sendAssistantMessageSchema } from './assistant.schemas';
...
const assistantRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{
    ...
    async (req, reply) => {
      ...
      try {
        const validated = sendAssistantMessageSchema.parse(req.body);
  ...
  fastify.get<{
    ...
    async (req, reply) => {
      ...
      try {
        const channel = (req.query.channel as 'chat' | 'social' | 'voice') || 'chat';
        ...
        if (data.fields) {
          for (const [key, value] of Object.entries(data.fields)) {
            if (typeof value === 'string') {
              ...
              fields[key] = value[0].value as string;
            }
          }
        }

        // 5. Processar voz via voice channel
        const result = await voiceChannel.processVoice(
          req.server,
          {
