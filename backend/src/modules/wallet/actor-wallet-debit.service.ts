...
export async function debitActorWalletForRecovery(
  ...
  const obligation = await runQueryWithTenant<ObligationRow>(
    tenantId,
  ...
  const approval = await runQueryWithTenant<{ status: string; operation_type: string }>(
    tenantId,
  ...
  const balance = await bankLedgerRepository.calculateBalance(tenantId, obligation.debtor_account_id);
  const amountToRecover = Math.min(remaining, balance.balanceCents);
