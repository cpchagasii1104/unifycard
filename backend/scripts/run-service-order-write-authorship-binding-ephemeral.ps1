# run-service-order-write-authorship-binding-ephemeral.ps1
# Orquestra o E2E F-SERVICE-ORDER-WRITE-AUTHORSHIP-BINDING em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_service_order_write_authorship_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e service-order-write-authorship-binding ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-service-order-write-authorship-binding.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E SERVICE-ORDER-WRITE-AUTHORSHIP-BINDING: OK' -ForegroundColor Green
exit 0
