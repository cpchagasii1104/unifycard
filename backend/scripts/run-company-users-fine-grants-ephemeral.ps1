# run-company-users-fine-grants-ephemeral.ps1
# Orquestra o E2E F-R2-COMPANY-USERS-FINE-GRANTS-MATERIALIZATION em DB EFÊMERA:
#   1. cria DB efêmera (drop se existir);
#   2. aplica TODAS as migrations (runner produtivo) contra a DB efêmera;
#   3. roda o e2e (validate-pipeline-e2e-company-users-fine-grants.ts);
#   4. dropa a DB efêmera SEMPRE (finally).
# NUNCA toca unificard_dev. Uso: pwsh -File scripts/run-company-users-fine-grants-ephemeral.ps1
...
$EPHEMERAL = "unificard_r2_grants_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e R2 fine-grants ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-company-users-fine-grants.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E R2 FINE-GRANTS: OK' -ForegroundColor Green
exit 0
