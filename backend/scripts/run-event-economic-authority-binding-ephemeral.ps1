# run-event-economic-authority-binding-ephemeral.ps1
# Orquestra o E2E B2f (binding de autoridade nas rotas econômicas v2) em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_event_economic_binding_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e event-economic-authority-binding ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-event-economic-authority-binding.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E EVENT-ECONOMIC-AUTHORITY-BINDING: OK' -ForegroundColor Green
exit 0
