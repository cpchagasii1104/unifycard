# run-economic-policy-authority-ephemeral.ps1
# Orquestra o E2E validate-pipeline-e2e-economic-policy-authority em DB EFEMERA. NUNCA toca unificard_dev.
# F-ECONOMIC-POLICY-ADMIN-FRONT FATIA 1 (authority key + read-only consumer) - prova que
# economic_policy:manage gateia de fato GET /economy/admin/policies (admin 200 / nao-admin 403 /
# anonimo 401 / isolamento cross-tenant); Delta bank = 0.
...
$EPHEMERAL = "unificard_economic_policy_authority_e2e"
...
try {
  ...
  Write-Host 'Rodando e2e economic-policy-authority ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-economic-policy-authority.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E ECONOMIC-POLICY-AUTHORITY: OK' -ForegroundColor Green
exit 0
