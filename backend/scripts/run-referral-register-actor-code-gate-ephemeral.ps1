# run-referral-register-actor-code-gate-ephemeral.ps1
# Orquestra o E2E F-REFERRAL-REGISTER-ACTOR-CODE-GATE em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_referral_register_actor_code_gate_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e referral-register-actor-code-gate ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-referral-register-actor-code-gate.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E REFERRAL-REGISTER-ACTOR-CODE-GATE: OK' -ForegroundColor Green
exit 0
