# run-internal-surfaces-containment-ephemeral.ps1
# Orquestra o E2E F-FINANCIAL-INTERNAL-SURFACES-P1-CONTAINMENT em DB EFÊMERA:
#   1. cria DB efêmera (drop se existir);
#   2. aplica TODAS as migrations (runner produtivo) contra a DB efêmera;
#   3. roda o e2e (validate-pipeline-e2e-internal-surfaces-containment.ts);
#   4. dropa a DB efêmera SEMPRE (finally).
# NUNCA toca unificard_dev. Uso: pwsh -File scripts/run-internal-surfaces-containment-ephemeral.ps1
...
$EPHEMERAL = "unificard_internal_surfaces_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e internal-surfaces-containment ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-internal-surfaces-containment.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E INTERNAL-SURFACES-CONTAINMENT: OK' -ForegroundColor Green
exit 0
