# run-performer-contracting-policy-ephemeral.ps1
# Orquestra o E2E validate-pipeline-e2e-performer-contracting-policy em DB EFÊMERA. NUNCA toca unificard_dev.
# F-PERFORMER-CONTRACTING-POLICY — aceita-direto × negocia + gate de DISTÂNCIA na oferta apresentacao-musical.
# Bank-free (Δbank=0). Reusa geo canônico (actor_active_location + haversine_distance_km) e o lock de confirm SELADO.
...
$EPHEMERAL = "unificard_performer_contracting_policy_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e performer-contracting-policy ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-performer-contracting-policy.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E PERFORMER-CONTRACTING-POLICY: OK' -ForegroundColor Green
exit 0
