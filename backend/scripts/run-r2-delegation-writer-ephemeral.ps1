...
$EPHEMERAL = "unificard_r2_delegation_writer_e2e"
...
try {
  ...
  Write-Host 'Provando invariantes R2.2 ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-r2-delegation-writer.ts
...
if ($failed) { exit 1 }
Write-Host 'R2.2 DELEGATION WRITER: OK' -ForegroundColor Green
exit 0
