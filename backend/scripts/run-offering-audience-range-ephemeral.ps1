# run-offering-audience-range-ephemeral.ps1
# Orquestra o E2E validate-pipeline-e2e-offering-audience-range em DB EFÊMERA. NUNCA toca unificard_dev.
# F-PERFORMER-AUDIENCE-RANGE — faixa de público preferida (audience_min/audience_max) na oferta apresentacao-musical,
# DISTINTA de audience_capacity (equipamento). Bank-free (Δbank=0).
...
$EPHEMERAL = "unificard_offering_audience_range_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e offering-audience-range ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-offering-audience-range.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E OFFERING-AUDIENCE-RANGE: OK' -ForegroundColor Green
exit 0
