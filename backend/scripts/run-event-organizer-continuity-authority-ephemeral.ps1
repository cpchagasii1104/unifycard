# run-event-organizer-continuity-authority-ephemeral.ps1
# Orquestra o E2E F-EVENT-ORGANIZER-CONTINUITY (item 3, autoridade plural da lista) em DB EFÊMERA.
...
$EPHEMERAL = "unificard_event_organizer_continuity_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e event-organizer-continuity-authority ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-event-organizer-continuity-authority.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E EVENT-ORGANIZER-CONTINUITY-AUTHORITY: OK' -ForegroundColor Green
exit 0
