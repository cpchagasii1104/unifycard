...
$EPHEMERAL = "unificard_r2_authorship_e2e"
...
try {
  ...
  Write-Host 'Provando fix Q3+RLS+unique ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-r2-authorship-and-isolation.ts
...
if ($failed) { exit 1 }
Write-Host 'R2.2 AUTHORSHIP+ISOLATION: OK' -ForegroundColor Green
exit 0
