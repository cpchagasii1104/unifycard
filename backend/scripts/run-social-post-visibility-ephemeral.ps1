# run-social-post-visibility-ephemeral.ps1
# Orquestra o E2E F-SOCIAL-POST-VISIBILITY-READ-ENFORCEMENT em DB EFEMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_social_post_visibility_e2e"
...
try {
  ...
  Write-Host 'Rodando e2e social-post-visibility-read-enforcement ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-social-post-visibility-read-enforcement.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E SOCIAL-POST-VISIBILITY-READ-ENFORCEMENT: OK' -ForegroundColor Green
exit 0
