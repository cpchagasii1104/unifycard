# run-product-scope-containment-ephemeral.ps1
# Orquestra o E2E F-PRODUCT-SCOPE-CONTAINMENT em DB EFÊMERA. NUNCA toca unificard_dev.
# NÃO cria as tabelas/funções fantasma: a DB efêmera roda as migrations VIVAS (que NÃO as
# contêm) → ghost real, igual unificard_dev.
...
$EPHEMERAL = "unificard_product_scope_e2e"
...
try {
  ...
  Write-Host '📦 Aplicando migrations (vivas; substrato fantasma de rides NÃO está entre elas) ...' -ForegroundColor Cyan
  npx tsx src/core/db/migrate.ts
  if ($LASTEXITCODE -ne 0) { throw "migrate falhou (rc=$LASTEXITCODE)" }
  Write-Host '🧪 Rodando e2e product-scope-containment ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-product-scope-containment.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E PRODUCT-SCOPE-CONTAINMENT: OK' -ForegroundColor Green
exit 0
