# run-contextual-thread-schema-ghost-containment-ephemeral.ps1
# Orquestra o E2E F-CONTEXTUAL-THREAD-SCHEMA-GHOST-FAIL-CLOSED-CONTAINMENT em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_contextual_thread_containment_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e contextual-thread-schema-ghost-containment ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-contextual-thread-schema-ghost-containment.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E CONTEXTUAL-THREAD-SCHEMA-GHOST-CONTAINMENT: OK' -ForegroundColor Green
exit 0
