# run-event-need-supplier-bridge-ephemeral.ps1
# Orquestra o E2E F-EVENT-SUPPLIER-BRIDGE em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_event_need_supplier_bridge_e2e"
...
try {
  ...
  Write-Host 'Rodando e2e event-need-supplier-bridge ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-event-need-supplier-bridge.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E EVENT-NEED-SUPPLIER-BRIDGE: OK' -ForegroundColor Green
exit 0
