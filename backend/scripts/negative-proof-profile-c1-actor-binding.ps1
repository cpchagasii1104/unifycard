# negative-proof-profile-c1-actor-binding.ps1
# Reproducible proof that audit-profile-c1-actor-binding.mjs BITES when the lifestyle authority binding is
# degraded — spoof the subject in resolveActorGuarded (canRepresentActor(tenantId, userId, actorId) ->
# canRepresentActor(tenantId, actorId, actorId): subject becomes the target). The dedicated guard then loses
# the canonical canRepresentActor(tenantId, userId, actorId) and FAILS. (The canal-1 detector also re-flags the
# lifestyle route, since SERVICE_BOUND_WRITERS proof requires that exact call.)
# Mutates lifestyle.service.ts (byte-exact backup), confirms GATE FAIL (exit 1), restores byte-identical and
# confirms GATE OK. ASCII-only + no BOM so it parses under Windows PowerShell 5.1 and PowerShell 7.
# R8B PROFILE-C1 BASELINE RECONCILIATION.
...
$file  = 'C:/unificard/backend/src/core/profile/lifestyle/lifestyle.service.ts'
$guard = 'C:/unificard/backend/scripts/audit-profile-c1-actor-binding.mjs'

$origBytes = [IO.File]::ReadAllBytes($file)
$preStatus = (& git -C C:/unificard status --porcelain -- backend/src/core/profile/lifestyle/lifestyle.service.ts)
$failed = $false
try {
  & node $guard
  ...
  # Simulated regression: subject spoof in the lifestyle gate (userId -> actorId as subject).
  $needle = 'canRepresentActor(tenantId, userId, actorId)'
  $inject = 'canRepresentActor(tenantId, actorId, actorId)'
  $mutated = $text.Replace($needle, $inject)
  if ($mutated -eq $text) { throw 'MUTATION NO-OP: lifestyle binding anchor not found (did the file change?).' }
  [IO.File]::WriteAllText($file, $mutated, (New-Object Text.UTF8Encoding($false)))

  & node $guard
  $rc = $LASTEXITCODE
  if ($rc -eq 0) { throw "NEGATIVE-PROOF FAILED: guard PASSED (exit 0) with the lifestyle subject spoofed -- expected FAIL." }
  Write-Host "[OK] NEGATIVE-PROOF: guard FAILED (exit $rc) with lifestyle subject spoofed (expected)." -ForegroundColor Green
}
...
$postStatus = (& git -C C:/unificard status --porcelain -- backend/src/core/profile/lifestyle/lifestyle.service.ts)
