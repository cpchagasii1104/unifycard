# run-risk-dashboard-schema-ghost-ephemeral.ps1
# Orquestra o E2E F-RISK-DASHBOARD em DB EFÊMERA. NUNCA toca unificard_dev.
# NÃO cria agreements/evidence_packs/payout_orders: a DB efêmera roda as migrations VIVAS
# (que NÃO contêm essas tabelas) → ghost real, igual unificard_dev.
...
$EPHEMERAL = "unificard_risk_dashboard_ghost_e2e"
...
try {
  ...
  Write-Host '📦 Aplicando migrations (vivas; agreements/evidence_packs/payout_orders NÃO estão entre elas) ...' -ForegroundColor Cyan
  npx tsx src/core/db/migrate.ts
  if ($LASTEXITCODE -ne 0) { throw "migrate falhou (rc=$LASTEXITCODE)" }
  Write-Host '🧪 Rodando e2e risk-dashboard-schema-ghost ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-risk-dashboard-schema-ghost.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E RISK-DASHBOARD-SCHEMA-GHOST: OK' -ForegroundColor Green
exit 0
