# run-search-omni-ephemeral.ps1
# Orquestra o E2E F-GLOBAL-SEARCH-OMNI-SLICE-A em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_search_omni_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e search-omni ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-search-omni.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E SEARCH-OMNI: OK' -ForegroundColor Green
exit 0
