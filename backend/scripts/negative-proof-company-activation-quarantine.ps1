# negative-proof-company-activation-quarantine.ps1
# Prova que o guard audit-company-activation-quarantine.mjs MORDE cada regressão.
...
function Invoke-Guard {
    node scripts/audit-company-activation-quarantine.mjs *> $null
    return $LASTEXITCODE
}
$baseOk = ((Invoke-Guard) -eq 0)

$bites = @(
    # 1) remover TODO o gate de quarentena da ativacao (replace_all no service) -> iGate<0 -> guard FAIL
    @{ name = 'activation-gate-removed'; file = $svc;
       find = "isActorEffectivelyBlocked";
       all = $true; repl = "__noQuarantine" },
    # 2) injetar uma escrita (FOR UPDATE) ANTES do gate -> gate tarde demais -> guard FAIL
    @{ name = 'gate-after-write'; file = $svc;
       find = "if \(\s*\n\s*\(await isActorEffectivelyBlocked\(tenantId, pageActor\.actor_id\)\)";
       repl = "await client.query('FOR UPDATE');`n    if (`n      (await isActorEffectivelyBlocked(tenantId, pageActor.actor_id))" },
    # 3) meter quarentena em canRepresentActor (representacao deve ficar pura) -> guard FAIL
...
Write-Host "[neg-proof company-activation-quarantine] baseOk=$baseOk"
