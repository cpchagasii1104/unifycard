# run-pdv-payment-authority-ephemeral.ps1
# Orquestra o E2E PDV-F2A (binding de autoridade do pagamento PDV) em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_pdv_payment_authority_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e pdv-payment-authority ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-pdv-payment-authority.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E PDV-PAYMENT-AUTHORITY: OK' -ForegroundColor Green
exit 0
