# run-rides-ghost-containment-ephemeral.ps1
# Orquestra o E2E F-RIDES-GHOST-CONTAINMENT em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_rides_ghost_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e rides-ghost-containment ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-rides-ghost-containment.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E RIDES-GHOST-CONTAINMENT: OK' -ForegroundColor Green
exit 0
