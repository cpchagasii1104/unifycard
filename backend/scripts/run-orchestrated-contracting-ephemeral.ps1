# run-orchestrated-contracting-ephemeral.ps1
# Orquestra o E2E validate-pipeline-e2e-orchestrated-contracting em DB EFÊMERA. NUNCA toca unificard_dev.
# F-ORCHESTRATED-CONTRACTING (C3) — fecha o loop propose→accept→BIND: booking de service_offering confirmado com
# metadata.eventId vincula o PERFORMER (provider derivado server-side) ao elenco via createCommitment. Bank-free (Δbank=0).
...
$EPHEMERAL = "unificard_orchestrated_contracting_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e orchestrated-contracting ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-orchestrated-contracting.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E ORCHESTRATED-CONTRACTING: OK' -ForegroundColor Green
exit 0
