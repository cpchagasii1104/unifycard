# run-event-lifecycle-authority-ephemeral.ps1
...
$EPHEMERAL = "unificard_event_authority_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e event-lifecycle-authority ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-event-lifecycle-authority.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E EVENT-LIFECYCLE-AUTHORITY: OK' -ForegroundColor Green
exit 0
