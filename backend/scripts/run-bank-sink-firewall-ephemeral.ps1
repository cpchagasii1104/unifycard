# run-bank-sink-firewall-ephemeral.ps1
# Orquestra o E2E F-BANK-TRANSACTION-SINK-FIREWALL em DB EFEMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_bank_sink_firewall_e2e"
...
try {
  ...
  Write-Host 'Rodando e2e bank-transaction-sink-firewall ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-bank-transaction-sink-firewall.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E BANK-TRANSACTION-SINK-FIREWALL: OK' -ForegroundColor Green
exit 0
