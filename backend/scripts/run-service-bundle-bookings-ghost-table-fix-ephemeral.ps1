# run-service-bundle-bookings-ghost-table-fix-ephemeral.ps1
# Orquestra o E2E DT-SERVICE-BUNDLE-BOOKINGS-GHOST-TABLE-LIVE em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_bundle_bookings_ghost_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e service-bundle-bookings-ghost-table-fix (book → GET, ida e volta) ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-service-bundle-bookings-ghost-table-fix.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E SERVICE-BUNDLE-BOOKINGS-GHOST-TABLE-FIX: OK' -ForegroundColor Green
exit 0
