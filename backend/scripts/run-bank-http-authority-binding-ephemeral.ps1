# run-bank-http-authority-binding-ephemeral.ps1
# Orquestra o E2E F-BANK-HTTP-AUTHORITY-BINDING em DB EFÊMERA:
#   1. cria DB efêmera (drop se existir);
#   2. aplica TODAS as migrations (runner produtivo);
#   3. roda o e2e (validate-pipeline-e2e-bank-http-authority-binding.ts);
#   4. dropa a DB efêmera SEMPRE (finally).
# NUNCA toca unificard_dev. Uso: pwsh -File scripts/run-bank-http-authority-binding-ephemeral.ps1
...
$EPHEMERAL = "unificard_bank_http_binding_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e bank-http-authority-binding ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-bank-http-authority-binding.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E BANK-HTTP-AUTHORITY-BINDING: OK' -ForegroundColor Green
exit 0
