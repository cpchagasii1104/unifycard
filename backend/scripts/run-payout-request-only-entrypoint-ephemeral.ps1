# run-payout-request-only-entrypoint-ephemeral.ps1
# Orquestra o E2E F-PAYOUT-REQUEST-ONLY-ENTRYPOINT em DB EFÊMERA (NÃO move dinheiro).
# NUNCA toca unificard_dev. Uso: pwsh -File scripts/run-payout-request-only-entrypoint-ephemeral.ps1
...
$EPHEMERAL = "unificard_payout_request_entrypoint_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e payout-request-only-entrypoint ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-payout-request-only-entrypoint.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E PAYOUT-REQUEST-ONLY-ENTRYPOINT: OK' -ForegroundColor Green
exit 0
