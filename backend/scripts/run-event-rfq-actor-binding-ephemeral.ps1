# run-event-rfq-actor-binding-ephemeral.ps1
# Orquestra o E2E R7a EVENT-RFQ ACTING-USER-GATE em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_event_rfq_actor_binding_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e event-rfq-actor-binding ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-event-rfq-actor-binding.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E EVENT-RFQ-ACTOR-BINDING: OK' -ForegroundColor Green
exit 0
