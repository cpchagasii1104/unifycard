# run-dashboard-actor-filter-ephemeral.ps1
# Orquestra o E2E F-AUTHORITY-Z2-R2-DASHBOARD-ACTOR-FILTER-BINDING em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_dashboard_actor_filter_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e dashboard-actor-filter-binding ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-dashboard-actor-filter-binding.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E DASHBOARD-ACTOR-FILTER-BINDING: OK' -ForegroundColor Green
exit 0
