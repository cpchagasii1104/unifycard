# run-intent-execute-buyer-actor-binding-ephemeral.ps1
# Orquestra o E2E F-AUTHORITY-Z2-R5-INTENT-EXECUTE-BUYER-ACTOR-BINDING em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_intent_execute_authority_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e intent-execute-buyer-actor-binding ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-intent-execute-buyer-actor-binding.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E INTENT-EXECUTE-BUYER-ACTOR-BINDING: OK' -ForegroundColor Green
exit 0
