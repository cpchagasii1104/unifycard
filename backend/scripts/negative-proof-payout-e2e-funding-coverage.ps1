# negative-proof-payout-e2e-funding-coverage.ps1
# Test-Bite proofs that audit-payout-e2e-ephemeral-guard.mjs BITES on funding regressions in the self-seed helper:
#   NP1 raw INSERT INTO bank_ledger in the funding helper       -> guard FAILS
#   NP2 DISABLE TRIGGER (bypass Bank coverage invariant)        -> guard FAILS
#   NP3 remove `await assertEphemeral()` before seed            -> guard FAILS
...
Write-Host '=== NP1 — raw INSERT INTO bank_ledger in funding helper ===' -ForegroundColor Cyan
Test-Bite -needle 'await ensureRecoveryConcept();' -inject "await ensureRecoveryConcept(); const _np1 = 'INSERT INTO bank_ledger (x) VALUES (1)';"

Write-Host '=== NP2 — DISABLE TRIGGER (bypass coverage invariant) ===' -ForegroundColor Cyan
Test-Bite -needle 'await ensureRecoveryConcept();' -inject "await ensureRecoveryConcept(); const _np2 = 'DISABLE TRIGGER trg_check_coverage';"

Write-Host '=== NP3 — remove await assertEphemeral() before seed ===' -ForegroundColor Cyan
Test-Bite -needle '  await assertEphemeral();' -inject '  // assertEphemeral removed (negative-proof)'

Write-Host 'PAYOUT FUNDING COVERAGE NEGATIVE-PROOF: all three bites confirmed and restored byte-identical. OK' -ForegroundColor Green
exit 0
