# negative-proof-service-mutations-quarantine-gate.ps1
# Prova que o guard audit-service-mutations-quarantine-gate.mjs MORDE cada regressão.
...
function Invoke-Guard {
    node scripts/audit-service-mutations-quarantine-gate.mjs *> $null
    return $LASTEXITCODE
}
$baseOk = ((Invoke-Guard) -eq 0)

$bites = @(
    # 1) remover gate scopeActor do createService -> guard FAIL
    @{ name = 'create-gate-removed'; file = $svc;
       find = "await this\.assertActorNotQuarantined\(tenantId, input\.actorId\);";
       repl = "/* sem gate */" },
    # 2) escrita ANTES do gate no createService -> guard FAIL
    @{ name = 'create-write-before-gate'; file = $svc;
       find = "await this\.assertActorNotQuarantined\(tenantId, input\.actorId\);";
       repl = "await servicesRepository.create(tenantId, {});`n    await this.assertActorNotQuarantined(tenantId, input.actorId);" },
    # 3) remover gate grantee do createService (grant antigo atravessaria) -> guard FAIL
    @{ name = 'create-grantee-gate-removed'; file = $svc;
       find = "await this\.assertActorNotQuarantined\(tenantId, granteeActorId\);";
       repl = "/* sem gate grantee */"; all = $true },
    # 4) remover gate scopeActor do updateService -> guard FAIL
    @{ name = 'update-gate-removed'; file = $svc;
       find = "await this\.assertActorNotQuarantined\(tenantId, currentService\.actorId\);";
       repl = "/* sem gate */" },
    # 5) gate usando userId CRU em vez de actorId resolvido -> guard FAIL
    @{ name = 'gate-raw-userid'; file = $svc;
       find = "await this\.assertActorNotQuarantined\(tenantId, input\.actorId\);";
       repl = "await this.assertActorNotQuarantined(tenantId, userId);" },
    # 6) relaxar required cap disable (cavalo-de-Troia) -> guard FAIL
    @{ name = 'relax-disable-cap'; file = $svc;
       find = "requiredCaps\.add\('services:disable'\)";
       repl = "requiredCaps.add('services:edit')" },
    # 7) enfiar quarentena em canRepresentActor -> guard FAIL
...
Write-Host "[neg-proof service-mutations-quarantine-gate] baseOk=$baseOk"
