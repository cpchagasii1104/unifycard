...
$EPHEMERAL = "unificard_r2_delegation_schema_e2e"
...
try {
  ...
  Write-Host 'Provando invariantes R2.1 ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-r2-delegation-schema.ts
...
if ($failed) { exit 1 }
Write-Host 'R2.1 DELEGATION SCHEMA: OK' -ForegroundColor Green
exit 0
