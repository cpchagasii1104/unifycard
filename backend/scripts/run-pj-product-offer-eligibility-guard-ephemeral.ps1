# run-pj-product-offer-eligibility-guard-ephemeral.ps1
# Orquestra o E2E F-PJ-PRODUCT-OFFER-ELIGIBILITY-GUARD (DECISION-0108 + W1 Slice-A/B) em DB EFÊMERA:
#   1. cria DB efêmera (drop se existir);
#   2. aplica TODAS as migrations (runner produtivo) contra a DB efêmera;
#   3. roda o e2e (validate-pipeline-e2e-pj-product-offer-eligibility-guard.ts);
#   4. dropa a DB efêmera SEMPRE (finally).
# NUNCA toca unificard_dev. Uso: pwsh -File scripts/run-pj-product-offer-eligibility-guard-ephemeral.ps1
...
$EPHEMERAL = "unificard_pj_product_offer_eligibility_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e PJ product-offer eligibility (DECISION-0108 + W1 bind) ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-pj-product-offer-eligibility-guard.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E PJ PRODUCT-OFFER ELIGIBILITY (W1 Slice-A/B): OK' -ForegroundColor Green
exit 0
