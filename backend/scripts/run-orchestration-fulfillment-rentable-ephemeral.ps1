...
# Orquestra o E2E F-EVENT-ORCHESTRATION-RENTABLE em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_orch_rentable_e2e"
$ephUrl = "$prefix/$EPHEMERAL"
if ($EPHEMERAL -eq 'unificard_dev') { Write-Host 'ABORT: alvo é unificard_dev' -ForegroundColor Red; exit 1 }

$migration = Join-Path $PSScriptRoot '..\migrations\20260803100000_orchestration_fulfillment_kind_rentable.sql'
$holdout = Join-Path $env:TEMP '20260803100000_orchestration_fulfillment_kind_rentable.sql.holdout'
...
try {
  ...
  npx tsx src/scripts/validate-pipeline-e2e-orchestration-fulfillment-rentable.ts red
  ...
  npx tsx src/scripts/validate-pipeline-e2e-orchestration-fulfillment-rentable.ts green
...
if ($failed) { exit 1 }
Write-Host 'E2E ORCHESTRATION-FULFILLMENT-RENTABLE (RED + GREEN 1/2/3): OK' -ForegroundColor Green
exit 0
