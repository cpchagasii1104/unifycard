# negative-proof-social-posts-actor-binding.ps1
# Reproducible proof that audit-social-posts-actor-binding.mjs BITES when the POST /social/posts gate is
# spoofed (subject swapped to the declared actor). Mutates social-2.0.routes.ts (byte-exact backup),
# confirms GATE FAIL (exit 1), restores byte-identical and confirms GATE OK. ASCII-only + no BOM so it
# parses under Windows PowerShell 5.1 and PowerShell 7. F-AUTHORITY-Z2-R6.2-SOCIAL-POSTS-ACTOR-BINDING.
...
$file  = 'C:/unificard/backend/src/modules/social/social-2.0.routes.ts'
$guard = 'C:/unificard/backend/scripts/audit-social-posts-actor-binding.mjs'
...
$preStatus = (& git -C C:/unificard status --porcelain -- backend/src/modules/social/social-2.0.routes.ts)
$failed = $false
try {
  & node $guard
  ...
  # Simulated regression: swap the subject to the declared actor (spoof) — the exact vector guarded.
  $needle = 'canRepresentActor(req.tenant.id, req.user.userId, validated.actor_id)'
  $inject = 'canRepresentActor(req.tenant.id, validated.actor_id, validated.actor_id)'
...
$postStatus = (& git -C C:/unificard status --porcelain -- backend/src/modules/social/social-2.0.routes.ts)
