# run-dispute-signal-ephemeral.ps1
# Orquestra o E2E F-DISPUTE-SIGNAL em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_dispute_signal_e2e"
...
try {
  ...
  npx tsx src/scripts/validate-pipeline-e2e-dispute-signal.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E DISPUTE-SIGNAL: OK' -ForegroundColor Green
exit 0
