# run-spr-read-authority-ephemeral.ps1
# Orquestra o E2E F-C1-MONEY-SPR-READ-AUTHORITY-HARDENING em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_spr_read_authority_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e spr-read-authority ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-spr-read-authority.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E SPR-READ-AUTHORITY: OK' -ForegroundColor Green
exit 0
