# run-event-update-allowlist-and-create-timezone-ephemeral.ps1
# Orquestra o E2E DT-EVENT-PUBLISHED-UPDATE-ALLOWLIST-CASE-MISMATCH + DT-EVENT-CREATE-TIMEZONE-DEFAULTS-UTC
# em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_event_allowlist_timezone_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e event-update-allowlist-and-create-timezone ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-event-update-allowlist-and-create-timezone.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E EVENT-UPDATE-ALLOWLIST-AND-CREATE-TIMEZONE: OK' -ForegroundColor Green
exit 0
