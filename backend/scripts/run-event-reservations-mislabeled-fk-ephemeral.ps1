# run-event-reservations-mislabeled-fk-ephemeral.ps1
# Orquestra o E2E F-EVENT-RESERVATIONS-MISLABELED-FK-CONTAIN (B7) em DB EFÊMERA. NUNCA toca unificard_dev.
# A DB efêmera roda as migrations VIVAS FULL — inclusive 20260703120000 (o DROP da FK que mentia).
...
$EPHEMERAL = "unificard_er_mislabeled_fk_e2e"
...
try {
  ...
  Write-Host '📦 Aplicando migrations VIVAS FULL (inclui 20260703120000 = DROP da FK que mentia) ...' -ForegroundColor Cyan
  npx tsx src/core/db/migrate.ts
  if ($LASTEXITCODE -ne 0) { throw "migrate falhou (rc=$LASTEXITCODE)" }
  Write-Host '🧪 Rodando e2e event-reservations-mislabeled-fk ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-event-reservations-mislabeled-fk.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E EVENT-RESERVATIONS-MISLABELED-FK: OK' -ForegroundColor Green
exit 0
