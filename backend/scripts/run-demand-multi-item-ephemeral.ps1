# run-demand-multi-item-ephemeral.ps1
# F4-b — PEDIDO COM VARIOS ITENS (GO Clayton 2026-08-06). Valida em DB EFEMERA.
# NUNCA toca unificard_dev (LEIS_OPERACIONAIS — REGRA DE AMBIENTE).
#
#   A  N itens = N demandas comparaveis (nao um pacote), cada uma com a SUA data
#   B  o lote e ATOMICO: item invalido derruba o lote inteiro
#   C  o pedido DIRIGIDO nao vaza no feed (defeito da F4)
#   D  broadcast segue espelhado · E teto do lote
...
$EPHEMERAL = "unificard_multi_item_e2e"
...
try {
  ...
  Write-Host 'Rodando provas do multi-item ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-demand-multi-item.ts
...
if ($failed) { exit 1 }
Write-Host 'F4-b multi-item: provas OK em DB efemera.' -ForegroundColor Green
