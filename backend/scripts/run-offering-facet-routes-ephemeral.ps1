# run-offering-facet-routes-ephemeral.ps1
# Orquestra o E2E validate-pipeline-e2e-offering-facet-routes em DB EFÊMERA. NUNCA toca unificard_dev.
# EVENT-ENGINE-COMPLETION · C1b/C1c-a — EXPOSIÇÃO HTTP dos facets de GÊNERO + EQUIPAMENTO da oferta
# (rotas THIN sobre os métodos SELADOS tag*/untag*/list*). Owner-gated no service; governança no service;
# HTTP-real via app.inject. Bank-free (Δbank=0).
...
$EPHEMERAL = "unificard_offering_facet_routes_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e offering-facet-routes ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-offering-facet-routes.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E OFFERING-FACET-ROUTES: OK' -ForegroundColor Green
exit 0
