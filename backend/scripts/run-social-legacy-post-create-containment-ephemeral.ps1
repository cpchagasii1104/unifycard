# run-social-legacy-post-create-containment-ephemeral.ps1
# Orquestra o E2E R8A SOCIAL LEGACY POST-CREATE CONTAINMENT em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_social_legacy_post_create_containment_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e social-legacy-post-create-containment ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-social-legacy-post-create-containment.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E SOCIAL-LEGACY-POST-CREATE-CONTAINMENT: OK' -ForegroundColor Green
exit 0
