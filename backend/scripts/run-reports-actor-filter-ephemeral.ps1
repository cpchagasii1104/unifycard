# run-reports-actor-filter-ephemeral.ps1
# Orquestra o E2E F-AUTHORITY-Z2-R3-REPORTS-ACTOR-FILTER-BINDING em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_reports_actor_filter_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e reports-actor-filter-binding ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-reports-actor-filter-binding.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E REPORTS-ACTOR-FILTER-BINDING: OK' -ForegroundColor Green
exit 0
