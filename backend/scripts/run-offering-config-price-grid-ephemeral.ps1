# run-offering-config-price-grid-ephemeral.ps1
# Orquestra o E2E validate-pipeline-e2e-offering-config-price-grid em DB EFÊMERA. NUNCA toca unificard_dev.
# ARCO FUNDAÇÃO EVENTOS · FATIA PREÇO — grade de preço por CONFIG (formação) × dia-da-semana × período,
# resolução em cascata "a partir de" de 3 níveis (célula → configs.default_price_cents → offerings.price_cents),
# soft-retire de config precificada, dualidade solo, autoridade, validação. Preço = catálogo DECLARADO (Δbank=0).
...
$EPHEMERAL = "unificard_offering_config_price_grid_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e offering-config-price-grid ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-offering-config-price-grid.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E OFFERING-CONFIG-PRICE-GRID: OK' -ForegroundColor Green
exit 0
