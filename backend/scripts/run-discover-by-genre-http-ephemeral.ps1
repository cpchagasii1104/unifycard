# run-discover-by-genre-http-ephemeral.ps1
# Orquestra o E2E validate-pipeline-e2e-discover-by-genre-http em DB EFÊMERA. NUNCA toca unificard_dev.
# SLICE-2A — DISCOVERY BY GENRE over HTTP: GET /services/discover expõe na querystring os filtros JÁ
# SELADOS (subject_concept_id / equipment_concept_id / audience_size) como pass-through THIN.
...
$EPHEMERAL = "unificard_discover_by_genre_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e discover-by-genre-http ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-discover-by-genre-http.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E DISCOVER-BY-GENRE-HTTP: OK' -ForegroundColor Green
exit 0
