# run-vaquinha-funding-rules-ephemeral.ps1
# Orquestra o E2E validate-pipeline-e2e-vaquinha-funding-rules em DB EFÊMERA. NUNCA toca unificard_dev.
# SLICE S1 (VAQUINHA RULES) — regras DECLARADAS da vaquinha (all-or-nothing) na linha events. Bank-free (Δbank=0).
...
$EPHEMERAL = "unificard_vaquinha_funding_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e vaquinha-funding-rules ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-vaquinha-funding-rules.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E VAQUINHA-FUNDING-RULES: OK' -ForegroundColor Green
exit 0
