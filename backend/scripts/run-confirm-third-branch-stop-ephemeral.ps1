# run-confirm-third-branch-stop-ephemeral.ps1
# F-CONFIRM-THIRD-BRANCH-STOP — prova de COMPORTAMENTO do STOP do 3o ramo, em DB EFEMERA.
# NUNCA toca unificard_dev (LEIS_OPERACIONAIS — REGRA DE AMBIENTE).
#
# O guard estatico prova que o STOP esta ESCRITO; isto prova que ele DISPARA:
#   A1  confirm de availability owner_type='user'  -> DEVE PARAR com 501 STOP_DECISION_REQUIRED
#   B1  o booking permanece 'requested'            -> fail-closed de verdade, sem meia-escrita
#   C1  confirm de availability owner_type='actor_asset' -> DEVE CONTINUAR CONFIRMANDO
#       (trava nova nao pode bloquear quem pode — o outro lado da moeda)
...
$EPHEMERAL = "unificard_confirm_stop_e2e"
...
try {
  ...
  Write-Host 'Rodando prova de comportamento do STOP ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-confirm-third-branch-stop.ts
...
if ($failed) { exit 1 }
Write-Host 'F-CONFIRM-THIRD-BRANCH-STOP: provas OK em DB efemera.' -ForegroundColor Green
