# run-auth-rate-limit-ephemeral.ps1
# Orquestra o E2E DT-AUTH-RATE-LIMIT-FAIL-OPEN-SUBSTRATE-AUSENTE em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_auth_rate_limit_e2e"
...
try {
  ...
  Write-Host '📦 Aplicando migrations VIVAS FULL (inclui auth_rate_limit_logs) ...' -ForegroundColor Cyan
  npx tsx src/core/db/migrate.ts
  if ($LASTEXITCODE -ne 0) { throw "migrate falhou (rc=$LASTEXITCODE)" }
  Write-Host '🧪 Rodando e2e auth-rate-limit-substrate (5 passam, 6ª/7ª bloqueadas) ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-auth-rate-limit-substrate.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E AUTH-RATE-LIMIT-SUBSTRATE: OK' -ForegroundColor Green
exit 0
