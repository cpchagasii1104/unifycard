# run-operator-service-order-view-grant-ephemeral.ps1
# Orquestra o E2E F-OPERATOR-SERVICE-ORDER-VIEW-GRANT em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_operator_service_order_view_grant_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e operator-service-order-view-grant ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-operator-service-order-view-grant.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E OPERATOR-SERVICE-ORDER-VIEW-GRANT: OK' -ForegroundColor Green
exit 0
