# negative-proof-reports-transfers-sla-representation.ps1
# Prova que o guard audit-reports-transfers-sla-representation.mjs MORDE cada regressão.
...
$routes = Join-Path (Get-Location) 'src\modules\reports\reports.routes.ts'

function Invoke-Guard {
    node scripts/audit-reports-transfers-sla-representation.mjs *> $null
    return $LASTEXITCODE
}
$baseOk = ((Invoke-Guard) -eq 0)

$bites = @(
    # 1) remover o gate de fromActorId (actorId cru) -> guard FAIL
    @{ name = 'from-gate-removed'; file = $routes;
       find = "await canRepresent\(String\(query\.fromActorId\)\)";
       repl = "true /* sem gate */" },
    # 2) remover o gate de toActorId (actorId cru) -> guard FAIL
    @{ name = 'to-gate-removed'; file = $routes;
       find = "await canRepresent\(String\(query\.toActorId\)\)";
       repl = "true /* sem gate */" },
    # 3) sem filtro vira tenant-wide: remover participantActorId/self -> guard FAIL
    @{ name = 'no-filter-tenant-wide'; file = $routes;
       find = "options\.participantActorId = self;";
       repl = "/* tenant-wide */" }
)
...
Write-Host "[neg-proof reports-transfers-sla-representation] baseOk=$baseOk"
