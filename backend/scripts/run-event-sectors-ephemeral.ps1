# run-event-sectors-ephemeral.ps1
# Orquestra o E2E validate-pipeline-e2e-event-sectors em DB EFÊMERA. NUNCA toca unificard_dev.
# SLICE S3 (SETORES) — setor SELF-CONTAINED (event_sectors) com inteira/meia (piso legal 40%). Bank-free (Δbank=0).
...
$EPHEMERAL = "unificard_event_sectors_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e event-sectors ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-event-sectors.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E EVENT-SECTORS: OK' -ForegroundColor Green
exit 0
