# run-event-publish-funnel-ephemeral.ps1
# Orquestra o E2E F-EVENT-PUBLISH-FUNNEL em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_event_publish_funnel_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e event-publish-funnel ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-event-publish-funnel.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E EVENT-PUBLISH-FUNNEL: OK' -ForegroundColor Green
exit 0
