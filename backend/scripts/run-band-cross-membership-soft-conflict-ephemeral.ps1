# run-band-cross-membership-soft-conflict-ephemeral.ps1
# Orquestra o E2E validate-pipeline-e2e-band-cross-membership-soft-conflict em DB EFÊMERA. NUNCA toca unificard_dev.
# ARCO FUNDAÇÃO EVENTOS · FATIA 4 (A ÚLTIMA) — aviso SUAVE de conflito de agenda POR PESSOA
# (cross-membership) no confirm de booking: detecção derivada por memberships ativas + solo,
# sink OP-2 = event_outbox (AVAILABILITY_CONFLICT_DETECTED, dedup determinístico), NUNCA bloqueio,
# hook NÃO-CRÍTICO, hard-lock selado intacto. Bank-free (Δbank=0).
...
$EPHEMERAL = "unificard_band_soft_conflict_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e band-cross-membership-soft-conflict ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-band-cross-membership-soft-conflict.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E BAND-CROSS-MEMBERSHIP-SOFT-CONFLICT: OK' -ForegroundColor Green
exit 0
