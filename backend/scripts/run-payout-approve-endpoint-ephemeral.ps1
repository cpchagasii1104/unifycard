# run-payout-approve-endpoint-ephemeral.ps1
# Orquestra o E2E F-PAYOUT-APPROVE-ENDPOINT-CORE-AUTHORITY (CAMINHO B) em DB EFÊMERA (NÃO move dinheiro).
# NUNCA toca unificard_dev. Uso: pwsh -File scripts/run-payout-approve-endpoint-ephemeral.ps1
...
$EPHEMERAL = "unificard_payout_approve_endpoint_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e payout-approve-endpoint (CAMINHO B) ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-payout-approve-endpoint.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E PAYOUT-APPROVE-ENDPOINT (CAMINHO B): OK' -ForegroundColor Green
exit 0
