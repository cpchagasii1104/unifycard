# run-contacts-schema-ghost-ephemeral.ps1
# Orquestra o E2E F-CONTACTS-SCHEMA-GHOST-FAIL-CLOSED-CONTAINMENT em DB EFÊMERA. NUNCA toca unificard_dev.
# NÃO cria a tabela contacts: a DB efêmera roda as migrations VIVAS (que NÃO contêm contacts) → ghost real.
...
$EPHEMERAL = "unificard_contacts_ghost_e2e"
...
try {
  ...
  Write-Host '📦 Aplicando migrations (vivas; contacts NÃO está entre elas) ...' -ForegroundColor Cyan
  npx tsx src/core/db/migrate.ts
  if ($LASTEXITCODE -ne 0) { throw "migrate falhou (rc=$LASTEXITCODE)" }
  Write-Host '🧪 Rodando e2e contacts-schema-ghost ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-contacts-schema-ghost.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E CONTACTS-SCHEMA-GHOST: OK' -ForegroundColor Green
exit 0
