# negative-proof-social-legacy-post-create-containment.ps1
# Reproducible proof that audit-social-legacy-post-create-containment.mjs BITES when the contained legacy
# route POST /social/posts/create is regressed back to writing (re-introduces the socialService.createPost
# write sink + actionContext authority ahead of the 501 hard-stop).
# Mutates social.routes.ts (byte-exact backup), confirms GATE FAIL (exit 1), restores byte-identical and
# confirms GATE OK. ASCII-only + no BOM so it parses under Windows PowerShell 5.1 and PowerShell 7.
# R8A SOCIAL LEGACY POST-CREATE CONTAINMENT.
...
$file  = 'C:/unificard/backend/src/modules/social/social.routes.ts'
$guard = 'C:/unificard/backend/scripts/audit-social-legacy-post-create-containment.mjs'

$origBytes = [IO.File]::ReadAllBytes($file)
$preStatus = (& git -C C:/unificard status --porcelain -- backend/src/modules/social/social.routes.ts)
$failed = $false
try {
  & node $guard
  ...
  # Simulated regression: the contained route writes again -- inject the write sink + actionContext authority
  # right before the 501 hard-stop (single-line, CRLF-agnostic; status(501) is unique to the contained route).
  $needle = 'return reply.status(501).send({'
  $inject = 'await socialService.createPost(req.server, req.tenant.id, req.actionContext.actorId, req.body); return reply.status(501).send({'
  ...
  & node $guard
  $rc = $LASTEXITCODE
  if ($rc -eq 0) { throw "NEGATIVE-PROOF FAILED: guard PASSED (exit 0) with the route writing again -- expected FAIL." }
  Write-Host "[OK] NEGATIVE-PROOF: guard FAILED (exit $rc) with write sink + actionContext reintroduced (expected)." -ForegroundColor Green
}
...
$postStatus = (& git -C C:/unificard status --porcelain -- backend/src/modules/social/social.routes.ts)
