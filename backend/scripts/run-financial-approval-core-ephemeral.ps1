# run-financial-approval-core-ephemeral.ps1
# Orquestra o E2E F-CORE-FINANCIAL-APPROVAL-MODEL em DB EFÊMERA:
#   1. cria DB efêmera (drop se existir);
#   2. aplica TODAS as migrations (runner produtivo, inclui 20260614140000);
#   3. roda o e2e (validate-pipeline-e2e-financial-approval-core-model.ts);
#   4. dropa a DB efêmera SEMPRE (finally).
# NUNCA toca unificard_dev. Uso: pwsh -File scripts/run-financial-approval-core-ephemeral.ps1
...
$EPHEMERAL = "unificard_financial_approval_core_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e financial-approval-core ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-financial-approval-core-model.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E FINANCIAL-APPROVAL-CORE: OK' -ForegroundColor Green
exit 0
