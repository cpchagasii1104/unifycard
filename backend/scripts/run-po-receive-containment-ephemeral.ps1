# run-po-receive-containment-ephemeral.ps1
# Orquestra o E2E F-C1-MONEY-PO-RECEIVE-EXPLICIT-CONTAINMENT em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_po_receive_containment_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e po-receive-containment ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-po-receive-containment.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E PO-RECEIVE-CONTAINMENT: OK' -ForegroundColor Green
exit 0
