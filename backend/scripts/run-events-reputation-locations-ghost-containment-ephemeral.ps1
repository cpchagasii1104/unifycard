# run-events-reputation-locations-ghost-containment-ephemeral.ps1
# Orquestra o E2E F-EVENTS-GHOST-CONTAINMENT em DB EFÊMERA. NUNCA toca unificard_dev.
# NÃO cria reputation_scores nem event_locations: a DB efêmera roda as migrations VIVAS (que NÃO as contêm)
# → ghost real (42P01) → o E2E exercita o caminho de contenção de verdade.
...
$EPHEMERAL = "unificard_events_ghost_containment_e2e"
...
try {
  ...
  Write-Host '📦 Aplicando migrations (vivas; reputation_scores e event_locations NÃO estão entre elas) ...' -ForegroundColor Cyan
  npx tsx src/core/db/migrate.ts
  if ($LASTEXITCODE -ne 0) { throw "migrate falhou (rc=$LASTEXITCODE)" }
  Write-Host '🧪 Rodando e2e events-reputation-locations-ghost-containment ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-events-reputation-locations-ghost-containment.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E EVENTS-REPUTATION-LOCATIONS-GHOST-CONTAINMENT: OK' -ForegroundColor Green
exit 0
