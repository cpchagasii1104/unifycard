# run-social-work-payment-ownership-ephemeral.ps1
# Orquestra o E2E B4 (ownership da superfície money GET payments) em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_social_work_payment_ownership_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e social-work-payment-ownership ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-social-work-payment-ownership.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E SOCIAL-WORK-PAYMENT-OWNERSHIP: OK' -ForegroundColor Green
exit 0
