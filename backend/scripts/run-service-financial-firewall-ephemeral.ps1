# run-service-financial-firewall-ephemeral.ps1
# Orquestra o E2E F-SERVICE-FINANCIAL-FIREWALL-CODE (DECISION-0110) + R8J SERVICES-DISCOVERY DIRECT-PAY RETIREMENT
# em DB EFÊMERA:
#   1. cria DB efêmera (drop se existir);
#   2. aplica TODAS as migrations (runner produtivo);
#   3. roda o e2e (validate-pipeline-e2e-service-financial-firewall.ts);
#   4. dropa a DB efêmera SEMPRE (finally).
# NUNCA toca unificard_dev (assertEphemeralDb no e2e + guard de nome aqui). Uso:
#   pwsh -File scripts/run-service-financial-firewall-ephemeral.ps1
...
$EPHEMERAL = "unificard_service_firewall_e2e"
...
try {
  ...
  Write-Host 'Aplicando migrations na DB efemera ...' -ForegroundColor Cyan
  npx tsx src/core/db/migrate.ts
  if ($LASTEXITCODE -ne 0) { throw "migrate falhou (rc=$LASTEXITCODE)" }

  Write-Host 'Rodando e2e service-financial-firewall (+R8J direct-pay retired) ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-service-financial-firewall.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E SERVICE-FINANCIAL-FIREWALL + R8J DIRECT-PAY: OK' -ForegroundColor Green
exit 0
