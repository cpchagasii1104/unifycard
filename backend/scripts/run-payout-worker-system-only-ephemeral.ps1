# run-payout-worker-system-only-ephemeral.ps1
# Orquestra o E2E F-PAYOUT-WORKER-SYSTEM-ONLY-SEAL em DB EFÊMERA (MOVE DINHEIRO — só em DB descartável).
# NUNCA toca unificard_dev. Uso: pwsh -File scripts/run-payout-worker-system-only-ephemeral.ps1
...
$EPHEMERAL = "unificard_payout_worker_seal_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e payout-worker-system-only (move dinheiro) ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-payout-worker-system-only.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E PAYOUT-WORKER-SYSTEM-ONLY: OK' -ForegroundColor Green
exit 0
