# run-venue-enrichment-ephemeral.ps1
# Orquestra o E2E validate-pipeline-e2e-venue-enrichment em DB EFÊMERA. NUNCA toca unificard_dev.
# SLICE S2 (VENUE ENRICHMENT) — captura do LOCAL do evento REUTILIZANDO vocabulário canônico. Bank-free (Δbank=0).
...
$EPHEMERAL = "unificard_venue_enrichment_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e venue-enrichment ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-venue-enrichment.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E VENUE-ENRICHMENT: OK' -ForegroundColor Green
exit 0
