# run-offering-config-lineup-ephemeral.ps1
# Orquestra o E2E validate-pipeline-e2e-offering-config-lineup em DB EFÊMERA. NUNCA toca unificard_dev.
# ARCO FUNDAÇÃO EVENTOS · FATIA 3 — cardápio de CONFIGS da oferta do performer com LINE-UP opcional
# (line-up = PESSOA/member_actor_id, doutrina DECISION-0188; DERIVED-INCOMPLETE; piso team_size nos
# dois lados; dualidade solo; negativos fail-closed; lifecycle leave/reentrada). Bank-free (Δbank=0).
...
$EPHEMERAL = "unificard_offering_config_lineup_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e offering-config-lineup ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-offering-config-lineup.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E OFFERING-CONFIG-LINEUP: OK' -ForegroundColor Green
exit 0
