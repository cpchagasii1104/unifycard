# run-votes-writes-containment-ephemeral.ps1
# Orquestra o E2E F-VOTES-WRITES-EXPLICIT-FAIL-CLOSED-CONTAINMENT em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_votes_containment_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e votes-writes-containment ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-votes-writes-containment.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E VOTES-WRITES-CONTAINMENT: OK' -ForegroundColor Green
exit 0
