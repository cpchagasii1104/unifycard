...
# Orquestra o E2E validate-pipeline-e2e-atomic-company-birth em DB EFÊMERA. NUNCA toca unificard_dev.
# Verificação pontual (Onda 1 zeragem de DT, 2026-07-05) de DT-PJ-EPHEMERAL-FIXTURES-STALE-VS-BASELINE-365:
# confirmar se a asserção "1d" (citada como falha em 2026-06-07 por expectativa de company_status
# legada PROVISIONAL/pending) já foi corrigida pra DRAFT (lifecycle vigente, DECISION-0097).
...
$EPHEMERAL = "unificard_atomic_birth_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e atomic-company-birth ...' -ForegroundColor Cyan
...
if ($failed) { exit 1 }
Write-Host 'E2E ATOMIC-COMPANY-BIRTH: OK' -ForegroundColor Green
exit 0
