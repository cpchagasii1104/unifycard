...
# Orquestra o E2E validate-pipeline-e2e-availability-owner-authority em DB EFÊMERA. NUNCA toca unificard_dev.
# Fecha R1 de DT-AVAILABILITY-OWNER-AUTHORITY-EXEMPLAR-RESIDUES (Onda 1 zeragem de DT, 2026-07-05):
# o e2e citava este runner no cabeçalho, mas ele não existia (prova órfã) — sem ele o e2e não podia
# rodar como documentado (assertEphemeralDb aborta contra unificard_dev).
...
$EPHEMERAL = "unificard_availability_owner_authority_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e availability-owner-authority ...' -ForegroundColor Cyan
...
if ($failed) { exit 1 }
Write-Host 'E2E AVAILABILITY-OWNER-AUTHORITY: OK' -ForegroundColor Green
exit 0
