# run-service-order-status-vocabulary-ephemeral.ps1
# Orquestra o E2E DT-SERVICE-ORDER-STATUS-VOCABULARY-MISMATCH em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_so_status_vocab_e2e"
...
try {
  ...
  npx tsx src/scripts/validate-pipeline-e2e-service-order-status-vocabulary.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E SERVICE-ORDER-STATUS-VOCABULARY: OK' -ForegroundColor Green
exit 0
