# run-event-rfq-acceptquote-containment-ephemeral.ps1
# Orquestra o E2E R7b ACCEPTQUOTE P0 CONTAINMENT em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_event_rfq_acceptquote_containment_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e event-rfq-acceptquote-containment ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-event-rfq-acceptquote-containment.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E EVENT-RFQ-ACCEPTQUOTE-CONTAINMENT: OK' -ForegroundColor Green
exit 0
