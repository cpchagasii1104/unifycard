# run-dashboard-metrics-tenant-scope-ephemeral.ps1
# Orquestra o E2E B4f (tenant-scope das métricas) em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_dashboard_metrics_tenant_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e dashboard-metrics-tenant-scope ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-dashboard-metrics-tenant-scope.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E DASHBOARD-METRICS-TENANT-SCOPE: OK' -ForegroundColor Green
exit 0
