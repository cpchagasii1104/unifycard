# run-organization-schema-ghost-containment-ephemeral.ps1
# Orquestra o E2E F-ORGANIZATION-SCHEMA-GHOST-FAIL-CLOSED-CONTAINMENT em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_organization_containment_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e organization-schema-ghost-containment ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-organization-schema-ghost-containment.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E ORGANIZATION-SCHEMA-GHOST-CONTAINMENT: OK' -ForegroundColor Green
exit 0
