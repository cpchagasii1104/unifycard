# run-event-from-price-coherence-ephemeral.ps1
# Orquestra o E2E F-EVENT-FROM-PRICE-COHERENCE (G0) em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_event_from_price_coherence_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e event-from-price-coherence ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-event-from-price-coherence.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E EVENT-FROM-PRICE-COHERENCE: OK' -ForegroundColor Green
exit 0
