# run-identity-update-authority-ephemeral.ps1
# Orquestra o E2E F-DISCOVERY-PUBLIC-PROFILE-SLICE-A em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_identity_authority_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e identity-update-authority ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-identity-update-authority.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E IDENTITY-UPDATE-AUTHORITY: OK' -ForegroundColor Green
exit 0
