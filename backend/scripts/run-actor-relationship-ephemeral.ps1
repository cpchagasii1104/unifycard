# run-actor-relationship-ephemeral.ps1
# Orquestra o E2E F-ACTOR-RELATIONSHIP-TYPED-EDGE-SLICE-1 em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_actor_relationship_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e actor-relationship ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-actor-relationship.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E ACTOR-RELATIONSHIP: OK' -ForegroundColor Green
exit 0
