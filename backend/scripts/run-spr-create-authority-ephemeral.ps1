# run-spr-create-authority-ephemeral.ps1
# Orquestra o E2E F-C1-MONEY-SPR-CREATE-AUTHORITY-HARDENING em DB EFÊMERA. NUNCA toca unificard_dev.
...
$EPHEMERAL = "unificard_spr_create_authority_e2e"
...
try {
  ...
  Write-Host '🧪 Rodando e2e spr-create-authority ...' -ForegroundColor Cyan
  npx tsx src/scripts/validate-pipeline-e2e-spr-create-authority.ts
...
if ($failed) { exit 1 }
Write-Host 'E2E SPR-CREATE-AUTHORITY: OK' -ForegroundColor Green
exit 0
