...
Dependências:
- tenants (001_schema_genesis_ssot)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = current_schema()
      ...
      AND policyname = 'tenant_configs_rls'
  ) THEN
...
COMMENT ON TABLE tenant_configs IS
  'Configurações dinâmicas por tenant e módulo';

COMMENT ON TABLE feature_flags IS
  'Feature flags com rollout gradual e whitelist de usuários';

COMMENT ON FUNCTION update_updated_at_column IS
  'Atualiza coluna updated_at automaticamente em UPDATEs';
