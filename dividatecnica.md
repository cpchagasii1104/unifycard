...
## DT-EVENTS-SPRINT76-ACTOR-HINT-AUTHORSHIP-FORGERY — 🟠 EM CONTENÇÃO · GATE Veredito B resolvido em A pela norma (AUDIT-004)

- `events-sprint76.routes.ts` cancel (L359) / checkin (L389) / checkout (L414) carimbam `actionContext.actorId` cru, DUAS vezes (actor + usuário), sem `canRepresentActor`. GATE read-only (Opus) confirmou: **dupla falha** — (i) forja de autoria (carimbo arbitrário de ator E de usuário) e (ii) autorização AUSENTE (qualquer autenticado do tenant cancela/faz check-in/out de QUALQUER ticket por ID). **Bank-free confirmado** (cancelTicket só opera em `status='pending'` não pago — `ticket.service.ts` L369-371; checkin/checkout são presença; nenhum toca dinheiro). Superfície viva ÓRFÃ (nenhuma UI viva chama; check-in real vai por `/cultural/events/:id/check-in` e `/presence/checkin/by-token`).
- **Modelo de contenção (resolvido pela norma, não é política nova):** checkin/checkout convergem ao modelo promulgado `cultural-checkin-target-authority` (DECISION-0113 residual B1: self OR `canRepresentActor` sobre o target + gate de validador no service). cancel = caller representa `buyerActorId` (server-resolved do ticket_sale; Art. I ownership). Carimbo = actor resolvido + `req.user.userId` real. **Aditivo deferido** (não necessário p/ conter): organizador-cancela-terceiro, `event_staff` além do validador.
- **Status:** GATE Veredito B → resolvido em A pela direção via exaustão normativa. Aguarda GO material (Opus). Depois: material → Yala → selo.
