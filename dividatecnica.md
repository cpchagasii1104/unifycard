...
- **PLANO EM FATIAS (cada fatia = Gate/GO/material/Yala próprio):** F0 verdade de schema + contenção honesta (Bank-free, GO-ready) · F1 catálogo de tipo governado (Bank-free, GO-ready) · F2 compra checkout→ticket_sales bank-wired (🔴 dinheiro, RATIFICAÇÃO-GATED) · F3 "ter um ingresso"/emissão + meus-ingressos (modelo a ratificar; schema pode mudar) · F4 check-in convergido ao modelo cultural — ABSORVE a contenção de autoria sprint76 (Bank-free, GO-ready) · F5 cancel/reembolso (🔴 dinheiro, RATIFICAÇÃO-GATED). **Espinha Bank-free F0→F1→F4 pronta para GO sem ratificação.**
