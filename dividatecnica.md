...
## DT-EVENTS-SPRINT76-ACTOR-HINT-AUTHORSHIP-FORGERY — ⚪ NÃO-EXPLORÁVEL (runtime morto vs schema) · contingente ao drift (AUDIT-004)

- `events-sprint76.routes.ts` cancel/checkin/checkout carimbam `actionContext.actorId` cru sem `canRepresentActor` (dupla falha: forja de autoria + autorização ausente). O GO material de contenção foi EMITIDO e a executora aplicou o fix na forma (typecheck 0, guard 11/11) — mas **PAROU e reverteu** ao descobrir que o runtime está MORTO contra o schema vivo (ver `DT-EVENTS-SPRINT76-TICKET-SALE-REPOSITORY-SCHEMA-DRIFT`).
- **Reclassificado 🟠→⚪:** a forja NUNCA foi viva-escritível — cancel/checkin/checkout passam por `getSaleById`, que estoura `coluna "event_ticket_id" não existe` ANTES de qualquer escrita (500). Risco era CONTINGENTE a corrigir o drift. Sem exploit vivo. Prova empírica: DB efêmera migrada FULL rc=0; a SELECT de getSaleById lançou coluna-inexistente.
- **Modelo de contenção (guardado para re-uso quando/se o drift for resolvido):** checkin/checkout = modelo promulgado `cultural-checkin-target-authority` (self OR `canRepresentActor` sobre o target + validador no service); cancel = representa `buyerActorId` (Art. I). Carimbo = actor resolvido + `req.user.userId`. Aditivo (organizador/staff) deferido.
- **Status:** contenção de autoria BLOQUEADA pelo drift; sem GO até a fate de sprint76 ticketing ser decidida (ver drift DT). Não há urgência (sem risco vivo).
