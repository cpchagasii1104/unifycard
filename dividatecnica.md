...
- **Correção:** a Fatia **F0-grupo** constrói o vínculo GOVERNADO correto (writer format-first + INSERT em `group_events` no schema real de 5 colunas + autoridade = `canRepresentActor` sobre o actor do grupo via fachada §4.9.8, transacional com o INSERT de events). O W2 quebrado é contido em F0-writer-único. Cross-ref: `EVENT_ENGINE_COUPLING_MAP.md` (F0-grupo).
