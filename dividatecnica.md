...
- **Provas:** typecheck 0 · runner **189/189** · guard verde · mutations **38+6** · unit **15/15** (D9.1+D9.2-A 27/27) · E2E **55/55** (concorrência real; revogação serializada; kill-no-COMMIT) · clone efêmero destruído; migrations aplicadas SÓ no clone; **dev intocado** (gam/gib INEXISTENTES, 0 colunas intent, 0 fns, group_members=1/invites=0) · **Bank 16/0/0 · Δbank=0**.
- **OBS não bloqueantes:** residual do UNIQUE legado de invites detectado e fail-closed (`GAM_INTENT_LEGACY_UNIQUE_RESIDUAL`), substituição RESERVADA ao D9.2-B; EOL revertido com hash idêntico; restore user_profiles benigno; ESM oficial; cartório lista 4 DTs (fonte de verdade).
- **DTs preservadas (4, nenhuma fechada).** **D9.2-B FECHADA** (cutover exige novo Gate + novo GO + envelope + Yala próprios); D9.3/D9.4 fechadas. **STOP obrigatório.**

### 2026-07-18 (167) — D9.2-A · FUNDAÇÃO ACTOR-FIRST DORMENTE DA MEMBERSHIP · MATERIAL: EXECUTADO E PROVADO · INTERNO E DORMENTE · NÃO SELADO · AGUARDA UMA ÚNICA AUDITORIA YALA
- **Material único consolidado** sob GO explícito, executando a DECISION-0188 (fase A). Commit material `2ef62dd2c` (base `bea280276`; 11 arquivos · 2542 inserções): migration `20260718120000` (casa `group_actor_memberships` — member_actor_id como identidade única; lifecycle active→left|removed terminal; unicidade ativa; FKs compostas reusando candidate keys D9.1; RLS FORCE; app sem DML; imutabilidade física) + evolução DORMENTE de `group_invites` (intent_kind invite|request explícito; NULL≠invite; pending-only uniques; UNIQUE legado PRESERVADO com residual SINALIZADO fail-closed) + 5 fns governadas (enter estrutural com classes v1 user·page·group-RAIZ e anti-composição; leave/remove com owner bloqueado; intent explícita; aceite ATÔMICO) + service interno transaction-owner (canRepresentActor no MESMO client; zero capability) + shadow validation read-only PURA + guard 189 (~55 marcadores A–I) + mutations 38+6 + unit 15/15 + E2E efêmero 55/55 (concorrência real incl. revogação-durante-entrada e kill-no-COMMIT).
- **Dormência total provada:** zero caller de produto (walk de src/) · 6 superfícies + events-B3 + role-authority + caps INTOCADOS (marcadores vivos verificados pelo guard) · zero dual-write nos dois sentidos · migrations NÃO aplicadas em dev (só clone efêmero, destruído com prova) · dev pós-tudo: gam/gib INEXISTENTES, 0 colunas intent, 0 fns, legado 1/0 inalterado, **Bank 16/0/0 · Δbank=0**.
- **Regressão:** typecheck 0 · runner **189/189** · units 27/27 · diff --check limpo · 0186/0187/0188 + material D9.1 byte-intactos · DTs preservadas (nenhuma fechada). **D9.2-B/D9.3/D9.4 FECHADAS.**
- **Próximo ato: STOP — arco material aguarda UMA única auditoria Yala. Selo futuro NÃO abre o cutover D9.2-B** (exige Yala + selo + novo Gate + novo GO).

### 2026-07-18 (166) — DECISION-0188 · MEMBERSHIP ACTOR-FIRST CONTRACT (D9.2): SELADA PELA YALA · VEREDITO A · SELO COMPLETO DOCS-ONLY · OFICIALMENTE ENCERRADA
- **Auditoria Yala READ-ONLY do arco (GATE D9.2 + DECISION-0188 + cartório) → Veredito A.** DECISION-0188 SELADA e ENCERRADA. Arco: `2ff31d6e3`(selo D9.1) → `e2087d905`(promulgação 0188) → **este selo** (commit docs-only, 2 arquivos). DECISION-0188 byte-intacta (`ed52391d`).
- **Contrato selado:** membro=Actor (classes user·page·group-raiz) · casa futura `group_actor_memberships` · `member_actor_id` identidade única · group_members legado→congelado (sem dual-write/fallback) · lifecycle active→left|removed terminal sem DELETE · owner-membership invariante · invites/requests = intenções explícitas com aceite TRANSACIONAL · role sem authority (admin perde poder no cutover) · cap 3 civil humano · 6 namespaces + reader events-B3 no cutover.
- **Material ESTAGIADO, ambos NÃO abertos:** D9.2-A (fundação dormente) e D9.2-B (cutover) exigem GOs materiais SEPARADOS; selo da DECISION não abre D9.2-A; selo de D9.2-A não abre D9.2-B.
- **2 OBS não bloqueantes:** defeitos vivos documentados (role-authority, actionContext→user_id, DELETE físico, UNIQUE invites, acceptInvite não-tx) convergem só no cutover; D9.2-A pode medir/shadow mas não backfill/flip/ativar.
- **D9.1 dormente** (`group_institutional_bindings` inexistente); `group_actor_memberships` inexistente; membership real=1 (owner) intocada; **Bank 16/0/0 · Δbank=0**; 0188/0187/0186/0157 + material D9.1 byte-intactos. **DTs preservadas. D9.3/D9.4 não abertas. STOP obrigatório.**

### 2026-07-18 (165) — DECISION-0188 · MEMBERSHIP ACTOR-FIRST CONTRACT (D9.2): PROMULGADA DOCS-ONLY · NÃO SELADA · MATERIAL NÃO INICIADO · AGUARDA UMA ÚNICA AUDITORIA YALA
- **Sequência:** GATE read-only D9.2 sobre `2ff31d6e3` (zero alteração; Veredito B — abertos: classes de elegibilidade, casa, lifecycle, role, invites/requests, owner-membership, cap, namespaces, estratégia de flip) → GO explícito → promulgação docs-only.
- **Fechado:** membro = ACTOR (classes fechadas user·page·group-raiz; representante não vira membro) · casa futura `group_actor_memberships` (group_members → legada→congelada; comparação=prova, nunca fallback) · identidade única `member_actor_id` (user/global só resolvem; mapper 0131-B3) · 1 ativa/tenant+group+actor; reentrada=nova linha · lifecycle `active→left|removed` terminal sem DELETE (ban/suspend fora) · owner-membership invariante · invites/requests = intenções explícitas `intent_kind` com aceite TRANSACIONAL (fim da convenção self-invite; rejeição não bloqueia p/ sempre) · autoridade só canRepresentActor existente (zero capability; D9.3 intacta) · role legado sem poder (admin perde efeito no flip) · cap 3 = civil humano (institucional não consome) · 6 superfícies de namespace nomeadas p/ cutover + reader events-B3 junto · **material estagiado D9.2-A (fundação dormente) / D9.2-B (cutover) com GOs separados** · guards mínimos · fronteiras.
- **Fatos vivos registrados sem correção:** /join persiste actionContext.actorId como user_id; /request injeta global em FK de actors; role='admin' concede poder (Art.17); DELETE físico; UNIQUE de invites bloqueia re-convite. Dados: 1 membership, backfill 100% determinístico.
- **Zero código · zero migration · zero dado · D9.1 não aplicado · Bank intocado · Δbank=0.** 0186/0187/material D9.1 byte-intactos. **Selo NÃO abre D9.2-A/B. Próximo ato: STOP — aguarda UMA única auditoria Yala.**

### 2026-07-18 (164) — D9.1 · GROUP INSTITUTIONAL BINDING: SELADO PELA YALA · VEREDITO A · SELO COMPLETO MATERIAL FINAL · OFICIALMENTE ENCERRADO
- **Reauditoria Yala FINAL do arco de remediação → Veredito A.** Material D9.1 SELADO e ENCERRADO. Arco completo: `953a4d8d4`(selo 0187) → `b6743b55e`(material) → `29497c1f8`(cartório) → **`380f68282`(remediação transacional)** → `a8260d9ed`(cartório remediação) → **este selo** (commit docs-only, 2 arquivos).
- **Bloqueante do Veredito B eliminado:** authority e writer agora no MESMO client/transação — service = único transaction owner (BEGIN→tenant LOCAL→advisory por tenant→ensureUserActorTx→canRepresentActor(client)×2/3→fn SQL→readback→COMMIT; falha=ROLLBACK; release único). `canRepresentActor` transaction-aware via `existingClient` REUTILIZANDO o SSOT (callers antigos byte-idênticos); evidência lida `FOR SHARE` nas linhas concretas → revogação concorrente serializada (resultado authority-obsoleta fisicamente impedido).
- **Provas seladas:** typecheck 0 · runner **188** · guard verde · mutations **53+6** · unit **12/12** · E2E **59/59** com concorrência real **T1–T5** (fonte revogável company_users) + fault-injections (sequência + falha no COMMIT) · clone efêmero destruído sem resíduo · migration aplicada só no clone · **unificard_dev intocado (tabela/funções D9.1 inexistentes)** · zero vínculo real · Bank **16/0/0** · **Δbank=0** · migration/DECISIONs/guard/service byte-intactos.
- **5 OBS não bloqueantes** registradas (bloqueio observado por ordenação; revogação sobre linha real de company_users; delegações temporais dormentes; ensureUserActorTx só user-actor; restore legado user_profiles fora do caminho de authority). **DTs preservadas** (GROUPS-TABLE-NO-RLS + 2 Bank congeladas + SPRINT78). **D9.2 NÃO aberto.**
- **STOP obrigatório.** O selo não aplica migration, não ativa binding, não cria vínculo, não abre D9.2 — qualquer etapa posterior exige novo GO humano explícito.

### 2026-07-17 (163) — D9.1 · REMEDIAÇÃO CONSOLIDADA PÓS-YALA (VEREDITO B) · AUTHORITY DUAL TRANSACTION BOUNDARY: EXECUTADA E PROVADA · NÃO SELADA · AGUARDA UMA ÚNICA REAUDITORIA YALA FINAL
- **Fato Yala (ratificado pela diretora):** os canRepresentActor rodavam em autocommit ANTES da transação do writer — janela real de revogação (TOCTOU); o relatório havia reclassificado a prova como "observação". Restante do material aceito; bloqueante único = fronteira transacional da authority.
- **Correção `380f68282`** (base `29497c1f8`; 10 arquivos, +977/−256; migration BYTE-INTACTA `dfcfec9c`; runner segue 188): transaction owner único no service (BEGIN→tenant→advisory por tenant→ensureUserActorTx→canRepresentActor(client)×2/3→fn SQL no MESMO client→COMMIT; falha=ROLLBACK; release 1×) · `canRepresentActor` transaction-aware via `existingClient` REUTILIZANDO o SSOT (5 fontes idênticas; evidência FOR SHARE; cadeia canManageCompany/resolveGlobalUserId/registry/delegação propagada com o mesmo client; caminho pool intacto p/ callers antigos) · proteção contra revogação = FOR SHARE nas LINHAS que fundamentaram a prova (revogação serializa: espera o COMMIT ou vence e a prova enxerga o estado revogado — resultado proibido fisicamente impedido).
- **Provas:** unit 12/12 (identidade de client; ordem; rollbacks; release 1×) · E2E **59/59** com **concorrência REAL 2-conexões sobre fonte revogável company_users** (T1 proteção-primeiro bloqueia revogador até COMMIT; T2/T2b revogação-vence→fail-closed+chave não consumida; T3/T4 revogação DURANTE bind/retire→espera+falha, nunca authority obsoleta; T5 3º lado no reparent→rollback integral) + 9 fault-injections + I2 falha no COMMIT · guard endurecido (+12 marcadores TX/lock/autocommit/TOCTOU-comment/second-guard) · mutations 53/53+6/6 byte-exatas · typecheck 0 · runner 188/188 · clone efêmero destruído · dev intocado · Δbank=0 · 0187/0186/0157 byte-intactas · DTs preservadas.
- **Próximo ato: STOP — aguarda UMA única reauditoria Yala final. Sem selo; D9.2–D9.8 trancadas.**
...
- **Ausência material:** migration=0 · DDL/DML=0 · dados=0 · runtime intocado · Bank intocado · Δbank=0. Intactos: 0157, módulo organization + blanket 501, tabelas tombstone, Actor sistêmico do tenant.
- **DTs:** `DT-GROUP-ACCOUNT-OWNERTYPE-COMPANY-MASQUERADE` e `DT-GROUP-ACCOUNTS-BALANCE-CENTS-PARALLEL-TRUTH` permanecem OPEN-CONGELADAS (não fechadas); `DT-ORGANIZATION-SPRINT78-FROZEN` segue OPEN (só lacuna decisória resolvida).
- **Nenhum GATE material aberto; nenhuma etapa de D9 iniciada.** Selo final = commit docs-only `docs(remediation): seal organizational actor composition decision after Yala`. **STOP obrigatório após o commit.**

### 2026-07-17 (158) — DECISION-0186 · ORGANIZATIONAL ACTOR COMPOSITION CONTRACT: PROMULGADA DOCS-ONLY · NÃO SELADA · MATERIAL NÃO INICIADO · AGUARDA UMA ÚNICA AUDITORIA YALA
- **Promulgação docs-only sob GO explícito de Clayton**, a partir do GATE READ-ONLY "condomínios/igrejas/organizações comunitárias" (Veredito B), com 2 correções soberanas incorporadas: (1) NÃO pré-decidir `ownerType='group'` no Bank — DTs financeiras registradas CONGELADAS, forma física remetida a GATE Bank próprio (hipótese preferencial não-vinculante: "Group é Actor → a conta tende a pertencer ao Actor"); (2) "Actor organizacional institucional" (humanas: condomínio/igreja/associação) expressamente SEPARADO do "Actor institucional sistêmico do tenant" (`DT-C1-INSTITUTIONAL-SYSTEM-ACTOR-PENDING` intocada).
- **Ratificado:** composição canônica = Actor institucional (page-actor via PJ se CNPJ; group-actor se informal) + Groups internos SEM herança de autoridade/conta/endereço · PROIBIDO novo `actor_type` por segmento (0157 = freio; proibido reviver `actor_organizational` como writer) · reconciliação categoria constitucional × forma operacional (`actor_organizational` → `page`·`group`) · formalização informal→formal = transição governada, nunca instituição duplicada · membership ≠ autoridade (papel descreve, grant concede) · `organization_*` REJEITADO como casa canônica (descontinuação arquitetural ≠ remoção física; 501/tombstone preservados; limpeza = frente posterior; critério de descongelamento da `DT-ORGANIZATION-SPRINT78-FROZEN` SUPERADO) · ordem vinculante de 8 integrações, TODAS atrás de GATE+GO próprios, financeiro POR ÚLTIMO.
- **Novas DTs (D8, congeladas):** `DT-GROUP-ACCOUNT-OWNERTYPE-COMPANY-MASQUERADE` (resolveGroupAccount usa máscara 'company'; físico Genesis = actor|system|escrow — user E company já viram 'actor') · `DT-GROUP-ACCOUNTS-BALANCE-CENTS-PARALLEL-TRUTH` (balance_cents local vs bank_ledger). Nenhum caller/leitor novo pode ampliar máscara ou tratar balance_cents como verdade.
- **Efeito na contagem:** +2 DTs OPEN-CONGELADAS (bucket A_DECISION/E_BLOCKED — zeram no GATE Bank D9.8); 1 DT anotada (SPRINT78). Zero código · zero migration · Δbank=0. **Próximo ato: STOP — arco aguarda UMA única auditoria Yala.**
...
- **Invariantes selados:** casa única `actor_capability_grants` · scope `regional_treasury` (tenant NOT NULL+scope_actor NULL+city NOT NULL) · actor/territory preservados · matriz 3 ramos (regional→2 keys financeiras) · capabilities CRITICAL_FINANCIAL separadas/não fusíveis · unicidade tenant+grantee+capability+city · 3 registries disjuntos (prefixo treasury: não é autoridade).
- **Ressalva DB:** 22/22 aceito por preservação byte-exata (não reprovisionado). **OBS não bloqueantes:** OBS-1 (casa paralela nome-arbitrário/1-key/INSERT fora da varredura — defesa em profundidade futura; sem 2ª casa viva) · OBS-2 (teste de índice em superfície inadequada, não-defeito). Nenhuma reabre a frente.
- **Fronteiras:** migration não aplicada em dev; scope_type vivo actor|territory; grants/policy/PORTA/caller **0**; firewall OFF; Bank **16/1/0/0/0**; Curitiba 0; **Δbank=0**. **SELO ≠ grant ≠ policy ≠ composição ≠ PORTA ≠ ativação.** Composição/PORTA trancadas — exigem novo GO humano explícito e separado.

### 2026-07-16 (155) — B-CITY-2 · SUBSTRATO regional_treasury · REMEDIAÇÃO PÓS-YALA (VEREDITO B): COBERTURA DE MUTATIONS 38 VETORES · GUARD/HARNESS-ONLY · NÃO SELADO
- **Yala Veredito B:** defeito único = harness provava 13 mutations (< mínimo 24). Material auditado CORRETO. Remediação guard/harness/provas-only → **38 vetores hostis isolados**. Arco: `52b0f854e`(material) → `bda93f9c1`(cartório) → `ff9b3565e`(remediação) → **este cartório**. Só 2 arquivos: guard 187 + harness (runner byte-intacto, guard count inalterado).
- **Guard 187 endurecido:** marcador `[X]` por trava; sub-checks (shape tenant/city/scope_actor/AND, unicidade tenant/city/capability/predicado, matriz cross-financeira, existência regional, scope_type) + **varredura runtime src/** ancorada em regional key (fusão, casa-paralela, grant por residência/city/role/self/bootstrap, cast entre registries, registry duplo, tenant-null-global). Comment-aware; sem depender de nome de arquivo.
- **Harness:** 38 hostis (M01–M38) — 18 via mutação byte-restore da migration + 20 via fixture hostil temporária produção-like (criada+removida) — cada um exit!=0 com marcador específico + 5 benignos. M15–M20 provados por fixture (não dispensados).
- **Provas:** guard OK · **38/38 hostis + 5/5 benignos** · runner **187/187** · typecheck 0 · git diff --check limpo · migration/protegidos **byte-idênticos** (sha antes==depois) · resíduo zero · DB 22/22 preservada (migration byte-idêntica). Dev SEM migration; scope_type `actor|territory`; grants/policy/PORTA/caller **0**; Bank **16/1/0/0/0**; **Δbank=0**. **NÃO SELADO — aguarda UMA reauditoria Yala final.**
...
- **SELO (2026-07-20):** DECISION-0190 · **VEREDITO A · SELO COMPLETO DOCS-ONLY · OFICIALMENTE SELADA**, após reauditoria independente (Opus 4.8) favorável. `PASSO 4 NARROW SUPERSESSION OFFICIALLY EFFECTIVE AFTER DECISION-0190 SEAL` — a supersessão estreita do PASSO 4 está oficialmente vigente. Restante da FASE_6_2 permanece vigente; princípio de sandbox sem dinheiro real preservado; firewall default-off/fail-closed preservado; PORTA-1 fechada; nenhuma execução financeira/sandbox real/caller de produto autorizado; frente `F-EVENT-ECONOMIC-V2-HONEST-CONTAINMENT` permanece FECHADA (exige novo GO material); integração do guard do firewall ao runner segue condicionada ao fechamento do AUDIT-002. DECISION-0190 e erratas não foram reeditadas neste ato — só o registro cartorial mudou.

## DT-WORKER-CROSS-TENANT-GLOBAL-READ-UNGOVERNED — 🟡 RECONHECIDA + DECIDIDA docs-only (DECISION-0191, 2026-07-20)

- **Estado:** RECONHECIDA + DECIDIDA docs-only (DECISION-0191 REDIGIDA/não-selada). Frentes materiais (tenant-loop + RLS por tranche) AGUARDAM GATEs + GO próprio + selo.
- **Fato:** 3 workers incondicionais liam/descobriam globalmente dados tenant-scoped sem aplicar a DECISION-0149: `governance-execution-worker` (`governance_proposals` global, sem RLS), `risk-identity-reconcile.worker` (`actor_events` global, sem RLS), `ledger-snapshot-worker` (`bank_ledger` com RLS+FORCE → retorna zero linhas sob `unificard_app`, feature silenciosamente inerte). Nenhum efeito cross-tenant de ESCRITA confirmado (escritas escopadas por tenant da própria linha); risco = leitura global sem governança + 1 feature morta. Resíduo #34 (DECISION-0149) nunca fechado nominalmente para estes.
- **Decisão:** DECISION-0191 (REDIGIDA/não-selada) — TENANT-LOOP reafirmado como único modelo canônico; `GLOBAL_READ_TENANT_SCOPED_WRITE` rejeitado como categoria autorizativa; governance-execution + risk-identity-reconcile = TENANT_LOOP_REQUIRED; ledger-snapshot = TENANT_LOOP_REQUIRED OR RETIRE (REVIVAL PROHIBITED WITHOUT MATERIAL GO); `governance_proposals`/`actor_events` = tenant-scoped com RLS_GAP; envelope material acopla conversão+RLS na ordem correta; causa-raiz = `audit-rls-tenant-context.mjs` exclui `src/workers/`.
- **Pendências não-bloqueantes (para GATEs/frentes futuras):** mapa de efeitos de `evaluateActorRisk`; consumers de `ledger_snapshots` (decide RETIRE vs TENANT_LOOP); auditoria de `saga-timeout`/`reconciliation-scheduled`/`payment-worker`; FK ausente `governance_proposals.tenant_id`/`actor_events.tenant_id` → `tenants`; `governance-execution` sem try/catch por item.
- **SELO (2026-07-20):** DECISION-0191 · **VEREDITO A · SELO COMPLETO DOCS-ONLY · OFICIALMENTE SELADA/ENCERRADA**, após auditoria independente (Opus 4.8) favorável, zero correção bloqueante (O1/O2 não-bloqueantes, não incorporadas). Commit de promulgação `4223d3651c24793a4ff5577d97a8b804f21d4198` byte-intacto; corpo D0-D15 não reeditado. TENANT-LOOP reafirmado como único modelo canônico; `GLOBAL_READ_TENANT_SCOPED_WRITE` rejeitado como autorizativo; 3 workers classificados (governance-execution/risk-identity-reconcile = TENANT_LOOP_REQUIRED; ledger-snapshot = TENANT_LOOP_REQUIRED OR RETIRE/HOLD). Resíduo #34 fechado nominalmente para a primeira tranche. Frentes materiais + GATEs (ledger_snapshots consumers, evaluateActorRisk) permanecem AGUARDANDO GO próprio; F-4 fechado; PORTA-1 fechada.

## DT-ROOT-003-DENOMINATOR-RECONCILIATION — 🟡 RECLASSIFICADA (R2, 2026-07-21)

- **Estado:** ROOT-003 ("runner 200/200 verde ignora 84 de 281 guards") reclassificado de "guards ativos sem enforcement" para **`R2 — COBERTURA EXISTE, MAS É OPACA`** (`PARTIALLY_RESOLVED_AND_CONTAINED`), após auditoria independente Opus 4.8 (Veredito A) confirmar que 77/84 guards rodam via 2 agregadores fail-closed (`execFileSync`) e 1/84 via comando CI próprio — restando **0** guards ativos válidos sem qualquer enforcement (6/84 são one-shot intencionais, não exigíveis como contínuos).
- **Fato corrigido:** "sem entrada nominal direta no runner" ≠ "sem execução em CI". O runner conta 200 comandos, não a quantidade efetiva de guards executados (≥278 via 2 agregadores). Achados de qualidade de F-1 a F-5 (pontos cegos, ACTIVE_BUT_INCOMPLETE, riscos de falso-negativo) **permanecem válidos e não foram invalidados** — só o eixo de enforcement estava mal descrito.
...
## DT-EVENTS-SPRINT76-ACTOR-HINT-AUTHORSHIP-FORGERY — ✅ FECHADA POR CONTENÇÃO · SELADA PELA YALA · VEREDITO A (2026-07-22)

- `events-sprint76.routes.ts` cancel/checkin/checkout carimbavam `actionContext.actorId` cru sem `canRepresentActor` (dupla falha: forja de autoria + autorização ausente). Nunca foi viva-escritível — o runtime está MORTO contra o schema vivo (as 3 rotas morrem no 1º read por drift de ticket_sales/event_checkins). Risco era contingente ao drift; sem exploit vivo.
- **Fechada por CONTENÇÃO (não por fix de autoridade):** material `86d675b83` — as 3 rotas retornam 501 honesto (`TICKET_CANCEL_DEFERRED` / `TICKET_CHECKIN_DEFERRED_FATIA4`) como PRIMEIRA instrução, ANTES de qualquer sink. Rota deferida não escreve → não forja. Espelha a contenção selada de /reserve e /pay (F1 addendum-1). Guard ancorado por região (18/18, mordida por handler provada; token compartilhado checkin/checkout SEM falso-lock). E2E 10/10, Δbank=0, escopo +76/−0 em 3 arquivos, services/repos/schema/Bank intocados. Selo docs-only 2026-07-22.
- **O fix de AUTORIDADE real (não só contenção)** vem na **Fatia 4** (check-in convergido ao modelo `cultural-checkin-target-authority`), que só é possível APÓS a Fatia 2 (compra → ticket_sales convergido) — check-in de verdade precisa de ingresso comprado. Modelo guardado: self OR canRepresentActor sobre o portador (buyerActorId) + validador no service; cancel = representa buyerActorId (Art. I). Aditivo organizador/staff deferido.

## DT-GROUP-ACTOR-NOT-EAGERLY-CREATED — 🟡 substrato (verificado pela direção, 2026-07-22)

- `groupsRepository.createGroup` cria grupo com `owner_actor_id` + `group_members` (role owner) mas NÃO materializa o group-actor (`actor_type='group'`) nem seta `groups.actor_id`. A migration `20260530576000` (1:1 group↔actor) é SCHEMA-ONLY (unique index, sem backfill). Logo `groups.actor_id` é NULL para grupos reais até algum fluxo chamar o lazy-heal governado `actorRepository.findOrCreateGroupActor` (social/actor.repository:579, §4.8.1/§4.8.2-compliant: valida âncora civil, lock, back-link).
- **Consequência:** qualquer operação que precise representar o group-actor tem de chamar findOrCreateGroupActor primeiro (lazy-heal). F0-grupo faz isso (Opção A' ratificada): materializar-se-preciso o group-actor é cura em direção ao 1:1 canônico, não anomalia.
- **Hardening futuro (fora de F0):** `createGroup` materializar o group-actor EAGERLY (todo grupo nasce com seu actor canônico), tornando o lazy-heal desnecessário. Fatia própria (toca criação de grupo + backfill de grupos existentes).

## DT-AUTHORITY-FACADE-NO-TRANSACTIONAL-REPRESENT — 🟡 convergência §4.9.8 (verificado pela direção, 2026-07-22)

- A fachada `authority.service` (modules/authority) expõe SÓ `canPerformAction`/`canActOnBehalf` (delegam a `authorizationService.canActAs`, permission-key, NÃO-transacional, sem `client`). **NÃO expõe `canRepresentActor`** nem qualquer check transaction-aware. O core (`authorization.service.ts:483` + caminho transacional `canRepresentActorOnClient:569`, remediação D9.1) provê representação transaction-aware, mas a fachada não a envolve.
...
## DT-EVENT-STAFF-OPERATIONAL-COMMITMENT-SCHEMA-DRIFT — ✅ FECHADA (C2a, 2026-07-22)
> **FECHADA:** a Fatia C2a (`6e9dc4706`) reconciliou o `event_staff` vivo ao contrato promulgado — materializou `checked_in_at/checked_out_at/failure_reason` (§3) e alinhou `chk_event_staff_status` ao vocabulário §4 (expected/checked_in/checked_out/failed). O writer actor-first `createCommitment` agora funciona (banda page/group vinculada, provado). +2 fixes mínimos (tenant_id no INSERT; ForbiddenError→403). Sem campo financeiro (§3/§11), sem materializar event_actors (§2). Yala Veredito A.



- **O writer actor-first canônico do vínculo actor↔evento está drifted contra o schema vivo.** `operationalCommitmentsService.createCommitment` (core/events/operational-commitments.service.ts) grava `status='expected'` e usa colunas `checked_in_at/checked_out_at/failure_reason` — MAS: (i) o CHECK vivo `chk_event_staff_status` (migration `20260530535000:107`) só aceita `('active','inactive','cancelled')` → INSERT de 'expected' = check_violation; (ii) as colunas de lifecycle só existem na migration ARQUIVADA `migrations_archive/0670_event_staff_commitments.sql`, NÃO na viva (`20260530170000_event_staff.sql`) → coluna inexistente. Ou seja, o caminho correto (que aceita banda/page/group via `responsible_actor_type`) estouraria em runtime.
- **Norma:** `docs/01_normative/operational_commitment_minimum_contract.md` §3/§4 define o schema+vocabulário de status EXATO do OperationalCommitment (event_staff é a tabela). O schema vivo diverge da norma promulgada.
- **Correção:** a Fatia **C2a** reconcilia `event_staff` ao contrato (materializa as colunas de lifecycle na migration VIVA + alinha o CHECK de status ao vocabulário do contrato), Bank-free — fazendo o vínculo actor-first (createCommitment) funcionar e destravando a C2 (line-up: contratar a banda para o evento). Materializar `event_actors` = REJEITADO (realidade paralela, §2). Papel econômico = substrato próprio Bank-free (contrato §3/§11 proíbe campo financeiro em event_staff).
...
- **Correção:** a Fatia **F0-grupo** constrói o vínculo GOVERNADO correto (writer format-first + INSERT em `group_events` no schema real de 3 colunas reais + autoridade = `canRepresentActor` transaction-aware sobre o group-actor, direto-core §4.9.8-transição, atômico com o INSERT de events + lazy-heal do group-actor §4.8.1). ✅ **F0-grupo SELADA (Yala Veredito A, `ce98ce41a`)** — o vínculo governado agora EXISTE e funciona (provado por API direta). **O W2 quebrado (que engole o erro) permanece OPEN até a F0-writer-único aposentá-lo.** Cross-ref: `EVENT_ENGINE_COUPLING_MAP.md` (F0-grupo).

## DT-EVENT-TICKETING-UNBUILT-FEATURE-FACADE — 🔴 estrutural · money · feature com UI viva e ZERO backend funcional (consolidado 2026-07-22)

- **Resposta à pergunta "bilheteria existe em outro lugar?": NÃO existe implementação viva em lugar nenhum.** Há um schema REAL (`event_tickets` = catálogo de TIPO: `ticket_type/price_cents/quantity_total/quantity_available`, migration `20260530120000`; `ticket_sales` = registro de COMPRA com `bank_transaction_id`, migration `20260530420000`), uma UI VIVA (`frontend/src/components/events/EventCheckoutModal.tsx`, `EventCheckout.tsx`, `api/checkout.ts`, botão de compra em `EventPage`/`CulturalEventCard`) — e TRÊS camadas de backend, TODAS mortas/drifted:
  1. `ticket-sale.repository.ts` (sprint76) → mira `ticket_sales` modelo reserva (inexistente) → 500 em getSaleById.
  2. `checkout-ticket.service.ts` (`/api/checkout/event-ticket`, montado no app.builder) → `purchaseTicket` e `checkIn` EXPLICITAMENTE DESATIVADOS com `throw` (remediação 2026-07-05): miram `event_tickets` modelo ingresso-emitido (`global_user_id/qr_code/price_paid`) que nunca existiu — o real é catálogo de tipo.
  3. `event-ticket.repository.ts` → a classificar no GATE de convergência.
- **"Varrido para debaixo do tapete" já aconteceu:** a remediação anterior conteve o checkout DESATIVANDO (throw), não consertando — deixando a UI viva prometer uma compra que o backend não entrega (promessa quebrada ao usuário).
- **Feature INTENCIONAL, nunca construída de ponta a ponta:** schema + UI + financial-firewall (`core/checkout/checkout-financial-firewall.ts`) existem; falta o backend coerente que conecte catálogo→compra(checkout+firewall)→registro(`ticket_sales`, bank-wired)→check-in(modelo `cultural-checkin-target-authority`)→cancel/refund. Envolve DINHEIRO (bank_transaction_id, settlement, AccountsReceivable).
- **Recomendação da direção:** GATE de CONVERGÊNCIA read-only (Opus) mapeando o ciclo intencional completo + a verdade de schema + as 3 camadas mortas + a UI viva + o firewall + o modelo de check-in, devolvendo um PLANO EM FATIAS. Sem material até o plano. NÃO quarentenar isoladamente (seria varrer de novo). Cross-ref: [[DT-EVENTS-SPRINT76-ACTOR-HINT-AUTHORSHIP-FORGERY]] (a autoridade que íamos adicionar entra como fatia deste plano), [[DT-EVENTS-SPRINT76-TICKET-PAY-UNAUTHENTICATED-AUTHORITY]].
- **GATE DE CONVERGÊNCIA EXECUTADO (2026-07-22, Opus, Veredito A).** DB virgem (sem dados) → schema reshapável a baixo risco; verdade = schema migrado (event_tickets tipo + ticket_sales compra bank-wired + event_checkins) — o código é que está drifted (mira `migrations_archive` nunca aplicado). Trilhos de dinheiro TODOS fail-closed hoje (checkout firewall 403 default-off `CHECKOUT_FINANCIAL_RUNTIME_DISABLED`; economic/v2 501 DECISION-0190; sprint76 dead/500; checkout-ticket disabled/throw). **NÃO existe DECISION de economia de venda de ingresso** (split/commission/settlement) — é ratificação soberana pendente.
- **CORREÇÃO da direção ao GATE (verificado no código):** a "fachada desonesta" foi superdimensionada — `EventCheckoutModal.tsx` TRATA erro (L109 lança se `!result.success`; L134 catch → step 'error' → "Erro ao comprar ingresso"). Num 403 o usuário vê ERRO, não sucesso falso. Enquadramento correto: fluxo de compra exposto que SEMPRE falha (firewall fechado), não fachada que finge sucesso. Remediação (conter a porta de entrada) inalterada.
- **PLANO EM FATIAS (cada fatia = Gate/GO/material/Yala próprio):** F0 verdade de schema + contenção honesta (dobrada nas fatias que reescrevem cada rota) · **F1 catálogo de tipo governado ✅ SELADA (Yala Veredito A, 2026-07-22)** · F2 compra checkout→ticket_sales bank-wired (🔴 dinheiro, RATIFICAÇÃO-GATED) · F3 "ter um ingresso"/emissão + meus-ingressos (modelo a ratificar; schema pode mudar) · F4 check-in convergido ao modelo cultural — ABSORVE a contenção de autoria sprint76 (Bank-free, GO-ready — PRÓXIMA) · F5 cancel/reembolso (🔴 dinheiro, RATIFICAÇÃO-GATED).
- **F1 SELADA:** núcleo `4a6b50c6e` (event-ticket.repository convergido ao schema vivo; rota governada criar/editar tipo com `canActAs`/`create_events`+`manage_events` server-resolved sobre o dono do evento, fail-closed 403; hint descartado) + addendum-1 `a5e5a06f8` (contenção honesta 501 `TICKET_PURCHASE_DEFERRED_FATIA2` de /reserve e /pay — fecha órfãos em orders/payment_intents que a convergência do repo reanimara) + addendum-2 `02e6cbe1c` (fix do falso-lock do guard do /pay). Provas Yala: E2E 7/7, guard 15/15 morde (incl. as 3 mutações a/b/c), typecheck 0, Δbank=0, escopo exato, incidente sem resíduo. **`DT-EVENTS-SPRINT76-ACTOR-HINT-AUTHORSHIP-FORGERY` (check-in) segue aberta — será fechada na F4.**
- **RATIFICAÇÕES SOBERANAS pendentes (gate das fatias de dinheiro/modelo):** (6.1) economia de ingresso faz split/commission/settlement? sob qual SSOT? — sem DECISION, ratificar; (6.2) abertura do firewall de checkout por PORTA institucional (não env var); (6.3) modelo de ingresso emitido: agregado (ticket_sales) vs linha individual+QR (schema reshapável, DB virgem); (6.4) política de reembolso; (6.5) destino das 3 camadas mortas (recomendado converger→remover); (6.6) **DOIS sistemas de bilheteria paralelos** (cultural events vs events-v2) — unificar? decisão de produto.

## DT-EVENTS-SPRINT76-TICKET-SALE-REPOSITORY-SCHEMA-DRIFT — 🟠 estrutural · money-adjacent · módulo morto · SUBSUMIDO em DT-EVENT-TICKETING-UNBUILT-FEATURE-FACADE (descoberto no material events-sprint76, 2026-07-22)

- **Dois designs irreconciliáveis de `ticket_sales`:** o schema vivo (migration `20260530420000_events_missing_tables.sql`) é modelo de COMPRA PAGA — `ticket_id, buyer_actor_id, quantity, unit_price_cents, total_price_cents, bank_transaction_id, status, purchased_at, metadata`. O repositório `ticket-sale.repository.ts` (SELECT/INSERT/UPDATE de reserve/pay/cancel/checkin/checkout) mira modelo de RESERVA→PAGAMENTO — `event_ticket_id, payment_intent_id, reserved_at, paid_at, cancelled_at, cancelled_by_actor_id, cancellation_reason, created_by_*` — NENHUMA existe no schema vivo. Nenhum ALTER posterior reconcilia (só `20260530420000` cria; `20260703120000` é sobre event_reservations).
- **Módulo morto confirmado:** NADA em `src/` referencia `ticket_sales` além do repositório incompatível; NADA escreve o schema vivo (quantity/unit_price_cents/purchased_at inexistentes no código). Os 5 endpoints sprint76 (reserve/pay/cancel/checkin/checkout) todos passam por `getSaleById` → 500 (coluna inexistente) antes de escrever. Sem UI viva. Check-in real do produto vive em `/cultural/events/:id/check-in` e `/presence/checkin/by-token` (governados).
- **Money-adjacent:** o schema vivo tem `bank_transaction_id` (FK a `bank_transactions`) e a tabela é DORMENTE (escrita por ninguém). Reconciliar o repositório ao schema (ou vice-versa) é decisão COM dinheiro: definir se bilheteria PAGA sprint76 é feature de produto real. Não é Trava B.8 (nada vivo/alcançável; é dead).
- **Decisão de fate PENDENTE (soberana):** (I) quarentenar/remover o módulo morto (padrão da quarentena de frontend); (II) reconciliar+reviver (decisão de produto+dinheiro, GATE próprio); (III) shelve como DT registrada (sem risco vivo). Sem GO. Precisa GATE de fate próprio se (I) ou (II).

## DT-EVENTS-SPRINT76-TICKET-PAY-UNAUTHENTICATED-AUTHORITY — 🟡 backlog, money-adjacent (descoberto no GATE events-sprint76, 2026-07-22)

- `events-sprint76.routes.ts` `POST /tickets/:id/pay` (L330-339) chama `confirmTicketPayment(tenantId, req.params.id)` com ZERO gate de autoridade (só checa tenant; nem olha actor). `confirmTicketPayment` TOCA dinheiro (AccountsReceivable + commission + event_settlement). NÃO consome o hint (keyed em ticketSaleId) e exige PaymentIntent `AUTHORIZED` — logo não é drenagem a frio, mas é "qualquer autenticado do tenant confirma o pagamento pendente de qualquer venda por ID". Distinto do cluster de forja (não é sobre actor) — money-write sem autoridade. **Precisa GATE próprio** para avaliar alcance/impacto (quem legitimamente confirma pagamento? o comprador? o gateway via webhook?). Sem GO. Não é Trava B.8 (gated por AUTHORIZED).
...
## F-GUARD-HARDENING-MIGRATION-DDL-RECOGNITION — 2ª TRANCHE (2A) — ✅ SELADA PELA YALA · VEREDITO A (2026-07-22)

- 3 guards (`category-input-audit-schema-ghost-fix`, `location-authority-classification`, `vehicle-fields-governed`) endurecidos via helper léxico já selado `sql-shape.mjs` (sem função nova) + harness selado `audit-migration-ddl-recognition-mutations.mjs` estendido (+26 cenários; 63/63). Material `5637c38e2`.
- **Evasões fechadas:** guard1 lia 1 migration hardcoded (migration nova = invisível) + regex FK sem case/schema-qual; guard2 não reconhecia CREATE schema-qualificado/`citext`/DDL em `EXECUTE` e dava falso-positivo por comentário/`ALTER TABLE ONLY`; guard3 lia a migration ERRADA (`.find()` por substring casava a tabela órfã `rental_resource_pricing`; a tabela de dinheiro VIVA `actor_asset_rental_pricing_tiers` nunca era lida — drift de `price_cents` invisível). Agora localizada por CONTEÚDO (`createsTarget` varrendo `migrations/`).
- **Selo:** GATE read-only (Veredito A) → GO material Sonnet → material `5637c38e2` → YALA independente (Opus) Veredito A com provas reproduzidas em 1ª mão (3 guards verdes, harness 63/63, bite-proof com o guard ANTIGO confirmando a regressão real, 2 agregadores verdes, diff --check 0, escopo exato 4 arquivos, helper/runner/declarations intocados). Estritamente ADITIVO — nenhum check verdadeiro removido. Ressalva não-bloqueante: escapes residuais (CTAS materializando loc-text; DROP+re-ADD de coluna) são limites declarados/esotéricos fora do escopo desta tranche de precisão — não regressões. Selo docs-only da direção 2026-07-22.
